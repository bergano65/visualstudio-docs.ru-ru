...
ms.locfileid: "66820610"
---
# <a name="include-a-nuget-package-in-your-project"></a>Включение пакета NuGet в проект

NuGet — это самый популярный диспетчер пакетов для разработки .NET, который встроен в Visual Studio для Mac и Visual Studio для Windows. Можно искать и добавление пакетов для Xamarin, проектов .NET Core и ASP.NET, с помощью IDE.
...
Откройте проект в Visual Studio для Mac, щелкнуть правой кнопкой мыши **пакетов** папку в **панели решения** и выберите **добавить пакеты NuGet**:
...
Используйте поле поиска в правом верхнем углу для поиска определенного пакета, например  `azure mobile`. Если вы нашли пакет, который вы хотите использовать, выберите его и нажмите кнопку **Add Package** кнопку, чтобы начать установку.
...
*   **Добавьте пакеты NuGet** -открывается окно для добавления дополнительных пакетов в проект.
