...
После установки расширения выберите **Управление версиями > TFS/VSTS > Open from Remote Repository** (Открыть из удаленного репозитория) в меню. 

Вначале выберите Visual Studio Team Services или Team Foundation Server и нажмите кнопку **Продолжить**:

  ![Соединение с сервером](media/tfvc-choose-server-type.png)

### <a name="vsts-authentication"></a>Проверка подлинности VSTS

При выборе проекта, размещенного в VSTS, вам предлагается ввести сведения о своей учетной записи Майкрософт:

  ![Соединение с сервером VSTS](media/tfvc-vsts-login.png)
...
- Учетная запись VSTS или коллекция — здесь отображаются все учетные записи, подключенные к учетной записи Майкрософт, с помощью которой вы вошли в систему.
- Командные проекты — в каждом VSTS можно иметь несколько командных проектов. В командном проекте размещены исходный код, рабочие элементы и автоматические сборки.

На этом этапе вы можете выполнять поиск и фильтрацию по имени проекта или учетной записи.
...
![Диалоговое окно с параметром для поставщика системы управления версиями](media/tfvc-add-new-creds.png)
...
Чтобы открыть обозреватель исходного кода, выберите **Управление версиями > TFS/VSTS > Обозреватель управления исходным кодом**:

![Элемент меню для открытия обозревателя исходного кода](media/tfvc-source-control-explorer.png)
...
1. Выберите папку "$" для сопоставления всех командных проектов на сервере с одной рабочей областью или выберите отдельный проект и нажмите кнопку **ОК**:
...
Для использования обычной проверки подлинности нужно включить **альтернативные учетные данные** в VSTS, выполнив следующие действия:

1. Войдите как владелец учетной записи VSTS (https://{ваша_учетная_запись}.visualstudio.com).
2. На панели инструментов учетной записи выберите значок шестеренки и пункт **Политика**:
