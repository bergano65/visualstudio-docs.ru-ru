...
В Visual Studio 2017 доступен инструментарий конструктора для создания нового формата манифеста VSIX v3. См. сопроводительный [документ как Перенесите проекты расширяемости в Visual](how-to-migrate-extensibility-projects-to-visual-studio-2017.md) Studio 2017, чтобы получить подробные сведения об использовании средств конструктора или внесении ручных обновлений в проект и манифест для разработки расширений VSIX v3.

## <a name="change-visual-studio-user-data-path"></a>Смен Путь к данным пользователя Visual Studio
...
## <a name="change-global-assembly-cache-gac"></a>Смен Глобальный кэш сборок (GAC)
...
> [!NOTE]
> [INSTALLDIR] ниже ссылается на корневой каталог установки Visual Studio. *Всиксинсталлер. exe* автоматически заполнит этот код, но для написания пользовательского кода развертывания ознакомьтесь с [разрасположением Visual Studio](locating-visual-studio.md).
...
* Сборки, которые были установлены в непроверенный путь и в глобальный кэш сборок:
  ...
  * Был добавлен *pkgdef* -файл для указания записи базы кода для сборки.
    ...
    Во время выполнения подсистема pkgdef Visual Studio объединяет эти записи в файл конфигурации среды выполнения процесса Visual Studio (в разделе *[всаппдата] \девенв.ЕКСЕ.конфиг*) как [`<codeBase>`](/dotnet/framework/configure-apps/file-schema/runtime/codebase-element) элементы. Это рекомендуемый способ разрешить процессу Visual Studio найти сборку, так как она позволяет избежать поиска по путям поиска.
...
* Если расширение выполняется вне процесса Visual Studio, выполните следующие действия.
  ...
  Рассмотрите возможность поиска основных сборок Visual Studio в <em>[INSTALLDIR]\*\Common7\IDE, * [INSTALLDIR] \Common7\IDE\PublicAssemblies</em> или *[INSTALLDIR] \Common7\IDE\PrivateAssemblies* с помощью файла конфигурации или сборки. сопоставитель.

## <a name="change-reduce-registry-impact"></a>Смен Уменьшение влияния на реестр

### <a name="global-com-registration"></a>Глобальная регистрация COM

* Ранее Visual Studio установила множество разделов реестра в кустах HKEY_CLASSES_ROOT и HKEY_LOCAL_MACHINE для поддержки собственной регистрации COM. Чтобы устранить это воздействие, Visual Studio теперь использует [активацию без регистрации для COM-компонентов](https://msdn.microsoft.com/library/ms973913.aspx).
...
* Ранее Visual Studio установило множество разделов реестра в кустах **HKEY_LOCAL_MACHINE** и **HKEY_CURRENT_USER** в разделе, относящемся к Visual Studio:

  * **Хклм\софтваре\микрософт\висуалстудио\{версия}** : Разделы реестра, созданные установщиками MSI и расширениями для компьютера.
  * **Хкку\софтваре\микрософт\висуалстудио\{версия}** : Разделы реестра, создаваемые Visual Studio для хранения пользовательских параметров.
