...
|*\<ИДЕНТИФИКАТОР GUID >*|REG_SZ|""|Ключ для логического представления, которые поддерживаются. При необходимости может иметь как многие из них. Имя записи реестра — важно, не значение, которое всегда является пустой строкой.|  
