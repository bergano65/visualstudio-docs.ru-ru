...
-   Для Оглавления добавьте порядок узлов в этом разделе. Если родительский узел имеет `n` число дочерних узлов, определяют порядок дочерних узлов расположение в этом разделе. Например в этом разделе — это номер 4 4 подразделов.
...
Текст (не включая верхний и нижний колонтитул) раздела будет содержать ссылки на страницы, раздел примечание, сворачиваемой области, фрагмент кода и раздел текста конкретного языка.  Сведения об этих областях раздела, представленных в разделе фирменной символики.
...
5.  Добавьте текст кода конкретного языка:  `<LanguageSpecificText devLangcs="CS" devLangvb="VB" devLangcpp="C++" devLangnu="F#" />` Обратите внимание, что `devLangnu=` можно ввести другие языки. Например `devLangnu="Fortran"` отображает Fortran при фрагмента кода DisplayLanguage = Fortran
...
**Пример в справке средства просмотра** код иллюстрирует способ определения метаданных, фрагмент кода, сворачиваемой области и зависящие от языка текста.
...
Значение: Отображение выходных данных отладки в данных розничной торговли: да
...
-   Резервный пакет фирменной символики (Branding_\<языкового стандарта > .mshc) устанавливается в корне приложения 2.3 средства просмотра справки (пример: C:\Program \Microsoft Help Viewer\v2.3 файлы (x86)), языковой пакет средства просмотра справки.  Это используется для случаев, где не установлен продукт, фирменная символика пакета (содержимое не установлен) или где поврежден установленный пакет фирменной символики.  Элементы Visual Studio (логотип и обратная связь) учитываются при использовании приложения корневой резервный фирменной настройки пакета.
...
| Использовать: | Набор конкретного случая заявления об отказе для машины переведенного контента. |
| **Элемент** | **Значение** |
...
Branding.CSS файлы содержат определения для Visual Studio раздел presentation (недостаток заключается в том, что branding.css содержатся в Branding_\<языкового стандарта > выполнения из пакета службы может измениться).
...
### <a name="add-help-to-the-visual-studio-shell-integrated-and-isolated"></a>Добавление справки для оболочки Visual Studio (интегрированная и изолированная)
...
2.  [Visual Studio 2013 изолированной оболочки Redist](https://visualstudio.microsoft.com/vs/older-downloads/isolated-shell/)
...
- HKLM\SOFTWARE\Wow6432Node\Microsoft\Help\v2.3\Catalogs\VisualStudio15\en-US
   ...
   Ключ CatalogName строковое значение: [!INCLUDE[vs_dev12](../../extensibility/includes/vs_dev12_md.md)] Документация по
...
4. Запустите редактор реестра, выбрав **запуска** в **запустить** меню и введя `Regedit`. В редакторе реестра выберите **файл**, а затем **импорта**. Перейдите в папку проекта ContosoHelpShell. В папке ContosoHelpShell выберите ContosoHelpShell.reg.
...
7. Добавьте следующие разделы реестра:
    ...
    Ключ CatalogName строковое значение: [!INCLUDE[vs_dev12](../../extensibility/includes/vs_dev12_md.md)] документация. Для оболочки ISO это имя вашего каталога.
...
Дополнительные сведения о том, как использовать API-Интерфейс справки см. в разделе [примеры кода для средства просмотра справки](https://marketplace.visualstudio.com/items?itemName=RobChandlerHelpMVP.HelpViewer20CodeExamples).

Вы можете отправить предложения по функциям на [сообщества разработчиков](https://developercommunity.visualstudio.com/content/idea/post.html?space=8).
