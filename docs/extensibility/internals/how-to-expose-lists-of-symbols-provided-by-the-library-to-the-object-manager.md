---
title: "Раскрывать списки символов, предоставленный диспетчера объектов | Документы Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
---
# <a name="how-to-expose-lists-of-symbols-provided-by-the-library-to-the-object-manager"></a>Практическое руководство: предоставлять список символов, предоставленный библиотекой диспетчера объектов
Средства просмотра символ **представление классов**, **обозревателя объектов**, **Call Browser** и **результаты поиска символа**, передачи запросов для новых данных [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] диспетчера объектов. Диспетчер объектов находит соответствующий библиотек и запрашивает новые списки символов. Библиотеки отвечает, предоставляя запрошенные данные для [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] диспетчера объектов через <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>интерфейса.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2> [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)] Диспетчера объектов вызывает методы <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>интерфейс для получения данных и использует его для заполнения или обновление представления средства просмотра символов.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>  
  
 При вызове средство, узел развернут или обновления представления библиотеки могут получить запросов данных. При вызове средством просмотра символа в первый раз диспетчера объектов запрашивает библиотеки для предоставления списка верхнего уровня. Когда пользователь разворачивает узел список, библиотека предоставляет список дочерних элементов в этом узле. Каждый запрос диспетчера объектов содержит индекс элемента интерес. Чтобы отобразить новый список, диспетчер объектов необходимо определить, сколько элементов в списке тип элементов, их имена, доступности и другие свойства.  
  
> [!NOTE]
>  В следующих примерах управляемого кода демонстрируют предоставления списков символы с помощью реализации <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2>интерфейса.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2> Диспетчер объектов вызывает методы этого интерфейса и использует полученные данные для заполнения или обновления средства просмотра символов.  
>   
>  Для реализации поставщика символов для машинного кода, используйте <xref:Microsoft.VisualStudio.Shell.Interop.IVsObjectList2>интерфейса.</xref:Microsoft.VisualStudio.Shell.Interop.IVsObjectList2>  
  
## <a name="providing-lists-of-symbols-to-the-object-manager"></a>Предоставление списка символов диспетчера объектов  
  
#### <a name="to-provide-lists-of-symbols-to-the-object-manager"></a>Чтобы предоставить списки символы в диспетчер объектов  
  
1.  Получение числа элементов в список символов, реализовав <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetItemCount%2A>метод.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetItemCount%2A> В следующем примере показано, как диспетчер объектов получает сведения о числе элементов в списке.  
...
2.  Получение сведений о категориях и атрибуты элемента данного списка путем реализации <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetCategoryField2%2A>метод.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetCategoryField2%2A> Категории элементов задаются в <xref:Microsoft.VisualStudio.Shell.Interop.LIB_CATEGORY>перечисления.</xref:Microsoft.VisualStudio.Shell.Interop.LIB_CATEGORY> В следующем примере показано, как диспетчер объектов получает атрибуты элементов для данной категории.  
...
3.  Получить текстовое представление заданного списка путем реализации <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetTextWithOwnership%2A>метод.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetTextWithOwnership%2A> Следующий пример демонстрирует получить полное имя данного элемента.  
...
4.  Получение сведений значок для элемента данного списка путем реализации <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetDisplayData%2A>метод.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetDisplayData%2A> Значок указывает на тип (класс, метод и т. д.) и специальных возможностей (закрытые, открытые и т. д.) для элемента списка. Ниже приведен пример, как получить сведения о ярлыка на основе атрибутов данного элемента.  
...
5.  Получите информацию о ли элемент заданного списка обеспечивается реализация <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetExpandable3%2A>метода.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetExpandable3%2A> Ниже приведен пример, как получить сведения о ли данный элемент могут быть развернуты.  
...
6.  Получить список дочерних символов из заданного списка путем реализации <xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetList2%2A>метод.</xref:Microsoft.VisualStudio.Shell.Interop.IVsSimpleObjectList2.GetList2%2A> Ниже приведен пример, как получить список дочерних символов для заданного элемента **вызвать** или **вызывающих объектов** диаграмм.  
