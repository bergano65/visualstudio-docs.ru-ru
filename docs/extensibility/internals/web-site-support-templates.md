...
## <a name="project-file"></a>Файл проекта
 @No__t_0 интегрированной среде разработки (IDE) требуется расширение файла проекта, чтобы сопоставлять шаблон с правильным типом проекта. Так как веб-проекты не имеют файла проекта, фиктивное расширение файла проекта регистрируется для того, чтобы сопоставлять шаблон с типом проекта.
...
### <a name="empty-web-site-projects"></a>Пустые проекты веб-сайтов
 Эти файлы создают пустой веб-сайт в ответ на команду **пустого веб-сайта** , которая доступна после выбора пункта **файл**  > **новый веб-сайт**:
...
### <a name="web-site-projects"></a>Проекты веб-сайтов
 Эти файлы создают новый веб-сайт в ответ на команду **ASP.NET Web site** , который доступен после выбора пункта **файл**  > **новый веб-сайт**:
...
### <a name="web-service-projects"></a>Проекты веб-служб
 Эти файлы создают новый веб-сайт в ответ на команду **веб-службы ASP.NET** , которая доступна после выбора **файла**  > **новый веб-сайт**:
...
- WebService. vstemplate
     ...
     Файл шаблона, определяющий содержимое решения веб-сайта и вызывающий принудительное создание папок App_Data и App_Code. Служба. файл *расширения* копируется в папку App_Code.
...
## <a name="project-item-template-folder"></a>Папка шаблона элемента проекта
 Веб-проект — шаблоны элементов обычно устанавливаются в папку [*путь установки Visual Studio*] \Common7\IDE\ItemTemplates\Web \\, каждый во вложенной папке, названной после языка веб-программирования.
...
### <a name="new-webform"></a>Новая форма
 ...
 Чтобы создать зависимый исходный файл с фоновым кодом, выберите **разместить код в отдельном файле**. В противном случае создается отдельная веб-страница с пустым блоком скрипта и без директивы \<% Page% > для подключения зависимого файла.
