...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsUIHierarchy>|Включает выполнение команды аналогично `IOleCommandTarget` для таких команд, как Cut и Rename, которые применяются только в том случае, если фокус находится в обозреватель решений.|
...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistHierarchyItem2>|Позволяет проекту управлять всеми аспектами сохраняемости для элементов проекта, как файлами на дисках, так и объектами в других системах хранения. Интерфейс `IVsPersistHierarchyItem2` используется для элементов, которые не реализуют интерфейс <xref:Microsoft.VisualStudio.Shell.Interop.IVsPersistDocData2>.|
...
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyDeleteHandler>|Реализуется иерархиями для управления параметрами удаления (уничтожения) или удаления (неразрушающих) для элементов иерархии. Вызов интерфейса запроса в интерфейсе `IVsHierarchyDeleteHandler` из интерфейса `IVsHierarchy`.|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsGetCfgProvider>|Предоставляет возможность реализации объекта, поддерживающего интерфейс `IVsCfgProvider2`, на различных удостоверениях COM, чем объект проекта, реализующий интерфейс `IVsHierarchy`.|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsProjectStartupServices>|Необязательный интерфейс, обеспечивающий расширение проекта другими разработчиками. Интерфейс `IVsProjectStartupServices` позволяет пакету VSPackage стороннего производителя зарегистрировать идентификатор GUID, который сохраняется в файле проекта, чтобы каждый раз при загрузке проекта в файл проекта загрузился идентификатор GUID сторонней службы, а для этого GUID вызывается `QueryService`.|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsUIHierWinClipboardHelperEvents>|Реализуется исходными иерархиями в окне `UIHierarchy` для координации операций с буфером обмена, таких как вырезание, копирование и вставка. Для регистрации событий буфера обмена используйте интерфейс `AdviseClipboardHelperEvents`.|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyDropDataSource2>|Предоставляет сведения о перемещенном элементе относительно его источника данных во время операции перетаскивания в окне иерархии пользовательского интерфейса. Вызывается из интерфейса `IVsHierarchy`.|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyDropDataTarget>|Предоставляет сведения о перемещенном элементе относительно целевого объекта перетаскивания во время операции перетаскивания в окне иерархии пользовательского интерфейса. Вызывается из интерфейса `IVsHierarchy`.|
...
|интерфейсов,|Комментарии|
|----------------|--------------|
|<xref:Microsoft.VisualStudio.Shell.Interop.IVsBuildableProjectCfg>|Управляет операцией сборки конфигурации проекта.|
...
|Служба|Комментарии|
|-------------|--------------|
|<xref:Microsoft.VisualStudio.Shell.Interop.SVsRegisterProjectTypes>|Используется пакетами VSPackage, которые реализуют типы проектов для регистрации, что их фабрика проектов существует в интегрированной среде разработки. Пакет VSPackage должен вызвать `QueryService` для этой службы и зарегистрировать фабрику проекта при вызове метода `IVsPackage::SetSite`. Если метод `SetSite` не вызывается, экземпляр проекта не создается.|
