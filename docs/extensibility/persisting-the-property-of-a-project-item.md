...
3.  Используйте точки останова или в противном случае определить загрузку пакета VSPackage и что SetItemAttribute выполняется.  
    ...
    > [!NOTE]
    >  Вы можете autoload VSPackage в контексте пользовательского интерфейса <xref:Microsoft.VisualStudio.VSConstants.UICONTEXT_SolutionExists>. Дополнительные сведения см. в разделе [загрузке пакетов VSPackage](../extensibility/loading-vspackages.md).  
