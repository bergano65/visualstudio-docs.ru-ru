...
Из-за этого перехода архитектуры сторонние расширения больше не загружаются в тот же процесс, что и сторонние библиотеки элементов управления. Расширения больше не могут иметь прямых зависимостей от библиотек элементов управления или непосредственного доступа к объектам среды выполнения. Расширения, написанные ранее для архитектуры изоляции конструктора с помощью API *Microsoft. Windows. Extensibility. dll* , должны быть перенесены на новый подход к работе с архитектурой изоляции Surface. На практике существующее расширение должно быть скомпилировано для новых сборок API расширения. Доступ к типам управления среды выполнения через экземпляры [typeof](/dotnet/csharp/language-reference/keywords/typeof) или Runtime необходимо заменить или удалить, так как теперь библиотеки элементов управления загружены в другой процесс.
...
## <a name="decouple-attribute-tables-from-runtime-types"></a>Отделение таблиц атрибутов от типов среды выполнения
...
API-интерфейсы `ModelItem` , использующие <xref:System.Object>вместо:
