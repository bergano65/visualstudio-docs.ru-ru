...
### <a name="host-ivscodewindow-and-ivstextview-in-your-own-content"></a>IVsCodeWindow узла и IVsTextView собственное содержимое
...
#### <a name="use-win32-and-wpf-versions-of-ivscodewindow"></a>Использовать версии IVsCodeWindow Win32 и WPF
...
#### <a name="use-win32-and-wpf-versions-of-ivstextview"></a>Использовать версии IVsTextView Win32 и WPF
 ...
 Установка текстового представления WPF режима, необходимо вызвать <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView.Initialize%2A> и передайте [TextViewInitFlags3.VIF_NO_HWND_SUPPORT](<xref:Microsoft.VisualStudio.TextManager.Interop.TextViewInitFlags3.VIF_NO_HWND_SUPPORT>) как часть инициализации флагом в `InitView` параметра. Вы можете получить <xref:System.Windows.FrameworkElement> путем вызова <xref:Microsoft.VisualStudio.Shell.Interop.IVsUIElementPane.CreateUIElementPane%2A>.
...
#### <a name="pass-arrays-as-parameters-in-native-code"></a>Передать массивы в качестве параметров в машинном коде
...
#### <a name="eventing-and-undo"></a>Регистрации событий и отмены
 В новом редакторе представление продолжает выполнять операции, такие как подготовка к просмотру и вызов событий, постоянно. Работа продолжается, даже когда открыт кластер отмены. Это поведение отличается от предыдущих версий представлений, которые не удалось выполнить эти операции до и после закрывающего кластера отмены.
