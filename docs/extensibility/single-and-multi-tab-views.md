...
## <a name="creating-multi-tabbed-views"></a>Создание представлений с несколькими вкладками
 ...
 Чтобы создать представление для редактора с несколькими вкладками, реализуйте интерфейс <xref:Microsoft.VisualStudio.Shell.Interop.IVsMultiViewDocumentView>, а затем свяжите другой GUID логического представления (<xref:Microsoft.VisualStudio.Shell.Interop.LogicalViewID>) с каждой создаваемой вкладкой.

 Редактор HTML Visual Studio — это пример редактора с представлением с несколькими вкладками. Он содержит вкладки **конструктора** и **исходного кода** . Чтобы сделать это, другое логическое представление связывается с каждой вкладкой, `LOGICALVIEWID_TextView` на вкладке « **конструктор** » и `LOGICALVIEWID_Code` на вкладке « **источник** ».
...
|`LOGVIEWID_TextView`|Представление текстового редактора. Это представление, которое возвращает <xref:Microsoft.VisualStudio.TextManager.Interop.IVsCodeWindow>, из которого можно получить доступ к <xref:Microsoft.VisualStudio.TextManager.Interop.IVsTextView>.|
