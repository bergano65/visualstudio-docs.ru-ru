...
|[SccSetOption](../extensibility/sccsetoption-function.md)|Универсальная функция, используемая для задания разнообразных параметров. Каждый параметр начинается с `SCC_OPT_xxx` и имеет свой собственный набор значений.|
...
|[SccPopulateList](../extensibility/sccpopulatelist-function.md)|Проверяет список файлов на предмет их текущего состояния. Кроме того, использует функцию `pfnPopulate` для уведомления вызывающего объекта о том, что файл не соответствует критериям для `nCommand`.|
