...
- Windows может автоматически масштабировать содержимое до 250% на новых устройствах, превышающих 280 PPI (начиная с Windows 8.1 S14).
  ...
  Windows (и Visual Studio) используют несколько технологий пользовательского интерфейса, которые имеют разные способы работы с коэффициентами масштабирования, заданными системой. Например:
...
Распространенные проблемы с макетом можно избежать в первую очередь, сохраняя точки в пользовательском интерфейсе и относительные, а не используя абсолютные расположения (в частности, в пикселных единицах). Например:
...
- Проект использования должен ссылаться на последнюю версию оболочки MPF. Например:
...
- В коде используйте пространство имен **Microsoft. VisualStudio. платформуи** и вызовите статические функции класса дпихелпер. Для поддерживаемых типов (точек, размеров, прямоугольников и т. д.) предусмотрены функции расширения, возвращающие новые масштабируемые объекты. Например:
...
Предобразировать изображение можно с помощью преобразователя, примененного к привязке, или с расширением разметки XAML. Например:
...
- Если размер исходного изображения неизвестен, можно использовать LayoutTransform для масштабирования объекта окончательного изображения. Например:
