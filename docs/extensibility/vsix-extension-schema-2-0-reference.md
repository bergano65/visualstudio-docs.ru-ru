...
-   `<Asset>` -Этот элемент содержит следующие атрибуты и элементы.  
    ...
    -   `TargetVersion` — диапазон версий, в котором к определенному активу. Используется для доставки несколько версий ресурсов для разных версий Visual Studio. Требуется Visual Studio 2017.3 или более поздней версии действовать.
