...
ms.locfileid: "72748969"
---
# <a name="ui-text-and-help-for-visual-studio"></a>Текст пользовательского интерфейса и Справка по Visual Studio
## <a name="BKMK_UITextAndTerminology"></a>Текст и терминология пользовательского интерфейса
...
#### <a name="title-bar-text"></a>Текст заголовка окна
...
#### <a name="supplemental-instructions"></a>Дополнительные инструкции
 ...
 ![Дополнительный текст в Visual Studio](../../extensibility/ux-guidelines/media/0601-b_supplementaltext1.png "0601-b_SupplementalText1")

 **Дополнительный текст в Visual Studio**

 ![Дополнительный текст в Visual Studio](../../extensibility/ux-guidelines/media/0601-c_supplementaltext2.png "0601-c_SupplementalText2")
...
#### <a name="infotips"></a>инфотипс
 ...
 ![Всплывающая подсказка в Visual Studio](../../extensibility/ux-guidelines/media/0601-d_infotip.png "0601-d_InfoTip")
...
|Термин|Объяснение|Добавление примечаний|
|----------|-----------------|-------------|
...
|Удаление|Деструктив. Используйте, только если удаляемый элемент будет безвозвратно удален с диска.|При удалении файла с диска обычно требуется запрос на подтверждение удаления.|
...
|Хорошо|Правильным|
|----------|---------|
|"Номер телефона больше не находится в службе. Проверьте номер и повторите вызов или наберите 0 для оператора. "|-"Ошибка (449): недопустимое число"<br />— "Эта ошибка необработанного исключения указывает на успешное завершение операции".<br /><br /> ![Неправильное сообщение об ошибке в Visual Studio](../../extensibility/ux-guidelines/media/0602-a_errordialog.png "0602-a_ErrorDialog ")|
...
##### <a name="dialogs-created-through-the-environment"></a>Диалоговые окна, созданные в среде
 ...
 В частности, функция **вбдиалогбокспарам** рассматривает шаблон диалогового окна для кнопки, идентификатор которой — **идхелп** (9) или label — **Справка** или **& справку**. Если кнопка "Справка" найдена, она скрыта, а стиль **WS_EX_CONTEXTHELP** добавляется в диалоговое окно, что помещает **?** в строке заголовка диалогового окна.

 Когда диалоговое окно будет создано, оно помещает процедуру диалогового окна в стек и вызывает диалоговое окно с предварительно обрабатывающей процедурой диалогового окна с именем **диалогпрепрок**. Когда **?** , она отправляет в диалоговое окно **WM_SYSCOMMAND** из **SC_CONTEXTHELP** . **Диалогпрепрок** захватывает эту команду и изменяет ее на сообщение **WM_HELP** , которое передается в исходную процедуру диалогового окна.
