...
|Редактор уведомления: волнистые линии|Используется для предупреждения пользователя проблемы с конкретного диапазона их открыть кода (например, красная волнистая линия для ошибок).|Не используйте для элементов, которые не связаны с определенного диапазона кода, их открыть.|  
