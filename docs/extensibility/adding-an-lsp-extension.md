...
В этой статье описывается создание расширения в Visual Studio, которая использует язык на основе LSP-сервер. Предполагается, что сервер язык на основе LSP уже разработали и просто хотите интегрировать в Visual Studio.
...
>**Примечание**: **URL-адрес загрузки** должно всегда быть заполнено, пользователи, устанавливающие расширение правильно установить необходимую зависимость.
...
Добавление поддержки для языка серверов LSP не требуется реализовывать собственную систему проектов в Visual Studio. Клиентам можно открыть один файл или папку в Visual Studio, чтобы приступить к использованию службы языка. На самом деле поддержка серверов языка LSP предназначен для работы только в сценариях, открыть папку или файл. Некоторые возможности, такие как параметры, не будет работать, если реализована в системе пользовательских проектов.

## <a name="advanced-features"></a>Дополнительные возможности

### <a name="settings"></a>Параметры

Поддержка пользовательского языка сервера, определенные параметры для предварительной версии LSP поддержки в Visual Studio, но это по-прежнему находится в процессе которое совершенствуется. Настройки, специфичные для что языка сервер поддерживает и обычно управлять как сервер языка передает данные. Например язык сервера могут быть параметр максимального числа ошибок. Авторы расширения может определить значение по умолчанию, который может быть изменен пользователями для конкретных проектов.
