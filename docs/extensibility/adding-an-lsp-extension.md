...
ms.locfileid: "66352320"
---
# <a name="add-a-language-server-protocol-extension"></a>Добавление расширения протокола языкового сервера

Протокол Server языка (LSP) — это общий протокол, в виде версии 2.0 JSON RPC, используемый для предоставления функций службы для различных редакторов кода языка. С помощью протокола, разработчики могут создавать что серверу предоставлять языковой службы для одного языка такими функциями, как IntelliSense, диагностические сведения об ошибках, найти все ссылки и т. д., для различных редакторов кода, которые поддерживают LSP. В большинстве случаев службы языка в Visual Studio можно добавлять с помощью файлов грамматики TextMate для предоставления основных функциональных возможностей, например выделение синтаксиса, или написав пользовательские языковые службы, использующие полный набор интерфейсов API расширяемости Visual Studio для обеспечения более подробные данные. Благодаря поддержке Visual Studio LSP имеется третий вариант.
...
Для получения поддержки в Visual Studio, язык серверы могут взаимодействовать с клиентом (Visual Studio) с помощью любого механизма потоковой передачи, например:

* Стандартных потоков ввода вывода
* Именованные каналы
* Сокеты (TCP)

Цель LSP и его поддержку в Visual Studio — встроенные языковые службы, которые не являются частью продукта Visual Studio. Он не должен расширить существующие службы языка (например C#) в Visual Studio. Чтобы расширить существующие языки, см. руководство по расширяемости языковой службы (например, [платформы компилятора .NET «Roslyn»](../extensibility/dotnet-compiler-platform-roslyn-extensibility.md)).
...
## <a name="language-server-protocol-supported-features"></a>Функции, поддерживаемые протоколом языкового сервера

В следующей таблице показаны компоненты LSP, которые поддерживаются в Visual Studio:
...
> [!NOTE]
> Начиная с Visual Studio 2017 версии 15.8, поддержка распространенных протоколом языкового сервера встроены в Visual Studio. Если вы создали модули LSP, с помощью предварительной версии [языка сервера клиента VSIX](https://marketplace.visualstudio.com/items?itemName=vsext.LanguageServerClientPreview) версии, они перестанет работать после обновления до версии 15,8 или более поздней версии. Необходимо выполнить следующие действия для получения расширений LSP работать снова:
>
> 1. Удалите Microsoft Visual Studio язык сервера протокола предварительную версию VSIX.
>
>    Начиная с версии 15.8, каждый раз при выполнении обновления в Visual Studio preview VSIX автоматически обнаруживать и удалять.
>
...
>
> 4. Убедитесь, что VSIX указывает Visual Studio 2017 версии 15.8 предварительной версии 3 в качестве нижней границы для целевого объекта установки.
>
> 5. Повторно создавать и развертывать.
...
Создайте проект VSIX, перейдя по адресу **файл** > **новый проект** > **Visual C#**   >  **Расширяемости** > **проект VSIX**:
...
По умолчанию расширений, созданных для поддержки языка на основе LSP серверов в Visual Studio не содержат самих серверов языка или среды выполнения, необходимые для их выполнения. Разработчики расширений несут ответственность за распределение серверов языка и среды выполнения требуется. Существует несколько способов для этого:
...
2. Внутри *грамматики* папки, содержащие любые  *\*.tmlanguage*,  *\*plist-файл в*,  *\*.tmtheme*, или  *\*.json* хотелось бы файлов, которые предоставляют пользовательские раскраски.

   > [!TIP]
   > Объект *.tmtheme* файл определяет, как области сопоставляются с классификацией Visual Studio (с именем цветовые ключи). Руководство, вы можете ссылаться на глобальную *.tmtheme* файл *% ProgramFiles (x86) %\Microsoft Visual Studio\\\<версии >\\\<SKU > \Common7\ IDE\CommonExtensions\Microsoft\TextMate\Starterkit\Themesg* каталога.

3. Создание *.pkgdef* файл и добавьте строку, аналогичную следующей:
...
4. Щелкните правой кнопкой мыши файлы, а затем выберите **свойства**. Изменить **построения** действие **содержимого** и измените **включить в VSIX** свойства **true**.
...
Затем можно создать новый класс, реализующий [ILanguageClient](/dotnet/api/microsoft.visualstudio.languageserver.client.ilanguageclient?view=visualstudiosdk-2017) интерфейс, который является основной интерфейс, необходимый для языка клиентов, подключающихся к серверу язык на основе LSP.
...
Visual Studio использует [MEF](https://github.com/Microsoft/vs-mef/blob/master/doc/index.md) (Managed Extensibility Framework) для управления точки расширения. [Экспорт](/dotnet/api/system.componentmodel.composition.exportattribute) атрибут указывает Visual Studio, что выбраны как точку расширения этого класса и они будут загружены в соответствующее время.
...
Нажмите кнопку **New** для создания нового ресурса:
...
Это осуществляется путем определения одной или нескольких `ContentTypeDefinition` классы:
...
В предыдущем примере создается определение типа содержимого для файлов, которые заканчиваются на *.bar* расширение файла. Определение типа содержимого присваивается имя «строка» и должен быть производным от [CodeRemoteContentTypeName](/dotnet/api/microsoft.visualstudio.languageserver.client.coderemotecontentdefinition.coderemotecontenttypename?view=visualstudiosdk-2017).
...
1. Добавить в JSON-файл (например, *MockLanguageExtensionSettings.json*) в проект, содержащий параметры и их значения по умолчанию. Пример:
...
2. Щелкните правой кнопкой мыши на JSON-файл и выберите **свойства**. Изменение **построения** действие «Content» и «включить в VSIX "Свойства **true**.
...
5. Щелкните правой кнопкой файл pkgdef и выберите **свойства**. Изменение **построения** действие **содержимого** и **включить в VSIX** свойства **true**.
...
#### <a name="receive-custom-messages"></a>Пользовательские сообщения.
