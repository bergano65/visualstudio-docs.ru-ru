---
title: Руководство. Миграция проектов расширяемости в Visual Studio 2017 | Документация Майкрософт
ms.date: 11/09/2016
ms.topic: conceptual
...
* Убедитесь, что `InstallationTarget` включает 15,0.
...
При поиске зависимостей вы обнаружите, что одна зависимость может сопоставляться с несколькими компонентами. Чтобы определить, какие зависимости следует указать в качестве необходимых компонентов, мы рекомендуем выбрать компонент, имеющий функциональность, аналогичный вашему расширению, а также принять во внимание пользователей и типы компонентов, которые, скорее всего, были установлены или не забывайте об установке. Мы также предлагаем создание расширений таким образом, чтобы необходимые условия соответствовали минимальным требованиям, которые позволят вашему модулю запускаться, а дополнительные функции — неактивным, если некоторые компоненты не обнаруживаются.
...
Список компонентов, отсортированных по продукту Visual Studio, находится в [рабочей нагрузке Visual studio 2017 и идентификаторах компонентов](/visualstudio/install/workload-and-component-ids?view=vs-2019). Используйте эти идентификаторы компонентов в манифесте для своих идентификаторов.
...
* Если у вас есть расширение отладчика и известно, что в проекте есть ссылка на *VSDebugEng. dll* и *всдебуг. dll*, нажмите кнопку Фильтр в заголовке **двоичные файлы/имена файлов** .  Выполните поиск по запросу "VSDebugEng. dll" и нажмите кнопку *ОК*.  Затем снова нажмите кнопку фильтра в заголовке **двоичные файлы/имена файлов** и выполните поиск по запросу "всдебуг. dll".  Установите флажок **Добавить текущее выделение для фильтрации** и нажмите кнопку **ОК**.  Теперь просмотрите **имя компонента** , чтобы найти компонент, который наиболее связан с типом расширения. В этом примере следует выбрать JIT-отладчик и добавить его в VSIXMANIFEST.
