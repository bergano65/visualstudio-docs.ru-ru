---
title: 'Практическое: перенос проектов расширяемости в Visual Studio 2017 | Документация Майкрософт'
ms.custom: ''
ms.date: 11/09/2016
ms.technology:
- vs-ide-sdk
...
>**Примечание:** как минимум, все расширения должны указать основного компонента редактора Visual Studio необходимый компонент.
...
### <a name="option-use-the-designer-to-make-changes-to-the-vsix-extension-manifest"></a>Вариант: Использование конструктора для изменения манифеста расширения VSIX
...
>**Примечание:** конструктор манифеста только позволяют выбрать компоненты (не рабочих нагрузок или пакеты), установленные в текущем экземпляре Visual Studio. Если вам нужно добавить необходимый компонент для рабочей нагрузки, пакета или компонента, который в настоящее время не установлен, отредактируйте сам файл манифеста XML.
...
* Обновите версию, если это необходимо.
  ...
  >Примечание: Значения поля версии будут уже заполнены с версией в настоящее время установленного компонента, с помощью диапазона охват до (но не включая) следующий основной номер версии компонента.
...
Это может выглядеть так: *\Microsoft Visual Studio\2017\Enterprise\Common7\IDE\devenv.exe C:\Program Files (x86)*
...
>**Примечание:** перед установкой любого расширения, завершите работу всех экземпляров Visual Studio.
...
>**Примечание:** этих процессов не будет завершать во время работы экземпляра Visual Studio. Убедитесь, что завершить работу всех экземпляров Visual Studio на компьютере - в том числе у других пользователей, и повторять попытки.
