---
title: 'Практическое: расширение перехода для Visual Studio | Документация Майкрософт'
ms.custom: ''
ms.date: 06/25/2017
ms.technology:
- vs-ide-sdk
...
В этом документе объясняется, как сделать приема-передачи между Visual Studio 2015 и Visual Studio 2017 проекты расширения среды. После завершения обновления, проекта будут иметь возможность открыть, построения, установить и запустить в Visual Studio 2015 и Visual Studio 2017.  Как ссылка, можно найти некоторые расширения, которые могут приема-передачи между Visual Studio 2015 и Visual Studio 2017 [здесь](https://github.com/Microsoft/VSSDK-Extensibility-Samples) в примерах расширения корпорации Майкрософт.
...
>**Примечание:** из-за изменения между версиями Visual Studio, некоторые вещи, которые работали в одной версии не будет работать другой. Убедитесь, что вы пытаетесь получить доступ к функции, доступны в обеих версиях или будет иметь расширение непредвиденные результаты.
...
* Из **обозревателе решений**, удалить *project.json* файл из проекта.
    * Это приведет к удалению *project.json* файл и удалите его из проекта.
...
>**Примечание:** Если проект содержал EnvDTE пакеты, их нужно добавить, щелкнув правой кнопкой мыши **ссылки** выбора **добавьте ссылку на** и добавив соответствующую ссылку.  Использование пакетов NuGet может вызывать ошибки при попытке сборки проекта.
...
>**Примечание:** Если вы решили сделать это с помощью конструктора VSIX в Visual Studio 2017, необходимо будет вручную изменить версия необходимого компонента, чтобы она стала совместима со всеми версиями Visual Studio 2017.  Это обусловлено конструктора будет вставлять минимальной версии в качестве текущей версией Visual Studio (например, 15.0.26208.0).  Тем не менее, так как другие пользователи могут иметь более ранней версии, необходимо вручную изменить на 15.0.
...
>**Примечание:** рекомендуется добавить ниже `<OutputType></OutputType>` тега.
...
>**Примечание:** зависание проекта с сообщением **при открытии файла**, принудительное завершение работы Visual Studio, перейдите в каталог проекта, Показать скрытые папки и удалите *.vs* папки.
