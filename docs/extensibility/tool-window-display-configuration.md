...
|----------|----------|----------|-----------------|  
|Имя|REG_SZ|«Краткое имя здесь»|Краткое имя, описывающее окна инструментов. Используется только для справки в реестре.|  
...
|*\<ИДЕНТИФИКАТОР GUID >*|REG_DWORD или REG_SZ|0 или описательная строка.|Необязательно. Имя входа должно быть GUID команды, требующие видимость. Значение содержит только строку подробное сообщение. Как правило, является значение `reg_dword` равно 0.|  
