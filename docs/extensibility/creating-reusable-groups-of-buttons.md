...
4.  Добавьте IDSymbol для каждой кнопки, который будет добавлен в группу, как показано в следующем примере.  
    ...
    ```  
     ...
     По умолчанию шаблон элемента команды создается группа с именем **MyMenuGroup** и кнопки, которая имеет имя, которое вы указали, вместе с IDSymbol запись для каждого.  
