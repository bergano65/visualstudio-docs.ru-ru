...
- Импортируйте файлы исходного кода для платформы управляемого пакета.
    ...
    2. Добавьте следующие блоки в файл проекта (непосредственно над блоками > импорта \<). Задайте для `ProjectBasePath` расположение файла *прожектбасе. Files* в только что загруженном коде платформы управляемого пакета. Может потребоваться добавить в путь обратную косую черту. В противном случае проект может не найти исходный код платформы управляемого пакета.
