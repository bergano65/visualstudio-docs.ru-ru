---
title: "Получение локальных значений | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
helpviewer_keywords: 
  ...
  - "Отладка локальных значений [отладка SDK]"
  - "вычисление выражений, получение локальных значений"
...
>  В Visual Studio 2015 этот способ реализации вычислители выражений является устаревшим. Сведения о реализации вычислители выражений CLR, см. в разделе [вычислители выражений CLR](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) и [управляемых образец средства оценки выражений](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample).  
 ...
 Чтобы получить значение локального, Visual Studio вызывает [GetPropertyInfo](../../extensibility/debugger/reference/idebugproperty2-getpropertyinfo.md) для этого локального. В этой реализации класса `CFieldProperty` реализует интерфейс IDebugProperty2 для каждой локальной.  
  
 Эта реализация `IDebugProperty2::GetPropertyInfo` выполняет следующие задачи:  
  
1.  Получает имя локальной переменной, свойства и атрибуты из [FIELD\_INFO](../../extensibility/debugger/reference/field-info.md) структуры заполнено, когда класс был создан и инициализирован.  
  
2.  Получает тип локальной переменной из [IDebugField](../../extensibility/debugger/reference/idebugfield.md) объекта.  
  
3.  Получает значение локальной переменной из `IDebugField` объекта. Это поле, привязанное к области памяти локального использования [IDebugBinder](../../extensibility/debugger/reference/idebugbinder.md) объект и значение получается из полученных [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) объекта.  
  
4.  Возвращает все запрошенные свойства в [DEBUG\_PROPERTY\_INFO](../../extensibility/debugger/reference/debug-property-info.md) структуру.  
  
## Управляемый код  
 В этом примере показана реализация `IDebugProperty2::GetPropertyInfo` метода в локальной в управляемом коде. Он также показывает вспомогательную функцию `Field.GetType`, который используется для получения типа поля.`Field.GetValue` отображается в [Определения значений локальных переменных](../../extensibility/debugger/evaluating-locals.md). Вспомогательная функция `Field.MapModifiersToAttributes` \(не показан\) просто преобразует значение поля [FIELD\_MODIFIERS](../../extensibility/debugger/reference/field-modifiers.md) флаги для [DBG\_ATTRIB\_FLAGS](../../extensibility/debugger/reference/dbg-attrib-flags.md) значения.  
...
## Неуправляемый код  
 В этом примере показана реализация `IDebugProperty2::GetPropertyInfo` метода в локальной в неуправляемом коде. В нем также показано две вспомогательные функции `FieldGetType` и `FieldGetValue` которые используются для получения типа поля и значения, соответственно. Обратите внимание, что `VARIANT`s используется для значения поля и введите в качестве `VARIANT` может обрабатывать самые разнообразные типы значений. В этой реализации `FieldGetValue` возвращает [IDebugField](../../extensibility/debugger/reference/idebugfield.md) объект, который затем преобразуется в значение при вызове `FieldGetPrimitiveValue` \(как показано в [Определения значений локальных переменных](../../extensibility/debugger/evaluating-locals.md)\).  
