...
|metricENC|Значение не равно нулю, чтобы указать поддержку для изменить и продолжить. **Примечание.**  Никогда не следует задавать пользовательского модуля отладки, или следует всегда равно 0.|
...
|metricEncUseNativeBuilder|Задайте ненулевое значение для указания, изменить и продолжить Manager следует использовать модуль отладки encbuild.dll для создания изменить и продолжить. **Примечание.**  Никогда не следует задавать пользовательского модуля отладки, или следует всегда равно 0.|
...
|*[метрик root]*|Это может быть либо `AD7Metrics` или `AD7Metrics(Debug)`зависимости от того, используется ли отладочная версия dbgmetric.lib. **Примечание.**  Независимо от того, имеется ли dbgmetric.lib используется, это соглашение об именовании следует соблюдать при наличии различий между отладочной и окончательной версий, которые должны быть отражены в реестре.|
...
## <a name="requirements"></a>Требования
 Эти файлы расположены в [!INCLUDE[vs_dev10_ext](../../../extensibility/debugger/reference/includes/vs_dev10_ext_md.md)] каталог установки SDK (по умолчанию *[диск]* \Program Files\Microsoft Visual Studio 2010 SDK\\).
