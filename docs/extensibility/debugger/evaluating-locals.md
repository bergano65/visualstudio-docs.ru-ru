---
title: "Определения значений локальных переменных | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
ms.topic: "article"
helpviewer_keywords: 
  - "отладка [пакет SDK для отладки], для определения значений локальных переменных"
  - "вычисление выражений, определения значений локальных переменных"
...
manager: "ghogen"
---
# Определения значений локальных переменных
[!INCLUDE[vs2017banner](../../code-quality/includes/vs2017banner.md)]

> [!IMPORTANT]
>  В Visual Studio 2015 этот способ реализации вычислители выражений является устаревшим. Сведения о реализации вычислители выражений CLR, см. в разделе [вычислители выражений CLR](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/CLR-Expression-Evaluators) и [управляемых образец средства оценки выражений](https://github.com/Microsoft/ConcordExtensibilitySamples/wiki/Managed-Expression-Evaluator-Sample).  
  
 [GetPropertyInfo](../../extensibility/debugger/reference/idebugproperty2-getpropertyinfo.md) вызывается, чтобы получить значение local, а также локальной переменной имя и тип. Поскольку значение локального зависит от текущего состояния программы, необходимо получить значение локальной переменной из памяти.[IDebugBinder](../../extensibility/debugger/reference/idebugbinder.md) Объект используется для привязки [IDebugField](../../extensibility/debugger/reference/idebugfield.md) объект, представляющий локальной в соответствующее расположение в памяти, содержащей значение. Это расположение в памяти представляется [IDebugObject](../../extensibility/debugger/reference/idebugobject.md) объекта.  
  
 Эта функциональность получение значения локальной инкапсулируется в вспомогательная функция, которая выполняет следующие задачи:  
  
1.  Привязывает `IDebugField` объект в памяти, чтобы получить `IDebugObject` объекта.  
...
## Управляемый код  
 Это реализация функции, которая возвращает значение локального в управляемом коде.  
...
## Неуправляемый код  
 Это реализация функции, которая возвращает значение локального в неуправляемом коде.`FieldGetType` отображается в [Получение локальных значений](../../extensibility/debugger/getting-local-values.md).  
