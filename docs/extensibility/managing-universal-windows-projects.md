...
1. Создайте проект C# VSIX с именем **тестуниверсалпрожект**. (**Файл**  > **Новый**  > **проект** , а **C#** затем  > **расширяемость**  > **пакета Visual Studio**). Добавьте шаблон **настраиваемого командного** элемента проекта (на **Обозреватель решений**щелкните правой кнопкой мыши узел проекта и выберите **добавить**  > **новый элемент**, затем перейдите в раздел **расширяемость**). Назовите файл **тестуниверсалпрожект**.

2. Добавьте ссылку на *Microsoft. VisualStudio. Shell. Interop. 12,1. DesignTime. dll* и *Microsoft. VisualStudio. Shell. Interop. DesignTime. dll* (в разделе **Extensions** ).

3. Откройте *TestUniversalProject.CS* и добавьте следующие директивы `using`:
...
4. В классе `TestUniversalProject` Добавьте закрытое поле, указывающее на окно **вывод** .
...
6. Удалите существующий код из метода `ShowMessageBox`:
...
8. Найдите общий проект. Общий проект является чистым контейнером; Он не создает и не создает выходные данные. Следующий метод находит первый общий проект в решении, выполняя поиск объекта <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchy>, имеющего возможность совместного использования проекта.
...
9. В методе `ShowMessageBox` выводится заголовок (имя проекта, которое отображается в **Обозреватель решений**) общего проекта.
...
11. В методе `ShowMessageBox` выводит заголовок активного проекта платформы.
...
12. Выполните итерацию по проектам платформы. Следующий метод получает все импортированные (платформы) проекты из общего проекта.
    ...
    > [!IMPORTANT]
    > Если пользователь открыл проект C++ универсального приложения Windows в экспериментальном экземпляре, приведенный выше код выдаст исключение. Это известная проблема. Чтобы избежать исключения, замените блок `foreach` выше на следующий:
...
13. В методе `ShowMessageBox` выводит заголовок каждого проекта платформы. Вставьте следующий код после строки, в которой выводится заголовок активного проекта платформы. В этом списке отображаются только загруженные проекты платформы.
...
15. В методе `ShowMessageBox` измените активный проект платформы. Вставьте этот код в блок `foreach`.
...
16. Теперь попробуйте. Нажмите клавишу F5, чтобы запустить экспериментальный экземпляр. Создайте проект C# универсального приложения-концентратора в экспериментальном экземпляре (в диалоговом окне " **Новый проект** " **Visual C#**   > **Windows**  > **Windows 8**  > **универсальное** **приложение концентратора**0). После загрузки решения перейдите в меню **Сервис** и выберите команду **вызвать тестуниверсалпрожект**, а затем проверьте текст в области **вывода** . Вы должны увидеть примерно следующее:
...
2. В методе `ShowMessageBox` добавьте следующий код для пошагового анализа элементов иерархии проекта платформы. Вставьте его в блок `foreach`.
...
4. Теперь попробуйте. Нажмите клавишу **F5** , чтобы запустить экспериментальный экземпляр. Создание проекта C# универсального приложения-концентратора в экспериментальном экземпляре (в диалоговом окне " **Создание проекта** " **Visual C#**   > **Windows**  > **Windows 8**  > **универсальное** **приложение концентратора**0) Перейдите в меню **Сервис** и выберите команду **вызвать тестуниверсалпрожект**, а затем проверьте текст в области **вывода** . Вы должны увидеть примерно следующее:
...
1. Вы можете использовать события иерархии и проекта для обнаружения изменений в общих проектах так же, как и для проектов платформы. Однако элементы проекта в общем проекте не отображаются, что означает, что определенные события не срабатывают при изменении общих элементов проекта.
   ...
   2. Файл проекта обновляется и включает новое имя файла.
      ...
      События иерархии (например, <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents>) обычно отписывают изменения, отображаемые в пользовательском интерфейсе, как в **Обозреватель решений**. События иерархии рассматривайте, что операция переименования файлов состоит из удаления файлов, а затем добавления файлов. Однако при изменении невидимых элементов система событий иерархии запускает событие <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A>, но не событие <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A>. Таким образом, при переименовании файла в проекте платформы вы получаете как <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A>, так <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A>, но при переименовании файла в общем проекте вы получаете только <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A>.

      Для наблюдения за изменениями в элементах проекта можно выполнять обработку событий элементов проекта DTE (которые находятся в <xref:EnvDTE.ProjectItemsEventsClass>). Однако при обработке большого количества событий можно повысить производительность обработки событий в <xref:Microsoft.VisualStudio.Shell.Interop.IVsTrackProjectDocuments2>. В этом пошаговом руководстве показаны только события иерархии и события DTE. В этой процедуре прослушиватель событий добавляется в общий проект и проект платформы. Затем при переименовании одного файла в общем проекте и другом файле в проекте платформы можно просмотреть события, которые запускаются для каждой операции переименования.
...
4. Реализуйте <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents> класса `HierarchyEventListener`:
...
6. В том же классе добавьте еще один обработчик событий для <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed> события DTE, который происходит при каждом переименовании элемента проекта.
...
10. Вызовите этот метод после того, как весь другой код в `ShowMessageBox` изменит имя файла на элемент в общем проекте. Вставьте его после кода, который получает полный путь к элементу в общем проекте.
...
11. Постройте и запустите проект. Создайте приложение C# универсального концентратора в экспериментальном экземпляре, откройте меню **Сервис** и выберите команду **вызвать тестуниверсалпрожект**и проверьте текст в области "общие выходные данные". Необходимо изменить имя первого элемента в общем проекте (предполагается, что он является файлом *app. XAML* ), и вы увидите, что событие <xref:EnvDTE.ProjectItemsEventsClass.ItemRenamed> вызвано. В этом случае, поскольку при переименовании файла *app. xaml* *app.XAML.CS* также должны отображаться четыре события (два для каждого проекта платформы). (События DTE не отслеживания элементов в общем проекте.) Вы должны увидеть два события <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A> (по одному для каждого проекта платформы), но не <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A> события.
...
13. Постройте и запустите проект. Создайте C# универсальный проект в экспериментальном экземпляре, перейдите в меню " **Сервис** " и выберите команду " **вызвать тестуниверсалпрожект**" и проверьте текст в области "общие выходные данные". После переименования файла в проекте платформы вы увидите как событие <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemAdded%2A>, так и событие <xref:Microsoft.VisualStudio.Shell.Interop.IVsHierarchyEvents.OnItemDeleted%2A>. Поскольку изменение файла не привело к изменению других файлов, и, поскольку изменения элементов в проекте платформы не будут распространяться в любом месте, существует только одно из этих событий.
