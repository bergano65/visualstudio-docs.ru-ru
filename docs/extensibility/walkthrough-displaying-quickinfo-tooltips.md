...
1. Создайте проект C# VSIX. (В диалоговом окне **Новый проект** выберите **элемент C# визуальный/расширяемый**, а затем **проект VSIX**.) Присвойте решению имя `QuickInfoTest`.
...
4. Объявите класс, реализующий <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSource>, и присвойте ему имя `TestQuickInfoSource`.

     [!code-vb[VSSDKQuickInfoTest#2](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-quickinfo-tooltips_2.vb)]
     [!code-csharp[VSSDKQuickInfoTest#2](../extensibility/codesnippet/CSharp/walkthrough-displaying-quickinfo-tooltips_2.cs)]

5. Добавьте поля для поставщика источника краткие сведения, текстового буфера и набора имен методов и сигнатур методов. В этом примере имена и подписи методов инициализируются в конструкторе `TestQuickInfoSource`.
...
8. Также необходимо реализовать метод Dispose (), так как <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSource> реализует <xref:System.IDisposable>:
...
1. Объявите поставщик источника краткие сведения с именем `TestQuickInfoSourceProvider`, который реализует <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSourceProvider>, и экспортируйте его с <xref:Microsoft.VisualStudio.Utilities.NameAttribute>ом "ToolTip краткие сведения Source", <xref:Microsoft.VisualStudio.Utilities.OrderAttribute> до = "default" и <xref:Microsoft.VisualStudio.Utilities.ContentTypeAttribute> "Text".
...
3. Реализуйте <xref:Microsoft.VisualStudio.Language.Intellisense.IQuickInfoSourceProvider.TryCreateQuickInfoSource%2A> для возврата нового `TestQuickInfoSource`.
...
1. Объявите класс, реализующий <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController>, и присвойте ему имя `TestQuickInfoController`.
...
5. Реализуйте метод <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController.Detach%2A>, чтобы он удаляет обработчик событий наведения указателя мыши при отсоединении контроллера от текстового представления.

     [!code-vb[VSSDKQuickInfoTest#14](../extensibility/codesnippet/VisualBasic/walkthrough-displaying-quickinfo-tooltips_14.vb)]
     [!code-csharp[VSSDKQuickInfoTest#14](../extensibility/codesnippet/CSharp/walkthrough-displaying-quickinfo-tooltips_14.cs)]

6. Реализуйте метод <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController.ConnectSubjectBuffer%2A> и метод <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseController.DisconnectSubjectBuffer%2A> в качестве пустых методов для этого примера.
...
1. Объявите класс с именем `TestQuickInfoControllerProvider`, который реализует <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseControllerProvider>, и экспортируйте его с <xref:Microsoft.VisualStudio.Utilities.NameAttribute>ом "Подсказка контроллера краткие сведения" и <xref:Microsoft.VisualStudio.Utilities.ContentTypeAttribute> "Text":
...
3. Реализуйте метод <xref:Microsoft.VisualStudio.Language.Intellisense.IIntellisenseControllerProvider.TryCreateIntellisenseController%2A>, создав экземпляр контроллера краткие сведения.
...
4. Наведите указатель мыши на одно из вхождений "Добавить". Должна отобразиться подпись и описание метода `add`.
