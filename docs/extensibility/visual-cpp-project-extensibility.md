...
Если интерпретируемые TLOG-файлы создаются задачей, желательно использовать эти вспомогательные классы для их создания. Тем не менее так как проверка наличия обновлений по умолчанию теперь использует только интерпретируемые TLOG-файлы, иногда удобнее для создания их в целевом объекте без задачу. Их можно написать с помощью `WriteLinesToFile` задачи. См. в разделе `_WriteMasmTlogs` target в `$(VCTargetsPath)` \\ *BuildCustomizations*\\*masm.targets* в качестве примера.
...
Общие сведения, см. в разделе [расширяемость платформы — часть 1](http://blogs.msdn.com/b/vsproject/archive/2009/06/10/platform-extensibility-part-1.aspx) и [расширяемость платформы — часть 2](http://blogs.msdn.com/b/vsproject/archive/2009/06/18/platform-extensibility-part-2.aspx).
