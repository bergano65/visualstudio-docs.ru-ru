...
|Строковое|Символ представляет произвольное строковое значение|
...
|-|-|
|**Attribute (XElement Dynamic Property)** (Attribute (динамическое свойство XElement))|**Определение**|
|URI|Необходимости URI, который определяет, откуда можно загрузить изображение. Может быть одним из следующих:<br /><br /> — URI типа " [Pack](/dotnet/framework/wpf/app-development/pack-uris-in-wpf) " с помощью центра Application:///.<br />— Абсолютная ссылка на ресурс компонента.<br />— Путь к файлу, содержащему машинный ресурс;|
...
1. Замените все >ные элементы \<Image в пользовательском интерфейсе элементами \<Криспимаже >.
...
3. Вкладка окно инструментов, когда вкладки достаточно малы (также используются в окне переключения **клавиш Ctrl**+**Tab** ).
   ...
   2. См. шаг #1 из раздела "Разделы справки использовать моникеры изображений в новом окне инструментов". разделе выше.

4. Команда для открытия окна инструментов.

   - См. шаг #2 из раздела "Разделы справки использовать моникеры изображений в новом окне инструментов". разделе выше.
 ...
 **Иконмаппингфиленаме** является либо относительным путем, неявно корнем в $PackageFolder $ (как в примере выше), либо абсолютным путем, явно корневым в каталоге, определенном переменной среды, например *@ "%UserProfile%\dir1\dir2\ Мимаппингфиле. csv*.
...
1. Реализуйте **VSHPROPID_SupportsIconMonikers** в **IVsHierarchy**проекта и возвращайте значение true.

2. Реализуйте любой из **VSHPROPID_IconMonikerImageList** (если исходный проект использовал **VSHPROPID_IconImgList**) или **VSHPROPID_IconMonikerGuid**, **VSHPROPID_IconMonikerId**, **VSHPROPID_OpenFolderIconMonikerGuid**, **VSHPROPID_OpenFolderIconMonikerId** (если исходный проект использовал **VSHPROPID_IconHandle** и **VSHPROPID_OpenFolderIconHandle**).
...
3. Измените реализацию исходного Вшпропидс для значков, чтобы создать "устаревшие" версии значков, если точки расширения запрашивают их. **IVsImageService2** предоставляет функциональные возможности, необходимые для получения этих значков
   ...
   **Дополнительные требования для разновидностейC# VB/Project**
   ...
   Реализуйте **VSHPROPID_SupportsIconMonikers** только в том случае, если обнаружите, что проект является самой **внешней разновидностью**. В противном случае фактическая внешний флаг может не поддерживать моникеры изображений в реальности, а базовый вариант может эффективно "скрывать" настроенные образы.
...
2. Если вы используете только **кновнмоникерс**, выполните следующие действия.
   ...
   - Добавьте < импорт манифеста = "$ (Манифестфолдер)\\< путь к папке с относительным каталогом установки,\>\Микрософт.висуалстудио.имажекаталог.имажеманифест"/\*> в начало раздела \<символы >.
...
## <a name="testing-your-images"></a>Тестирование образов
 Средство просмотра библиотеки изображений можно использовать для тестирования манифестов изображений, чтобы убедиться, что все создано правильно. Это средство можно найти в [пакете SDK для Visual Studio 2015](visual-studio-sdk.md). Документацию по этому средству и другим пользователям можно найти [здесь](https://aka.ms/VSImageThemeTools).
