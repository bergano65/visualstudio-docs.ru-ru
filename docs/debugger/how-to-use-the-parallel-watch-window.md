---
title: Задать контрольное значение для переменных в параллельных потоках | Документация Майкрософт
ms.date: 04/25/2017
ms.topic: conceptual
f1_keywords:
...
---
# <a name="set-a-watch-on-variables-in-parallel-threads-in-visual-studio-c-visual-basic-c"></a>Задать контрольное значение для переменных в параллельных потоках вC#Visual Studio ( C++, Visual Basic,)
...
- Столбец текущего потока, в котором желтая стрелка указывает текущий поток (зеленая стрелка с фигурным хвостовиком указывает, что у нетекущего потока есть текущий контекст отладчика).

- Настраиваемый столбец, который может отображать компьютер, процесс, блок потоков, задачу и поток.

  > [!TIP]
  > Чтобы отобразить сведения о задаче в окне **контроль параллельных** данных, необходимо сначала открыть окно **задачи** .

- Пустые столбцы *добавления контрольных значений* , в которых можно вводить выражения для просмотра.
...
- Выберите одно из пустых столбцов *Добавить контрольное значение* , а затем введите выражение контрольного значения.

### <a name="to-flag-or-unflag-a-thread"></a>Пометка и снятие пометки потока

- Выберите столбец флага для строки (первый столбец) или откройте контекстное меню для потока и выберите **флаг или снять** **пометку**.

### <a name="to-display-only-flagged-threads"></a>Отображение только помеченных потоков

- Нажмите кнопку **Показывать только помеченные** в левом верхнем углу окна " **Параллельные контрольные значения** ".

### <a name="to-switch-to-another-thread"></a>Переключение на другой поток

- Дважды щелкните столбец текущий поток (второй столбец). (Клавиатура: выберите строку и нажмите клавишу ВВОД.)
