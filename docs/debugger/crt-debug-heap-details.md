...
Отладочные версии функций кучи вызывают стандартные или базовые версии, используемые в окончательных построениях. Когда запрашивается блок памяти, диспетчер отладочной кучи выделяет из основной кучи блок памяти немного больше требуемого, и возвращает указатель на часть этого блока. Например, допустим, приложение содержит вызов: `malloc( 10 )`. В сборке выпуска [malloc](/cpp/c-runtime-library/reference/malloc) вызывает подпрограммы выделения базовой кучи, запрашивая выделение 10 байт. Однако в отладочной сборке `malloc` вызовом [_malloc_dbg](/cpp/c-runtime-library/reference/malloc-dbg), который затем вызывает подпрограммы выделения базовой кучи для запроса выделения 10 байтов плюс примерно 36 байт дополнительной памяти. Все результирующие блоки памяти в отладочной куче подключаются к единому связанному списку, который упорядочивает их по времени выделения.
...
Номансланд (0xFD). буферы "Номансланд" с любой стороны памяти, используемой приложением, в настоящее время заполнены 0xFD.

Освобожденные блоки (0xDD). освобожденные блоки, которые не используются в связанном списке отладочной кучи, когда установлен флаг `_CRTDBG_DELAY_FREE_MEM_DF`, в настоящее время заполняются 0xDD.

Новые объекты (0xCD) заполняются 0xCD при их выделении.

К [содержимому](#BKMK_Contents) ![сверху](../debugger/media/pcs_backtotop.png "PCS_BackToTop")
...
**_NORMAL_BLOCK** Вызов функции [malloc](/cpp/c-runtime-library/reference/malloc) или [calloc](/cpp/c-runtime-library/reference/calloc) создает нормальный блок. Если планируется использование только обычных блоков и не требуются клиентские боки, можно определить [_CRTDBG_MAP_ALLOC](/cpp/c-runtime-library/crtdbg-map-alloc), преобразующий все вызовы выделений кучи в их отладочные эквиваленты в отладочном построении. Такой подход позволяет хранить сведения об имени файла и номере строки для каждого выделения в заголовке соответствующего блока.
...
**_FREE_BLOCK** Обычно освобожденные блоки удаляются из списка. Чтобы удостовериться, что в освобожденную память не продолжается запись, или чтобы эмулировать условия нехватки памяти, можно выбрать хранение освобожденных блоков в связанном списке, помеченных как свободные и заполненных фиксированным значением байта (на данный момент 0xDD).

**_IGNORE_BLOCK** Операции отладочной кучи можно отключить в течение определенного периода времени. В течение этого времени блоки памяти хранятся в списке, но помечаются как пропускаемые блоки.
...
2. Включите любой бит путем `OR` (Побитовый &#124; символ) в качестве временной переменной с соответствующими битовыми масками (представленными в коде приложения с помощью констант манифеста).

3. Отключите другие биты путем `AND` (Побитовый символ &) для переменной с `NOT` (битовый символ ~) соответствующих битовых масок.
