...
Эффективное использование отладчика также навыка, требуется затратить время и способ узнать, но в конечном счете — это основная задача для любой разработчик программного обеспечения. В этой статье мы описываются основные принципы отладки затем ряд советов, чтобы приступить к работе.
...
* Что произошло на самом деле?
    ...
    В случае ошибки (исключение) при выполнении приложения, который может быть хорошо! Исключение при выполнении кода, обычно ошибка какого-либо непредвиденное событие. Средства отладки можно перейти к точное место в коде, в которой возникло исключение, а также помогают исследовать возможные исправления.
...
* Вы используете API правильно? Возможно вы использовали справа API, но не использовали ее надлежащим образом.
...
1. Выберите шаблон из кода приложения.
    ...
    Если вы не видите эти шаблона, необходимо установить соответствующую рабочую нагрузку (см. в статье ранее).
...
1. Нажмите кнопку **перезапустите** ![перезапустить приложение](../debugger/media/dbg-tour-restart.png "RestartApp") кнопку на панели инструментов отладки (**Ctrl** + **Shift**   +  **F5**).
    ...
    Приложение приостановит выполнение в заданной точке останова. Желтый hightlighting указывает, где работа отладчика была приостановлена (желтая линия кода еще не выполнена).
