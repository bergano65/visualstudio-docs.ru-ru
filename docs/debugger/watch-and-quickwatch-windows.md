...
## <a name="dynamic-view-and-the-watch-window"></a>Динамическое представление и окно контрольных значений  
 ...
 Когда окно наблюдения отображает объект, созданный из типа, который реализует [интерфейс IDynamicMetaObjectProvider](/dotnet/api/system.dynamic.idynamicmetaobjectprovider?view=netframework-4.7), отладчик добавляет специальный **динамическое представление** узел **"Видимые"**  отображения. В этом узле показаны динамические члены динамического объекта, но не разрешено изменять их значения.  
