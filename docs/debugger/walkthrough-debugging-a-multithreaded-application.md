 ...
 **Потоков** окно содержит таблицу, где каждая строка описывает отдельный поток в приложении. По умолчанию в таблице перечислены все потоки в приложении, но можно отфильтровать список, чтобы показать только нужные потоки. Каждый столбец описывает свой тип сведений. Можно также скрывать некоторые столбцы. Если отображаются все столбцы, отображаются следующие столбцы слева направо:  
  
- **Флаг**: В этом столбце без метки, можно пометить поток, к которому требуется уделить особое внимание. Сведения о способе пометки потока см. в разделе [как: флаг и снять отметку с потока](../debugger/how-to-flag-and-unflag-threads.md).  
...
- **Идентификатор**: отображается идентификационный номер для каждого потока.  
...
- **Приоритет**: дополнительным столбцом и (скрытый по умолчанию) отображает приоритет или приоритет, назначенный системой каждому потоку.  
...
- **Имя процесса**: дополнительным столбцом и (скрытый по умолчанию) отображает процесс, к которому принадлежит каждый поток. Данные в этом столбце можно использовать при отладке много процессов.  

- **Идентификатор процесса**: идентификатор дополнительным столбцом и (скрытый по умолчанию) отображает процесс, к которому принадлежит каждый поток. 

- **Квалификатор транспортного протокола**: расширенные столбец (скрытый по умолчанию), однозначно определяющий машины, к которому подключен отладчик. 
...
## <a name="display-flagged-threads"></a>Отображение помеченных потоков  
 Поток, которому требуется уделить особое внимание, помечая его с ним значок в можно пометить **потоков** окна. Дополнительные сведения см. в разделе [как: флаг и снять отметку с потока](../debugger/how-to-flag-and-unflag-threads.md). В **потоков** окна, можно выбрать для отображения всех потоков или только помеченных потоков.  
...
## <a name="group-and-sort-threads"></a>Группировка и Сортировка потоков  
 При группировании потоков в таблице появляется заголовок для каждой группы. В заголовке содержится описание группы, такие как **рабочий поток** или **непомеченные потоки**и древовидным элементом управления. Потоки-элементы каждой группы отображаются под заголовком группы. Если вы хотите скрыть потоки-элементы для группы, используйте элемент управления дерева, чтобы свернуть группу.  
...
## <a name="search-for-specific-threads"></a>Поиск конкретных потоков  
 Можно искать потоки, соответствующие указанной строке в **потоков** окна. При поиске потоков в окне отображаются все обсуждения, соответствующие строке поиска в любом столбце. Эти сведения включают расположение потока, которое отображается в верхней части стека вызовов в **расположение** столбца. По умолчанию не поиск полный стек вызова.  
  
### <a name="to-search-for-specific-threads"></a>Поиск конкретных потоков  
  
1. На панели инструментов в верхней части **потоков** окно, перейдите к **поиска** поле и либо:  
...
2. (Необязательно) Чтобы включить полный стек вызовов в поиск, выберите **поиск в стеке вызова**.   
