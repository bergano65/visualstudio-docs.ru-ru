...
::: moniker range=">=vs-2019"
 При запуске 64-разрядной версии удаленного отладчика по умолчанию использует основной порт (4024).  При отладке 32-разрядного процесса в 64-разрядной версии удаленного отладчика запускается 32-bit версия удаленного отладчика через порт 4025 (номер основного порта, увеличивающийся на 1). Если вы запускаете 32-разрядный удаленный отладчик, он использует порт 4024, а порт 4025 не используется.
::: moniker-end
::: moniker range="vs-2017"
 При запуске 64-разрядной версии удаленного отладчика по умолчанию использует основной порт (4022).  При отладке 32-разрядного процесса в 64-разрядной версии удаленного отладчика запускается 32-bit версия удаленного отладчика через порт 4023 (номер основного порта, увеличивающийся на 1). Если вы запускаете 32-разрядный удаленный отладчик, он использует порт 4022, а порт 4023 не используется.
:::moniker-end
