...
ms.locfileid: "50050116"
---
# <a name="find-memory-leaks-with-the-crt-library"></a>Находите утечки памяти с помощью библиотеки CRT
...
**Чтобы задать точку останова для выделения памяти в окне контрольных значений:**  
...
1. Нажмите клавишу **ВВОД**.  
   ...
   Отладчик выполнит оценку вызова и поместит результат в столбец **Значение** . Это значение будет **-1** Если вы не указали ни одной точки останова на выделение памяти.  
...
Точки останова для выделения памяти можно также задать в коде. Можно задать:  
