...
-   Первые 16 байт данных в блоке, в шестнадцатеричном формате.  
 ...
 Эти способы работают для памяти, выделенной с помощью стандартной функции `malloc` библиотеки CRT. Если программа выделяет память с использованием C++ `new` оператора, однако может отображается только имя файла и номер строки где реализация глобального `operator new` вызовы `_malloc_dbg` в отчете об утечке памяти. Так как это поведение не очень полезен, можно изменить его для передачи строки, выделение, выполненные с помощью макрос, который выглядит следующим образом: 
...
Теперь вы можете заменить `new` оператора с помощью `DBG_NEW` макрос в коде. В отладочных построениях, этот механизм использует перегрузку глобального `operator new` , принимает дополнительные параметры для типа блока, файла и номер строки. Эта перегрузка `new` вызовы `_malloc_dbg` для записи дополнительных сведений. При использовании `DBG_NEW`, утечка памяти сообщает, Показать имя файла и номер строки, где было выделено потерянные объекты. В окончательных сборках, используется значение по умолчанию `new`. (Не рекомендуется создавать макрос препроцессора с именем `new`, или ключевым словом.) Ниже приведен пример этого метода:  
...
Это означает, что потерянная выделение было предназначено в строке 20 debug_new.cpp.  
...
4.  Нажмите клавишу **ВВОД**.  
     ...
     Отладчик выполнит оценку вызова и поместит результат в столбец **Значение** . Это значение будет значение -1, если не заданы ни одной точки останова для выделения памяти.  
