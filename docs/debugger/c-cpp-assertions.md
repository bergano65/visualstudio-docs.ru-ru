...
##  <a name="BKMK_MFC_assertions"></a> Утверждения MFC  
 ...
 Когда утверждение ложно, диалоговое окно сообщения отображает имя исходного файла и номер строки утверждения. Если выбрать "Повторить" в диалоговом окне поле вызов [AfxDebugBreak](/cpp/mfc/reference/diagnostic-services#afxdebugbreak) прерывает выполнение программы в отладчике. В этот момент можно для определения причины ложности утверждения проверить стек вызовов и использовать другие функции отладчика. При включении [непосредственно времени отладки](../debugger/just-in-time-debugging-in-visual-studio.md)и отладчик еще не запущен, диалоговое окно может запустить отладчик.  
...
```  
 ...
 Макрос `ASSERT` не создает никакого кода в окончательной версии. Если вам нужно оценить выражение в окончательной версии, используйте [ПРОВЕРЬТЕ](/cpp/mfc/reference/diagnostic-services#verify) макрос вместо ASSERT.  
