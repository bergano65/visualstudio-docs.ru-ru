---
title: C/C++ утверждения | Документация Майкрософт
ms.date: 11/04/2016
ms.topic: conceptual
dev_langs:
- CSharp
...
Visual Studio поддерживает C++ операторы утверждений, основанные на следующих конструкциях:
...
Если программа работает корректно, эти операторы ничего не делают. Если логическая ошибка приводит к тому, что `numMols` меньше нуля, утверждение прекращает выполнение программы и отображает [диалоговое окно «Сбой утверждения](../debugger/assertion-failed-dialog-box.md)».
...
Если код, обрабатывающий ошибку, работает правильно, ошибка будет обработана и `myErr` сбросится в ноль, не доходя до утверждения. Если `myErr` имеет другое значение, утверждение не выполняется, программа останавливается и появляется [диалоговое окно "сбой утверждения](../debugger/assertion-failed-dialog-box.md) ".
