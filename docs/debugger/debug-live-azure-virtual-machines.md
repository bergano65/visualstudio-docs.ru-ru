---
title: Динамической отладки ASP.NET Azure виртуальные машины и масштабируемые наборы виртуальных машин Azure
...
ms.locfileid: "57007279"
---
# <a name="debug-live-aspnet-apps-on-azure-virtual-machines-and-azure-virtual-machine-scale-sets-using-the-snapshot-debugger"></a>Отладка работающих приложений ASP.NET на виртуальных машинах Azure и Azure масштабируемых наборов виртуальных машин с помощью отладчика моментальных снимков
...
* Отладчик моментальных снимков для виртуальных машин (ВМ) Azure и Azure наборов масштабирования виртуальных машин (VMSS) только доступной в предварительной версии Visual Studio Enterprise 2019 г. или более поздней версии с **рабочую нагрузку разработки Azure**. (В разделе **отдельные компоненты** вкладке его найти в разделе **отладка и тестирование** > **Snapshot debugger**.)
...
* Коллекция моментальных снимков доступна для следующих веб-приложений Azure VM/VMSS:
...
1. Откройте проект, который вы хотите отладки моментальных снимков.
    ...
    > [!IMPORTANT]
    > Отладка моментальных снимков, необходимо открыть *же версию исходного кода* , опубликованный в службе Azure VM/VMSS.

1. Подключите Snapshot Debugger. Можно использовать один из нескольких способов:

    * Выберите **Отладка > подключить Snapshot Debugger...** . Выберите Azure VM/VMSS развертывается веб-приложения и учетную запись хранения, а затем нажмите кнопку **Attach**.

      ![Отладчик моментальных снимков, из меню "Отладка"](../debugger/media/snapshot-debug-menu-attach.png)

    * Щелкнуть правой кнопкой мыши проект и выберите **публикации**, а затем на странице публикации щелкните **подключить отладчик моментальных снимков**. Выберите Azure VM/VMSS развертывается веб-приложения и учетную запись хранения, а затем нажмите кнопку **Attach**.
    ![Запустить отладчик моментальных снимков на странице публикация](../debugger/media/snapshot-publish-attach.png)

    * В отладочной ориентироваться в раскрывающемся меню выберите **Snapshot Debugger**, нажатия **F5** и при необходимости выберите Azure VM/VMSS развертывается веб-приложения и службы хранилища Azure учетной записи и нажмите кнопку  **Присоединение**.
    ![Отладчик моментальных снимков, в раскрывающемся меню F5](../debugger/media/snapshot-F5-dropdown-attach.png)

    * С помощью Cloud Explorer (**представление > Cloud Explorer**), щелкните правой кнопкой мыши Azure VM/VMSS развертывается веб-приложения и выберите учетную запись хранения Azure и нажмите кнопку **подключить отладчик моментальных снимков**.

      ![Запустить отладчик моментальных снимков из Cloud Explorer](../debugger/media/snapshot-launch.png)

    > [!IMPORTANT]
    > Когда вы впервые щелкаете **подключить отладчик моментальных снимков** для виртуальной Машины IIS автоматически перезапускается.
    > Когда вы впервые щелкаете **подключить отладчик моментальных снимков** для вашей масштабируемого набора виртуальных МАШИН требуется обновление вручную каждого экземпляра масштабируемого набора виртуальных МАШИН.

    Метаданные для **модули** изначально не будут активированы, перейдите к веб-приложения и **начать сбор** кнопка становится доступной. Visual Studio теперь находится в режиме отладки моментальных снимков.
    ...
    > Для масштабируемого набора виртуальных МАШИН пользователь потребуется вручную обновить экземпляры в их масштабируемого набора виртуальных МАШИН после присоединения отладчика моментальных снимков в первый раз.

   ![Режим отладки моментальных снимков](../debugger/media/snapshot-message.png)

   **Модули** окне отображается, когда все модули, загруженные для Azure VM/VMSS (выберите **Отладка > Windows > модули** чтобы открыть это окно).
...
В этом руководстве вы узнали, как использовать отладчик моментальных снимков для виртуальных машин Azure и масштабируемые наборы виртуальных машин Azure. Вы можете прочитать дополнительные сведения об этой функции.
