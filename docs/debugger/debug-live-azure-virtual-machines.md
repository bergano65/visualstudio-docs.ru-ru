...
* Snapshot Debugger для Виртуальных машин Azure и Масштабируемых наборов виртуальных машин Azure доступно только в предварительной версии Visual Studio 2019 Enterprise или более поздней версии с **рабочей нагрузкой разработки Azure**. (На вкладке **Отдельные компоненты** его можно найти в разделе **Отладка и тестирование** > **Отладчик моментальных снимков**.)
...
* Коллекция моментальных снимков доступна для следующих веб-приложений Виртуальной машины или Масштабируемых наборов виртуальных машин Azure:
...
1. Откройте проект, моментальный снимок которого хотите отладить.
    ...
    > [!IMPORTANT]
    > Для отладки моментальных снимков необходимо открыть *ту же версию исходного кода*, которая опубликована в службе виртуальных машин Azure или в Масштабируемых наборах виртуальных машин Azure.

1. Выберите **Отладка > Подключить Snapshot Debugger...**. Выберите виртуальную машину или Масштабируемые наборы виртуальных машин Azure, где развернуто ваше веб-приложение, и учетную запись хранения Azure, а затем нажмите **Подключить**.
    ...
    > Если впервые щелкнуть **Подключить Snapshot Debugger** для масштабируемого набора виртуальных машин, каждый экземпляр VMSS необходимо будет обновить вручную.
   ...
   ![Режим отладки моментальных снимков](../debugger/media/snapshot-message.png)
    ...
    > Для Масштабируемых наборов виртуальных машин пользователь должен вручную обновить экземпляры в своем VMSS после первого подключения Snapshot Debugger.

   Окно **Модули** показывает, когда все модули для виртуальных машин или Масштабируемых наборов виртуальных машин Azure загружены (чтобы открыть это окно, выберите **Отладка > Windows > Модули**).
