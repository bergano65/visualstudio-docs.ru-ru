...
![Панель инструментов в окне "Параллельные стеки"](../debugger/media/parallel_stackstoolbar.png "Панель инструментов окна \"Параллельные стеки\"")
...
|![Значок "Показывать только помеченные"](media/parallel_toolbar2.png "Значок \"Показывать только помеченные\"")|Показывать только помеченные|Отображает стеки вызовов только для потоков, которые помечены в других окнах отладки, таких как окно **Потоки GPU** и окно **Контроль параллельных данных**.|
|![Значок "Переключить представление метода"](media/parallel_toolbar3.png "Значок \"Переключить представление метода\"")|Переключение **представления метода**|Переключает представления стека и **представление метода**. Дополнительные сведения см. в разделе [Представление "Метод"](#method-view).|
|![Значок "Автопрокрутка к текущему кадру стека"](media/parallel_toolbar4.png "Значок \"Автопрокрутка к текущему кадру стека\"")|Автопрокрутка к текущему кадру стека|Автоматически прокручивает граф для отображения текущего кадра стека. Этот компонент применяется при изменении текущего кадра стека из других окон или при попадании в новую точку останова в крупных графах.|
...
![Контекстное меню в окне "Параллельные стеки"](../debugger/media/parallel_contmenu.png "Контекстное меню в окне \"Параллельные стеки\"")
...
На следующем рисунке показан один поток, идущий от основного кода к управляемому, а затем — к машинному. В текущем методе находятся шесть потоков. Один из них переходит в Thread.Sleep, другой переходит в Console.WriteLine, а затем в SyncTextWriter.WriteLine.
 ...
 ![Представление "Потоки" в окне "Параллельные стеки"](../debugger/media/parallel_stack1.png "Представление \"Потоки\" в окне \"Параллельные стеки\"")
...
![Представления "Потоки" и "Задачи"](../debugger/media/parallel_threads-tasks.png "Представления \"Потоки\" и \"Задачи\"")
...
![Подсказки в представлениях "Потоки" и "Задачи"](../debugger/media/parallel_threads-tasks-tooltips.png "Подсказки в представлениях \"Потоки\" и \"Задачи\"")
