...
|`$exceptionstack`|Отображает трассировку стека текущего исключения среды выполнения Windows. `$ exceptionstack`работает только в приложениях UWP и Windows 8.1 или более поздней версии. `$ exceptionstack` не поддерживается для исключений C++ и SHE|  
