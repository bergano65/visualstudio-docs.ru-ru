...
## <a name="understanding-the-graphics-object-table"></a>Основные сведения о таблице графических объектов
 С помощью таблицы объектов можно анализировать объекты Direct3D, поддерживающие отрисовку определенного кадра. Можно точно определить проблему отрисовки для определенного объекта, изучив его свойства и данные (используя другие средства диагностика графики, приведенные ранее в ходе диагностики, можно уменьшить список объектов, которые могут не быть нужными.) После обнаружения объекта, вызывающего ошибку, можно использовать визуализацию, относящуюся к его типу, например, можно использовать редактор изображений для просмотра текстур или *Визуализатор буфера* для просмотра содержимого буфера.
 ...
 Кроме того, можно использовать раскрывающийся список **тип** в левом верхнем углу для переключения просмотра объектов **буферов**типа, **шейдеров** , **текстур**или всех этих элементов одновременно.  Кроме того, можно использовать поле поиска в правом верхнем углу, чтобы найти определенные строки во всех представленных данных.  Например, можно выполнить поиск *D32_FLOAT* , чтобы найти все экземпляры объектов этого формата в списке.
...
## <a name="graphics-object-viewers"></a>Средства просмотра объектов графики
 ...
 ![Предварительный просмотр текстуры, отображающий выход слияния](media/gfx_diag_texture_preview.png "gfx_diag_texture_preview")
...
### <a name="device-context-object"></a>Объект контекста устройства
 В Direct3D 11 и Direct3D 10 объект контекста устройства (**Контекст устройства D3D11** или **Устройство D3D10**) особенно важен, поскольку он содержит наиболее важную информацию о состоянии и ссылки на другие объекты состояния, которые в настоящее время заданы. Сведения о контексте устройства отображаются в новом окне документа, и каждая категория сведений отображается на отдельной вкладке. Контекст устройства изменяется при выборе нового события для отражения текущего состояния устройства.
