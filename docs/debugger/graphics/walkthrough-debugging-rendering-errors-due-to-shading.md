...
## <a name="scenario"></a>Сценарий  
 ...
 В этом случае вы недавно добавили объект в приложение. Вы также добавили новый вершин и построителем текстур для преобразования объекта и ему присваивается уникальный внешний вид. При запуске приложения во время теста объект окрашивается сплошным черным цветом. С помощью диагностики графики можно записать данные о проблеме в журнал графики, чтобы можно было выполнить отладку приложения. Проблема выглядит как на изображении в приложении:  
...
1. В [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)], загрузите журнал графики, имеющий кадра, который показывает отсутствующей модели. Откроется новое окно документа журнала графики в [!INCLUDE[vsprvs](../../code-quality/includes/vsprvs_md.md)]. В верхней части этого окна находится вывод целевого объекта отрисовки для выбранного кадра. В нижней части находится **Список кадров**, в котором каждый захваченный кадр отображается как эскиз.  
  
2. В **список кадров**, выберите кадр, в котором этот объект не имеет неправильный внешний вид. Однобуферная прорисовка обновляется, и в ней отображается выбранный кадр. В этом случае выглядит этот образ окно документа журнала графики:  
...
2. Выберите пиксель для изучения. В окне документа журнала графики выберите один из пикселей на неправильно окрашенном объекта.  
    ...
    Обратите внимание на то, что результат работы построителя текстуры — полностью непрозрачный черный цвет (0, 0, 0, 1) и что **средство слияния вывода** сочетании этот шейдер пикселей с **Назад** цвет пикселя таким образом,  **Результат** — тоже полностью непрозрачный черный цвет.  
  
   Проверка пикселя будет проверен неправильно окрашенный, и Обнаружив, что результат работы построителя текстуры не является ожидаемым цвет, отладчик HLSL можно использовать для изучения построителя текстуры и узнать, что случилось с цветом объекта. Отладчик HLSL можно использовать для проверки состояния переменных HLSL при выполнении, пошагового выполнения кода HLSL и указания точек останова для помощи в диагностике проблемы.  
...
3. Наведите указатель на `input.color`. Обратите внимание, что его значение — полностью непрозрачный черный (0, 0, 0, 1).  
    ...
    ![Член «color» объекта «input» черным. ](media/gfx_diag_demo_render_error_shader_step_5.png "gfx_diag_demo_render_error_shader_step_5")  
