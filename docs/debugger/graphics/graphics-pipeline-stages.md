 ...
 **Входной ассемблер** Входной ассемблер считывает индекс и данные вершин, указанные в приложении, и собирает их для графического оборудования.
 ...
 **Шейдер вершин** На стадии шейдера вершин обрабатываются вершины, обычно выполняющие такие операции, как преобразование, выбор обложки и освещение. Шейдеры вершин создают то же число вершин, которое они принимают в качестве входных данных.
 ...
 **Шейдер поверхности** (только Direct3D 11 и Direct3D 12). этап шейдера поверхности обрабатывает контрольные точки, которые определяют поверхность низкого порядка, например линию, треугольник или четыре. В качестве выходных данных этот этап создает исправление геометрии более высокого порядка и константы исправления, которые передаются на этап тесселяции с фиксированными функциями.

 Этап шейдеров поверхностей не визуализируется в окне этапов конвейера.

 **Стадия тесселяции** (только Direct3D 11 и Direct3D 12) этап тесселяции представляет собой фиксированную (непрограммируемую) аппаратную единицу, которая предварительно обрабатывает домен, представленный выходными данными шейдера поверхности. В качестве выходных данных он создает шаблон выборки домена и набор примитивов меньшего размера — точки, линии, треугольники, соединяющие эти выборки.

 Этап тесселяции не визуализируется в окне этапов конвейера.

 **Шейдер доменов** (только Direct3D 11 и Direct3D 12). этап шейдера доменов обрабатывает обновления геометрии более высокого порядка из шейдера поверхности, а также факторы тесселяции из этапа тесселяции. Факторы тесселяции могут включать в себя как факторы входных данных тесселяции, так и факторы выходных данных. В качестве выходных данных этап вычисляет позицию вершины точки на выходном исправлении в соответствии с факторами тесселяции.

 Этап шейдеров доменов не визуализируется в окне этапов конвейера.

 **Шейдер геометрии** На стадии шейдера геометрии обрабатываются целые примитивы — точки, линии или треугольники, а также необязательные данные вершин для примитивов, смежных с краями. В отличие от шейдеров вершин шейдеры геометрии могут создавать больше или меньше примитивов, чем они приняли в качестве входных данных.
 ...
 **Стадия вывода потока** На этапе вывода потока можно перехватывать преобразованные примитивы до растрирования и записывать их в память. из него данные можно повторно передать в качестве входных данных на более ранних стадиях графического конвейера или считывать с помощью ЦП.

 Этап потокового вывода не визуализируется в окне этапов конвейера.

 **Этап средства развертки** Этап средства создания программной прорисовки — это фиксированная функция (непрограммируемая), преобразующая примитивы векторов — точки, линии, треугольники — в растровое изображение, выполняя преобразование «сканирование-строка». Во время растеризации вершины преобразуются в однородное пространство обрезки и обрезаются. В выходных данных построители текстуры сопоставляются, а атрибуты отдельных вершин интерполируются по примитиву и подготавливаются для построителя текстуры.

 Этап растеризации не визуализируется в окне этапов конвейера.

 **Шейдер пикселей** На этапе шейдера пикселей обрабатываются Растровые примитивы вместе с интерполяцией данных вершин для создания значений для каждого пикселя, таких как цвет и глубина.
 ...
 **Слияние выходных данных** На этапе слияния выходных данных объединяются недавно обработанные Пиксели вместе с существующим содержимым соответствующих буферов (цвет, глубина и набор элементов) для создания новых значений в этих буферах.
...
### <a name="vertex-and-geometry-shader-preview"></a>Предварительный просмотр вершин и шейдеров Geometry
 При выборе стадии шейдера вершин или геометрии в окне **Этапы конвейера** можно просмотреть входные и выходные данные из шейдера на панели ниже.  Здесь вы найдете сведения о списке вершин, предоставленных шейдерам, после того, как они были собраны этапом входного ассемблера.
