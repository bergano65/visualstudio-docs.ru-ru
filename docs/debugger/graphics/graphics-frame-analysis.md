...
#### <a name="hot-draw-calls"></a>"Горячие" вызовы Draw  
 ...
 ![Этот вызов DrawIndexed имеет горячие и холодные вариантов. ] (media/pix_frame_analysis_hot_calls.png "pix_frame_analysis_hot_calls")  
...
### <a name="gpu-counters"></a>Счетчики GPU  
 ...
 Так как ни одно из устройств GPU, в настоящее время предлагаемых компаниями Intel, AMD или nVidia для компьютеров, не обеспечивает надежной поддержки аппаратных счетчиков GPU, при анализе кадров данные этих счетчиков не собираются. Однако анализ кадров сбор аппаратные счетчики из следующих GPU, поддерживающего надежно их:  
...
### <a name="warp"></a>WARP  
 Анализ кадров предназначен для сбора данных и оптимизации производительности отрисовки на реальном оборудовании. Запуск анализа кадров на устройствах WARP возможен, но это не обычно стоит производительность WARP на быстром ЦП выполняется медленнее, чем даже самого простого современных графических процессоров, а производительность WARP может существенно различаться в зависимости от конкретного ЦП Эта функция включена.  
...
|**0x MSAA**|Многовыборочное сглаживание (MSAA) отключается для всех целевых объектов отрисовки.<br /><br /> Дополнительные сведения см. в разделе [0 x / 2 x / 4 x MSAA варианты](0x-2x-4x-msaa-variants.md)|  
