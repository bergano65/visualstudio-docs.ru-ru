---
title: Графический список событий (ru) Документы Майкрософт
ms.date: 11/04/2016
ms.topic: conceptual
f1_keywords:
...
|![Пользователь&#45;определенный значок маркера события](media/vsg_eventlist_icon_user.png "vsg_eventlist_icon_user")|Пользовательская группа событий<br /> Группа связанных событий, определенная приложением.|
|![Пользователь&#45;определенный значок маркера события](media/vsg_eventlist_icon_user.png "vsg_eventlist_icon_user")|Пользовательский маркер события<br /> Помечает расположение, определенное приложением.|
...
Visual Studio 2017 и больше содержат окно **истории ресурсов.**  Выбор значка](media/gfx_watch.png) ![часов значок значок часы рядом с записью в окне **списка событий** будет воспитывать окно **истории ресурсов** показано ниже:

![Журнал ресурсов](media/gfx_diag_resource_history.png)

Это окно позволяет просматривать историю выбранного элемента в списке событий.  Выпадение в верхней части может быть использовано для выбора других элементов для просмотра истории.  Верхняя половина окна содержит **события настройки кадра.**  Это события, которые попадают в категорию типа *«Создание»* и являются вызовами, которые обычно инициализируют и создают ресурс.  Нижняя половина окна содержит раздел **Событий кадра.**  Это обычные события чтения и записи, которые происходят во время использования ресурса.

| Столбец | Описание |
|-----------| - |
| **Тип** | Отображает тип записи, как правило, *Создать,* *Читать* и *писать*. |
| **Представление** | Показывает эскиз ресурса в этот момент времени.  Дважды щелкните эскиз, чтобы открыть представление деталей ресурса в то время. |
| **Событие** | Отображает сяпоивший вызов метода, сгенерированный событием.  Любую дополнительную историю по отдельным элементам можно ![просмотреть, выбрав значок](media/gfx_watch.png) часов icon на соответствующей строке.  Кроме того, любой элемент, который `m_commandList` нарисован синим текстом, например, на скриншоте выше, может быть выбран для более подробной информации. |
