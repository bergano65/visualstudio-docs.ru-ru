---
title: Список событий графики | Документы Microsoft
ms.custom: ''
ms.date: 11/04/2016
ms.reviewer: ''
ms.suite: ''
...
Список событий графики в анализаторе графики Visual Studio служит для изучения событий Direct3D, возникших при отрисовке определенного кадра игры или приложения.  
 ...
 ![Список событий, имеющих в имени «Индекс». ] (media/gfx_diag_demo_event_list_orientation.png "gfx_diag_demo_event_list_orientation")  
...
### <a name="event-list-views"></a>Представления списка событий  
 Список событий поддерживает два разных представления, в которых события графики упорядочиваются по-разному в соответствии с рабочим процессом и вашими предпочтениями. Первое представление является *представление рабочих GPU* которого иерархически организует события и соответствующие состояния. Второе — это *представление временной шкалы*, в котором события упорядочиваются хронологически в виде плоского списка.  
  
 **Графического** представление  
...
-   В **список событий графики** над списком событий **представление** раскрывающийся список и выберите **временная шкала** представление или **работы GPU** представления.  
...
|![Пользователь-45 — значок маркера определенное событие](media/vsg_eventlist_icon_user.png "vsg_eventlist_icon_user")|Пользовательская группа событий<br /> Группа связанных событий, определенная приложением.|  
|![Пользователь-45 — значок маркера определенное событие](media/vsg_eventlist_icon_user.png "vsg_eventlist_icon_user")|Пользовательский маркер события<br /> Помечает расположение, определенное приложением.|  
...
##Ресурс журнала Visual Studio 2017 и более поздней версии содержат **журнал ресурсов** окна.  При выборе значок ![значок просмотра](media/gfx_watch.png) рядом с записью в **список событий** появится окно **журнал ресурсов** показанная ниже:

![Журнал ресурсов](media/gfx_diag_resource_history.png)

В этом окне можно просмотреть журнал элемента, выбранного в списке событий.  Выберите другие элементы для просмотра журнала можно использовать раскрывающийся список в верхней.  В верхней части окна содержит **события кадра установки**.  Это события, которые попадают в *создать* категории типов и вызовов, которые обычно инициализации и создания ресурса.  Нижней половине окна содержит **события кадров** раздела.  Это обычный чтения и записи событий, происходящих во время использования ресурса.  

Столбец|Описание:
---|---
**Type** | Показывает тип операции, обычно *создать*, *чтения* и *записи*.  
**Вид** | Эскиз ресурса отображается в данный момент времени.  Дважды щелкните его, чтобы открыть подробное представление ресурса в это время.  
**Event**| Показан вызов метода, который произошла, который создал событие.  Любые дополнительные данные журнала для отдельных элементов, которые можно просмотреть, выбрав значок ![значок просмотра](media/gfx_watch.png) на соответствующую строку.  Кроме того, любой элемент, который рисуется синим цветом, такие как `m_commandList` на снимке экрана выше, может быть выбран для получения дополнительных сведений.
<!-- /VERSIONLESS -->
