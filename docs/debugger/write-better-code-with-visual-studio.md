...
Обратите внимание, что эта ошибка отображается в нижнем левом углу значка лампочки. Вместе с значком отвертки ![screwdriver Icon @ no__t-1, значок лампочки ![light значок лампы @ no__t-3 представляет быстрые действия, которые могут помочь в исправлении или реструктуризации кода. Лампочка представляет проблемы, которые *следует* исправить. Отвертка предназначена для устранения проблем, которые можно выбрать для исправления. Чтобы устранить эту ошибку, используйте первое предлагаемое исправление, нажав кнопку **использовать System. Text** слева.
...
Описанная выше ошибка является распространенной, которая обычно исправляется путем добавления новой инструкции `using` в код. Существует несколько распространенных подобных ошибок, таких как ```The type or namespace `Name` cannot be found.```. Эти ошибки могут указывать на отсутствующую ссылку на сборку (щелкните проект правой кнопкой мыши, выберите пункт **добавить** > **ссылку**), неправильно написанное имя или отсутствующую библиотеку. чтобы добавить (для C#, щелкните проект правой кнопкой мыши и выберите пункт **Управление пакетами NuGet**).
...
Как правило, это представляет проблему, которую необходимо исправить. Однако в примере приложения фактически данные хранятся в переменной `points` в процессе десериализации, а затем добавляются в элемент данных `totalpoints`. В этом примере вы узнали назначение кода и можете спокойно проигнорировать предупреждение. Однако если вы хотите устранить это предупреждение, можно заменить следующий код:
...
Сообщение об ошибке указывает, что значение `4o` не может быть проанализировано как целое число. Итак, в этом примере вы узнали, что данные неверны: `4o` должно быть `40`. Но если вы не контролируете данные в реальной ситуации (скажем, вы получаете их из веб-службы), что делать с ней? Как это исправить?
...
Если это первое, исправьте ошибку. (В примере приложения это означает исправление неверных данных.) Если это последнее, может потребоваться обрабатывайте исключение в коде с помощью блока `try/catch` (мы рассмотрим другие возможные стратегии в следующем разделе). В примере приложения замените следующий код:
...
Блок `try/catch` имеет некоторые затраты на производительность, поэтому вы хотите использовать их только тогда, когда они действительно нужны, то есть, где (а) они могут возникнуть в окончательной версии приложения, и где (b) документация для метода указывает, что следует проверить наличие исключения (например, суминг, что документация завершена!). Во многих случаях можно соответствующим образом обойти исключение, и пользователю никогда не нужно знать о нем.
...
* Используйте блок `try/catch` вокруг конкретной функции, которая вызывает исключение (`ReadObject` в примере приложения). Если вы используете его с большим фрагментом кода, вы в итоге скройтее расположение ошибки. Например, не используйте блок `try/catch` вокруг вызова родительской функции `ReadToObject`, как показано здесь, или вы не узнаете точно, где произошло исключение.
...
Это хороший момент для изучения полезного программирования, который часто используется, что заключается в использовании в функциях операторов `assert`. Добавив следующий код, вы включите проверку среды выполнения, чтобы убедиться, что `firstname` и `lastname` не `null`. Замените следующий код в методе `UpdateRecords`:
...
Добавив такие инструкции `assert` в функции во время процесса разработки, вы можете помочь указать цель кода. В предыдущем примере мы указываем следующее:
...
При перезапуске отладчик приостанавливается на операторе `assert`, поскольку выражение `users[i].firstname != null` принимает значение `false` вместо `true`.

![Assert разрешается в значение false](../debugger/media/write-better-code-using-assert.png)

Ошибка `assert` сообщает о проблеме, которую необходимо изучить. `assert` может охватывать множество сценариев, в которых не всегда отображается исключение. В этом примере пользователь не увидит исключение, а значение `null` добавляется как `firstname` в список записей. Это может вызвать проблемы позже (например, в выходных данных консоли) и может быть труднее для отладки.

> [!NOTE]
> В сценариях, где вызывается метод для значения `null`, результаты `NullReferenceException`. Обычно следует избегать использования блока `try/catch` для общего исключения, то есть исключения, не привязанного к определенной функции библиотеки. Любой объект может вызывать исключение `NullReferenceException`. Если вы не уверены, проверьте документацию по библиотечной функции.

Во время процесса отладки рекомендуется использовать определенную инструкцию `assert`, пока вы не захотите заменить ее фактическим исправлением кода. Допустим, вы решили, что пользователь может столкнуться с исключением в сборке выпуска приложения. В этом случае необходимо выполнить рефакторинг кода, чтобы убедиться в том, что приложение не создает неустранимое исключение или приводит к возникновению какой-либо другой ошибки. Итак, чтобы исправить этот код, замените следующий код:
...
Используя этот код, вы выполняете требования к коду и следите за тем, чтобы в данные не добавлялась запись со значением `firstname` или `lastname`, равным `null`.

В этом примере мы добавили две инструкции `assert` в цикле. Как правило, при использовании `assert` лучше всего добавить инструкции `assert` в точку входа (начало) функции или метода. В настоящее время вы ищете метод `UpdateRecords` в примере приложения. В этом методе вы можете столкнуться с проблемами, если любой из аргументов метода имеет значение `null`, поэтому проверьте их как с помощью оператора `assert` в точке входа функции.
...
Можно использовать `assert` с любым видом выражения, которое разрешается в `true` или `false`. Например, можно добавить оператор `assert`, подобный этому.
