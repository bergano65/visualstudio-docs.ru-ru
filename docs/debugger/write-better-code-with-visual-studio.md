---
title: Позволить Visual Studio позволяет создавать C# кода с меньшим количеством ошибок
description: Ситуации использования отладчика для отладки приложения
...
ms.locfileid: "51561833"
---
# <a name="write-better-c-code-using-visual-studio"></a>Лучше писать C# кода с помощью Visual Studio

Отладка кода может занимать много времени — и иногда раздражающим--задачи. Потребуется время, чтобы узнать, как для эффективной отладки. Это мощная интегрированная среда разработки, как Visual Studio можно делают работу гораздо проще. Интегрированная среда разработки может помочь вам в отладке кода быстрее, а не просто, но он может также помочь в написании качественного кода с меньшим количеством ошибок. Нашей целью этой статьи — дать вам целостное представление о процесс отладки, поэтому вы будете знать, когда следует использовать анализатор кода, для использования отладчика и когда следует использовать другие средства.
...
> [!NOTE]
> Если вы не видите **консольное приложение** шаблон проекта, щелкните **открыть установщик Visual Studio** ссылку в левой части **новый проект** диалоговое окно. Запускается Visual Studio Installer. Выберите **разработка классических приложений .NET** или **кросс Платформенная разработка .NET Core** рабочей нагрузки, затем выберите **изменить**.
...
Нажмите клавишу **F5** (**Отладка > Начать отладку**) или **начать отладку** кнопку ![начать отладку](../debugger/media/dbg-tour-start-debugging.png "начать отладку ") на панели инструментов отладки.
...
* Для ознакомления методов, включенных в приложение обратитесь к документации, чтобы увидеть, какие исключения метод является, вероятно, создаст исключение. Это может быть критически важные сведения для соответствующей обработки ошибок, а также для отладки приложения.
...
Нажмите кнопку **перезапустите** ![перезапустить приложение](../debugger/media/dbg-tour-restart.png "RestartApp") кнопку на панели инструментов отладки (**Ctrl** + **Shift**   +  **F5**). Это перезапускает приложение за несколько шагов. Можно увидеть следующий результат в окне консоли.
...
`assert` Ошибка сообщает, что имеется проблема, вам нужно проанализировать. `assert` можно охватывают множество сценариев, где вы не видите обязательно исключение. В этом примере пользователь не увидит исключение (в других сценариях `NullReferenceException` возникает) и `null` значение добавляется как `firstname` в базе данных. Это может вызвать проблемы позже (например, см. в выходных данных консоли) и может быть сложнее для отладки.
...
В следующем коде у некоторые ошибки, которые можно исправить с помощью Visual Studio IDE. Здесь приложение представляет собой простое приложение, которое имитирует получение данных JSON с некоторой операции десериализации данных к объекту и обновление простой базы данных в памяти с новыми данными.
