...
ms.locfileid: "51561677"
---
# <a name="tutorial-learn-to-debug-using-visual-studio"></a>Руководство. Отладка с помощью Visual Studio
...
Несмотря на то, что демонстрационное приложение написано на C# и C++, доступные функции применяются к Visual Basic, JavaScript и другим языкам, поддерживаемым Visual Studio (если не указано иное). На снимках экрана представлены примеры на C#.
...
* Необходимо установить Visual Studio 2017 и рабочую нагрузку **Разработка классических приложений .NET** или **Разработка классических приложений на C++**.
    ...
    Если вам нужно установить рабочую нагрузку, но среда Visual Studio уже имеется, щелкните ссылку **Открыть установщик Visual Studio** в левой области диалогового окна **Новый проект** (выберите пункты **Файл** > **Создать** > **Проект**). Запускается Visual Studio Installer. Выберите рабочую нагрузку **Разработка классических приложений .NET** или **Разработка классических приложений на C++**, а затем нажмите кнопку **Изменить**.

## <a name="create-a-project"></a>Создание проекта

1. В Visual Studio последовательно выберите **Файл > Создать проект**.

2. В разделе **Visual C#** или **Visual C++** выберите **Рабочий стол Windows**, а затем в средней области выберите **Консольное приложение** (**Консольное приложение Windows** на C++).

    Если шаблон проекта **Консольное приложение** отсутствует, щелкните ссылку **Открыть Visual Studio Installer** в левой области диалогового окна **Создание проекта**. Запускается Visual Studio Installer. Выберите рабочую нагрузку *Разработка классических приложений .NET* * или **Разработка классических приложений на C++**, а затем нажмите кнопку **Изменить**.
...
3. Введите имя, например **get-started-debugging**, и нажмите кнопку **ОК**.
    ...
    > [!NOTE]
    > Для переключения между примерами кода на C# и C++ в этой статье используйте фильтр язык в правом верхнем углу этой страницы.

4. В файле *Program.cs* (C#) или *get-started-debugging.cpp* (C++) замените код
...
1. В цикле `foreach` функции `Main` (цикл `for` в функции `main` C++) установите точку останова, щелкнув левое поле следующей строки кода:

    `shape.Draw()` (или `shape->Draw()` в C++)

    В месте установки точки останова появится красный круг.

    Точки останова — это один из самых простых и важных компонентов надежной отладки. Точка останова указывает, где Visual Studio следует приостановить выполнение кода, чтобы вы могли проверить значения переменных или поведение памяти либо выполнение ветви кода. 

6. Нажмите клавишу **F5** или кнопку **Начать отладку**. Будет запущено приложение и отладчик перейдет к строке кода, где задана точка останова.
...
1. Во время приостановки в вызове метода `shape.Draw` в методе `Main` (`shape->Draw` в C++) нажмите клавишу **F11** (или выберите **Отладка > Шаг с заходом**), чтобы перейти в код для класса `Rectangle`.
...
2. Нажимайте клавишу **F10** (или выберите **Отладка > Шаг с обходом**) несколько раз, пока отладчик не остановится в вызове метода `base.Draw` (`Shape::Draw` в C++), а затем еще раз нажмите клавишу **F10**.

     ![Использование клавиши F10 для шага с обходом кода](../debugger/media/get-started-step-over.png "F10 — шаг с обходом")

     Обратите внимание, что в этот раз отладчик не заходит в метод `Draw` базового класса (`Shape`). Клавиша **F10** перемещает отладчик без захода в функции или методы в коде приложения (код продолжает выполняться). Нажав клавишу F10 (а не **F11**) в вызове метода `base.Draw` (или `Shape::Draw`), мы пропускаем код реализации для `base.Draw` (пока это нас не интересует).

## <a name="navigate-code-using-run-to-click"></a>Переход по коду с помощью команды "Выполнение до щелкнутого"

5. В редакторе кода прокрутите вниз и наведите указатель мыши на метод `Console.WriteLine` (`std::cout` в C++) в классе `Triangle`, пока в левой части не появится зеленая кнопка **Выполнение до щелкнутого** ![Выполнение до щелкнутого](../debugger/media/dbg-tour-run-to-click.png "Выполнение до щелкнутого").
...
6. Нажмите кнопку **Выполнение до щелкнутого** ![Выполнение до щелкнутого](../debugger/media/dbg-tour-run-to-click.png "Выполнение до щелкнутого").
    ...
    Отладчик перемещается к реализации метода `Console.WriteLine` для класса `Triangle` (`std::cout` в C++).
...
1. Нажмите сочетание клавиш **SHIFT** + **F11** (или **Отладка > Шаг с выходом**).
     ...
     Вы должны вернуться в цикл `foreach` в методе `Main` (цикл `for` в C++).
...
Отладчик еще раз останавливается в заданной вами точке останова в методе `shape.Draw()` (`shape->Draw()` в C++).
...
1. Во время приостановки в методе `shape.Draw()` (`shape->Draw()` в C++) наведите указатель мыши на объект `shapes`. Вы увидите значение его свойства `Count` по умолчанию.

1. Разверните объект `shapes`, чтобы увидеть все его свойства, такие как первый индекс массива `[0]` со значением `Rectangle` (C#) или адресом памяти (C++).
...
1. Взгляните на окно **Видимые** в нижней части редактора кода.
    ...
    В окне **Видимые** отображаются переменные и их текущие значения. В окне **Видимые** приводятся все переменные, используемые в текущей строке или предыдущей строке (В C++ в окне отображаются переменные в трех предыдущих строках кода. Сведения о зависящем от языка поведении см. в соответствующей документации.).
...
2. Затем посмотрите на окно **Локальные** на вкладке рядом с окном **Видимые**.
    ...
    В окне **Локальные** показаны переменные, которые находятся в текущей [области](https://www.wikipedia.org/wiki/Scope_(computer_science)).
...
1. Во время приостановки в цикле `foreach` (цикл `for` в C++) щелкните окно **Стек вызовов**, которое по умолчанию открыто в нижней правой области.

2. Несколько раз нажимайте клавишу **F11**, пока отладчик не приостановится в методе `Circle.Draw` в редакторе кода. Взгляните на окно **Стек вызовов**.

    ![Просмотр стека вызовов](../debugger/media/get-started-call-stack.png "Просмотр стека вызовов")

    В окне **Стек вызовов** показан порядок вызова методов и функций. В верхней строке приводится текущая функция (метод `Circle.Draw` или `Circle::Draw` в этом приложении). Во второй строке показано, что функция `Circle.Draw` была вызвана из метода `Main` (`main` в C++), и т. д.
   ...
   > Окно **Стек вызовов** аналогично перспективе "Отладка" в некоторых интегрированных средах разработки, например Eclipse.
    ...
    Для выполнения других задач можно воспользоваться контекстными меню из окна **Стек вызовов**. Например, можно вставлять точки останова в указанные функции, перемещать отладчик с помощью функции **Выполнение до текущей позиции** и изучать исходный код. Дополнительные сведения см. в статье о [просмотре стека вызовов](../debugger/how-to-use-the-call-stack-window.md).

## <a name="change-the-execution-flow"></a>Изменение потока выполнения

1. Во время приостановки отладчика в вызове метода `Circle.Draw` нажимайте клавишу **F11** несколько раз, пока отладчик не приостановится в вызове метода `base.Draw` (`Shape::Draw` в C++).

1. С помощью мыши захватите желтую стрелку (указатель выполнения) в левой части и переместите ее вверх на одну строку в вызов метода `Console.WriteLine` (`std::cout` в C++).

1. Еще раз нажмите клавишу **F11**.

    Отладчик повторно запустит метод `Console.WriteLine` (`std::cout` в C++).
...
> [Советы и рекомендации по работе с отладчиком](../debugger/debugger-tips-and-tricks.md)
