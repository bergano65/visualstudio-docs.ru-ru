...
3. Введите имя, например **get работы Отладка** и нажмите кнопку **ОК**.
    ...
    > [!NOTE]
    > Для переключения между C# и C++ примера кода в этой статье, используйте фильтр язык в правом верхнем углу этой страницы.
...
1. В `foreach` цикла `Main` функция (`for` цикл в C++ `main` функции), установите точку останова, щелкнув левое следующую строку кода:
...
6. Нажмите клавишу **F5** или **начать отладку** кнопку, запускается приложение, и отладчик переходит к строке кода, где вы задали точку останова.
    ...
    ![Установка и срабатывание точки останова](../debugger/media/get-started-set-breakpoint.gif)
...
1. Во время паузы в `shape.Draw` вызов метода `Main` метод (`shape->Draw` в C++), нажмите клавишу **F11** (или выберите **Отладка > шаг с заходом**) для перехода в код для `Rectangle` класс.
...
Отладчик останавливается в точке останова, вы установки на `shape.Draw()` метод (`shape->Draw()` в C++).
...
1. Во время паузы на `shape.Draw()` метод (`shape->Draw()` в C++), наведите указатель мыши `shapes` и объект см. в разделе значением свойства по умолчанию `Count` свойство.
...
1. Разверните `shapes` объекту, чтобы увидеть все его свойства, такие как первый индекс в массиве `[0]`, который имеет значение `Rectangle` (C#) или адрес памяти (C++).
     ...
     ![Просмотреть подсказке](../debugger/media/get-started-data-tip.gif "просмотреть подсказке по данным")

    Далее можно развернуть объекты, чтобы просмотреть их свойства, такие как `Height` свойство прямоугольника.
...
2. Нажмите кнопку **F11** несколько раз, пока не увидите приостановки в отладчике `Circle.Draw` метода в редакторе кода. Рассмотрим **стек вызовов** окна.
