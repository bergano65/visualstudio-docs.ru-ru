---
title: Написать визуализатор в C# | Документация Майкрософт
ms.custom: seodec18
ms.date: 04/12/2019
ms.topic: conceptual
dev_langs:
...
1. Создайте новый проект библиотеки классов.
    ...
    Нажмите клавишу **ESC**, чтобы закрыть окно запуска. Введите **CTRL + Q** , чтобы открыть поле поиска, введите **Библиотека классов**, выберите **шаблоны**, а затем щелкните **создать новую библиотеку классов (.NET Standard)** . В появившемся диалоговом окне выберите **Создать**.
    ::: moniker-end
    ::: moniker range="vs-2017"
    В верхней строке меню выберите **Файл** > **Создать** > **Проект**. В левой области диалогового окна **Новый проект** в разделе **визуальный C#** элемент выберите **.NET Standard**, а затем в средней области выберите **Библиотека классов (.NET Standard)** .
    ::: moniker-end
...
6. В DebuggerSide.cs добавьте следующее в директивы `using`:
...
4. В DebuggerSide.cs добавьте следующее в директивы `using`:
...
1. В обозреватель решений щелкните решение правой кнопкой мыши, выберите **Добавить**, а затем — **Новый проект**.
    ...
    ::: moniker range=">=vs-2019"
    В поле поиска введите **консольное приложение**, выберите **шаблоны**, а затем выберите **создать новое консольное приложение (.NET Framework)** . В появившемся диалоговом окне выберите **Создать**.
    ::: moniker-end
    ::: moniker range="vs-2017"
    В верхней строке меню выберите **Файл** > **Создать** > **Проект**. В левой области диалогового окна **Новый проект** в разделе **Visual C#** выберите **Рабочий стол Windows**, а затем в средней области выберите **Консольное приложение (.NET Framework)** .
    ::: moniker-end
...
2. В диалоговом окне **Добавление ссылки** перейдите на вкладку **Обзор** и выберите Microsoft. VISUALSTUDIO. DebuggerVisualizers. dll.
...
3. В TestConsole.cs добавьте следующий код в директивы `using`:
...
2. В диалоговом окне **Новый проект** в разделе **визуальный C#** элемент выберите **.NET Standard**.
...
3. В диалоговом окне **Добавление нового элемента** в разделе **визуальные C# элементы**выберите **Визуализатор отладчика**.
