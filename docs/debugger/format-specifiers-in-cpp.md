...
С помощью описателей формата можно изменить формат, в котором значение отображается в окне **Контрольные значения** .  
 ...
 Можно также использовать описатели формата в **Интерпретация** окне **команда** окна в [точки трассировки](../debugger/using-breakpoints.md#BKMK_Print_to_the_Output_window_with_tracepoints)и даже в окнах исходного кода. Если вы приостановите выполнение на выражении в этих окон, результат отображается в подсказке по данным. В подсказке по данным указывается описатель формата.  
  
> [!NOTE]
>  При изменении новое ядро отладки отладчик машинного кода Visual Studio были добавлены ряд новых описателей формата, и некоторые старые описатели были удалены. Прежний отладчик все еще используется при отладке взаимодействия (то есть смешанной отладке машинного и управляемого кода) с использованием C++/CLI. В подразделах этого раздела показаны описатели формата для каждого ядра отладки.
>   
...
```  
 ...
 Добавить `my_var1` переменной **Контрольные значения** окна (во время отладки, **Отладка > Windows > Контрольные значения > Контрольные значения 1**) и затем задайте Шестнадцатеричный вывод (в **смотреть**, щелкните правой кнопкой мыши переменную и выберите пункт **Шестнадцатеричный вывод**). Теперь в окне "Контрольные значения" показано значение 0x0065. Чтобы представить это значение в символьном, а не целочисленном формате, в столбце "Имя" после имени переменной добавьте описатель символьного формата **, c**. В столбце **Значение** теперь отображается значение **101 'e'**.  
...
|**sb**|const char * строки (без кавычек)|\<расположение > «hello world»|Здравствуй, мир!|  
|s8|Строка UTF-8|\<расположение > «Является â˜• кофе cup UTF-8»|«Is ☕ кофе cup UTF-8»|
|**s8b**|Строка UTF-8 (без кавычек)|\<расположение > «hello world»|Здравствуй, мир!|  
|su|Строка в Юникоде (кодировка UTF-16)|\<расположение > L «hello world»|L"Здравствуй, мир!"<br /><br /> u"Здравствуй, мир!"|  
|sub|Юникод (UTF-16) строки (без кавычек)|\<расположение > L «hello world»|Здравствуй, мир!|  
|bstr|Строка BSTR|\<расположение > L «hello world»|L"Здравствуй, мир!"|  
|env|Блок среды (завершенных строка double null)|\<расположение > L "=:: =::\\\\»|L "=:: =::\\\\\\0 = C: = C:\\\\windows\\\\system32\\0ALLUSERSPROFILE =...|
...
###  <a name="BKMK_Size_specifier_for_pointers_as_arrays_in_interop_debugging_and_C___edit_and_continue"></a> Спецификатор размера для указателей как массивов при отладке взаимодействия с использованием C + +/ CLI  
