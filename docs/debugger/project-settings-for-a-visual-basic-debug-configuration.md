---
title: Параметры проекта для настройки отладки VB | Документация Майкрософт
ms.custom: seodec18
ms.date: 11/04/2016
ms.topic: reference
f1_keywords:
...
| **Использовать удаленный компьютер** | Когда этот флажок установлен, разрешена удаленная отладка. В текстовом поле можно ввести имя удаленного компьютера, на котором приложение будет работать в отладочных целях, или [Имя сервера Msvsmon](../debugger/remote-debugging.md). Расположение EXE-файла на удаленном компьютере указывается свойством выходной путь на вкладке Сборка. Расположение должно быть общим каталогом на удаленном компьютере. |
