...
2.  Установите точку останова в легкодоступном месте кода.  
     ...
     Например, ее можно установить в обработчике кнопки. Чтобы сделать это, откройте файл MainWindow.xaml и добавьте элемент управления Button из области элементов, а затем дважды щелкните кнопку, чтобы открыть ее обработчик.
  
3.  В обозревателе решений щелкните правой кнопкой мыши проект и выберите команду **свойства**.  
  
4.  На **свойства** выберите **отладки** вкладки.  
  
     ![RemoteDebuggerCSharp](../debugger/media/remotedebuggercsharp.png "RemoteDebuggerCSharp")  
  
5.  Убедитесь, что **рабочий каталог** текстовое поле будет пустым.  
  
6.  Выберите **использовать удаленный компьютер**и тип **MJO-DL:4022** в текстовом поле. (4022 — номер порта, показываемый в окне удаленного отладчика. Номер порта увеличивает значение 2 в каждой версии Visual Studio).
  
7.  Убедитесь, что **включить отладку машинного кода** не выбран.  
  
8.  Выполните построение проекта.  
  
9. Создайте папку на удаленном компьютере, который является тем же путем, как **отладки** папку на компьютере Visual Studio:  **\<исходный путь > \MyWPF\MyWPF\bin\Debug**.  
...
10. Скопируйте исполняемый файл, сборку которого вы только что выполнили, с компьютера с Visual Studio в созданную на удаленном компьютере папку.
    ...
    > [!CAUTION]
    >  Не следует вносить изменения в код или rebuild (или необходимо повторить этот шаг). Исполняемый файл, скопированный на удаленный компьютер, должен в точности совпадать с локальным исходным кодом и символами.

    Можно вручную скопировать проект, использовать Xcopy, Robocopy, Powershell или другие параметры.
  
11. Убедитесь, что удаленный отладчик запущен на целевом компьютере (если это не так, поиск **удаленный отладчик** в **запустить** меню). В окне удаленного отладчика выглядит следующим образом.  
...
14. При необходимости выполните действие попадет на точке останова. Она должна быть активна. Если это не так, еще не загружены символы для приложения. Повторите попытку и, если это не поможет, получить сведения о загрузке символов и устранении связанных с ними в [основные сведения о файлах символов и Visual Studio символов параметры](http://blogs.msdn.com/b/visualstudioalm/archive/2015/01/05/understanding-symbol-files-and-visual-studio-s-symbol-settings.aspx).
