...
ms.locfileid: "72730891"
---
# <a name="native-run-time-checks-customization"></a>Настройка проверок во время выполнения машинного кода
При компиляции с параметром **/RTC** (проверки во время выполнения) или при использовании директивы pragma `runtime_checks` библиотека времени выполнения языка C предоставляет проверки во время выполнения в машинном режиме. В некоторых случаях необходимо настроить проверки времени выполнения:
...
- написать функцию, сообщающую об ошибке времени выполнения. Дополнительные сведения см. [в разделе Практические руководства. Написание функции отчетов об ошибках во время выполнения](../debugger/how-to-write-a-run-time-error-reporting-function.md).
