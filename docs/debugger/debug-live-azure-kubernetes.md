...
* Snapshot Debugger для Azure Kubernetes Services доступны только для Visual Studio 2019 Enterprise или более поздней версии с рабочей нагрузкой для **разработки Azure**. (На вкладке **Отдельные компоненты** его можно найти в разделе **Отладка и тестирование** > **Отладчик моментальных снимков**.)
...
1. Выберите **Отладка > Подключить Snapshot Debugger...** . Выберите ресурс AKS, в котором развернуто ваше веб-приложение, и учетную запись хранения Azure, а затем нажмите **Подключить**. Snapshot Debugger также поддерживает [службы приложений Azure](debug-live-azure-applications.md) и [виртуальные машины Azure & масштабируемые наборы виртуальных машин](debug-live-azure-virtual-machines.md).
