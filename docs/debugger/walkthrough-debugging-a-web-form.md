---
title: "Пошаговое руководство: Отладка веб-формы | Документы Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
2.  На **файл** меню, нажмите кнопку **New**, а затем нажмите кнопку **веб-сайта**.  
  
     **Новый веб-сайт** откроется диалоговое окно.  
  
3.  В **шаблоны** области, нажмите кнопку **веб-сайт ASP.NET**.  
  
4.  На **расположение** строки, нажмите кнопку **HTTP** из списка и в текстовом поле введите **http://localhost/WebSite**.  
  
5.  В **язык** выберите **Visual C#** или **Visual Basic**.  
  
6.  Нажмите кнопку **ОК**.  
  
     [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] создаст новый проект и отобразит исходный код HTML, создаваемый по умолчанию. Он также создает новый виртуальный каталог с именем **веб-сайт** под **веб-сайт по умолчанию** в службах IIS.  
  
7.  Нажмите кнопку **разработки** вкладку на нижней границе окна.  
  
8.  Нажмите кнопку **элементов** вкладка на левой границе или выберите ее в **представление** меню.  
  
     Откроется **Панель элементов** .  
  
9. В **элементов**, нажмите кнопку **кнопку** управления и добавьте его в область конструирования Default.aspx.  
...
3.  **Отладка не включена** откроется диалоговое окно. Выберите **измените файл Web.config для включения отладки** и нажмите кнопку **ОК**.  
...
5.  На **отладки** меню, нажмите кнопку **Windows**, нажмите кнопку **Контрольные значения**, а затем нажмите кнопку **Контрольное значение 1**.  
  
6.  В **Контрольные значения** введите **TextBox1.Text**.  
  
     **Контрольные значения** показывает значение переменной `TextBox1.Text`:  
  
    ```  
    ""  
    ```  
  
7.  На **отладки** меню, нажмите кнопку **шаг с обходом**.  
  
     Значение `TextBox1.Text` изменения в **Контрольные значения** на следующее:  
  
    ```  
    "Button was clicked!"  
    ```  
  
8.  На **отладки** меню, нажмите кнопку **Продолжить**.  
...
11. На **отладки** меню, нажмите кнопку **остановить отладку**.  
...
3.  На **отладки** меню, нажмите кнопку **Запуск без отладки**.  
  
     Веб-форма будет запущена в Internet Explorer без присоединения отладчика.  
  
4.  Присоединитесь к процессу[!INCLUDE[vstecasp](../code-quality/includes/vstecasp_md.md)]. Дополнительные сведения см. в разделе [отладки развертывания веб-приложений](../debugger/debugging-deployed-web-applications.md).  
...
6.  По окончании отладки в **отладки** меню, нажмите кнопку **остановить отладку**.  
