...
- [Подключение к удаленной конечной точке было прекращено](#connection_terminated)
...
## <a name="connection-with-the-remote-endpoint-was-terminated"></a><a name="connection_terminated"></a>Подключение к удаленной конечной точке было прекращено
...
- Если вы пытаетесь подключиться с помощью имени хоста, попробуйте вместо этого IP-адрес.

- Проверьте журнал приложения на сервере (Viewer событий на Windows) для получения более подробной информации, чтобы помочь решить проблему.

- В противном случае попробуйте перезапустить Visual Studio с привилегиями администратора, а затем повторите попытку.
