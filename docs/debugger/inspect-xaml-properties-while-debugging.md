...
![Главное окно приложения](../debugger/media/livevisualtree-app.png "ливевисуалтри-App")

Теперь нажмите кнопку **Добавить элемент** несколько раз, чтобы добавить новые элементы в список.

Затем откройте окно **динамического визуального дерева** , нажав кнопку с крайней левой кнопкой на панели инструментов в приложении (или выбрав **Отладка > Windows > динамическое визуальное дерево**). После открытия перетащите его с позиции закрепления, чтобы мы могли взглянуть на это окно, а окно "Свойства" в **реальном времени** — на сторону. В окне **динамического визуального дерева** разверните узел **ContentPresenter**. Он должен содержать узлы для кнопки и списка. Разверните список (а затем **ScrollContentPresenter** и **ItemsPresenter**), чтобы найти элементы списка. Окно должно выглядеть следующим образом:
