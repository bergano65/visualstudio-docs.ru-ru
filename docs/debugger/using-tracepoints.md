---
title: Сведения о журнале с точками трассировки | Документация Майкрософт
ms.date: 10/28/2019
ms.topic: conceptual
helpviewer_keywords:
...
ms.locfileid: "73187309"
---
# <a name="log-info-to-the-output-window-using-tracepoints-in-visual-studio"></a>Запись сведений о журнале в окно вывода с помощью точек трассировки в Visual Studio

Точки трассировки позволяют записывать данные в окно вывода в области настраиваемые условия без изменения или остановки кода. Эта функция поддерживается как для управляемых языков (C#Visual Basic), F#так и для машинного кода, а также для таких языков, как JavaScript и Python.
...
Для условия фильтра укажите, для каких устройств, процессов или потоков отображается выход.
...
Иногда при проверке свойства или атрибута объекта его значение может измениться. Если значение изменяется во время проверки, это не ошибка, вызванная самой функцией трассировки. Однако использование точек отслеживания для проверки объектов не позволяет избежать этих случайных изменений.
