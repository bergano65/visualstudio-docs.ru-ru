...
- Если вы используете файл HOSTs, убедитесь, что он настроен правильно. Например, если отладка выполняется с помощью **клавиши F5** (вместо **присоединения к процессу**), то файл hosts должен включать тот же URL-адрес проекта, что и в свойствах проекта, **свойства > серверы веб->** или **Свойства > отладки**в зависимости от типа проекта.
...
## <a name="operation_not_supported"></a>Операция не поддерживается. Неизвестная ошибка: *номер ошибки*
