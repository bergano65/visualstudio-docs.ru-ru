...
* На панели инструментов выберите либо **Отладка** или **выпуска** из **конфигураций решения** списка.
...
> [!IMPORTANT]
> Отладчик загружает PDB-файл для исполняемого файла, только если он точно соответствует PDB-файлу, который был создан при сборке исполняемого файла (то есть это должен быть либо оригинальный PDB-файл, либо его копия). Дополнительные сведения см. в разделе [почему Visual Studio требуется, отладчик символ точного соответствия файлов двоичные файлы, которые они были построены?](https://blogs.msdn.microsoft.com/jimgries/2007/07/06/why-does-visual-studio-require-debugger-symbol-files-to-exactly-match-the-binary-files-that-they-were-built-with/)
