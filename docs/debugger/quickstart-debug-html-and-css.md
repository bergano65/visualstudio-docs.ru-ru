...
4.  В элементе BODY index.html добавьте следующий код:  
...
6.  Замените код в файле default.js следующим кодом:  
    ...
    ```  
     ...
     На следующем рисунке, мы хотим видеть, если запустим это приложение. Тем не менее для доведения приложения до такого состояния нам потребуется сначала устранить несколько ошибок.  
  
     ![Приложение FlipView ожидаемым результатам](../debugger/media/js_dom_appfixed.png "JS_DOM_AppFixed")  
  
7.  Выберите **локального компьютера** из раскрывающегося списка рядом с **начать отладку** кнопку **отладки** инструментов:  
...
8.  Выберите **Отладка** > **Начать отладку**или нажмите клавишу F5, чтобы запустить приложение в режиме отладки.  
     ...
     При этом приложение запустится, но вы увидите экран практически пустым, поскольку в стилях имеется несколько ошибок в нем. Первое изображение `FlipView` появляется в малом квадрате около середины экрана.  
...
15. В главном окне дважды щелкните `width: 100px;height: 100px;`, нажмите клавишу **удаление** ключа, а затем нажмите клавишу **ввод**. После нажатия клавиши ВВОД новые значения немедленно отражаются в приложении, хотя сеанс отладки не был остановлен.  
...
16. Перейдите в приложение, выбрав его или использовав сочетание клавиш Alt + Tab.  
...
20. Чтобы изучить вопрос, переключитесь в Visual Studio и выберите **макета** для просмотра рамочной модели элемента.  
    ...
    -   255px (смещение) и 255px (поле) или схожие значения в зависимости от разрешения вашего устройства. 
