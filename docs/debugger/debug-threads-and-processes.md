...
- Для потоков основное средство отладки потоков — это окно **потоков** , маркеры потоков в окнах исходного кода, окно **параллельных стеков** , окно **контрольных значений** и панель инструментов **место отладки** .

- Для кода, использующего <xref:System.Threading.Tasks.Task> в [библиотеке параллельных задач (TPL)](/dotnet/standard/parallel-programming/task-parallel-library-tpl), [Среда выполнения с параллелизмом](/cpp/parallel/concrt/concurrency-runtime/) (машинный код), основные средства для отладки многопоточных приложений — это окно **параллельных стеков** , а также **параллельное наблюдение** . и в окне **задачи** (окно **задачи** также поддерживает объект Promise JavaScript).

- Для отладки потоков на GPU основным средством являются окна **потоков GPU** .
