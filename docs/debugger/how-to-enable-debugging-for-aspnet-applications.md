...
<a name="iis"></a>
- Выбрать **Поддержка времени разработки в IIS** при установке Visual Studio. (При необходимости перезапустите Visual Studio Installer, выберите **Изменить** и добавьте этот компонент.)
- Запустить Visual Studio от имени администратора.
- Установить и правильно настроить IIS, указав соответствующие версии ASP.NET и (или) ASP.NET Core. Дополнительные сведения и инструкции см. в разделе [IIS 8.0 — использование ASP.NET 3.5 и ASP.NET 4.5](/iis/get-started/whats-new-in-iis-8/iis-80-using-aspnet-35-and-aspnet-45) или [Размещение ASP.NET Core в Windows с помощью IIS](/aspnet/core/host-and-deploy/iis/index).
...
1. Для отладки приложения установите точки останова в некоторых местах кода проекта. В панели инструментов Visual Studio убедитесь, что для конфигурации задано значение **Отладка**, и нужный вам браузер отображается в поле эмулятора **IIS Express (\<Имя браузера>)** или **Локальный сервер IIS (\<имя браузера>)** .

1. Чтобы начать отладку, в панели инструментов выберите **IIS Express (\<имя браузера>)** или **Локальный сервер IIS (\<имя браузера>)** , а затем выберите **Начать отладку** в меню **Отладка** или нажмите клавишу **F5**. Отладчик приостанавливает выполнение в точках останова. Если отладчик не может попасть в точки останова, см. раздел [Устранение неполадок при отладке](#troubleshoot-debugging).
...
1. Чтобы начать отладку, в панели инструментов выберите **IIS Express** или **\<Имя профиля IIS>** , а затем выберите **Начать отладку** в меню **Отладка** или нажмите клавишу **F5**. Отладчик приостанавливает выполнение в точках останова. Если отладчик не может попасть в точки останова, см. раздел [Устранение неполадок при отладке](#troubleshoot-debugging).
...
3. Выберите **IIS, FTP и т. п.** и нажмите **Опубликовать**.
    ...
    ![Публикация в IIS](media/dbg-aspnet-local-iis.png "Публикация в службах IIS")
...
1. Нажмите **Публиковать**.
    ...
    ![Публикация в IIS](media/dbg-aspnet-local-iis-select-site.png "Публикация в службах IIS")
