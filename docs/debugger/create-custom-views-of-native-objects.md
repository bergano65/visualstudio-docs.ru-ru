...
- В **разделе сервис** > **Параметры** (или **Параметры** > **отладки** ) **> Отладка** > **окно вывода**, установка **диагностическихC++ сообщений Natvis (только) в "** **Ошибка**", " **предупреждение"** или **verbose**, а затем нажмите кнопку **ОК**.
...
`Exec` можно использовать для выполнения кода внутри `CustomListItems` расширения с помощью переменных и объектов, определенных в расширении. С `Exec` можно использовать логические операторы, арифметические операторы и операторы присваивания. Для вычисления функций нельзя использовать `Exec`.
