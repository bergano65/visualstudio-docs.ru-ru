...
Например, на следующем рисунке показана переменная типа [Windows:: UI:: XAML:: Controls:: TextBox](http://go.microsoft.com/fwlink/?LinkId=258422) в окне отладчика без примененных пользовательских визуализаций.

![](../debugger/media/dbg_natvis_textbox_default.png "Визуализация") по умолчанию текстового поля визуализации TextBox
...
Тот же `TextBox` выглядит гораздо проще в окне переменных, когда применяются пользовательские правила визуализатора Natvis. Важные элементы класса отображаются вместе, и отладчик отображает базовое строковое значение типа настраиваемой строки.

Использование ![данных текстового поля визуализатора]с(../debugger/media/dbg_natvis_textbox_visualizer.png "помощью визуализатора")
...
Базовая структура *natvis* -файла — один или несколько элементов `Type`, представляющих записи визуализации. Полное имя каждого элемента `Type` указывается в его атрибуте `Name`.
...
![Окно контрольных значений с]простым представлением(../debugger/media/watch-simpleview.png "окно контрольных значений") простого представления
...
- В **разделе сервис** > **Параметры** (или **Отладка**@no__t **-4)** > **Отладка** > **окно вывода**, установка **диагностическихC++ сообщений Natvis (только) в "** **Ошибка**", " **предупреждение"** или **verbose**, а затем нажмите кнопку **ОК**.
...
3. Как должны выглядеть элементы типа, когда пользователь расширяет тип в окне переменной (`Expand`).

#### <a name="templated-classes"></a>Шаблонные классы
Атрибут `Name` элемента `Type` принимает звездочку `*` в качестве подстановочного знака, который можно использовать для шаблонов имен классов.

В следующем примере используется та же визуализация, что и объект `CAtlArray<int>` или `CAtlArray<float>`. Если имеется определенная запись визуализации для `CAtlArray<float>`, то она имеет приоритет над универсальным.
...
Атрибут `Optional` можно разместить на любом узле. Если часть выражения внутри необязательного узла не удается проанализировать, отладчик игнорирует этот узел, но применяет остальные правила `Type`. В следующем типе `[State]` является обязательным, а `[Exception]` — необязательным.  Если `MyNamespace::MyClass` содержит поле с именем _ @ no__t-1, появляются как узел `[State]`, так и узел `[Exception]`, но если `_M_exceptionHolder` нет, появится только узел `[State]`.
...
Атрибуты `IncludeView` и `ExcludeView` указывают элементы, которые должны отображаться или не отображаться в конкретных представлениях. Например, в следующей спецификации Natvis `std::vector` представление `simple` не отображает элементы `[size]` и `[capacity]`.
...
Для типов и отдельных элементов можно использовать атрибуты `IncludeView` и `ExcludeView`.
...
Означает, что переменные типа `CPoint` отображаются, как показано на рисунке.

 ![Использование элемента DisplayString]с элементом(../debugger/media/dbg_natvis_cpoint_displaystring.png "DisplayString")

В выражении `DisplayString` `x` и `y`, являющиеся членами `CPoint`, находятся внутри фигурных скобок, поэтому их значения оцениваются. В примере также показано, как можно поэкранировать фигурные скобки с помощью двойных фигурных скобок (`{{` или `}}`).
...
Элемент `StringView` определяет значение, которое отладчик может отправить во встроенный визуализатор текста. Например, при наличии следующей визуализации для типа `ATL::CStringT`:
...
Элемент ![CStringT DisplayString](../debugger/media/dbg_natvis_displaystring_cstringt.png "CStringT элемент DisplayString")
...
Во время отладки можно выбрать значок лупы рядом с переменной, а затем выбрать **Визуализатор текста** , чтобы отобразить строку, на которую **m_pszData** указывает.

 ![CStringT данных с помощью стрингвиев визуализатора](../debugger/media/dbg_natvis_stringview_cstringt.png "CStringT данных с помощью визуализатора стрингвиев")
...
Необязательный узел `Expand` настраивает дочерние элементы визуального типа при развертывании типа в окне переменной. Узел `Expand` принимает список дочерних узлов, которые определяют дочерние элементы.

- Если в записи визуализации не указан узел `Expand`, то дочерние элементы используют правила расширения по умолчанию.

- Если узел `Expand` не имеет дочерних узлов, этот тип нельзя развернуть в окнах отладчика.
...
![Крект с расширением](../debugger/media/dbg_natvis_expand_item_crect1.png "крект элемента Item с расширением элемента Item")
...
![std:: Vector, использующий ArrayItems расширение](../debugger/media/dbg_natvis_expand_arrayitems_stdvector.png "std:: Vector с помощью расширения ArrayItems")
...
- Выражение `ValuePointer`, которое указывает на первый элемент (который должен быть указателем на тип элемента, который не `void*`).
...
- Элемент `Size` принимает неявный параметр `$i`, вместо которого подставляет индекс измерения для нахождения длины массива в этом измерении. В предыдущем примере выражение `_M_extent.M_base[0]` должно дать длину нулевого измерения, `_M_extent._M_base[1]` первый и т. д.

Вот как двухмерный объект `Concurrency::array` выглядит в окне отладчика:

![Двухмерный массив с ArrayItems расширением]двумерного(../debugger/media/dbg_natvis_expand_arrayitems_2d.png "массива с ArrayItems расширением")

#### <a name="BKMK_IndexListItems_expansion"></a> Расширение IndexListItems

Расширение `ArrayItems` можно использовать только в том случае, если элементы массива располагаются последовательно в памяти. Отладчик получает к следующему элементу, просто увеличивая его указатель. Если необходимо управлять индексом в узле значение, используйте узлы `IndexListItems`. Ниже приведена Визуализация с узлом `IndexListItems`:
...
Единственным различием между `ArrayItems` и `IndexListItems` является `ValueNode`, что подразумевает полное выражение с<sup>элементом i,</sup> который имеет неявный параметр `$i`.
...
Отладчик вычисляет выражения `NextPointer` и `ValueNode` в контексте элемента `LinkedListItems`, а не типа родительского списка. В предыдущем примере `CAtlList` имеет класс `CNode` (находится в `atlcoll.h`), который является узлом связанного списка. `m_pNext` и `m_element` являются полями этого класса `CNode`, а не класса `CAtlList`.
...
Расширение `CustomListItems` позволяет записывать настраиваемую логику для обхода структуры данных, например хэш-таблицы. Используйте `CustomListItems` для визуализации структур данных, которые могут использовать C++ выражения для всех элементов, которые необходимо оценить, но не вполне соответствуют определение для `ArrayItems`, `IndexListItems` или `LinkedListItems`.
...
@No__t-0 можно использовать для выполнения кода внутри `CustomListItems`, используя переменные и объекты, определенные в расширении. С `Exec` можно использовать логические операторы, арифметические операторы и операторы присваивания. Для вычисления функций нельзя использовать `Exec`.
...
#### <a name="BKMK_TreeItems_expansion"></a> Расширение TreeItems
 Если визуализированный тип представляет дерево, отладчик может пройти дерево отображать его дочерние элементы с помощью узла `TreeItems` . Ниже приведена визуализация для типа `std::map` с помощью узла `TreeItems`:
...
Например, тип интеллектуального указателя `auto_ptr<vector<int>>` обычно отображается как:
 ...
 Автоопределение(../debugger/media/dbg_natvis_expand_expandeditem_default.png "расширения по") умолчанию для ![автоматического&#95;&#60;вектора&#60;int&#62; &#62; по умолчанию]

 Чтобы увидеть значения вектора, необходимо детализировать два уровня в окне переменных, передав элемент `_Myptr`. При добавлении элемента `ExpandedItem` можно исключить переменную `_Myptr` из иерархии и просмотреть непосредственно элементы вектора elements:
...
```
 ...
 ![Автоматическое&#95;расширение&#60;типа int&#62; &#60;&#62; в векторном расширение expandeditem](../debugger/media/dbg_natvis_expand_expandeditem_visualized.png "расширение expandeditem")
...
```
 ...
 Concurrency ![:: Array с параллельным развертыванием искусственного элемента](../debugger/media/dbg_natvis_expand_synthetic.png ":: Array с расширением искусственного элемента")
...
- Атрибут `MenuName` определяет имя визуализатора, которое будет отображаться в раскрывающемся списке рядом со значком лупы в отладчике. Пример:
  ...
  Контекстное меню меню(../debugger/media/dbg_natvis_vectorvisualizer.png "UIVisualizer") ![контекстного меню UIVisualizer]
...
```
 ...
 Пример `UIVisualizer` можно увидеть в расширении [просмотра изображения](https://marketplace.visualstudio.com/items?itemName=VisualCPPTeam.ImageWatch2017) , используемом для просмотра растровых изображений в памяти.
