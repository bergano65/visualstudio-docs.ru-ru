...
|Отладка .NET Core в Linux|Используйте функцию **Подключиться к процессу**|*dotnet.exe*|Сведения об использовании см. в статье [Удаленная отладка .NET Core в Linux с помощью SSH](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md). Сведения о контейнерных приложениях см. в предыдущих разделах этой статьи.|
