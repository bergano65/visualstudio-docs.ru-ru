...
2. **Тип соединения** должно быть **по умолчанию** для большинства случаев. В **цель подключения** окне выбрать удаленный компьютер, с помощью одного из следующих методов:
   ...
   - Введите имя компьютера в **цель подключения** поле и нажмите клавишу **ввод**.
...
Для отладки скриптов на стороне клиента необходимо включить отладку скриптов в браузере. Отладка клиентского сценария в Chrome, выберите **веб-пакета** как тип кода и в зависимости от типа приложения, может потребоваться закрыть все экземпляры Chrome и запустите браузер в режиме отладки (тип `chrome.exe --remote-debugging-port=9222` из командной строки).
...
|Отладка приложения C#, Visual Basic или C++ на локальном компьютере|Используйте стандартные отладку (**F5**) или **присоединение к процессу**|*\<имя_приложения > .exe*|В большинстве случаев использовать стандартный отладку и не **присоединение к процессу**.|
