...
|Отладка .NET Core в Linux|Использовать **Присоединение к процессу**|*dotnet.exe*|Сведения об использовании SSH см. в статье [Удаленная отладка .NET Core под управлением Linux с помощью SSH](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md). |
