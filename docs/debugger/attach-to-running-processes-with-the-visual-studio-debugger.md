...
Дополнительные сведения см. [в статье Удаленная отладка .NET Core под управлением Linux с помощью SSH](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md).
...
|Отладка .NET Core в Linux|Использовать **Присоединение к процессу**|*dotnet.exe*|Сведения об использовании SSH см. в статье [Удаленная отладка .NET Core под управлением Linux с помощью SSH](../debugger/remote-debugging-dotnet-core-linux-with-ssh.md). Сведения об использовании контейнера DOCKER для Linux см. в статье [Подключение к процессу, выполняющемуся в контейнере DOCKER для Linux](#BKMK_Docker_Attach).|
