...
description: Отладка с помощью окон параллельных задач и параллельных стеков в Visual Studio
ms.date: 03/22/2018
ms.topic: conceptual
dev_langs:
- CSharp
...
ms.locfileid: "71252519"
---
# <a name="walkthrough-debugging-a-parallel-application-in-visual-studio-c-visual-basic-c"></a>Пошаговое руководство. Отладка параллельного приложения в Visual StudioC#(, Visual Basic C++,)

В этом пошаговом руководстве описывается использование окон **Параллельные задачи** и **Параллельные стеки** для отладки параллельного приложения. Эти окна помогают понять и проверить поведение во время выполнения кода, использующего [библиотеку параллельных задач (TPL)](/dotnet/standard/parallel-programming/task-parallel-library-tpl) или [Среда выполнения с параллелизмом](/cpp/parallel/concrt/concurrency-runtime). Примеры кода, приведенные в этом пошаговом руководстве, имеют встроенные точки останова. Возможности диалоговых окон **Параллельные задачи** и **Параллельные стеки** показаны после прерывания выполнения программы.
...
## <a name="prerequisites"></a>Предварительные требования
 В этом пошаговом руководстве предполагается, что **только мой код** включен (по умолчанию он включен в более поздних версиях Visual Studio). В меню **Сервис** выберите пункт **Параметры**, разверните узел **Отладка**, выберите пункт **Общие** и затем установите флажок **Включить режим "Только мой код" (только управляемый код)** . Если эта возможность не задана, можно продолжить выполнение инструкций настоящего пошагового руководства, однако полученные результаты могут отличаться от описанных здесь.
...
1. Откройте Visual Studio и создайте новый проект.
    ...
    Нажмите клавишу **ESC**, чтобы закрыть окно запуска. Введите **CTRL + Q** , чтобы открыть поле поиска, введите **Console** (или **c++** ), выберите **шаблоны**, а затем:

    - Для C# или Visual Basic выберите **создать новый проект консольного приложения (.NET Framework)** для C# или Visual Basic. В появившемся диалоговом окне выберите **Создать**.
    - Для C++выберите **создать новый проект консольного приложения** для C++. В появившемся диалоговом окне выберите **Создать**.

    Затем введите имя или используйте имя по умолчанию и нажмите кнопку **создать**.
    ::: moniker-end
    ...
    Затем введите имя или используйте имя по умолчанию и нажмите кнопку **ОК**.
    ::: moniker-end
...
1. В меню **Отладка** наведите указатель на пункт **Окна** и затем выберите пункт **Параллельные стеки**. Убедитесь, что в верхнем левом углу выбрано значение **Потоки**.
     ...
     С помощью окна **Параллельные стеки** можно одновременно просматривать несколько стеков вызовов в одном представлении. На следующем рисунке показано окно **Параллельные стеки** над окном **стека вызовов** .

     ![Представление "потоки" в окне "Параллельные стеки] " (../debugger/media/pdb_walkthrough_1.png "PDB_Walkthrough_1")

     Стек вызовов главного потока отображается в одном окне, а стеки вызовов для других четырех потоков сгруппированы в другом окне. Четыре потока сгруппированы вместе, поскольку их кадры стека совместно используют одни и те же контексты методов: `A`, `B` и `C`. Чтобы просмотреть идентификаторы потоков и имена потоков, совместно использующих одно и то же поле, наведите указатель мыши на поле с заголовком (**4 потока**). Текущий поток выводится полужирным шрифтом.

     ![Подсказка, показывающая идентификаторы и имена потоков](../debugger/media/pdb_walkthrough_1a.png "PDB_Walkthrough_1A")
     ...
     ![Выделенный основной поток в окне "Параллельные стеки] " (../debugger/media/pdb_walkthrough_1c.png "PDB_Walkthrough_1C")
...
1. Чтобы возобновить выполнение до попадания во вторую точку останова, в меню **Отладка** выберите пункт **Продолжить**. На следующем рисунке показано дерево потоков для второй точки останова.
     ...
     ![Окно параллельных стеков, в котором отображается много ветвей](../debugger/media/pdb_walkthrough_2.png "PDB_Walkthrough_2")
     ...
     Значок ткани — потоки (с переплетенными линиями) указывает активные кадры стека для неактуальных потоков. В окне **Стек вызовов** дважды щелкните S.B, чтобы переключить кадры. Текущий кадр стека текущего потока показан в окне **Параллельные стеки** с помощью зеленой круговой стрелки.
     ...
     ![Меню стеков с 2 параметрами на C, когда J является текущим](../debugger/media/pdb_walkthrough_3.png "PDB_Walkthrough_3")
...
1. Чтобы возобновить выполнение до попадания в третью точку останова, в меню **Отладка** выберите пункт **Продолжить**.
     ...
     ![Представление методов в окне "Параллельные стеки] " (../debugger/media/pdb_walkthrough_4.png "PDW_Walkthrough_4")
    ...
    - **Показать потоки в источнике** переключает отображение маркеров потоков в исходном коде, который показывает расположение потоков в исходном коде.
...
1. Чтобы возобновить выполнение до попадания в четвертую точку останова, в меню **Отладка** выберите пункт **Продолжить**.
     ...
     При наличии больших схем в окне **Параллельные стеки** также удобно воспользоваться функцией **Вид с высоты птичьего полета**. По умолчанию используется **представление "глаз" с высоты птичьего полета** . Но вы можете переключить ее, нажав кнопку между полосами прокрутки в правом нижнем углу окна, как показано на следующем рисунке.

     ![Представление&#45;глаз с высоты птичьего полета в окне параллельных стеков](../debugger/media/pdb_walkthrough_5.png "PDB_Walkthrough_5")

     В представлении "глаз" с высоты можно перемещать прямоугольник для быстрого панорамирования диаграммы.
...
5. В меню **Отладка** укажите пункт **окна**, а затем выберите пункт **задачи**. На следующем рисунке показано окно **задачи** .
...
1. Чтобы возобновить выполнение до попадания во вторую точку останова, в меню **Отладка** выберите пункт **Продолжить**.
     ...
     Ранее в столбце **состояние** были показаны все задачи как активные, но теперь две задачи заблокированы. Задачи могут быть заблокированы по разным причинам. Чтобы узнать причину блокирования задачи, наведите указатель мыши на задачу в столбце **Состояние**. Например, на следующем рисунке задача 3 ожидает задачу 4.

     ![Две ожидающие задачи в окне задач](../debugger/media/pdb_walkthrough_7.png "PDB_Walkthrough_7")

     Задача 4, в свою очередь, ожидает монитор, принадлежащий потоку, назначенному задаче 2. (Щелкните правой кнопкой мыши строку заголовка и выберите **столбцы** > **Назначение потоков** , чтобы просмотреть значение назначения потока для задачи 2).

     ![Ожидание задачи и подсказка в окне задач](../debugger/media/pdb_walkthrough_7a.png "PDB_Walkthrough_7A")
     ...
     ![Представление "задачи" в окне "Параллельные стеки] " (../debugger/media/pdb_walkthrough_8.png "PDB_Walkthrough_8")
...
1. Чтобы возобновить выполнение до попадания в третью точку останова, в меню **Отладка** выберите пункт **Продолжить**.
     ...
     Теперь запускается новая задача под номером 5, а задача 4 переходит в режим ожидания. Для просмотра причины перехода в режим ожидания наведите указатель мыши на задачу в окне **Состояние**. Обратите внимание, что в **родительском** столбце Задача 4 является родителем задачи 5.

     Чтобы лучше визуализировать связь «родители-потомки», щелкните правой кнопкой мыши строку заголовка столбца и выберите пункт **родительское подчиненное представление**. Должно отображаться такое же окно, как на следующем рисунке.

     ![Родительское&#45;подчиненное представление в окне задач](../debugger/media/pdb_walkthrough_9.png "PDB_Walkthrough_9")

     Обратите внимание, что задачи 4 и 5 выполняются в одном потоке (отобразится столбец **назначения потока** , если он скрыт). Эти сведения не отображаются в окне **потоки** . Здесь можно увидеть, что это еще одно преимущество окна **задач** . Чтобы проверить это, откройте окно **Параллельные стеки**. Убедитесь, что в качестве представления выбран пункт **Задачи**. Чтобы выбрать задачи 4 и 5, дважды щелкните их в окне **задачи** . После выполнения этого действия синяя граница в окне **Параллельные стеки** будет обновлена. Кроме того, расположение задач 4 и 5 можно определить путем просмотра подсказок в окне **Параллельные стеки**.

     ![Представление задач в окне "Параллельные стеки] " (../debugger/media/pdb_walkthrough_9a.png "PDB_Walkthrough_9A")
...
1. Чтобы возобновить выполнение до попадания в третью точку останова, в меню **Отладка** выберите пункт **Продолжить**. Щелкните заголовок столбца **ИД**, чтобы выполнить сортировку по идентификатору. Должно отображаться такое же окно, как на следующем рисунке.
     ...
     Задачи 3 и 4 теперь ожидают друг друга и блокируются. 5 новых задач являются дочерними по отношению к задаче 2 и в данный момент запланированы. Запланированными задачами являются задачи, которые были запущены в коде, но еще не запущены на компьютере. Поэтому для этих задач значения в столбцах **Расположение** и **Назначение потоков** отсутствуют.
     ...
     Можно сгруппировать задачи по столбцу. В окне **задачи** щелкните правой кнопкой мыши заголовок столбца **состояние** и выберите пункт **Группировать по состоянию**. На следующем рисунке показано окно **задачи** , сгруппированное по состоянию.

     ![Сгруппированные задачи в окне задач](../debugger/media/pdb_walkthrough_12.png "PDB_Walkthrough_12")
     ...
     Последняя функция в окне **задачи** для проверки — это контекстное меню, отображаемое при щелчке правой кнопкой мыши по задаче.
     ...
     Можно зафиксировать поток задачи или задачи, либо зафиксировать все потоки, за исключением назначенного. Замороженный поток представлен в окне **задачи** в том виде, в котором оно находится в окне **потоки** , с помощью синего значка *паузы* .
