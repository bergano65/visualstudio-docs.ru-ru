---
title: Параметры проекта для конфигурации C++ отладки
ms.custom: seodec18
ms.date: 11/26/2018
ms.topic: reference
f1_keywords:
...
Можно изменить параметры проекта для конфигурации C или C++ отладки в диалоговом окне **страницы свойств** , как описано в разделе [как задать конфигурации отладки и выпуска](../debugger/how-to-set-debug-and-release-configurations.md). В следующих таблицах показано, где в диалоговом окне **Страницы свойств** можно найти параметры, связанные с отладчиком.

> [!NOTE]
> Параметры проекта отладки в категории **Свойства конфигурации/отладка** отличаются для приложений UWP и для компонентов, написанных на C++. См. раздел [Запуск сеанса отладки (VB C#, C++ и XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md).

 Каждый параметр свойства Debug автоматически записывается и сохраняется в файле "на пользователя" (VCXPROJ. User) для решения при сохранении решения.

 Укажите, какой отладчик следует использовать в окне списка **отладчик для запуска** , как описано в следующей таблице. В зависимости от выбора, будут отображены те или иные свойства.
...
| **Среда** (локальный отладчик Windows и удаленный отладчик Windows) | Указывает переменные среды для отлаживаемой программы. Используйте стандартный синтаксис переменных среды (например, `PATH="%SystemRoot%\..."`). Эти переменные переопределяют системную среду или объединяются с ее переменными, в зависимости от значения параметра **Объединение среды**. Если щелкнуть левой кнопкой мыши в столбце параметры, то изменить... отображен. Выберите эту ссылку, чтобы изменить переменные среды. |
...
|**Имя файла отображения** ([/Map:](/cpp/build/reference/map-generate-mapfile)*имя*)|Если выбрано создание файла сопоставления, в этом поле можно указать имя файла сопоставления. Дополнительные сведения см. в разделе [Параметр /MAP (создание файла сопоставления)](/cpp/build/reference/map-generate-mapfile).|
|**Сопоставлять экспортируемые функции** ([/MAPINFO:EXPORTS](/cpp/build/reference/mapinfo-include-information-in-mapfile))|Включение экспортируемых функций в файл сопоставления. По умолчанию выбрано значение "Нет". Дополнительные сведения см. [в разделе/MapInfo (включение сведений в параметр сопоставления)](/cpp/build/reference/mapinfo-include-information-in-mapfile).|
...
Для отладки типов проектов, таких как статические библиотеки и библиотеки DLL, проект Visual Studio должен иметь возможность найти правильные файлы. Если исходный код доступен, можно добавить статические библиотеки и библиотеки DLL в качестве отдельных проектов для одного решения, чтобы упростить отладку. Сведения о создании этих типов проектов см. в статьях [Создание и использование библиотеки динамической компоновки (DLL)](/cpp/build/walkthrough-creating-and-using-a-dynamic-link-library-cpp) и [Создание с помощью статической библиотеки](/cpp/windows/walkthrough-creating-and-using-a-static-library-cpp). Если доступен исходный код, можно также создать новый проект Visual Studio, выбрав **файл**  > **Новый**  > **проект из существующего кода**.

Сведения об отладке библиотек DLL, которые являются внешними по отношению к проекту, см. в разделе [Отладка проектов DLL](../debugger/debugging-dll-projects.md#vxtskdebuggingdllprojectsexternal). Если вам нужно выполнить отладку проекта библиотеки DLL, но нет доступа к проекту для вызывающего приложения, см. раздел [Отладка из проекта DLL](../debugger/how-to-debug-from-a-dll-project.md).

## <a name="see-also"></a>См. также
- [Отладка машинного кода](../debugger/debugging-native-code.md)
- [Параметры отладчика и подготовка](../debugger/debugger-settings-and-preparation.md)
- [Создание проектов и C++ управление ими](/cpp/ide/creating-and-managing-visual-cpp-projects)
