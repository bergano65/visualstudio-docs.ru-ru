...
- **Включить поддержку сервера системы управления версиями**  
  ...
  >[!IMPORTANT]
  >В приложении могут быть внедрены произвольные команды *PDB-файл* файл, поэтому убедитесь, что только те команды, которые необходимо выполнить в *srcsrv.ini* файл. Любая попытка выполнить команду не из файла *srcsvr.ini* вызовет диалоговое окно подтверждения. Для получения дополнительной информации см. [Security Warning: Debugger Must Execute Untrusted Command](../debugger/security-warning-debugger-must-execute-untrusted-command.md). 
  >
...
[Изменения принципов загрузки удаленных символов .NET в Visual Studio 2012 и 2013](https://blogs.msdn.microsoft.com/devops/2013/10/16/net-remote-symbol-loading-changes-in-visual-studio-2012-and-2013/)
