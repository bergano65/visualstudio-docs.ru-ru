...
- **Включить поддержку сервера системы управления версиями**  
  ...
  >[!IMPORTANT]
  >В приложении могут быть внедрены произвольные команды *PDB-файл* файл, поэтому убедитесь, что только те команды, которые необходимо выполнить в *srcsrv.ini* файл. Любая попытка выполнить команду не из файла *srcsvr.ini* вызовет диалоговое окно подтверждения. Дополнительные сведения см. в разделе [предупреждение системы безопасности: Отладчик должен выполнить ненадежную команду](../debugger/security-warning-debugger-must-execute-untrusted-command.md). 
  >
