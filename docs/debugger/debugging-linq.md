...
## <a name="BKMK_EditandContinueNotSupportedforLINQ"></a> Операция "Изменить и продолжить" для LINQ не поддерживается
 "Изменить и продолжить" поддерживает изменения в запросах LINQ с ограничениями. Дополнительные сведения см. в разделе [Поддерживаемые изменения для ENC](https://github.com/dotnet/roslyn/wiki/EnC-Supported-Edits).
