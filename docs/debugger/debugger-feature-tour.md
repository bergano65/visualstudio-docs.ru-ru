...
ms.locfileid: "72911640"
---
# <a name="first-look-at-the-visual-studio-debugger"></a>Знакомство с отладчиком Visual Studio

В этом разделе представлены средства отладчика, предоставляемые Visual Studio. В контексте Visual Studio *отладка приложения* обычно означает запуск приложения с подключенным отладчиком (то есть в режиме отладчика). При этом в отладчике доступно множество способов наблюдения за выполнением кода. Вы можете пошагово пройти код и просмотреть значения, хранящиеся в переменных. Вы можете установить контроль над переменными, чтобы увидеть, когда значения изменяются, можно проверить путь выполнения кода, то есть выполнить команду et al. Если вы впервые пытались выполнить отладку кода, вы можете ознакомиться с разделом [Отладка для](../debugger/debugging-absolute-beginners.md) начинающих, прежде чем перейти к этому разделу.
...
Нажмите клавишу **F5** (**Отладка > начать отладку**) или нажмите кнопку **начать отладку** , чтобы ![начать отладку](../debugger/media/dbg-tour-start-debugging.png "Начало отладки") на панели инструментов Отладка, и отладчик выполняется до первой обнаруженной точки останова. Если приложение еще не запущено, при нажатии клавиши F5 запускается отладчик и выполняется остановка в первой точке останова.
...
Клавишу F11 удобно использовать для более детальной проверки потока выполнения. (Чтобы быстрее перемещаться по коду, мы также покажем некоторые другие параметры.) По умолчанию отладчик пропускает код, не являющийся пользовательским (если требуются дополнительные сведения, см. раздел [только мой код](../debugger/just-my-code.md)).
...
![Шаг с заходом](../debugger/media/dbg-tour-step-into-specific.png "Шаг с заходом")

В этом примере команда **Шаг с заходом в конкретное расположение** приводит нас к коду для `Path.set`.

![Шаг с заходом](../debugger/media/dbg-tour-step-into-specific-2.png "Шаг с заходом")

## <a name="run-to-a-point-in-your-code-quickly-using-the-mouse"></a>Быстрое выполнение до точки в коде с помощью мыши

В отладчике наведите указатель мыши на строку кода до нажатия кнопки **Запуск (выполнение** ), ![чтобы](../debugger/media/dbg-tour-run-to-click.png "рунтокликк") открыть ее слева.

![Запустить, чтобы щелкнуть](../debugger/media/dbg-tour-run-to-click-2.png "Выполнение до щелкнутого")
...
Закройте отладчик, нажав кнопку " **закончить отладку** ", а затем — " ![Отключить отладку](../debugger/media/dbg-tour-stop-debugging.png "Остановить отладку") " или " **SHIFT** " + **клавишу F5**.
...
Нажмите кнопку **перезапуск** ![приложения перезапустить приложение](../debugger/media/dbg-tour-restart.png "Перезапустить приложение") на панели инструментов Отладка (**CTRL + SHIFT + F5**).
...
Если вы хотите прерывать работу отладчика и вернуться в редактор кода, нажмите красную кнопку "закончить ![отладку](../debugger/media/dbg-tour-stop-debugging.png "Остановить отладку") ", а не **перезапустить**.
...
![Просмотр Совета по данным](../debugger/media/dbg-tour-data-tips.gif "Просмотр Совета по данным")
...
![Окно видимых переменных](../debugger/media/dbg-tour-autos-window.png "Видимые - окно")
...
![Окно контрольных значений](../debugger/media/dbg-tour-watch-window.png "Контрольные значения - окно")
...
![Проверка стека вызовов](../debugger/media/dbg-tour-call-stack.png "Просмотр стека вызовов")
...
![Вспомогательное приложение исключения](../debugger/media/dbg-tour-exception-helper.png "Вспомогательное приложение исключения")
...
![Запуск отладчика моментальных снимков](../debugger/media/snapshot-launch.png "Запустить Snapshot Debugger")
...
![Кнопки перехода назад и вперед](../debugger/media/intellitrace-step-back-icons-description.png  "Кнопки перехода назад и вперед")
