---
title: Использование точек останова в отладчике Visual Studio | Документация Майкрософт
...
2.  В **адрес** введите адрес памяти или выражение, результатом которого будет адрес памяти. Например, для прерывания при изменении содержимого переменной `&avar` введите `avar` .  
...
-   Процесс, который не отлаживается записывает область памяти.  
...
>  
>- Если задать точку останова по данным на локальную переменную, точка останова остается включенной, когда функция заканчивается, но адрес памяти больше не применяется, поэтому поведение точки останова будет непредсказуемым. Если задать точку останова по данным на локальную переменную, следует удалить или отключить точку останова до конца функции.  
...
1. В раскрывающемся списке выберите **условное выражение**, **число попаданий**, или **фильтра**и задайте значение соответствующим образом. 
  
1. Выберите **закрыть** или нажмите клавишу **Ctrl**+**ввод** закрыть **параметры точки останова** окна. Или с **точки останова** выберите **ОК** чтобы закрыть диалоговое окно. 
...
При выборе **условное выражение**, можно выбрать два условия: **верно** или **при изменении**. Выберите **верно** чтобы прервать выполнение при истинности выражения, или **при изменении** чтобы прервать выполнение при изменении значения выражения.  
 ...
 >[!NOTE]
 >Поведение **при изменении** поле отличается для различных языков программирования. 
...
3. Добавить новую точку останова в точке, которую вы хотите изучить; Например, если объект находится для добавления в коллекцию. Щелкните правой кнопкой мыши точку останова и выберите **условия**.  
   
4. Используйте идентификатор объекта в **условное выражение** поля. Например если переменная `item` — это объект, добавляемый в коллекцию, выберите **верно** и тип **item == $\<n >**, где \<n > — номер идентификатора объекта .  
...
##  <a name="BKMK_Print_to_the_Output_window_with_tracepoints"></a> Действия точки останова и точки отслеживания  
 Объект *трассировки* — это точка останова, которая выводит сообщение для **вывода** окна. Точка трассировки может играть роль временного оператора трассировки в языке программирования.  
