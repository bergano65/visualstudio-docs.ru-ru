...
ms.locfileid: "71814361"
---
# <a name="use-breakpoints-in-the-visual-studio-debugger"></a>Использование точек останова в отладчике Visual Studio
Точки останова — один из наиболее важных методов отладки на панели элементов разработчика. Точки останова устанавливаются везде, где нужно приостановить выполнение отладчика. Например, может потребоваться просмотреть состояние переменных кода или просмотреть стек вызовов в определенной точке останова. Если вы не знакомы с процессом отладки, перед выполнением задач в этой статье рекомендуется прочесть документ об [отладке для начинающих](../debugger/debugging-absolute-beginners.md).

## <a name="BKMK_Overview"></a>Установка точек останова в исходном коде
 Можно установить точку останова в любой строке исполняемого кода. Например, в следующем C# коде можно задать точку останова в объявлении переменной, цикле `for` или любом коде внутри цикла `for`. Нельзя задать точку останова в объявлении пространства имен или класса или в сигнатуре метода.

 Чтобы задать точку останова в исходном коде, щелкните в крайнем левом поле рядом со строкой кода. Можно также выбрать линию и нажать клавишу **F9**, выбрать **Отладка** > **Переключить точку останова**или щелкнуть правой кнопкой мыши и выбрать пункт **точка останова** > **Вставить точку останова**. Точка останова отображается в виде красной точки в левом поле.

В C# коде точки останова и текущие строки выполнения выделяются автоматически. Для C++ кода можно включить выделение точек останова и текущих строк, выбрав **инструменты** (или **отладка**) > **Параметры** > **Отладка** >  **выделить всю исходную строку для точек останова и текущих оператор (C++ только)** .

 Установка(../debugger/media/basicbreakpoint.png "базовой точки") останова ![точки останова]
 ...
 В точке останова в следующем примере значение `testInt` по-прежнему равно 1.

 Выполнение точки останова ![остановлено](../debugger/media/breakpointexecution.png "выполнение точки останова")

 При остановке отладчика в точке останова можно просмотреть текущее состояние приложения, включая значения переменных и стек вызовов. Дополнительные сведения о стеке вызовов см. в разделе [How: Используйте окно Стек вызовов @ no__t-0.

- Точка останова — это переключатель. Можно щелкнуть его, нажать клавишу **F9**или использовать **Debug** > , чтобы удалить или повторно вставить**точку останова** .
...
1. Чтобы открыть окно **стека вызовов** , необходимо приостановить во время отладки. Выберите **отладка**@no__t-**1 Windows**@no__t-**3 стек вызовов**или нажмите клавиши **CTRL**+**ALT**+**C**.
...
Дополнительные сведения о стеке вызовов см. в разделе [How: Используйте окно Стек вызовов @ no__t-0.
...
## <a name="BKMK_Set_a_breakpoint_in_a_source_file"></a>Задание точек останова для функций
  ...
  Выполнение можно прерывать при вызове функции.

**Чтобы задать точку останова функции, сделайте следующее:**

1. Выберите **отладка** > **Новая точка останова** > **точка останова функции**или нажмите клавиши **ALT**+**F9** > **CTRL**0**B**.

   В окне **точки останова** можно также выбрать пункт **Новая** **точка останова по функциям**  > .
...
### <a name="set-a-function-breakpoint-using-a-memory-address-native-c-only"></a>Задание точки останова функции с помощью адреса памяти ( C++ только машинный код)
 Адрес объекта можно использовать для задания точки останова функции в методе, вызываемом конкретным экземпляром класса.  Например, при наличии объекта с адресацией типа `my_class` можно установить точку останова функции в методе `my_method`, который вызывает экземпляр.

1. Установка точки останова после создания экземпляра класса.

2. Найдите адрес экземпляра (например, `0xcccccccc`).

3. Выберите **отладка** > **Новая точка останова** > **точка останова функции**или нажмите клавиши **ALT**+**F9** > **CTRL**0**B**.
...
2. В окне " **видимые**", " **Контрольные значения**" или " **локальные** " щелкните правой кнопкой мыши свойство и выберите команду **прервать при изменении значения** в контекстном меню.
    ...
    (../debugger/media/managed-data-breakpoint.png "Точка останова управляемых данных") точка ![останова управляемых данных]
...
1. В C++ проекте запустите отладку и дождитесь достижения точки останова. В меню **Отладка** выберите пункт **создать точку**останова  > **точка останова по данным**

    Можно также выбрать пункт **Новая** **точка останова по данным**  >  в окне **точки останова** или щелкнуть правой кнопкой мыши элемент **в окне** **видимые**, **Контрольные значения**и выбрать команду **прервать при изменении значения** в контекстном меню.
...
Чтобы открыть окно **точки останова** , выберите **Отладка** > **Windows** > **точки останова**или нажмите клавиши **ALT**+**F9** или **CTRL**0**ALT**2**B**.

Окна точек ![останова](../debugger/media/breakpointswindow.png "окно точек останова")
...
3. Нажмите кнопку **Закрыть** или сочетание клавиш **CTRL**+**Enter** , чтобы закрыть окно **Параметры точки останова** . Или в окне **точки останова** нажмите кнопку **ОК** , чтобы закрыть диалоговое окно.

Точки останова с установленными условиями отображаются с символом **+** в окнах исходный код и **точки останова** .
...
При выборе **условного выражения**можно выбрать одно из двух условий: **Имеет значение true** или **при изменении**. Выберите **значение true** , чтобы прервать выполнение при удовлетворении выражения, или **при** изменении значения выражения на Break.
 ...
 ![Условие точки останова является истинной](../debugger/media/breakpointconditionistrue.png "точкой останова")

 В следующем примере точка останова достигается только при изменении значения `testInt`:

 ![Точка останова при](../debugger/media/breakpointwhenchanged.png "изменении измененной точки останова")
...
### <a name="using-object-ids-in-conditional-expressions-c-and-f-only"></a>Использование идентификаторов объектов в условныхC# выражениях (и F# только)
...
2. Запустите отладку и, когда выполнение приостанавливается на точке останова, выберите **отладка** > **Windows** > **локальные** или **ALT**+**4** , чтобы открыть окно **локальные** .
...
### <a name="hit-count"></a>Число попаданий
 ...
 ![Число попаданий точки останова](../debugger/media/breakpointhitcount.png "бреакпоинситкаунт")
...
**Чтобы задать точку трассировки, сделайте следующее:**

1. Щелкните правой кнопкой мыши точку останова и выберите **действия**. Или в окне **Параметры точки останова** наведите указатель мыши на точку останова, щелкните значок **Параметры** , а затем выберите **действия**.

1. Введите сообщение в поле " **записать сообщение в окно вывода** ". Сообщение может включать в себя универсальные текстовые строки, значения переменных или выражений, заключенные в фигурные скобки, и описатели[C#](../debugger/format-specifiers-in-csharp.md) формата [C++](../debugger/format-specifiers-in-cpp.md)(и) для значений.

   В сообщении также можно использовать следующие специальные ключевые слова:

   - **$Address** -текущая инструкция
   - Имя функции, вызывающей **$Caller**
   - **$CallStack** стека вызовов
   - **$Function** — имя текущей функции
   - **$PID** — идентификатор процесса
   - **$Pname** -имя процесса
   - **$Tid** — идентификатор потока
   - **$TNAME** -имя потока
   - Число тактов **$Tick** (из Windows `GetTickCount`)

1. Чтобы напечатать сообщение в окне **вывода** без разрыва, установите флажок **продолжить выполнение** . Чтобы напечатать сообщение и прервать выполнение на точке трассировки, снимите флажок.

Точки трассировки отображаются красными ромбами в левой области окна исходного кода и **точек останова** .
