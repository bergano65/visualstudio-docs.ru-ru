...
1. В Visual Studio Enterprise, перейдите к **Сервис > Параметры > IntelliTrace** и выберите параметр **IntelliTrace события и моментальные снимки**. 
...
3. Задайте один или несколько точек останова в проекте и начните отладку (нажмите клавишу **F5**), или запустите отладку, пошаговое выполнение кода (**F10** или **F11**).
...
1. Перемещаться между событиями с помощью **шаг назад (Alt + [)** и **шаг вперед (Alt +])** кнопок на панели инструментов отладки.
    ...
    Эти кнопки перехода события, которые появляются в **событий** вкладке **окно средств диагностики**. При переходе на шаг назад или вперед к событию автоматически активируется отладка с ведением журнала для выбранного события.
