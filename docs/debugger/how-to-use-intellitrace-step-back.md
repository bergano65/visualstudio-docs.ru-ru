---
title: Просмотр моментального снимка с помощью IntelliTrace обратной шаг
...
1. Если вы хотите настроить параметры для просмотра моментальных снимков на исключения, выберите **IntelliTrace** > **Дополнительно** из **параметры** диалоговое окно.

    Эти параметры доступны, начиная с Visual Studio Enterprise 2017 г. версия 15,7.

    ![Настройка поведения моментальные снимки по исключениям](../debugger/media/intellitrace-enable-snapshots-on-exceptions.png)

    При включении события и моментальные снимки, моментальные снимки для исключения включена по умолчанию. Моментальные снимки по исключениям можно отключить, отменив их выделение **осуществлять сбор моментальных снимков на событиях исключений**. Если эта функция включена, моментальные снимки создаются для необработанных исключений. Для обработки исключений моментальные снимки создаются только в том случае, если исключение создано и не повторного вызова исключения, исключение ранее. Можно задать максимальное количество моментальных снимков по исключениям, выбрав значение из раскрывающегося списка. Максимально применяется при каждом переходе, приложения в режиме приостановки выполнения (например, если приложение достигнет точки останова).

    > [!NOTE]
    > Моментальные снимки создаются только для событий исключения, записываемых IntelliTrace. Можно указать события записи IntelliTrace, выбрав **средства** > **параметры** > **события IntelliTrace**.
...
1. В проекте, задать один или несколько точек останова и начните отладку (нажмите клавишу **F5**), или запустите отладку, пошаговое выполнение кода (**F10** или **F11**).
    ...
    IntelliTrace делает моментальный снимок процесса приложения на каждый шаг отладчика, событие точки останова и события необработанного исключения. Эти события записываются в **событий** вкладке **средства диагностики** окно вместе с другими событиями IntelliTrace. Чтобы открыть это окно, выберите **отладки** > **Windows** > **Показать средства диагностики**.
