---
title: Создание визуализатора в Visual Basic | Документация Майкрософт
ms.custom: seodec18
ms.date: 04/12/2019
ms.topic: conceptual
dev_langs:
...
1. Создайте новый проект библиотеки классов.
    ...
    Нажмите клавишу **ESC**, чтобы закрыть окно запуска. Нажмите **клавиши CTRL + Q** , чтобы открыть поле поиска, введите текст **Visual Basic**, выберите **шаблоны**, а затем щелкните **создать новую библиотеку классов (.NET Framework)** . В появившемся диалоговом окне выберите **Создать**.
    ::: moniker-end
...
2. Введите соответствующее имя для библиотеки классов, например `MyFirstVisualizer`, а затем нажмите кнопку **создать** или **ОК**.
...
2. Измените имя с Class1.vb на что-нибудь более осмысленное, например DebuggerSide.vb.
   ...
   > [!NOTE]
   > [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] автоматически изменяет объявление класса в Дебугжерсиде. vb в соответствии с новым именем файла.
...
4. В диалоговом окне **Добавление ссылки** на вкладке **Обзор** выберите **Обзор** и найдите файл Microsoft. VisualStudio. DebuggerVisualizers. dll.

    Библиотеку DLL можно найти в *каталоге установки\<Visual studio > подкаталог \Common7\IDE\PublicAssemblies* в каталоге установки Visual Studio.
...
2. Введите соответствующее имя для библиотеки классов, например `MyTestConsole`, а затем нажмите кнопку **создать** или **ОК**.
...
2. В диалоговом окне **Добавление ссылки** на вкладке **Обзор** щелкните Microsoft. VisualStudio. DebuggerVisualizers.
...
3. В Тестконсоле. VB добавьте следующую инструкцию `Imports`:
...
2. В меню **Отладка** выберите команду **Запуск**.
   ...
   Если вам удобнее вызывать визуализатор из [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)], а не из специально подготовленной тестовой программы, визуализатор необходимо установить. Дополнительные сведения см. [в разделе руководство. Установка визуализатора](../debugger/how-to-install-a-visualizer.md).
