...
ms.locfileid: "49919472"
---
# <a name="use-the-memory-windows-in-the-visual-studio-debugger"></a>Используйте Windows памяти в отладчике Visual Studio
...
2. В **Отладка** последовательно выберите пункты **Windows**. Наведите указатель на **памяти** и нажмите кнопку **память 1**, **память 2**, **память 3**, или **память 4**. (Более низкого уровня выпусков [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] имеется только одно **памяти** окна. Если вы используете один из этих выпусков, просто щелкните **памяти**.)  
