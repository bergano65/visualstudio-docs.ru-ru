...
|**Имитатор**|Отладка приложения в имитаторе Visual Studio для UWP и [!INCLUDE[win8_appname_long](../debugger/includes/win8_appname_long_md.md)] приложения. Имитатор представляет окно на рабочем столе, которое позволяет отлаживать функциональные возможности устройства, такие как сенсорный ввод и поворот устройства —, могут быть недоступны на локальном компьютере. Этот параметр доступен, только если ваше приложение **Минимальная версия целевой. Версия** меньше или равно операционной системы на компьютере разработки. В разделе [UWP запуска приложений в симуляторе](../debugger/run-windows-store-apps-in-the-simulator.md).|  
...
|**Эмулятор мобильных устройств**|Загрузки эмулятор с конфигурацией, указанной в поле имя эмулятора, развернуть приложение и запустить отладку. Эмуляторы доступны только на машинах Hyper-V включен под управлением Windows 8.1 или более поздней версии.|  
...
2.  Это сделать, чтобы открыть страницу свойств отладки для проекта:  
    ...
    -   В Visual C# и Visual Basic выберите **Отладка**.  
         ...
         ![И &#35; &#47; Страница свойств отладки проекта VB](../debugger/media/dbg_csvb_debugpropertypage.png)  
  
    -   В Visual C++ и JavaScript разверните **свойства конфигурации** узел и выберите **Отладка**.  
  
         ![C# 43; &#43; Страницы свойств отладки приложения UWP](../debugger/media/dbg_cpp_debugpropertypage.png)  
...
-   Для приложений Visual C++ и JavaScript, выбрать **Да** из **запустить приложение** списке **Отладка** страницу свойств.  
...
> [!NOTE]
> Windows 8.1 выберите **отладки**, а затем выберите **отлаживать установленный пакет приложения**.
