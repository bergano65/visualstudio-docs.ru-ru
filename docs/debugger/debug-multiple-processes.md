...
ms.locfileid: "37057971"
---
# <a name="debug-multiple-processes"></a>Отладка нескольких процессов
Вот как можно начать отладку процессов, переключения между процессами, прервать и продолжить выполнение, пошаговое выполнение исходного, остановить отладку и окончание или отсоединение от процессов.  
...
##  <a name="BKMK_Find_the_source_and_symbol___pdb__files"></a> Поиск источника и файлов символов (PDB) файлы  
 ...
 Если не удается получить доступ к файлам процесса, можно перейти с помощью окна дизассемблирования. См. в разделе [как: использование окна дизассемблирования](../debugger/how-to-use-the-disassembly-window.md)  
 ...
 **Присоединение к процессу, который выполняется на локальном компьютере**  
 ...
 Нажмите кнопку **Отладка > присоединение к процессу**. На **присоединение к процессу** диалоговое окно, выберите процесс, из **доступные процессы** , а затем выберите **Attach**.  
...
-   [Прервать, пошагового выполнения и продолжения команды](#BKMK_Break__step__and_continue_commands)  
...
-   Откройте **процессы** окна (ярлык **Ctrl + Alt + Z**), найдите процесс, который требуется задать в качестве текущего процесса и дважды щелкните его.  
 ...
 ![К началу страницы](../debugger/media/pcs_backtotop.png "PCS_BackToTop") [переключение процессов, приостановка и продолжение выполнения, пошагового выполнения исходного](../debugger/debug-multiple-processes.md#BKMK_Switch_processes__break_and_continue_execution__step_through_source)  
...
|**Процессы** контекстное меню окна:<br /><br /> -   **Отсоединиться при остановке отладки**|Переключает поведение **Отладка**, **остановить отладку** для выбранного процесса:<br /><br /> -Checked: Отладчик отсоединяется от процесса.<br />-Флажок снят: Процесс завершается.|  
 ...
 ![К началу страницы](../debugger/media/pcs_backtotop.png "PCS_BackToTop") [остановка отладки, окончание или отсоединение от процессов](../debugger/debug-multiple-processes.md#BKMK_Stop_debugging__terminate_or_detach_from_processes)  
