...
Начиная с Visual Studio 2017 версии 15.8, Just My Code для кода также поддерживается пошаговое выполнение. Этот компонент также требует использования [/JMC (Отладка только собственного кода)](/cpp/build/reference/jmc) параметр компилятора. Переключатель включен по умолчанию в C++ проектов. Для **стек вызовов** окна и вызов стека поддержки в Just My Code /JMC коммутатора не требуется.
...
Для поведения стека вызовов, таких как **стек вызовов** только мой код в окне C++ считает эти функции следует *непользовательский код*:
...
Для пошагового выполнения поведения кода только мой код в C++ считает эти функции следует *непользовательский код*:

- Функции, для которых соответствующие PDB-файл не был загружен в отладчик.
- Функции, определенные в  *\*.natjmc* файлы в *%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers* папки.

> [!NOTE]
> Для поддержки пошагового выполнения кода в Just My Code C++ необходимо компилировать код с помощью компиляторов MSVC в Visual Studio 15.8 предварительной версии 3 или более поздней версии, и должен быть включен параметр компилятора /JMC (он включен по умолчанию). Дополнительные сведения см. в разделе [Настройка C++ стек вызовов и поведение пошагового выполнения кода](#BKMK_CPP_Customize_call_stack_behavior)) и это [блога](https://devblogs.microsoft.com/cppblog/announcing-jmc-stepping-in-visual-studio/). Для кода, компилируемого с помощью более старой версией компилятора *.natstepfilter* файлы являются единственным способом для настройки пошагового выполнения кода, который не зависит от Just My Code. См. в разделе [Настройка C++ поведение пошаговой](#BKMK_CPP_Customize_stepping_behavior).
...
### <a name="BKMK_CPP_Customize_call_stack_behavior"></a> Настройка C++ стек вызовов и поведение пошагового выполнения кода
...
### <a name="BKMK_CPP_Customize_stepping_behavior"></a> Настройка C++ поведение проверки зависит от параметров только мой код
