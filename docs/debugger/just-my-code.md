...
Начиная с Visual Studio 2017 версии 15.8, Just My Code для кода также поддерживается пошаговое выполнение. Этот компонент также требует использования [/JMC (Отладка только собственного кода)](/cpp/build/reference/jmc) параметр компилятора. Переключатель включен по умолчанию в проектах C++. Для **стек вызовов** окна и вызов стека поддержки в Just My Code /JMC коммутатора не требуется.

<a name="BKMK_CPP_User_and_non_user_code"></a> Классифицироваться как пользовательский код, PDB-ФАЙЛ для двоичного файла, содержащего код пользователя должен быть загружен в отладчике (использовать **модули** окне, чтобы проверить это).

Для поведения стека вызовов, таких как **стек вызовов** окно, только мой код на C++ рассматривает только эти функции, чтобы быть *непользовательский код*:
...
Для поведения пошагового выполнения кода, только мой код на C++ рассматривает только эти функции, чтобы быть *непользовательский код*:
  
- Функции, для которых соответствующие PDB-файл не был загружен в отладчик.
- Функции, определенные в  *\*.natjmc* файлы в *%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers* папки.

> [!NOTE]
> Для поддержки пошагового выполнения кода в Just My Code, код C++, должны компилироваться с помощью компиляторов MSVC в Visual Studio 15.8 предварительной версии 3 или более поздней версии, и должен быть включен параметр компилятора /JMC (он включен по умолчанию). Дополнительные сведения см. в разделе [C++ настроить стек вызовов и поведение пошагового выполнения кода](#BKMK_CPP_Customize_call_stack_behavior)) и это [блога](https://blogs.msdn.microsoft.com/vcblog/2018/06/29/announcing-jmc-stepping-in-visual-studio/). Для кода, компилируемого с помощью более старой версией компилятора *.natstepfilter* файлы являются единственным способом для настройки пошагового выполнения кода, который не зависит от Just My Code. См. в разделе [поведение пошаговой настройки C++](#BKMK_CPP_Customize_stepping_behavior).
...
###  <a name="BKMK_CPP_Customize_call_stack_behavior"></a> Настройка стека вызовов C++ и поведение пошагового выполнения кода  
...
###  <a name="BKMK_CPP_Customize_stepping_behavior"></a> Настройка поведения пошагового выполнения C++ зависит от параметров только мой код
