...
- Чтобы указать код, не написанный пользователем для отдельного пользователя, добавьте *.natstepfilter* файл *%USERPROFILE%\My Documents\Visual Studio 2017\Visualizers* папки.  
...
|Элемент|Описание|  
|-------------|-----------------|  
|`Function`|Обязательно. Указывает одну или несколько функций как функцию, не написанную пользователем.|  
...
|`Action`|Обязательно. Одно из следующих чувствительных к регистру значений:<br /><br /> `NoStepInto`  — указывает отладчику шаг с обходом функции.<br /> `StepInto`  — указывает отладчику выполнять шаги с заходом в функцию, отменяя все остальные `NoStepInto` для соответствующей функции.|  
...
-   Чтобы указать встроенный код для всех пользователей компьютера с Visual Studio, добавьте *.natjmc* файл *%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers* папки.  
-   Чтобы указать код, не написанный пользователем для отдельного пользователя, добавьте *.natjmc* файл *%USERPROFILE%\My Documents\Visual Studio 2017\Visualizers* папки.  
...
|Атрибут|Описание|  
|---------------|-----------------|  
|`Name`|Обязательно. Полный путь к модулю или модулям. Можно использовать подстановочные знаки Windows `?` (ноль или один символ) и `*` (ноль или более символов). Например, примененная к объекту директива<br /><br /> `<Module Name="?:\3rdParty\UtilLibs\*" />`<br /><br /> указывает отладчику необходимо рассматривать все модули в *\3rdParty\UtilLibs* на любом диске как внешний код.|  
...
<a name="BKMK_JS_User_and_non_user_code"></a> JavaScript Just My Code элементы управления пошагового выполнения и отображением стека вызовов путем распределения кода по одной из следующих классификаций.  
...
- Когда пользователь кода шаг приводит к выходу из текущего контекста выполнения, отладчик останавливается на следующей строке кода выполненного пользователем. Например, если обратный вызов выполняется в **LibraryCode** кода, отладчик продолжает пока не будет выполнена следующая строка кода пользователя.
 ...
 **Eval**, **функция**, и **ScriptBlock** пар ключ-значение как динамически определить классифицируется созданный код:  
