...
Чтобы запустить приложение с подключенным отладчиком, нажмите клавишу **F5**выберите **Отладка** > **начать отладку**, или выберите зеленую стрелку на панели инструментов Visual Studio.
 ...
 ![DBG&#95;основы&#95;запустить&#95;отладки](../debugger/media/dbg_basics_start_debugging.png "DBG_Basics_Start_Debugging")
...
Если прервать выполнение кода, не имеет источника или символ (*.pdb*) файлов, загруженных, отладчик отображает **исходный файл не найден** или **символы не найдены** страницы, которые могут помочь Поиск и загрузка файлов. См. статью [Указание файлов символов (.pdb) и файлов с исходным кодом в отладчике Visual Studio](../debugger/specify-symbol-dot-pdb-and-source-files-in-the-visual-studio-debugger.md). Если не удается загрузить символов или исходные файлы, можно выполнять отладку инструкций ассемблера в **Дизассемблированный код** окна.
...
Действия отладчика через операторов кода, а не физических строк. Например, предложение `if` может быть записано в одной строке:
...
Можно настроить отладчик для запуска, пока не достигнет указанной функции. Можно задать функцию по имени или выбрать ее из стека вызовов.
...
> [!NOTE]
> **Выполнение до щелкнутого** доступен, начиная с версии [!include[vs_dev15](../misc/includes/vs_dev15_md.md)].
