...
|Удаление, переподчинение или повторное связывание связанных элементов при удалении элемента.|Установите значение параметра **распространяет удаление** для роли отношений. Для более сложных эффектов переопределяйте методы `ShouldVisitRelationship` и `ShouldVisitRolePlayer` в классе `MyDslDeleteClosure`, определенном в **DomainModel.CS**.|
