...
## <a name="running-validation-when-a-change-occurs"></a>Запуск проверки при внесении изменений
 ...
 В дополнение к коду проверки добавьте файл пользовательского кода в вашей **DslPackage** проекта с содержимым, как в следующем примере. В этом коде используется `ValidationController`, который вложен в документ. Этот контроллер отображает ошибки проверки в списке ошибок [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)].
