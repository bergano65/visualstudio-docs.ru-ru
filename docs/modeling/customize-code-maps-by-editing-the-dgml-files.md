...
Чтобы настроить карту кода, можно изменить ее файл DGML. Например, можно изменить элементы, чтобы указать пользовательские стили, назначить свойства и категории кодовым точкам и связям либо связать узлы с документы или URL-адреса с кодовыми точками или связями. Дополнительные сведения о DGML-элементы в разделе [направленный граф разметки языка (DGML) ссылка](../modeling/directed-graph-markup-language-dgml-reference.md).  
 ...
 Измените DGML-файл карты кода в текстовом редакторе или редакторе XML. Если сопоставление является частью решения Visual Studio, выберите ее в **обозревателе решений**, откройте контекстное меню и выберите **открыть с помощью**, **редактор XML (текстовый)**.  
...
##  <a name="OrganizeNodes"></a>Элементы группы кода  
...
4.  В разделе `<Links>` убедитесь, что элемент `<Link/>` со следующими атрибутами существует для каждой связи между кодовой точкой группы и дочерними кодовыми точками:  
    ...
    ```  
     ...
     Дополнительные сведения о `Category` см. в разделе [присвоить категории элементов кода и ссылками](#AssignCategories).  
  
##  <a name="ChangeGraphStyle"></a>Изменение стиля карты  
 Цвет фона и цвет границы карты можно изменить в DGML-файле. Изменение стиля элементов кода и ссылок, в разделе [изменение стиля элементов кода и ссылками](#Highlight).  
...
2.  В элементе `<Style/>` добавьте элемент `<Condition/>`, который содержит атрибут `Expression`, чтобы указать выражение, возвращающее логическое значение.  
    ...
    ```  
     ...
     Можно указать несколько `<Condition/>` элементы, которые должны быть true, чтобы применить стиль.  
...
##  <a name="AssignProperties"></a>Назначьте свойства элементов кода и ссылок  
...
##  <a name="AssignCategories"></a>Назначение категорий для элементов кода и ссылок  
...
##  <a name="AddReferences"></a>Связать документы или URL-адресов для элементов кода и ссылок  
...
-   Диаграмма зависимостей может связать группу элементов кода, который представляет слой в логической архитектуре программного обеспечения.  
...
-   Элемент кода связывайте рабочий элемент Team Foundation Server или ошибки или другие сведения, относящиеся к элементу кода.  
...
3.  Выполните одну из задач, представленных в следующей таблице:  
    ...
    ```  
     ...
     На карте имя кодовой точки отображается подчеркнутым. Можно, открыв контекстное меню для элемента кода или по ссылке, вы увидите **перейти по ссылке** контекстное меню, содержащее элементы связанного кода для выбора.  
