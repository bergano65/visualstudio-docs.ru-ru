...
При задании имени свойства в определении DSL его отображаемое имя автоматически присваивается копии имени. При написании имени в стиле Pascal, например "Фуелгауже", отображаемое имя автоматически будет содержать пробел: "Датчик горючего". Однако можно явно задать для отображаемого имени другое значение.
...
## <a name="forwarding-display-properties-of-related-elements"></a>Сылая Отображение свойств связанных элементов
...
1. Создайте решение, содержащее по крайней мере два класса, которые в этом примере называются Book и **Author**. [!INCLUDE[dsl](../modeling/includes/dsl_md.md)] Между **книгой** и **автором**должна быть связь любого вида.
