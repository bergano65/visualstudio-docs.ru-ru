...
> [!NOTE]
>  В текущей реализации [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] ModelBus связанные модели должны быть элементами одного и того же решения [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)].  
...
-   [Пакет SDK для моделирования для Visual Studio](http://www.microsoft.com/download/details.aspx?id=40754)  
...
2.  В обозревателе DSL разверните **поведением при сериализации Xml**, затем **данные класса**.  
  
3.  Для каждого класса, к которому необходимо создать ссылки ModelBus, сделайте следующее:  
  
     Щелкните узел класса и в окне «Свойства» убедитесь, что **сериализации идентификатора** равно `true`.  
...
1.  В схема определения DSL, щелкните правой кнопкой мыши основную часть диаграммы и нажмите кнопку **включить Modelbus**.  
  
2.  В диалоговом окне выберите **необходимо включить эту модель для использования ссылок шины модели**.  
  
3.  В проекте Dsl принимающего DSL к ссылкам проекта добавьте указанные ниже сборки (файлы .DLL). Можно найти эти сборки (DLL-файлы) в ModelBusAdapter\bin\\* каталог предоставляемых DSL.  
  
    -   Предоставленный DSL сборки, например **Fabrikam.FamilyTree.Dsl.dll**  
  
    -   Предоставленный модели шины сборки адаптера, например **Fabrikam.FamilyTree.ModelBusAdapter.dll**  
...
2.  В окне свойств со свойством домена установлен, задайте **тип** для `ModelBusReference`.  
  
 На данном этапе программный код может определять значение свойства, но в окне "Свойства" он доступен только для чтения.  
  
 Можно разрешить пользователям устанавливать свойство с помощью специализированного редактора ссылок ModelBus. Существуют две версии этого редактора или *выбора:* один позволяет пользователям выбрать файл модели и другой позволяет пользователям выбрать файл модели и элемент внутри модели.  
...
1.  Щелкните правой кнопкой мыши свойства домена, а затем нажмите кнопку **ModelBusReference изменить определенные свойства**. Откроется диалоговое окно. Это *выбора шины модели*.  
  
2.  Выберите соответствующий **вида ModelBusReference**: в модель или элемент внутри модели.  
...
7.  Нажмите кнопку **Обзор** и выберите экземпляр предоставляется DSL.  
...
## <a name="creating-references-in-program-code"></a>Создание ссылок в программном коде  
 ...
 Для создания ссылки на модель требуется AdapterManager доменного языка, в котором модель является экземпляром, и имя файла или элемент проекта [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] модели.  
  
 Для создания ссылки на элемент требуется адаптер для файла модели и элемент, на который необходимо сослаться.  
  
> [!NOTE]
>  С помощью [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] ModelBus можно создавать ссылки только на элементы в том же решении [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)].  
...
1.  DSL, к которому необходимо получить доступ, должен иметь адаптер ModelBus, настроенный с помощью текстовых шаблонов на получение доступа. Дополнительные сведения см. в разделе [предоставление доступа к DSL](#provide).  
  
2.  Как правило, получение доступа к целевому DSL осуществляется с помощью ссылки ModelBus (MBR), которая хранится в исходном DSL. В связи с этим шаблон включает директиву исходного DSL и код, необходимый для разрешения MBR. Дополнительные сведения о текстовых шаблонах см. в разделе [формирование кода из доменного языка](../modeling/generating-code-from-a-domain-specific-language.md).  
...
```  
 ...
 MBR, сериализованная подобным образом, не зависит от контекста. При использовании простого файлового адаптера ModelBus MBR содержит абсолютный путь к файлу. Этого достаточно, если файлы модели экземпляра никогда не перемещаются. Тем не менее файлы модели обычно являются элементами в проекте [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]. Пользователи будут рассчитывать на возможность перемещать весь проект в различные части файловой системы, а также сохранять исходный контроль над проектом и открывать его на разных компьютерах. В связи с этим пути следует сериализовать относительно расположения проекта, содержащего файлы.  
...
-   Адаптер ModelBus работает с адаптерной частью MBR. Он может использовать дополнительные сведения, сохраненные в значении ReferenceContext ссылки ModelBus. Простой адаптер файловых отслеживает корневого пути к файлам с помощью клавиш `FilePathLoadContextKey` и `FilePathSaveContextKey`.  
...
## <a name="actions-performed-by-the-modelbus-extension"></a>Действия, выполняемые расширением ModelBus  
 ...
 Щелкните правой кнопкой мыши схема определения DSL **включить Modelbus**и выберите **включить этот DSL использовать ModelBus**:  
...
-   В окне "Определение DSL" добавляется ссылка внешнего типа: `Microsoft.VisualStudio.Modeling.Integration.ModelBusReference`.  
     ...
     Вы увидите ссылку в **обозреватель DSL**в разделе **типы доменов**. Чтобы добавить ссылки внешнего типа вручную, щелкните корневой узел правой кнопкой мыши.  
  
-   Добавлен новый файл шаблона, **Dsl\GeneratedCode\ModelBusReferencesSerialization.tt**.  
  
 Задайте тип свойства домена ModelBusReference, а затем щелкните правой кнопкой мыши свойство при нажатии **ModelBusReference включить конкретные свойства**:  
...
-   В свойство домена будут добавлены несколько атрибутов CLR. Их можно увидеть в поле "Пользовательские атрибуты" окна "Свойства". В **Dsl\GeneratedCode\DomainClasses.cs**, можно просмотреть атрибуты в объявлении свойства:  
 ...
 При щелчке правой кнопкой мыши схема определения DSL, нажмите кнопку **включить ModelBus**и выберите **предоставлять этот DSL для ModelBus**:  
