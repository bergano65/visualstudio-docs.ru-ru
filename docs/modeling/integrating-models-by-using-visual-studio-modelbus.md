...
> [!NOTE]
> В текущей реализации Visual Studio ModelBus связанные модели должны быть элементы в одном решении Visual Studio.
...
## <a name="provide"></a> Предоставление доступа к DSL
 Перед созданием ссылок ModelBus на модель или ее элементы необходимо определить ModelBusAdapter для DSL. Для этого проще всего использовать Visual Studio модели шины расширение, которое добавляет команды конструктора DSL.
...
### <a name="ensure-that-elements-can-be-referenced"></a>Убедитесь, что могут быть ссылки на элементы
...
Чтобы убедиться, что элемент сохранности идентификаторов:
...
3.  В проект "Отладка" в экспериментальном экземпляре Visual Studio добавьте файлы, являющиеся экземплярами каждого DSL.

    > [!NOTE]
    > Visual Studio ModelBus может разрешать только ссылки на модели, которые являются элементами в том же решении Visual Studio. Например, создать ссылку на файл модели, расположенный в другой части файловой системы, нельзя.
...
Чтобы создать ссылку на модель, требуется AdapterManager доменного языка, элементом которой является модель экземпляра и имя файла или элемент проекта Visual Studio модели.

Для создания ссылки на элемент требуется адаптер для файла модели и элемент, на который необходимо сослаться.

> [!NOTE]
> С помощью Visual Studio ModelBus можно создать ссылки только на элементы в одном решении Visual Studio.
...
MBR, сериализованная подобным образом, не зависит от контекста. При использовании простого файлового адаптера ModelBus MBR содержит абсолютный путь к файлу. Этого достаточно, если файлы модели экземпляра никогда не перемещаются. Тем не менее файлы модели обычно будет элементов в проект Visual Studio. Пользователи будут рассчитывать на возможность перемещать весь проект в различные части файловой системы, а также сохранять исходный контроль над проектом и открывать его на разных компьютерах. В связи с этим пути следует сериализовать относительно расположения проекта, содержащего файлы.
...
### <a name="creating-opening-and-editing-a-model"></a>Создание, открытие и изменение модели
 Следующий фрагмент взят из примера конечный автомат на веб-сайте VMSDK. В нем показывается использование ссылок ModelBus для создания и открытия модели, а также для получения связанной с моделью схемы.
