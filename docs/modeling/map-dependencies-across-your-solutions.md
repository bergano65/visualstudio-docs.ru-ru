...
3.  На **обозревателе решений** панели инструментов, выберите **Показать на карте кода**![создания новой диаграммы из выбранных узлов кнопки](../modeling/media/createnewgraphfromselectedbutton.gif "CreateNewGraphFromSelectedButton"). Либо откройте контекстное меню и выберите пункт **Показать на карте кода**. Также можно перетащить элементы из представления классов или обозревателя объектов на новую или уже существующую карту кода.  
...
1.  В меню **Архитектура** выберите пункт **Создать карту кода для решения**.  
     ...
     ![Создание команды map кода](../modeling/media/codemapsarchitecturemenu.png "CodeMapsArchitectureMenu")  
...
2.  Чтобы открыть список значков, обозначающих различные типы проектов (такие как тестовый, веб- или телефонный проект), элементов кода (таких как классы, методы и свойства) и типов отношений (таких как «Наследует от», «"Реализует» и «Вызывает»), нажмите кнопку **Условные обозначения** .  
     ...
     ![Граф зависимостей верхнего уровня сборки](../modeling/media/dependencygraph_toplevelassemblies.png "DependencyGraph_TopLevelAssemblies")  
...
9. Чтобы сосредоточиться на определенных фрагментах карты, удалите все элементы, которые вас пока не интересуют. Например, чтобы получить детализированное представление класса и его членов, просто отфильтруйте все узлы пространства имен на панели **Фильтры** .  
     ...
     ![Детализация до уровня классов и членов](../modeling/media/dependencygraph_expandedselectedgroups_2012.png "DependencyGraph_ExpandedSelectedGroups_2012")  
...
2.  Сопоставьте элементы с их членами. На **обозревателе решений** инструментов нажмите **Показать на карте кода**![создания новой диаграммы из выбранных узлов кнопки](../modeling/media/createnewgraphfromselectedbutton.gif "CreateNewGraphFromSelectedButton").  
  
     ![Выберите элементы, которые требуется сопоставить](../modeling/media/codemapsselectinsolutionexplorer.png "CodeMapsSelectInSolutionExplorer")  
...
5.  Продолжайте разворачивать интересующие вас элементы вплоть до уровня классов и членов, если это понадобится.  
     ...
     Для просмотра элементов, которые существуют в коде, но не отображаются на карте, нажмите кнопку **повторно извлечь дочерние элементы** значок ![повторно извлечь дочерние элементы значок](../modeling/media/dependencygraph_deletednodesicon.png "DependencyGraph_DeletedNodesIcon") в левом верхнем углу группы.  
...
-   Чтобы просмотреть зависимости между открытым файлом и связанными с ним исходными файлами и файлами заголовков, откройте либо исходный файл, либо файл заголовка. Откройте контекстное меню в любом месте файла. Выберите пункт **Создать диаграмму включаемых файлов**.  
     ...
     ![Граф зависимостей первого уровня для h-файла](../modeling/media/dependencygraph_native_firstlevel.png "DependencyGraph_Native_FirstLevel")  
