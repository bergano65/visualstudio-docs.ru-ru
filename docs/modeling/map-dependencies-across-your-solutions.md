...
Вы можете сопоставлять зависимости для кода на следующих языках:
...
1. На панели инструментов **Обозреватель решений** выберите пункт **Показывать в карте кода** ![Create кнопка "создать диаграмму из выбранных узлов" ](../modeling/media/createnewgraphfromselectedbutton.gif). Также можно открыть контекстное меню для одной или группы элементов и выбрать команду « **отобразить на карте кода**».
...
5. Продолжайте развертывать интересующие вас элементы, если это необходимо, на уровне класса и члена.
   ...
   Чтобы просмотреть элементы, которые находятся в коде, но не отображаются на карте, щелкните значок **дочерние объекты повторной выборки** ![Refetch значок дочерние ](../modeling/media/dependencygraph_deletednodesicon.png) в левом верхнем углу группы.
