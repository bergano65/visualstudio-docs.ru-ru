...
- Визуальный C# элемент или Visual Basic в решении или сборках ( *. dll* или *. exe*)

- Машинный или управляемый C++ C или код C++ в визуальных проектах, файлы заголовков (*h* или `#include`) или двоичные файлы

- Проекты и сборки X++, созданные в модулях .NET для Microsoft Dynamics AX
...
Аналогичным образом можно создать новый файл с картой кода, не добавляя его в решение, выбрав **архитектура** > **новую карту кода** или **файл** > **Новый** **файл** > .
...
1. На панели инструментов **Обозреватель решений** нажмите кнопку **Показывать на карте кода** ![создать новый граф из выбранных узлов кнопка](../modeling/media/createnewgraphfromselectedbutton.gif). Также можно открыть контекстное меню для одной или группы элементов и выбрать команду « **отобразить на карте кода**».
...
4. Для исследования элементов разверните их. Наведите указатель мыши на элемент и щелкните значок шеврона (стрелка вниз), когда он появится.
   ...
   Чтобы развернуть все элементы, выберите их с помощью **клавиши Ctrl**+**A**, откройте контекстное меню для этой схемы и выберите **Группа** > **развернуть**. Этот параметр недоступен, если при развертывании всех групп карта кода станет непригодна к использованию или возникнут проблемы с памятью.
...
5. Продолжайте развертывать интересующие вас элементы, если это необходимо, на уровне класса и члена.
   ...
   Чтобы просмотреть элементы, которые находятся в коде, но не отображаются на карте, щелкните значок **дочерние элементы повторной выборки** ![значок дочерние элементы повторной выборки](../modeling/media/dependencygraph_deletednodesicon.png) в левом верхнем углу группы.

6. Для просмотра дополнительных элементов, связанных с элементами на карте, выделите нужный элемент и выберите команду **Показать связанные** на панели инструментов карты кода. После этого выберите тип связанных элементов, которые нужно добавить на карту. Кроме того, можно выбрать один или несколько элементов, открыть контекстное меню и выбрать параметр **Показывать** для типа связанных элементов, добавляемых на карту. Например:
...
7. На карте показаны отношения. В этом примере на карте показаны методы, вызываемые методом `Find` и их расположением в решении или извне.
...
- [Видео. Общие сведения о проектировании кода с помощью карт кода Visual Studio 2015](https://channel9.msdn.com/Events/Visual-Studio/Connect-event-2015/502)
