---
title: "Сопоставление зависимостей в решениях | Документы Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
Чтобы разобраться в реализованных в коде зависимостях, визуализируйте их, создав карты кода. Это поможет увидеть, как код сочетается друг с другом без чтения файлов и строк кода.  
 ...
 ![Просмотр зависимостей в решениях](~/docs/modeling/media/codemapsmainintro.png "CodeMapsMainIntro")  
...
##  <a name="a-namegetstarteda-get-started-with-code-maps"></a><a name="GetStarted"></a>Приступая к работе с картами кода  
...
3.  На **обозревателе решений** панели инструментов, выберите **Показать на карте кода**![создания новой диаграммы из выбранных узлов кнопки](~/docs/modeling/media/createnewgraphfromselectedbutton.gif "CreateNewGraphFromSelectedButton"). Либо откройте контекстное меню и выберите пункт **Показать на карте кода**. Также можно перетащить элементы из представления классов или обозревателя объектов на новую или уже существующую карту кода.  
...
###  <a name="a-namecreateemptymapa-to-add-a-new-empty-code-map-to-your-solution"></a><a name="CreateEmptyMap"></a>Добавление новой пустой карты кода в решении  
...
##  <a name="a-nameseeoverviewsourcea-see-overall-dependencies"></a><a name="SeeOverviewSource"></a>Просмотр всех зависимостей  
  
###  <a name="a-nameoverviewsourcea-see-dependencies-across-your-solution"></a><a name="OverviewSource"></a>Просмотр зависимостей во всем решении  
  
1.  В меню **Архитектура** выберите пункт **Создать карту кода для решения**.  
  
     ![Создание команды map кода](~/docs/modeling/media/codemapsarchitecturemenu.png "CodeMapsArchitectureMenu")  
...
2.  Чтобы открыть список значков, обозначающих различные типы проектов (такие как тестовый, веб- или телефонный проект), элементов кода (таких как классы, методы и свойства) и типов отношений (таких как «Наследует от», «"Реализует» и «Вызывает»), нажмите кнопку **Условные обозначения** .  
     ...
     ![Граф зависимостей верхнего уровня сборки](~/docs/modeling/media/dependencygraph_toplevelassemblies.png "DependencyGraph_TopLevelAssemblies")  
...
3.  Чтобы детализировать карту, разверните группы, представляющие проекты и сборки. Чтобы развернуть все компоненты, выделите все узлы нажатием клавиш **CTRL+A** , а затем последовательно выберите в контекстном меню пункты **Группа**и **Развернуть** .  
     ...
     ![Развертывание всех групп на карте кода](~/docs/modeling/media/codemapsexpandallgroups.png "CodeMapsExpandAllGroups")  
...
4.  В большом решении это не пригодится. Более того, если решение комплексное, ограничения памяти могут не позволить развернуть все группы. Вместо этого разворачивайте отдельные интересующие вас узлы. Наведите указатель мыши на верхнюю часть узла и нажмите на значок шеврона (стрелка вниз), когда он появится.  
     ...
     ![Развертывание узла на карте кода](~/docs/modeling/media/dependencygraph_containment.png "DependencyGraph_Containment")  
...
5.  Чтобы упростить карту и выделить ее отдельные части, выберите **Фильтры** на панели элементов в карте кода и отметьте только те типы узлов и связей, которые вас интересуют. Например, можно скрыть все контейнеры папки решения и сборки.  
     ...
     ![Упростить карту путем фильтрации контейнеров](~/docs/modeling/media/codemapsfilterfoldersassemblies.png "CodeMapsFilterFoldersAssemblies")  
...
7.  Для получения дополнительных сведений об элементе или связи наведите указатель на элемент или связь и дождитесь появления подсказки. Подсказка будет содержать подробные сведения об элементе кода или представляемых связью категориях.  
     ...
     ![Показать категории связи](~/docs/modeling/media/codemapsshowlinkcatgories.png "CodeMapsShowLinkCatgories")  
...
9. Чтобы сосредоточиться на определенных фрагментах карты, удалите все элементы, которые вас пока не интересуют. Например, чтобы получить детализированное представление класса и его членов, просто отфильтруйте все узлы пространства имен на панели **Фильтры** .  
     ...
     ![Детализация до уровня классов и членов](~/docs/modeling/media/dependencygraph_expandedselectedgroups_2012.png "DependencyGraph_ExpandedSelectedGroups_2012")  
...
11. Выбранные элементы будут добавлены в новую карту вместе с контекстом. Скройте папки решений и другие ненужные контейнеры с помощью панели **Фильтры** .  
     ...
     ![Фильтрация контейнеров для упрощения просмотра](~/docs/modeling/media/codemapsexpandnewgroups.png "CodeMapsExpandNewGroups")  
  
12. Разверните группы и выберите элементы на карте, чтобы просмотреть отношения.  
  
     ![Выберите элементы для просмотра связей](~/docs/modeling/media/codemapsviewnewrelationships.png "CodeMapsViewNewRelationships")  
...
###  <a name="a-nameoverviewcompileda-see-dependencies-across-assemblies-or-binaries"></a><a name="OverviewCompiled"></a>Просмотр зависимостей в сборках или двоичных файлов  
...
##  <a name="a-nameseespecificsourcea-see-specific-dependencies"></a><a name="SeeSpecificSource"></a>Просмотр конкретных зависимостей  
 ...
 ![Показать конкретные зависимости на карте кода](~/docs/modeling/media/codemapsspecificdependenciesintro.png "CodeMapsSpecificDependenciesIntro")  
...
2.  Сопоставьте элементы с их членами. На **обозревателе решений** инструментов нажмите **Показать на карте кода**![создания новой диаграммы из выбранных узлов кнопки](~/docs/modeling/media/createnewgraphfromselectedbutton.gif "CreateNewGraphFromSelectedButton").  
  
     ![Выберите элементы, которые требуется сопоставить](~/docs/modeling/media/codemapsselectinsolutionexplorer.png "CodeMapsSelectInSolutionExplorer")  
  
3.  На картах кода выбранные элементы отображаются в сборках, частью которых они являются.  
  
     ![Выбранные элементы показаны как группы на карте](~/docs/modeling/media/codemapsshowitemsfromsolnexplorer.png "CodeMapsShowItemsFromSolnExplorer")  
...
5.  Продолжайте разворачивать интересующие вас элементы вплоть до уровня классов и членов, если это понадобится.  
     ...
     ![Развернуть группы до уровня классов и членов](~/docs/modeling/media/codemapsexpandtoclassandmember.png "CodeMapsExpandToClassAndMember")  
  
     Для просмотра элементов, которые существуют в коде, но не отображаются на карте, нажмите кнопку **повторно извлечь дочерние элементы** значок ![повторно извлечь дочерние элементы значок](~/docs/modeling/media/dependencygraph_deletednodesicon.png "DependencyGraph_DeletedNodesIcon") в левом верхнем углу группы.  
...
6.  Для просмотра дополнительных элементов, связанных с элементами на карте, выделите нужный элемент и выберите команду **Показать связанные** на панели инструментов карты кода. После этого выберите тип связанных элементов, которые нужно добавить на карту. Также можно выбрать один или несколько элементов, открыть контекстное меню и выбрать пункт **Показать...** для типа связанных элементов, которые нужно добавить на карту. Пример:  
    ...
    |||  
    |-|-|  
    |**Показать сборки, на которые ссылается этот**|Добавьте сборки, на которые ссылается данная сборка. Внешние сборки приводятся в группе **Внешние** .|  
    |**Показать сборки, ссылающегося на этот**|Добавьте в решение сборки, которые ссылаются на данную сборку.|  
    ...
    |**Показать типы, на которые ссылается этот**|Добавьте все классы и их члены, используемые данным классом.|  
    |**Показать типы, ссылающегося на этот**|Добавьте все классы и их члены, которые используют данный класс.|  
    |**Показать содержащий пространство имен**|Добавьте родительское пространство имен.|  
    ...
    |**Показать поля, на которые ссылается этот**|Добавьте поля, на которые ссылается данный метод.|  
    |**Показать содержащий тип**|Добавьте родительский тип.|  
    |**Показать содержащий тип, пространства имен и сборки**|Добавьте родительскую иерархию контейнера.|  
    ...
    |**Показать содержащий тип, пространства имен и сборки**|Добавьте родительскую иерархию контейнера.|  
     ...
     ![Показать методы, вызванные этим членом](~/docs/modeling/media/codemapsshowrelatedmethods.png "CodeMapsShowRelatedMethods")  
...
8.  Чтобы упростить карту и выделить ее отдельные части, выберите **Фильтры** на панели элементов в карте кода и отметьте только те типы узлов и связей, которые вас интересуют. Например, отключите отображение папок решения, сборок и пространств имен.  
     ...
     ![Используйте панель «Фильтр» для упрощения отображения](~/docs/modeling/media/almcodemapfilterpane.png "ALMCodeMapFilterPane")  
  
##  <a name="a-nameseesourceheadera-see-dependencies-between-c-and-c-source-files-and-header-files"></a><a name="SeeSourceHeader"></a>Просмотр зависимостей между исходными файлами C и C++ и файлами заголовков  
...
-   Чтобы просмотреть зависимости между всеми исходными файлами и файлами заголовков в решении, в меню **Архитектура** выберите пункт **Создать диаграмму включаемых файлов**.  
     ...
     ![Граф зависимостей для машинного кода](~/docs/modeling/media/dependencygraphgeneral_nativecode.png "DependencyGraphGeneral_NativeCode")  
...
-   Чтобы просмотреть зависимости между открытым файлом и связанными с ним исходными файлами и файлами заголовков, откройте либо исходный файл, либо файл заголовка. Откройте контекстное меню в любом месте файла. Выберите пункт **Создать диаграмму включаемых файлов**.  
     ...
     ![Граф зависимостей первого уровня для h-файла](~/docs/modeling/media/dependencygraph_native_firstlevel.png "DependencyGraph_Native_FirstLevel")  
  
###  <a name="a-nametroubleshootinga-troubleshoot-maps-for-c-and-c-code"></a><a name="Troubleshooting"></a>Устранение неполадок в картах для кода C и C++  
...
|[!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]не отвечает при попытке создать карту кода из **архитектура** меню.|Файл базы данных программы (PDB) может быть поврежден.<br /><br /> В PDB-файле хранятся сведения для отладки, такие как сведения о типе, методе и исходном файле.|Выполните сборку решения еще раз и повторите попытку.|  
|Некоторые параметры базы данных обзора IntelliSense отключены.|Возможно, некоторые параметры IntelliSense отключены в [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] **параметры** диалоговое окно.|Включите нужные параметры.<br /><br /> В разделе [Дополнительные параметры "," Текстовый редактор "," C/C++,](../ide/reference/options-text-editor-c-cpp-advanced.md).|  
...
##  <a name="a-namerendermorequicklya-make-code-maps-render-more-quickly"></a><a name="RenderMoreQuickly"></a>Повышение скорости отрисовки карты кода  
...
-   [Сопоставление зависимостей, которые вас интересуют.](#SeeSpecificSource)  
...
-   Удалите ненужные узлы и ссылки, отредактировав файл карты кода. Изменение карты не влияет на базовый код. В разделе [Настройка карт кода путем редактирования DGML-файлов](../modeling/customize-code-maps-by-editing-the-dgml-files.md).  
  
 ![Пропустить сборку и включить родительские элементы кнопки](~/docs/modeling/media/codemapsfilterskipbuildicons.png "CodeMapsFilterSkipBuildIcons")  
...
##  <a name="a-namesavingexportinga-share-code-maps"></a><a name="SavingExporting"></a>Карт кода  
...
### <a name="share-the-map-with-other-visual-studio-users"></a>Совместное использование карты с другими пользователями Visual Studio  
 ...
 Чтобы сохранить схему как часть определенного проекта на панели инструментов карты, выберите **общего ресурса**, **перемещение** \< *CodeMapName*>**.dgml в**и выберите проект, где требуется сохранить схему.  
  
 ![Переместите карту в другой проект](~/docs/modeling/media/codemapsmovemapmenu.png "CodeMapsMoveMapMenu")  
...
3.  Укажите имя для карты кода. Убедитесь, что **тип** установлен на **XPS-файлы (\*.xps)**. Нажмите кнопку **Сохранить**.  
