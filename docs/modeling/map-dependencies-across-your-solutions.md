...
ms.locfileid: "34265569"
---
# <a name="map-dependencies-with-code-maps"></a>Сопоставление зависимостей с картами кода

Вы можете визуализировать зависимости в коде путем создания карты кода. Карты кода помогают увидеть, как код сочетается друг с другом без чтения файлов и строк кода.

![Просмотр зависимостей во всех решениях](../modeling/media/codemapsmainintro.png)

Для работы с картами кода, необходимо Visual Studio Enterprise или Professional edition. Функциональность карты кода в выпуске Professional edition ограничено немного более чем в выпуске Enterprise edition.

> [!NOTE]
> Прежде чем делать картам, созданным в Visual Studio Enterprise, пользователям программы Visual Studio Professional, убедитесь, что отображаются все элементы на карте (например, скрытые элементы, развернутые группы и связи между группами).

Можно сопоставить зависимостей для кода на следующих языках:

- Visual C# или Visual Basic в решении или сборках (*.dll* или *.exe*)

- Машинный или управляемый код C или C++ в проектов Visual C++, файлы заголовков (*.h* или `#include`), или двоичные файлы

- проекты и сборки X++, созданные из модулей .NET для Microsoft Dynamics AX.

> [!NOTE]
> Для проектов, отличный от C# и Visual Basic доступно меньше способов создания карты кода или добавления элементов в существующую карту кода. Например, нельзя щелкнуть объект правой кнопкой мыши в редакторе текста проекта C++ и добавить его на карту кода. Тем не менее, вы можно перетаскивать элементы или файлы из отдельных кода **обозревателе решений**, **представление классов**, и **обозревателя объектов**.

## <a name="install-code-map-and-live-dependency-validation"></a>Карта кода установки и проверки динамических зависимостей

Чтобы создать карту кода в Visual Studio 2017 г., следует сначала установить **карта кода** и **быстрой проверки зависимостей** компоненты:

1. Откройте **установщик Visual Studio**. Его можно открыть из меню «Пуск» или в среде Visual Studio, выбрав **средства** > **получение средств и функций**.

1. Откройте вкладку **Отдельные компоненты**.

1. Прокрутите вниз до **средства кода** , выберите **карта кода** и **быстрой проверки зависимостей**.

   ![Карта кода и быстрой проверки зависимостей компоненты в установщик Visual Studio](media/modeling-components.png)

1. Нажмите кнопку **Изменить**.

   **Карта кода** и **быстрой проверки зависимостей** компоненты начать установку. Может появиться запрос закрыть Visual Studio.

## <a name="add-a-code-map"></a>Добавление карты кода

Можно создать пустой карты кода и перетащите элементы на него, включая ссылки на сборки, файлы и папки, или можно создать карту кода для всех или части вашего решения.

Чтобы добавить пустой карты кода:
...
3. Выберите **направленный граф Document(.dgml)** шаблона, а затем выберите **добавить**.
   ...
   > [!TIP]
   > Этот шаблон может не отобразиться в алфавитном порядке, поэтому прокрутите список вниз до нижней части списка шаблона, если вы не видите.

   Откроется пустой карты в вашем решении **элементы решения** папки.

Аналогичным образом, можно создать новый файл карты кода без добавления в решение, выбрав **архитектура** > **новую карту кода** или **файл**  >  **Новый** > **файл**.

## <a name="generate-a-code-map-for-your-solution"></a>Создать карту кода для решения

Для просмотра всех зависимостей в решении:

1. В строке меню выберите **архитектура** > **создать карту кода для решения**. Если код не изменился с момента последней сборки, можно выбрать **архитектура** > **создать карту кода для решения без сборки** вместо него.

   ![Команда "Создать карту кода"](../modeling/media/codemapsarchitecturemenu.png)

   Создается карта кода со сборками верхнего уровня и совокупными связями между ними. Чем шире совокупная связь, тем больше зависимостей она представляет.
...
4. В большом решении это не пригодится. Более того, если решение комплексное, ограничения памяти могут не позволить развернуть все группы. Вместо этого разворачивайте отдельные интересующие вас узлы. Наведите указатель мыши на верхнюю часть узла и нажмите на значок шеврона (стрелка вниз), когда он появится.
   ...
   > [!TIP]
   > Дополнительные сведения о работе с кодом сопоставляется с помощью мыши, клавиатуры и жестов см. в разделе [Просмотр и переупорядочение карт кода](../modeling/browse-and-rearrange-code-maps.md).
...
10. Еще один способ концентрации в комплексном решении — это создать новую карту с выбранными элементами существующей карты. Удерживайте **Ctrl** при выделении необходимо сосредоточиться на элементы, откройте контекстное меню и выберите **новый граф по выделенному фрагменту**.
...
## <a name="view-specific-dependencies-in-a-code-map"></a>Просмотр конкретных зависимостей на карте кода
...
1. На **обозревателе решений** инструментов, выберите **Показать на карте кода** ![создания графа из выбранных узлов кнопка создания](../modeling/media/createnewgraphfromselectedbutton.gif). Или, откройте контекстное меню для одного или нескольких элементов и выберите **Показать на карте кода**.

   Можно также перетаскивать элементы из **обозревателе решений**, **представление классов**, или **обозревателя объектов**, в [новый](#add-a-code-map) или сопоставить существующего кода. Чтобы включить родительскую иерархию для элементов, нажмите и удерживайте **Ctrl** , перетащите нужные элементы, или использовать **включить родительские элементы** кнопку на панели инструментов карты кода и укажите действие по умолчанию. Можно также перетащить файлы сборки из вне Visual Studio, такие как из **Проводник**.

   > [!NOTE]
   > При добавлении элементов из проекта, который является общим для нескольких приложений, таких как Windows Phone или магазина Microsoft эти элементы отобразятся на карте с текущим активным проектом приложения. Если изменить контекст на другой проект приложения и добавить дополнительные элементы из общего проекта, эти элементы отобразятся на карте с новым активным проектом приложения. Операции, выполняемые с элементом на карте, применяются только к тем элементам, которые имеют общий контекст.
...
4. Для исследования элементов разверните их. Наведите указатель мыши на элемент и щелкните значок шеврона (стрелка вниз), когда он появится.
   ...
   Чтобы развернуть все элементы, выберите их с помощью **Ctrl**+**A**, откройте контекстное меню для карты и выберите **группы**  >   **Разверните**. Этот параметр недоступен, если при развертывании всех групп карта кода станет непригодна к использованию или возникнут проблемы с памятью.

5. Продолжайте разворачивать элементы, которые вас интересуют, вплоть до уровня классов и членов, если это необходимо.

   ![Развернуть группы до уровня классов и членов](../modeling/media/codemapsexpandtoclassandmember.png)

   Чтобы просмотреть элементы, которые имеются в коде, но не отображаются на карте, щелкните **повторно извлечь дочерние элементы** значок ![повторно извлечь дочерние элементы значок](../modeling/media/dependencygraph_deletednodesicon.png) в левом верхнем углу группы.
...
7. На карте показаны отношения. В этом примере на карте кода показаны методы, вызываемые `Find` метод и их расположение в решении или внешне.
...
- [Видео: понять конструктора из кода с помощью карт кода Visual Studio 2015](https://channel9.msdn.com/Events/Visual-Studio/Connect-event-2015/502)]
