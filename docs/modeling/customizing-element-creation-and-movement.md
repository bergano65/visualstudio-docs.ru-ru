...
![DSL&#45;EMD&#95;слияния](../modeling/media/dsl-emd_merge.png)
...
![DSL&#45;EMD&#95;сведения](../modeling/media/dsl-emd_details.png)
...
4. В разделе **обработать объединение путем создания ссылки**, добавьте два пути:
   ...
   - Один путь связывает новый элемент родительской модели. Выражение пути, необходимо ввести переходит от существующего элемента вверх через отношение внедрения к родительской модели. Наконец он указывает роль в новой ссылке, к которому будет назначен новый элемент. Путь выглядит следующим образом:

      `ExampleModelHasElements.ExampleModel/!ExampleModel/.Elements`

   - Другой путь связывает новый элемент на существующий элемент. Выражение пути Задает ссылочное отношение и роль, к которому будет назначен новый элемент. Этот путь выглядит следующим образом:
...
5. Проверьте DSL:
   ...
   1.  Нажмите клавишу **F5** перестроить и запустить решение.
   ...
   2.  После запуска экспериментальном экземпляре Visual Studio откройте файл модели DSL. Создайте некоторые элементы пример.
...
6.  Проверьте DSL:
    ...
    1.  Нажмите клавишу **F5** для перестроения решения. Когда Откроется экспериментальный экземпляр Visual Studio, откройте экземпляр DSL.

    2.  Создайте новые элементы несколькими способами:

        - Перетащите из **пример элемента** средство на схему.

        - В **обозревателя моделей пример**, щелкните правой кнопкой мыши корневой узел и нажмите кнопку **добавить новый элемент пример**.
...
10. Постройте и запустите это решение. Появляется новый экземпляр Visual Studio.
...
- [Пример схемы канала DSL](https://code.msdn.microsoft.com/Visualization-Modeling-SDK-763778e8)