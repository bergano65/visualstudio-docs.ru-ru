...
- *Моникеры идентификаторов* цитата. идентификатор GUID целевого элемента. Например:
...
1. Убедитесь, что **ключ моникера** `false` для каждого свойства домена в классе и его базовых классах.
    ...
    1. В обозревателе DSL разверните **Бехавиор\класс данные XML-сериализации,\\\<класс домена > данные \елемент**.
...
- Set **является ключом моникера** для свойства предметной области существующего доменного класса. Тип свойства должен быть `string`.
    ...
    1. В обозревателе DSL разверните **Бехавиор\класс данные XML-сериализации,\\\<класс домена > данные \елемент**, а затем выберите свойство Domain.
...
|Имя типа моникера|Имя XSD-типа, создаваемого для моникеров для элементов данного класса. XSD находится в **коде дсл\женератед\\\*Schema. xsd**|
