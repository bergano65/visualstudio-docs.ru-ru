...
|Основное приложение|Узел — это интерфейс между системой и среды пользователя. [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]представляет собой узел процесса преобразования текста.|Да. Можно написать пользовательское основное приложение.|  
