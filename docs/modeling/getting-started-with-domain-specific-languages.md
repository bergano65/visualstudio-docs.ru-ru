...
Если вы не знакомы с DSL впервые, мы советуем работать через **лаборатории средства DSL**, который можно найти на этом сайте: [Визуализации и моделирования SDK](http://go.microsoft.com/fwlink/?LinkID=186128)
...
При созданной DSL можно распределить его как часть пакета Visual Studio Integration Extension (VSIX). Пользователи работают с DSL в Visual Studio:
...
Остальная часть в этом обзоре — Пошаговое руководство, которое представляет основные операции создания и использования доменного языка в Visual Studio.
...
Обратите внимание, что новое решение следующие аспекты:
...
- **Проект DslPackage** этот проект содержит код, который позволяет экземпляры DSL, чтобы открыть и редактировать в Visual Studio.
...
2. Нажмите клавишу **F5**или выберите в меню **Отладка** пункт **Начать отладку**.
     ...
     DSL, сборок и является в экспериментальном экземпляре Visual Studio.

     Запускает экспериментальный экземпляр Visual Studio. Экспериментальный экземпляр принимает его параметры из отдельное поддерево реестра, где регистрируются расширений Visual Studio для отладки. Обычный экземпляры Visual Studio нет доступа к расширениям, зарегистрированы.

3. В экспериментальном экземпляре Visual Studio, откройте файл модели, с именем **теста** из **обозревателе решений**.
...
В экспериментальном экземпляре Visual Studio будет выглядеть следующим образом:
...
Содержимое файла, который является экземпляром класса DSL называется *модели*. В модели содержится *модели* <em>элементы</em> и *ссылки* между элементами. Определение DSL определяет, какие типы элементов модели и ссылки могут существовать в модели. Например в DSL, созданном из шаблона минимальный язык, имеется один тип элемента модели и одного типа ссылки.
...
Если обозреватель в экземпляре отладки Visual Studio, не виден на **представление** пункты меню **Other Windows**и нажмите кнопку  *\<ваш язык >* **Explorer**.
...
В решении "Отладка" откройте файл шаблона с расширением «.tt». Эти примеры демонстрируют, как создание текста из моделей и использовать при тестировании API вашего DSL. Один из примеров создается на языке [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)], другие в [!INCLUDE[csprcs](../data-tools/includes/csprcs_md.md)].
...
4. Отладка решения в экспериментальном экземпляре Visual Studio откройте файл модели тестирования. Перетащите элементы на его из области элементов. Обратите внимание, что средство заголовки и имена типов в обозревателе DSL были изменены.
...
9. В экспериментальном экземпляре Visual Studio откройте файл модели тестирования. Используйте новые средства для создания городам и ссылки между городам и лиц. Обратите внимание на то, что можно создать только связи между правильные типы элемента.
...
## <a name="validation-and-commands"></a>Проверка и команды
 ...
 Можно также определить команды меню, которые пользователь может вызывать. Команды можно изменить модель. Они также могут взаимодействовать с другими моделями в Visual Studio и внешние ресурсы. Дополнительные сведения см. в разделе [Практическое руководство. Изменение стандартной команды меню](../modeling/how-to-modify-a-standard-menu-command-in-a-domain-specific-language.md).

## <a name="deploying-the-dsl"></a>Развертывание DSL
 Чтобы разрешить другим пользователям использовать доменный язык, распространение файлов Visual Studio Extension (VSIX). Это будет создано при построении решения DSL.

 Найдите VSIX-файл в папке bin для вашего решения. Скопируйте его на компьютер, на котором вы хотите установить его. На этом компьютере дважды щелкните VSIX-файл. DSL можно использовать во всех экземплярах Visual Studio на этом компьютере.

 Можно использовать ту же процедуру для установки DSL на своем компьютере, таким образом, нет необходимости использовать экспериментальный экземпляр Visual Studio.
...
## <a name="Reset"></a> Удалить старые экспериментальные доменных языков
 Если вы создали экспериментальный доменных языков, которые больше не требуется, их можно удалить с компьютера с Сброс Visual Studio экспериментальный экземпляр.

 Это приведет к удалению с компьютера все экспериментальные доменных языков и другие экспериментальные расширения Visual Studio. Это расширения, которые были выполнены в режиме отладки.

 Эта процедура не приводит к удалению DSL или другими расширениями Visual Studio, которые были полностью установлены, выполнив файл VSIX.
...
2. Перестройте любой экспериментальный DSL или другие экспериментальные расширения Visual Studio, которые по-прежнему хотите использовать.
