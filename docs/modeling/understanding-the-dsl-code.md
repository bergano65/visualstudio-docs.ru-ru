 ...
 **Dsl** проекта определяет API для вашего DSL. **DslPackage** проекта определяет, как он интегрируется с [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)]. Вы можете также добавить собственные проекты, которые могут содержать код, созданный из модели.
