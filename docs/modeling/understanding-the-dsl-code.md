...
> [!NOTE]
> Этот шаблон DSL схемы компонента вызывается **конструктора доменного языка**.

Нажмите клавишу **F5** и эффективнее, если вы не знакомы с этим шаблоном решения. В частности, обратите внимание на то, что порты создаются перетаскиванием инструмента порта на компонент, и порты можно подключать.
