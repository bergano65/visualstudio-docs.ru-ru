...
Используйте такие средства, как карты кода, схемы зависимостей и схемы классов, чтобы:
...
В этом сценарии описываются эпизоды из жизненных циклов разработки программного обеспечения для двух вымышленных компаний: Dinner Now и Lucerne Publishing. Dinner Now предоставляет услуги доставки еды по Сиэтлу с заказом через Интернет. Клиенты могут заказать еду и платить за них на веб-сайте компании Dinner Now. после чего заказы направляются в соответствующий местный ресторан для выполнения доставки. Нью-йоркская компания Lucerne Publishing работает в нескольких областях с использованием и без использования Интернета. Например, они запускают веб-сайт, на котором клиенты могут публиковать отзывы о ресторане.

Недавно компания Lucerne приобрела компанию Dinner Now и хочет произвести указанные ниже изменения.

- Интегрируйте свои веб-сайты, добавив возможности проверки ресторана в компании Dinner Now.
...
Для рисования схем зависимостей необходимо создать проект моделирования как часть существующего решения или создать его. Эти схемы должны создаваться в проекте моделирования.
...
Обе команды также используют проверку зависимостей, чтобы убедиться, что код в разработке остается согласованным с архитектурой. Пример

- [Поддержание соответствия кода и структуры](#ValidatingCode)

- [Опишите логическую архитектуру: схемы зависимостей](#DescribeLayers)

- [Проверка кода по схемам зависимостей](../modeling/validate-code-with-layer-diagrams.md)

> [!NOTE]
> Некоторые версии Visual Studio поддерживают проверку зависимостей и версии карт кода только для чтения для визуализации и моделирования. Чтобы узнать, какие выпуски Visual Studio поддерживают эту функцию, см. раздел [Поддержка архитектуры и средств моделирования в выпуске](../modeling/what-s-new-for-design-in-visual-studio.md#VersionSupport).

## <a name="understand-and-communicate-information-about-the-system"></a>Изучение и передача сведений о системе
...
Компании Dinner Now и Lucerne взаимодействуют друг с другом и с заинтересованными сторонами проекта, используя схемы как общий язык. Например, в компании Dinner Now схемы используются для выполнения следующих задач.
...
Перед тем, как команды проверяют свои изменения, они проверяют код на соответствие тестам и проектированию, запуская сборки, включающие проверку зависимостей и автоматические тесты. Это помогает предотвратить противоречия между обновленным кодом и дизайном, а также нарушение работавших прежде функциональных возможностей.
...
### <a name="keep-code-consistent-with-the-design"></a><a name="ValidatingCode"></a>Обеспечение соответствия кода дизайну
 Компания Dinner Now должна удостовериться, что обновленный код соответствует дизайну. Они создают схемы зависимостей, описывающие уровни функциональности в системе, указывают разрешенные зависимости между ними и связывают артефакты решения с этими слоями.

|**Схема**|**Что описывает**|
|-|-|
|*Схема зависимостей*<br /><br /> Пример<br /><br /> - [Создание схем зависимостей из кода](../modeling/create-layer-diagrams-from-your-code.md)<br />- [Схемы зависимостей: справочные материалы](../modeling/layer-diagrams-reference.md)<br />- [Схемы зависимостей: рекомендации](../modeling/layer-diagrams-guidelines.md)<br />- [Проверка кода с помощью схем зависимостей](../modeling/validate-code-with-layer-diagrams.md)|Логическая архитектура кода.<br /><br /> Схема зависимостей организует артефакты в решении Visual Studio и сопоставляет их с абстрактными группами, называемыми *слоями*. Эти слои описывают роли, задачи или функции, выполняемые этими артефактами в системе.<br /><br /> Схемы зависимостей полезны для описания предполагаемой структуры системы и проверки кода на соответствие этому проекту.<br /><br /> Чтобы создать слои, перетащите элементы из обозревателя решений, карт кода, представления классов и обозревателя объектов. Чтобы нарисовать новые слои, используйте панель элементов или щелкните поверхность схемы правой кнопкой мыши.<br /><br /> Чтобы просмотреть существующие зависимости, щелкните правой кнопкой мыши область Схема зависимостей и выберите команду **создать зависимости**. Чтобы указать целевые зависимости, нарисуйте новые зависимости.|

Например, следующая схема зависимостей описывает зависимости между слоями и число артефактов, связанных с каждым слоем.

![Схема зависимостей интегрированной системы платежей](../modeling/media/layer_integrated_dnlucerne.png)

 **Схема зависимостей**

Чтобы гарантировать, что конфликты с архитектурой не возникают во время разработки кода, группы используют проверку зависимостей для сборок, которые выполняются в Azure DevOps. Они также создают пользовательскую задачу MSBuild, которая требует проверки зависимостей в операциях возврата. Для сбора ошибок проверки используются отчеты о сборках.

Пример

- [Использование визуального конструктора](/azure/devops/pipelines/get-started-designer)

- [TFVC запись с проверкой изменений](/azure/devops/pipelines/build/triggers)
...
- Ежедневные *отчеты* , которые показывают, будут ли они завершать запланированную работу в ожидаемое время. Для контроля за ходом устранения ошибок они создают в Team Foundation Server и другие отчеты подобного рода.
...
- [О гибких средствах и управлении проектами Agile](/azure/devops/boards/backlogs/backlogs-overview?view=vsts)
...
По мере того как команды завершают каждую задачу, они проверяют свой код в системе управления версиями и получают напоминания от Team Foundation Server, если они забывают. Прежде чем Team Foundation Server принимает свои записи после возврата, команды выполняют модульные тесты и проверку зависимостей для проверки кода на соответствие тестовым случаям и проектированию. Они используют Team Foundation Server для регулярного выполнения сборок, автоматизированных модульных тестов и проверки зависимостей. Это позволяет им контролировать соответствие кода перечисленным ниже критериям.
...
Чтобы убедиться, что код соответствует проекту, команды настраивают свои сборки в Azure DevOps для включения проверки зависимостей. Если возникают какие-либо конфликты, создается отчет с подробными сведениями.
...
- Поиск областей сложности, естественных зависимостей, закономерностей или других областей, которые могут оказаться полезными при улучшении.
...
|**Схема**|**Что описывает**|
|-|-|
|Схема зависимостей|Логическая архитектура системы. Используйте проверку зависимостей, чтобы убедиться, что код остается согласованным с макетом.<br /><br /> Чтобы помочь вам определить существующие зависимости или зависимости, создайте карту кода и сгруппируйте связанные элементы. Сведения о создании схемы зависимостей см. в следующих статьях:<br /><br /> - [Создание схем зависимостей из кода](../modeling/create-layer-diagrams-from-your-code.md)<br />- [Схемы зависимостей: рекомендации](../modeling/layer-diagrams-guidelines.md)|
...
### <a name="define-a-glossary-of-types-class-diagrams"></a><a name="DefineClasses"></a>Определение глоссария типов: схемы классов
...
|**Схема**|**Описание**|
|-|-|
|Схема зависимостей|Определение логической архитектуры системы относительно классов.<br /><br /> Используйте проверку зависимостей, чтобы убедиться, что код остается согласованным с макетом.<br /><br /> Пример<br /><br /> - [Создание схем зависимостей из кода](../modeling/create-layer-diagrams-from-your-code.md)<br />- [Схемы зависимостей: справочные материалы](../modeling/layer-diagrams-reference.md)<br />- [Схемы зависимостей: рекомендации](../modeling/layer-diagrams-guidelines.md)<br />- [Проверка кода с помощью схем зависимостей](../modeling/validate-code-with-layer-diagrams.md)|
|Карта кода|Визуализация организации и отношений в имеющемся коде.<br /><br /> Для идентификации классов, их отношений и методов создайте карту кода, показывающую эти элементы.<br /><br /> Пример<br /><br /> - [Сопоставьте зависимости в решениях](../modeling/map-dependencies-across-your-solutions.md)|

### <a name="describe-the-logical-architecture-dependency-diagrams"></a><a name="DescribeLayers"></a>Опишите логическую архитектуру: схемы зависимостей
 Схемы зависимостей описывают логическую архитектуру системы путем организации артефактов в решении в абстрактные группы или *слои*. Артефактами могут быть различные сущности, такие как пространства имен, проекты, методы и т. п. Слои представляют и описывают роли или задачи, выполняемые артефактами в системе. Включив проверку слоев в операции построения и возврата, можно обеспечить соответствие кода его дизайну.

 Чтобы код соответствовал дизайну, компания Dinner Now и компания Lucerne используют следующую схему зависимостей для проверки кода по мере развития:

 ![Схема зависимостей интегрированной системы платежей](../modeling/media/layer_integrated_dnlucerne.png)

 **Схема зависимостей для компании Dinner Now, интегрированная с компанией Lucerne**
...
#### <a name="drawing-a-dependency-diagram"></a>Рисование схемы зависимостей

Схема зависимостей имеет следующие основные характеристики.
...
- *Связь* — это ассоциация между слоем и артефактом.
  ...
  - Если слой содержит другие слои, связанные с артефактами, то слой-контейнер также связан с этими артефактами, даже если число в слое-контейнере не включает эти артефакты.
    ...
    Чтобы просмотреть артефакты, связанные с слоем, щелкните правой кнопкой мыши зависимость и выберите **Просмотреть ссылки** , чтобы открыть **Обозреватель слоев**.
...
- *Зависимость* указывает, что один слой может использовать функции другого слоя, но не наоборот. *Двунаправленная зависимость* указывает, что один слой может использовать функции другого слоя и наоборот.
     ...
     Чтобы отобразить существующие зависимости на схеме зависимостей, щелкните правой кнопкой мыши поверхность схемы и выберите команду **создать зависимости**. Чтобы описать требуемые зависимости, создайте новые зависимости.
...
|**Схема**|**Описание**|
|-|-|
|Карта кода|Визуализация организации и отношений в имеющемся коде.<br /><br /> Чтобы создать слои, сформируйте карту кода, а затем сгруппируйте элементы на карте как потенциальные слои. Перетащите группы из схемы на схему зависимостей.<br /><br /> Пример<br /><br /> - [Сопоставьте зависимости в решениях](../modeling/map-dependencies-across-your-solutions.md)<br />- [Просмотр и реорганизация карт кода](../modeling/browse-and-rearrange-code-maps.md)|
