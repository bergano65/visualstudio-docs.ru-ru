...
Таким образом действия выполняются в проекте DslPackage следующим образом:
...
3. В новом файле напишите частичное объявление, используя то же пространство имен и имя, что и в созданном частичном классе. Например:
     ...
     > [!NOTE]
     > Если для создания нового файла использовался шаблон класса, необходимо откорректировать пространство имен и имя класса.
...
Необходимо определить два метода: один для определения, когда команда будет отображаться в меню щелкните правой кнопкой мыши (контекста), а другой для выполнения команды. Эти методы не переопределяются. Наоборот, их нужно зарегистрировать в списке команд.
