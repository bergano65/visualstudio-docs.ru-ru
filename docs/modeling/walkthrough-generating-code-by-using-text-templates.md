...
Установка атрибута `hostspecific` директивы `<#@template#>` позволяет шаблону получить доступ к API Visual Studio. Благодаря этому шаблон может получить сведения о расположении файлов проекта, чтобы избежать использования абсолютного пути к файлу в коде шаблона.
