---
title: "Файл DslDefinition.dsl | Документы Microsoft"
ms.custom: 
ms.date: 11/04/2016
ms.reviewer: 
ms.suite: 
...
В этом разделе описывается структура DslDefinition.dsl файла в проект Dsl [!INCLUDE[dsl](../modeling/includes/dsl_md.md)] решение, которое определяет *доменного языка*. Файл DslDefinition.dsl описаны классы и отношения доменный язык, вместе с диаграммы, фигур, соединители, формат сериализации и **элементов** доменного языка и его средства редактирования. В решении доменного языка код, который определяет эти инструменты, генерируется согласно информации из файла DslDefinition.dsl.  
 ...
 Примеры в данном разделе взяты из шаблона решения "Схема компонентов". Чтобы посмотреть пример, создайте решение доменного языка на основе шаблона решения "Модели компонентов". Как только вы создадите решение, в Конструкторе доменного языка появится файл DslDefinition.dsl. Закройте файл, щелкните правой кнопкой мыши в **обозревателе решений**, пункты **открыть с помощью**, нажмите кнопку **редактора XML**и нажмите кнопку **ОК**.  
  
## <a name="sections-of-the-dsldefinitiondsl-file"></a>Разделы файла DslDefinition.dsl  
 Корневой элемент — \<Dsl > и его атрибуты определяют имя доменный язык, пространство имен и номера основной и дополнительный номера версий для управления версиями. Схема `DslDefinitionModel` определяет содержание и структуру действительного файла DslDefinition.dsl.  
 ...
 Типы  
 В этом разделе определяется каждый тип и его пространство имен. Свойства домена бывают двух типов. `DomainEnumerations`определены в модели и Создание типов в DomainModel.cs. `ExternalTypes`ссылаться на типы, определенные в другом месте (например, `String` или `Int32`) и не создает никаких действий.  
 ...
 ExplorerBehavior  
 В этом разделе определены как **обозреватель DSL** появится окно, когда пользователь редактирует модели.  
...
-   **Id.** Этот атрибут является GUID. Если вы не укажете значение в файле, Конструктор доменного языка создаст значение сам. (Для экономии места в иллюстрациях к данному документу этот атрибут обычно опускается.)  
...
-   **Отображаемое имя.** Этот атрибут имеет имя, отображаемое в **свойства** окна. Атрибут DisplayName может содержать пробелы и другие знаки препинания.  
...
-   **IsBrowsable**. Этот атрибут определяет, отображается ли свойство в **свойства** окна по щелчку объекта класса-родителя.  
  
-   **IsUIReadOnly**. Данный атрибут определяет, может ли пользователь изменить свойство в **свойства** окна или с помощью декоратор, в котором представлены свойства.  
...
-   **Категория** — это заголовок, под которым свойство будет отображаться в **свойства** окна.  
...
-   `RolePlayer` Атрибут ссылается на класс домена связанных экземпляров: OutPort источника, InPort для целевого объекта.  
...
## <a name="designer-and-toolbox-tabs"></a>Конструктор и вкладки панели элементов  
 Основная часть **конструктор** раздел файла DslDefinition.dsl **ToolboxTab** элементов. Один конструктор может содержать несколько этих элементов, каждый из которых представляет заголовками раздел в конструкторе созданный **элементов**. Каждый **ToolboxTab** элемент может содержать один или несколько **ElementTool** элементов, **ConnectionTool** элементы или оба.  
...
## <a name="paths"></a>Пути  
 ...
 Каждый сегмент начинается с имени взаимосвязи. В прыжок связи объекта связи предшествует точка и имя свойства: "`Relationship . Property`«. В перехода объекта связи связь предшествует восклицательный знак и имя роли: «`Relationship ! Role`».  
...
```  
 ...
 При написании C# для данной модели, можно перейти по ссылке в один шаг, используя свойство, которое приводит к возникновению ошибки связи на каждый из классов, которым они относятся:  
...
## <a name="element-merge-directives"></a>Директивы слияния элементов  
 Если язык пользователь перетаскивает элемент из **элементов** на схему, создается экземпляр класса этого средства. Кроме того, создаются связи между данным экземпляром и существующими элементами модели. Некоторые элементы, такие как компоненты или комментарии, создаются при их перемещении пользователем язык **элементов** фигуру на пустую область схемы. Другие элементы создаются, когда пользователь языка перетаскивает их на другие размещенные элементы. Например, OutPort или InPort создаются, когда пользователь перетаскивает их на компонент.  
...
-   **XmlPropertyData** для каждого свойства, определенные в классе.  
  
-   **XmlRelationshipData** для каждой связи, чем класс. (У отношений также есть собственные узлы XmlClassData.)  
  
-   **TypeName** строковый атрибут, который определяет имя вспомогательного класса сериализации в созданном коде.  
  
-   **ElementName** строку, которая определяет XML-тега сериализованных экземпляров этого класса. Как правило, ElementName совпадает с именем класса за тем исключением, что начинается со строчной буквы. Например, файл образца модели начинается следующим образом:  
  
    ```  
    <componentModel ...  
    ```  
  
-   **MonikerElementName** файлы сериализованной модели. Данный атрибут вводит моникер, который ссылается на этот класс.  
  
-   **MonikerAttributeName**, определяющий имя XML-атрибут в специальное имя. В этом фрагменте сериализованного файла пользователя, определенных автором доменный язык **MonikerElementName** как «inPortMoniker» и **MonikerAttributeName** как «путь»:  
...
### <a name="xmlpropertydata"></a>XmlPropertyData  
 Объект **DomainPropertyMoniker** атрибутов определяет свойство, на который ссылается данные. Данный атрибут должен быть свойством включающего класса ClassData.  
  
 **XmlName** атрибут предоставляет имя соответствующего атрибута, отображаемое в XML. Как правило, эта строка совпадает с именем свойства за тем исключением, что начинается со строчной буквы.  
  
 По умолчанию **представление** атрибута задано значение атрибута. Если **представление** задан элемент, дочерний узел создается в XML. Если **представление** — установить значение Ignore, свойство не сериализуется.  
  
 **IsMonikerKey** и **IsMonikerQualifier** атрибуты предоставляют свойство роль при определении экземпляров класса-родителя. Можно задать **IsMonikerKey** значение true для одного свойства, определенные в или унаследованное классом. Этот атрибут определяет отдельный экземпляр родительского класса. Свойство, для которого устанавливается значение `IsMonikerKey`, обычно является именем или другим ключевым идентификатором. Например, строковое свойство `Name` является ключом моникера для NamedElement и его производных классов. Когда пользователь сохраняет модель в файл, этот атрибут должен содержать уникальные значения для каждого экземпляра одноуровневых элементов в дереве отношений внедрения.  
...
```  
 ...
 Можно задать **IsMonikerQualifier** атрибутов для свойства строки и обеспечивают дополнительный способ создания полного имени элемента. Например, в файле DslDefinition.dsl **имен** является квалификатором моникера.  
 ...
 **XmlRelationshipData** атрибута в **XmlClassData** атрибут определяет точно как дочерние узлы являются вложенными исходного элемента. Каждая связь, являющейся источником для класса домена имеет один **XmlRelationshipData** атрибута.  
  
 **DomainRelationshipMoniker** атрибута определяет одну из связей, исходный код для класса.  
  
 **RoleElementName** атрибут предоставляет имя тега XML, включающий дочерний узел в сериализованных данных.  
...
```  
 ...
 Если **UseFullForm** атрибута задано значение true, впервые появился дополнительный уровень вложенности. Этот слой представляет собой отношение. Если это отношение имеет свойства, данному атрибуту необходимо присвоить значение true.  
...
```  
 ...
 Если **OmitElement** атрибута задано значение true, связь опущен имя роли, который сокращает сериализованного файла и однозначный, если класс имеет не более одного отношения. Пример:  
...
-   **DSL** RootClass узла и класс схемы. DomainClass, DomainRelationship и другие элементы внедрены в `Dsl`.  
  
-   **Классы** — **RoleElementName** связи между доменами на разных языках и DomainClass.  
...
-   **XmlSerializationBehavior** атрибут внедряется в `Dsl` атрибута, но **OmitElement** связи внедренные установлен атрибут. Следовательно, атрибут`RoleElementName` не внедряется. В отличие от этого **данные класса** атрибут `RoleElementName` атрибут внедрения связи между **XmlSerializationBehavior** атрибут и **XmlClassData** атрибута.  
...
-   **ExposesFillColorAsProperty** и несколько аналогичных атрибутов. Эти логические атрибуты позволяют пользователю менять соответствующее свойство. Как правило, щелчку языка фигуре на схеме, свойства, отображаемые в **свойства** окна зависят от экземпляра класса домена, с которым сопоставлен фигуры. Если `ExposesFillColorAsProperty` имеет значение true, также отображается свойство фигуры.  
