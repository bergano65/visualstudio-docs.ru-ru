...
2.  В **Добавление нового элемента** выберите **текстовый шаблон времени выполнения**. (В [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] найдите **общие Items\General**.)  
...
1.  Включите файл в вашей [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] проекта. В обозревателе решений в контекстном меню проекта выберите **добавить**, **существующий элемент**.  
...
### <a name="the-code-built-from-the-template"></a>Код, построенный на основе шаблона  
 При каждом сохранении **.tt** файл дочерняя компания **.cs** или **.vb** будет создан файл. Чтобы просмотреть этот файл в обозревателе решений, разверните **.tt** узел файла. В проекте Visual Basic можно будет развернуть узел, после нажатия кнопки **Показать все файлы** в панели инструментов обозревателя решений.  
  
 Обратите внимание, что этот дочерний файл содержит разделяемый класс, который содержит метод с именем `TransformText()`. Этот метод можно вызывать из приложения.  
...
#### <a name="constructor-parameters-in-visual-basic"></a>Параметры конструктора в Visual Basic  
 В [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)], используется отдельный файл **MyWebPageCode.vb** содержит:  
...
```  
 ...
 И шаблон будет вызываться с помощью передачи параметра в конструктор.  
...
#### <a name="passing-data-in-template-properties"></a>Передача данных в свойствах шаблона  
 Добавление открытых свойств в класс шаблона в определении разделяемого класса — альтернативный метод передачи данных в шаблон. Приложение может задать свойства перед вызовом `TransformText()`.  
  
 Кроме того, можно добавить поля в определении разделяемого класса шаблона. Это позволит передавать данные между последовательными выполнениями шаблона.  
  
### <a name="use-partial-classes-for-code"></a>Использование разделяемых классов для кода  
 Многие разработчики предпочитают избежать написания больших фрагментов кода в шаблонах. Вместо этого определите методы в разделяемый класс с тем же именем, что файл шаблона. Вызовите эти методы из шаблона. Таким образом, шаблон отображает более четко определить какие целевая выходная строка будет выглядеть. Обсуждения внешнего результата можно отделить от логики создания данных, которые отображаются.  
  
### <a name="assemblies-and-references"></a>Сборки и ссылки  
 Если требуется, чтобы код шаблона для ссылки .NET или другую сборку, таких как **System.Xml.dll**, его следует добавить в проект **ссылки** обычным образом.  
...
-   Базовый класс не содержит свободных текст. Вместо этого все его текстовые блоки размещены внутри методов функций класса.  
...
## <a name="see-also"></a>См. также  
 ...
 [T4 понимание: Предварительно обработанных текстовых шаблонов по Oleg Sych](http://www.olegsych.com/2009/09/t4-preprocessed-text-templates/)