...
По умолчанию каждое отношение внедрения имеет `PropagatesDelete` значение true в целевой роли. Удаление `henry` не приводит к `familyTree`удалению, `familyTree.Delete()` но приведет к `Persons`удалению всех.
