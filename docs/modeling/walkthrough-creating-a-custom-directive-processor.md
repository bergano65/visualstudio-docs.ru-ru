...
В этом примере текстовый шаблон вызывает директиву и передает ей имя XML-файла, содержащего документацию для файла класса. В текстовом шаблоне используется свойство <xref:System.Xml.XmlDocument>, которое создается директивой для навигации по XML и печати комментариев к документации.
...
1. Создайте файл с именем *докфиле. XML* с помощью любого текстового редактора (например, блокнота).

    > [!NOTE]
    > Этот файл можно создать в любом расположении (например, *к:\тест\докфиле.ксмл*).
...
4. Измените содержимое TestDP.tt на следующий текст.
    ...
    > [!NOTE]
    > Замените строку `<YOUR PATH>` путем к файлу *докфиле. XML* .
...
1. В **Обозреватель решений**щелкните правой кнопкой мыши TestDP.tt и выберите пункт **Запустить пользовательский инструмент**.
   ...
   Для Visual Basic пользователей Тестдп. txt может не отображаться в **Обозреватель решений** по умолчанию. Чтобы отобразить все файлы, назначенные проекту, откройте меню **проект** и выберите пункт **Показать все файлы**.
