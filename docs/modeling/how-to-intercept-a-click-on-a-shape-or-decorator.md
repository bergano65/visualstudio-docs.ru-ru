...
## <a name="to-intercept-clicks-on-shapes"></a>Перехват щелчков на фигурах
 В проекте DSL в файле кода, отдельном от созданных файлов кода, напишите определение разделяемого класса для класса Shape. Переопределите `OnDoubleClick()` или другой метод с именем, начинающимся с `On...`. Например:
...
> [!NOTE]
> Задайте для `e.Handled` значение `true`, если не требуется передавать событие в содержащую фигуру или схему.
...
4. Измените доменные имена классов и фигур в этом коде в соответствии с собственными языками DSL.
   ...
   В целом, код работает следующим образом. В этом примере `ClassShape` является именем фигуры секции.

- Набор обработчиков событий мыши присоединяется к каждому экземпляру секции при его создании.

- Событие `ClassShape.MouseDown` сохраняет текущий элемент.
