...
- Создание пакета службы: этот пакет и файл конфигурации службы можно использовать для публикации приложения в среде развертывания на [портале Azure](https://portal.azure.com).
...
- **Только веб-роли могут обновляться**: невозможно обновить рабочие роли. Кроме того, нельзя обновлять параметр `RoleEntryPoint` в файле `web role.cs`.

- **Поддерживает только один экземпляр веб-роли**: ни одна роль не может существовать в среде развертывания в нескольких экземплярах. При этом несколько веб-ролей с одним экземпляром поддерживаются.
