...
Опция **https endpoint** доступна только в том случае, если вы уже определили конечную точку HTTPS для вашей роли. Вы можете определить конечную точку HTTPS на странице свойств **Конечные точки** .
...
На странице **Параметры** можно добавлять параметры в конфигурацию в виде пары "имя — значение". Код, работающий в роли, может считывать значения параметров конфигурации во время выполнения, используя классы, предоставляемые [Управляемой библиотекой Azure,](/previous-versions/azure/dn602775(v=azure.11))в частности, метод [GetConfigurationSettingValue.](/previous-versions/azure/reference/ee772857(v=azure.100))
