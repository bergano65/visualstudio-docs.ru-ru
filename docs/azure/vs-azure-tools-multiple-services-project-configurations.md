...
Параметр **конечной точки HTTPS** доступен только в том случае, если конечная точка HTTPS уже определена для вашей роли. Вы можете определить конечную точку HTTPS на странице свойств **Конечные точки** .
...
Для новых или существующих строк подключения выберите **...** _ справа от поля _ *значение**, чтобы открыть диалоговое окно **Создание строки подключения к хранилищу** :
