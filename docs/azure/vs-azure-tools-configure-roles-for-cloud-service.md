...
- **ServiceDefinition.csdef** - Файл определения службы определяет параметры времени выполнения для облачной службы, включая, какие роли требуются, конечные точки и размер виртуальной машины. Данные хранящиеся в файле `ServiceDefinition.csdef`, нельзя изменить во время выполнения роли.
...
1. В **Solution Explorer**расширьте узла проекта. В узле **Роли** щелкните правой кнопкой мыши роль, которую нужно обновить, и в контекстном меню выберите пункт **Свойства**.
...
1. Из визуальной студии, панели инструментов, выберите **Сохранить**.
...
1. В диалоговом окне **Создание строки подключения к хранилищу** выберите один из вариантов для параметра **Подключиться с помощью**. Затем следуйте инструкциям для выбранного варианта.
    ...
    - **Введенные вручную учетные данные**. Введите имя учетной записи хранения, а также первичный или вторичный ключ. Выберите опцию для **подключения** (HTTPS рекомендуется для большинства сценариев).) Выберите **OK**.
