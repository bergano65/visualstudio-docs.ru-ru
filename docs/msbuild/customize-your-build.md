...
Может потребоваться использовать общие свойства для всех проектов `(1)`, общие свойства для проектов `src` `(2-src)` и общие свойства для проектов `test` `(2-test)`.

Чтобы система MSBuild правильно объединила "внутренние" файлы (`2-src` и `2-test`) с "внешними" (`1`), следует помнить, что как только MSBuild находит файл `Directory.Build.props`, она прекращает сканирование. Чтобы продолжить его и включить в слияние внешний файл, добавьте в оба внутренних файла следующее:
...
И еще проще: MSBuild останавливается на первом `Directory.Build.props`, который ничего не импортирует.
