...
|`EntryPoint`|Необязательный параметр <xref:Microsoft.Build.Framework.ITaskItem>`[]`.<br /><br /> Указывает точку входа для создаваемой сборки манифеста. Для манифеста развертывания [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] этот входной параметр указывает манифест приложения [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)].<br /><br /> Для создания манифеста приложения в [!INCLUDE[vsprvslong](../code-quality/includes/vsprvslong_md.md)] [задаче GenerateApplicationManifest](../msbuild/generateapplicationmanifest-task.md) необходимо иметь `EntryPoint`. (Для сборки или собственного манифеста не требуется `EntryPoint`). Это требование было предписано ошибкой сборки: "MSB3185: в качестве манифеста не указано значение EntryPoint".<br /><br /> [!INCLUDE[vstecmsbuild](../extensibility/internals/includes/vstecmsbuild_md.md)] не выдает эту ошибку, если не задан параметр задачи `EntryPoint`. Вместо этого тег \<customHostSpecified> вставляется в качестве дочернего элемента для тега \<entryPoint>, например:<br /><br /> `<entryPoint xmlns="urn:schemas-`<br /><br /> `microsoft-com:asm.v2">`<br /><br /> `<co.v1:customHostSpecified />`<br /><br /> `</entryPoint>`<br /><br /> В манифест приложения можно добавить зависимости DLL следующим образом:<br /><br /> 1.  Разрешите ссылки на сборку, осуществив вызов к <xref:Microsoft.Build.Tasks.ResolveAssemblyReference>.<br />2.  Передайте выходные данные предыдущей задачи и саму сборку в <xref:Microsoft.Build.Tasks.ResolveManifestFiles>.<br />3.  Передайте зависимости в <xref:Microsoft.Build.Tasks.GenerateApplicationManifest>, используя параметр `Dependencies`.|  
|`ErrorReportUrl`|Необязательный параметр [String](assetId:///String?qualifyHint=False&autoUpgrade=True) .<br /><br /> Указывает URL-адрес веб-страницы, который отображается в диалоговых окнах во время установки ClickOnce.|  
...
## <a name="remarks"></a>Примечания  
 Помимо перечисленных выше параметров, эта задача наследует параметры от класса <xref:Microsoft.Build.Tasks.GenerateManifestBase>, который в свою очередь наследует их от класса <xref:Microsoft.Build.Utilities.Task>. Список параметров класса Task см. в статье [Базовый класс Task](../msbuild/task-base-class.md).  
