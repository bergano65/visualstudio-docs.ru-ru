...
|**AssemblyIdentity**|Необязательный параметр **String** .<br /><br /> Задает значения атрибута элемента **assemblyIdentity** манифеста. Укажите разделенный запятыми список, где первый компонент — это значение атрибута `name`, за которым следуют одна или несколько пар "имя-значение" в формате *\<имя_атрибута>=<значение_атрибута>* .<br /><br /> Дополнительные сведения см. в описании параметра **/identity** в разделе [Mt.exe](/windows/desktop/SbsCs/mt-exe).|
