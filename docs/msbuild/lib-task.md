...
|**Имя**|Необязательный параметр типа **String**.<br /><br /> При сборке библиотеки импорта задает имя DLL, для которой собирается библиотека импорта.<br /><br /> Этот параметр соответствует параметру **/NAME** в lib.exe, который принимает аргумент `filename`.|  
