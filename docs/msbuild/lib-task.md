...
|**ForceSymbolReferences**|Необязательный параметр типа **String**.<br /><br /> Вынуждает *lib.exe* включать ссылку на указанный символ.<br /><br /> Этот параметр соответствует параметру **/INCLUDE:** в *lib.exe*.|
