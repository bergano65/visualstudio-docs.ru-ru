...
1. Откройте Visual Studio и создайте проект.
    ...
    В поле **Имя** введите `BuildApp`. Введите **расположение** решения, например *D:\\* . Примите значения по умолчанию для **Решение**, **Имя решения** (**BuildApp**) и **Платформа**.
    ::: moniker-end
    ...
    В поле **Имя** введите `BuildApp`. Введите **расположение** решения, например *D:\\* . Примите параметры по умолчанию **Создать каталог для решения** (выбран), **Добавить в систему управления версиями** (не выбран) и **Имя решения** (**BuildApp**).
    ::: moniker-end
...
```
 ...
 Свойства сборки можно переопределить в любое время. Если
...
4. Изучение выходных данных. Вы должны увидеть следующие две строки (ваша версия .NET Framework может быть другой):
...
2. Изучение выходных данных. Вы должны увидеть следующую строку:
...
4. Изучение выходных данных. Вы должны увидеть следующую строку:
...
4. Изучение выходных данных. Должны отобразиться следующие строки:
...
```
 ...
 добавляет все файлы с расширением *.cs* в тип элемента Compile, за исключением файлов, имена которых содержат строку *Designer*. Дополнительные сведения см. в статье [Практическое руководство. Исключение файлов из сборки](../msbuild/how-to-exclude-files-from-the-build.md).
...
5. Изучение выходных данных. Вы должны увидеть следующую строку:
...
## <a name="item-metadata"></a>Метаданные элементов
 ...
 Для объявления элементов в файле проекта создается элемент с именем метаданных, являющийся дочерним по отношению к элементу. Элемент может содержать несколько значений метаданных (или ни одного). Например, следующий элемент CSFile содержит метаданные Culture со значением Fr:
...
## <a name="whats-next"></a>Дальнейшие действия
 ...
 Сведения о пошаговом создании файла простого проекта см. в статье [Пошаговое руководство. Создание файла проекта MSBuild с нуля](../msbuild/walkthrough-creating-an-msbuild-project-file-from-scratch.md).
