...
Задача <xref:Microsoft.Build.Tasks.Windows.GenerateTemporaryTargetAssembly> создает сборку, если хотя бы одна страница [!INCLUDE[TLA#tla_xaml](../msbuild/includes/tlasharptla_xaml_md.md)] в проекте ссылается на любой тип, локально объявленный в этом проекте. Созданная сборка удаляется после успешного или неудачного завершения процесса сборки.  
