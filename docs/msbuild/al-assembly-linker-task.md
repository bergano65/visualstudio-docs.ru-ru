...
|`OutputAssembly`|Обязательный параметр вывода <xref:Microsoft.Build.Framework.ITaskItem>.<br /><br /> Указывает имя файла, создаваемого этой задачей. Этот параметр соответствует параметру `/out` в статье [Al.exe (компоновщик сборок)](http://msdn.microsoft.com/Library/b5382965-0053-47cf-b92f-862860275a01).|  
