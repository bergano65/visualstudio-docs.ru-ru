...
ms.locfileid: "47561714"
---
# <a name="adding-a-shortcut-menu-in-a-tool-window"></a>Добавление контекстного меню в окно инструментов
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Последнюю версию этого раздела можно найти в [Добавление контекстного меню в окне инструментов](https://docs.microsoft.com/visualstudio/extensibility/adding-a-shortcut-menu-in-a-tool-window).  
  
В этом пошаговом руководстве помещает контекстное меню в окне инструментов. Контекстное меню является меню, которое открывается при щелчке кнопки, текстовое поле или фона окна. Команды контекстного меню ведут себя так же, как команды на другие меню или панели инструментов. Чтобы обеспечить поддержку контекстное меню, она указывается в vsct-файле и отобразить ее в ответ на правой кнопки мыши.  
...
## <a name="creating-the-tool-window-shortcut-menu-package"></a>Создание пакета меню ярлык для окна инструментов  
  
1.  Создайте проект VSIX с именем `TWShortcutMenu` и добавьте шаблон окно инструментов с именем **контекстномменю** к нему. Дополнительные сведения о создании окна инструментов, см. в разделе [создания расширения с окном инструментов](../extensibility/creating-an-extension-with-a-tool-window.md).  
...
1.  В ShortcutMenuPackage.vsct найдите в названный guidShortcutMenuPackageCmdSet элемент GuidSymbol и объявить контекстное меню, контекстное меню группы и параметры меню. Элемент GuidSymbol теперь должен выглядеть следующим образом:  
...
5.  В ShortcutMenuPackageGuids.cs добавьте набор определений для команды GUID, в контекстном меню и пунктов меню.  
    ...
    ```  
     ...
     Это же идентификаторы команд, которые определены в разделе "символы" файла ShortcutMenuPackage.vsct. Контекст группы не включается здесь так как он необходим только в vsct-файле.  
  
## <a name="implementing-the-shortcut-menu"></a>Реализация в контекстном меню  
 В этом разделе реализует контекстное меню и команд.  
  
1.  В ShortcutMenu.cs окна инструментов можно получить службу команд меню, но элемент управления, содержащиеся в ней невозможно. Ниже показано, как сделать службу команд меню, доступными в пользовательский элемент управления.  
...
3.  Переопределите метод Initialize() окна инструментов, чтобы получить службу команд меню и добавьте элемент управления, передачи службу команд меню в конструктор:  
...
6.  Добавьте в ShortcutMenuControl.xaml, <xref:System.Windows.UIElement.MouseRightButtonDown> событий на верхний уровень <xref:System.Windows.Controls.UserControl> элемент. Файл XAML теперь должен выглядеть следующим образом:  
