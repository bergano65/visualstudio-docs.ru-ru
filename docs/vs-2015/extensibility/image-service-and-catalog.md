...
|**Attribute (XElement Dynamic Property)** (Attribute (динамическое свойство XElement))|**Определение**|  
|-|-|  
| URI | Необходимости URI, который определяет, откуда можно загрузить изображение. Может принимать одно из следующих значений:<br /><br /> — URI типа " [Pack](https://msdn.microsoft.com/library/aa970069\(v=vs.100\).aspx) " с помощью центра Application:///.<br />— Абсолютная ссылка на ресурс компонента.<br />— Путь к файлу, содержащему машинный ресурс; |
...
1. Замена всех \<Image> элементов в пользовательском интерфейсе \<CrispImage> элементами  
...
3. Вкладка окно инструментов, когда вкладки достаточно малы (также используются в окне переключения клавиш CTRL + TAB).  
   ...
   2. См. шаг #1 из раздела "как использовать моникеры изображений в новом окне инструментов". "Как реагировать на критические изменения".  

4. Команда для открытия окна инструментов.  

   - См. шаг #2 из раздела "как использовать моникеры изображений в новом окне инструментов". "Как реагировать на критические изменения".  
...
2. Если вы используете только **кновнмоникерс**, выполните следующие действия.  
   ...
   - Добавьте <импортировать манифест = "$ (Манифестфолдер) \\<относительный путь установки dir к \> \микрософт.висуалстудио.имажекаталог.имажеманифест"/ \> в начало \<Symbols> раздела.  
