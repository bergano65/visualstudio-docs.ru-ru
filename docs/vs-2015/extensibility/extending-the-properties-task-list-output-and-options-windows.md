---
title: Расширение свойств, список задач, вывода и параметров Windows | Документация Майкрософт
ms.date: 11/15/2016
...
3. Перетащите на холст **текстовое поле**, **кнопку**и элемент управления **ListBox** . Разместите элементы таким образом, чтобы текстовое поле и кнопка находящиеся на одном уровне, а элемент управления ListBox заполняет оставшуюся часть окна под ними, как показано на рисунке ниже.  
     ...
     ![Окно завершенных инструментов.](../extensibility/media/t5-toolwindow.png "T5 — окно инструментов")  
...
1. В файле TodoWindowControl.xaml.cs добавьте следующую инструкцию using:  
...
1. Добавьте класс с именем `ToolsOptions.cs`. Сделайте класс Тулсоптионс производным от <xref:Microsoft.VisualStudio.Shell.DialogPage> .  
...
3. На странице Параметры в этом пошаговом руководстве представлен только один параметр с именем Дайсахеад. Добавьте закрытое поле с именем **дайсахеад** и свойство с именем **Дайсахеад** в класс тулсоптионс:  
...
## <a name="make-data-available-to-the-properties-window"></a>Предоставление доступа к данным в окне "Свойства"  
 Чтобы получить список дел, можно создать класс с именем TodoItem, в котором хранятся сведения об отдельных элементах в списке ToDo.  
...
2. Добавьте следующий оператор using для файла TodoItem.cs.  
...
3. Добавьте `public` Модификатор доступа в объявление класса.  
    ...
    ```  
     ...
     Добавьте два свойства: Name и DueDate. Мы сделаем Упдателист () и Чеккфореррорс () позже.  
...
4. Добавьте закрытую ссылку на пользовательский элемент управления. Добавьте конструктор, который принимает пользовательский элемент управления и имя для этого элемента ToDo. Чтобы найти значение для Дайсахеад, оно получает свойство страницы Options.  
...
6. В TodoWindowControl.xaml.cs добавьте методы-заглушки в класс Тодовиндовконтрол для `CheckForError` методов и `UpdateList` . Вставьте их после ProcessDialogChar и до конца файла.  
...
## <a name="integrate-into-the-properties-window"></a>Интеграция в окно "Свойства"  
...
1. Замените существующую `button1_Click` функцию кодом, который создает новый TodoItem и добавляет его в список ListBox. Он вызывает Траккселектион (), который будет определен позже.  
...
3. Реализуйте метод Траккселектион (). Поскольку вам потребуется получить <xref:Microsoft.VisualStudio.Shell.Interop.SVsUIShell> <xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection> службы, вам потребуется сделать их <xref:Microsoft.VisualStudio.Shell.WindowPane.GetService%2A> доступными для тодовиндовконтрол. Добавьте следующий метод в класс Тодовиндов:  
...
4. Добавьте в TodoWindowControl.xaml.cs следующие операторы using:  
...
9. Откройте страницу « **Сервис/параметры** ». На левой панели отобразится категория ToDo. Категории перечислены в алфавитном порядке, поэтому их следует искать в службах терминалов.  
  
10. На странице Параметры TODO вы увидите, что свойство Дайсахеад имеет значение **0**. Измените его на **2**.  
...
## <a name="add-text-to-the-output-window-and-items-to-the-task-list"></a>Добавление текста в окно вывода и элементы в список задач  
 Для **список задач**создайте новый объект типа Task, а затем добавьте этот объект task в **список задач** , вызвав метод Add. Для записи в окно **вывода** вызывается его метод GetObject для получения объекта панели, а затем вызывается метод аутпутстринг объекта панели.  
...
2. Чтобы добавить элементы в список задач, необходимо добавить вложенный класс в класс Тодовиндовконтрол. Вложенный класс должен быть производным от <xref:Microsoft.VisualStudio.Shell.TaskProvider> . Добавьте следующий код в конец класса Тодовиндовконтрол.  
...
3. Затем добавьте в класс Тодовиндовконтрол закрытую ссылку на Тодотаскпровидер и метод Креатепровидер (). Код должен выглядеть следующим образом:  
...
4. Добавьте Клеареррор (), который очищает список задач и ReportError (), который добавляет запись в список задач классу Тодовиндовконтрол.  
...
5. Теперь реализуйте метод Чеккфореррорс, как показано ниже.  
...
## <a name="trying-it-out"></a>Попробуйте  
  
1. Выполните сборку решения и запустите отладку. Откроется экспериментальный экземпляр.  
  
2. Откройте Тодовиндов (**представление/другие окна или тодовиндов**).  
...
4. Теперь измените значение параметра на странице **Сервис/параметры/TODO** с **2** на **0**.  
