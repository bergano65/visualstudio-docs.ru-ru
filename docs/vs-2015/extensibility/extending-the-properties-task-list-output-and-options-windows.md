...
Последнюю версию этого раздела можно найти в [расширение свойств, список задач, выходные данные и параметры Windows](https://docs.microsoft.com/visualstudio/extensibility/extending-the-properties-task-list-output-and-options-windows).  
...
1.  В TodoWindow.xaml удалите элементы управления Button, TextBox и StackPanel от UserControl.  
...
1.  Добавьте в TodoWindowPackage.cs, <xref:Microsoft.VisualStudio.Shell.ProvideOptionPageAttribute> к классу TodoWindowPackage:  
...
## <a name="make-data-available-to-the-properties-window"></a>Сделать данные доступными в окне «Свойства»  
 Вы можете предоставить сведения о списке, чтобы сделать, создав класс с именем TodoItem, в котором содержатся сведения об отдельных элементов в списке ToDo.  
...
4.  Добавьте ссылку на закрытый в пользовательский элемент управления. Добавьте конструктор, принимающий пользовательский элемент управления и имя для данного элемента списка дел. Чтобы найти значение для daysAhead, он получает свойства параметров страницы.  
...
5.  Так как экземпляры `TodoItem` класса будут храниться в поле со списком и ListBox будет вызывать `ToString` функции, то необходимо перегрузить `ToString` функции. Добавьте следующий код к TodoItem.cs, после конструктора и до конца класса.  
...
1.  Замените существующий `button1_Click` функции с помощью кода, который создает нового объекта TodoItem и добавляет его в список ListBox. Он вызывает TrackSelection(), которые будут определены позже.  
...
2.  В режиме конструктора выберите элемент управления ListBox. В **свойства** окну, нажмите кнопку **обработчики событий** кнопку и найдите событие SelectionChanged. Заполните текстовое поле с **listBox_SelectionChanged**. Это добавляет заглушку для обработчика SelectionChanged и присваивает его к событию.  
  
3.  Реализуйте метод TrackSelection(). Так как вам потребуется получить <xref:Microsoft.VisualStudio.Shell.Interop.SVsUIShell> <xref:Microsoft.VisualStudio.Shell.Interop.STrackSelection> служб, вы должны сделать <xref:Microsoft.VisualStudio.Shell.WindowPane.GetService%2A> доступном TodoWindowControl. Добавьте следующий метод к классу TodoWindow:  
...
9. Откройте **Сервис / Параметры** страниц. Вы увидите категории ToDo в левой области. Категории перечислены в алфавитном порядке, поэтому перейдите в раздел служб терминалов.  
  
10. На странице параметров Todo, вы увидите DaysAhead набора свойств, к **0**. Измените его на **2**.  
  
11. В представлении / Other Windows откройте в меню **TodoWindow**. Тип **EndDate** в текстовое поле и нажмите кнопку **добавить**.  
...
## <a name="add-text-to-the-output-window-and-items-to-the-task-list"></a>Добавление текста в окне выходных данных и элементы в список задач  
 Для **список задач**, можно создать объект типа задач, а затем добавьте этот объект задачи для **список задач** путем вызова его метода Add. Записываемый **вывода** окна, вызовите его getpane-метод, чтобы получить объект панели и затем вызвать метод OutputString объекта области.  
...
2.  Чтобы добавить элементы в список задач, необходимо добавить вложенный класс в класс TodoWindowControl. Вложенный класс должно быть производным от <xref:Microsoft.VisualStudio.Shell.TaskProvider>. Добавьте следующий код в конец класса TodoWindowControl.  
...
3.  Далее добавьте закрытый ссылку TodoTaskProvider и CreateProvider() метод в класс TodoWindowControl. Код должен выглядеть следующим образом:  
...
## <a name="trying-it-out"></a>Пробном использовании  
  
1.  Выполните сборку решения и запустите отладку. Откроется экспериментальный экземпляр.  
  
2.  Откройте TodoWindow (**представления и других Windows / TodoWindow**).  
  
3.  Введите в текстовом поле и нажмите кнопку **добавить**.  
  
     Срок 2 дней, после добавления сегодня в поле со списком. Ошибки не создаются и **список задач** (**Просмотр / Task List**) нет записей, следует.  
  
4.  Теперь измените параметр на **Сервис / Параметры / ToDo** странице из **2** обратно **0**.  
