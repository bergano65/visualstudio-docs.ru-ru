...
1. Создайте проект VSIX с именем `TestToolWindowSearch` и окном инструментов с именем **тестсеарч**. Если вам необходима помощь, см. раздел [Creating an Extension with a Tool Window](../extensibility/creating-an-extension-with-a-tool-window.md).  
...
3. В файле TestSearchControl.xaml.cs добавьте следующую инструкцию using:  
...
6. В строке меню выберите **вид**, **другие окна**, **тестсеарч**.  
...
1. В файл TestSearch.cs добавьте следующие `using` инструкции:  
...
5. Проверьте новые параметры, перестройте решение и запустите дебугб.  
...
2. В `TestSearchTask` классе раскомментируйте матчкасе строку в `OnStartSearch` методе:  
