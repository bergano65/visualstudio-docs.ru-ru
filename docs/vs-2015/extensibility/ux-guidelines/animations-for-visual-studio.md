...
- **Время и скорость важны** для обеспечения быстрого и естественного перехода:
  ...
  - Анимации, которые часто возникают, должны быть достаточно быстрыми, чтобы не прерывать рабочий процесс пользователя.

  - Анимация не должна быть настолько быстрой или жарринг, что ее сложно понять, но не настолько медленна, что она делает один из них недостаточным для завершения перехода.
...
- **Используйте постепенную нелинейную замедление** от одного состояния к другому, давая смысл приходом и естественного перемещения
...
- Если вы сильно полагаетесь на анимацию в своих функциях, то можете **отключить их** локально (для всех функций) в качестве параметра в диалоговом окне " **Сервис > параметры** ".

- **Только одна анимация должна выполняться за раз** и передавать только один фрагмент информации.

- **Тонкость важна.** В большинстве случаев анимация не требует вмешательства пользователя для обслуживания своей цели. Незначительные изменения в времени, последовательности и поведении могут значительно повлиять на восприятие, а также могут сделать разницу между эффективной и неэффективной анимацией.

- При использовании анимации, чтобы привлечь внимание к чему-то, **Убедитесь, что стоит прерывать обучение пользователя**.

- **При отображении хода выполнения или состояния** через анимацию:

  - Перестает отображать перемещение хода выполнения, если базовый процесс не двигается.
...
#### <a name="do-not"></a>Чего не следует делать:

- Используйте небольшие перемещения (перемещение в небольшом объеме), предместив постепенное уменьшение и изменения при перемещении объектов.

- Используйте анимации, которые выполняются в большой области экрана. Независимо от размера этот стиль анимации отвлекается от пользователя.

- Используйте анимацию, не связанную с объектом, на который в настоящее время связан пользователь или который взаимодействует с.

- Используйте анимацию, требующую взаимодействия с пользователем, чтобы сбросить состояние, например заставить пользователя реагировать на мигание уведомления, чтобы сделать его немигающим. Для их закрытия достаточно взаимодействия с ними.
...
### <a name="animation-as-communicator"></a>Анимация как Communicator
 В пользовательском интерфейсе Visual Studio функции анимации действуют только в качестве средства связи.  Он используется для передачи разнообразных сведений, например структурных изменений в пользовательском интерфейсе. Например, при открытии или закрытии меню. Анимация может помочь визуализировать зависящее от времени поведение сложных систем, таких как визуализация хода выполнения установки или использование для привлечения внимания к оповещениям и уведомлениям.

 Анимация пользовательского интерфейса обычно работает четырьмя способами: Визуализация, привлечение внимания, моделирование и указание времени отклика и хода выполнения.
...
#### <a name="attract-attention"></a>Привлечь внимание
 Если цель состоит в том, чтобы привлечь внимание пользователя к одному элементу из нескольких или предупредить пользователя об изменении информации, может оказаться уместной анимация. Например, начальная страница приложений может использовать начало работы кнопку, которую слайды помещают после загрузки страницы.
...
- Аккуратно уведомляет пользователя о завершении или изменении процесса, например о завершении загрузки.
...
### <a name="overview"></a>Обзор
 Анимация в Visual Studio предназначена для обслуживания определенной функции и не мешает производительности пользователей. Общие характеристики анимации, которые должны быть соблюдены:
...
- Неявное, не урезан
  ...
  На следующем рисунке показаны стили анимации, Рекомендуемые для использования в Visual Studio. Анимация и небольшие анимации, такие как исчезновение/исчезновение, являются наиболее часто используемыми. Существует ограниченное применение таких эффектов анимации, как расширение и контракт, изменение позиций X и Y, а также вращение.
...
#### <a name="appear-and-disappear"></a>Появление и исчезновение
 ...
 ![Отображаются&#47;исчезнуть анимацию в Visual Studio](../../extensibility/ux-guidelines/media/1202-b-appearanddisappear.png "1202 — b_AppearAndDisappear")

##### <a name="correct-usage"></a>Правильное использование
 Новые элементы пользовательского интерфейса, которые должны мгновенно появляться или исчезать, чтобы пользователь не был ни отвлекается, ни неограниченным. Кроме того, медленный перенос анимации может восприниматься как перетаскивание, что не происходит при использовании стиля «появление» и «исчезновение».
...
- Пользовательский интерфейс редактора, активируемого с клавиатуры, например IntelliSense и Справка по параметрам
...
#### <a name="fade-in-and-fade-out"></a>Появление и исчезание
 ...
 ![Эффект затухания&#45;в&#47;исчезание&#45;out в Visual Studio](../../extensibility/ux-guidelines/media/1202-c-fadeinfadeout.png "1202 — c_FadeInFadeOut")

##### <a name="correct-usage"></a>Правильное использование
 Это наиболее часто Рекомендуемая Анимация пользовательского интерфейса. Это незаметное воздействие, которое добавляет интерес без прерывания потока. В некоторых случаях пользователь может даже не понимать, что существует анимация и просто воспринимает гладкую, потоковую систему пользовательского интерфейса.
...
#### <a name="color-blend-from-a-to-b"></a>Смешение цветов от A к B
 ...
 ![Анимация "цветовой переход" в Visual Studio](../../extensibility/ux-guidelines/media/1202-d-colorblend.png "1202 — d_ColorBlend")
...
#### <a name="expand-and-contract"></a>Развернуть и свернуть
 При использовании этого шаблона элемент пользовательского интерфейса расширяется по оси X, Y или обоим направлениям:

 ![Развертывание&#47;анимации контракта в Visual Studio](../../extensibility/ux-guidelines/media/1202-e-expandcontract.png "1202 — e_ExpandContract")
...
- Развернуть и свернуть элемент начальной страницы

#### <a name="x-y-position-change"></a>Изменение расположения X-Y
 При использовании этого шаблона элемент пользовательского интерфейса изменяет свое расположение X или Y или оба:

 ![Анимация изменения расположения X&#47;Y в Visual Studio](../../extensibility/ux-guidelines/media/1202-f-xypositionchange.png "1202 — f_XYPositionChange")
...
- Длительность: ноль секунд
  ...
  ![Анимация открытия вкладки в Visual Studio](../../extensibility/ux-guidelines/media/1202-h-tabopen.png "1202 — h_TabOpen")
...
- Длительность: 200 мс
  ...
  ![Анимация закрытия вкладки в Visual Studio](../../extensibility/ux-guidelines/media/1202-i-tabclose.png "1202 — i_TabClose")
...
- Длительность: 200 мс
  ...
  ![Анимация переупорядочивания вкладок в Visual Studio](../../extensibility/ux-guidelines/media/1202-j-tabreorder.png "1202 — j_TabReorder")
...
- Длительность: 200 мс
  ...
  ![Анимация закрытия плавающего документа в Visual Studio](../../extensibility/ux-guidelines/media/1202-k-closefloatingdocument.png "1202 — k_CloseFloatingDocument")
...
- Длительность: ноль секунд
  ...
  ![Автоматически&#45;скрыть анимацию окна инструментов в Visual Studio](../../extensibility/ux-guidelines/media/1202-o-autohidetoolwindowreveal.png "1202 — o_AutoHideToolWindowReveal")
