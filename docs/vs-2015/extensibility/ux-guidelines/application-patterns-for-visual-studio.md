...
### <a name="overview"></a>Обзор
 Двумя основными типами окон, используемыми в Visual Studio, являются редакторы документов и окна инструментов. Редкие, но возможные, большие безрежимные диалоги. Хотя все они невозможны в оболочке, их модели принципиально отличаются. Эта тема охватывает разницу между окнами документов, окнами инструментов и безрежимными диалогами. Шаблоны модальных диалогов описаны в [Диалогах.](../../extensibility/ux-guidelines/application-patterns-for-visual-studio.md#BKMK_Dialogs)
 ...
 **Окно инструмента** чаще всего отображается как отдельное, меньшее окно, которое может быть видно, скрыто или автоматически скрыто, рухнувшее на край IDE. Тем не менее, иногда они представлены в документе хорошо, путем unchecking **окно / Docking собственности** на окне. Это приводит к большему большему набору недвижимости, но также и к общему дизайнерским решениям: при попытке интеграции в Visual Studio необходимо решить, следует ли отображать окно инструмента или окно документа.

 В Visual Studio не поощряются **диалоги без беспрестанных.** В значительной степени они являются - по определению - плавающие окна инструмента и должны быть реализованы как таковые. Безмерные диалоги допускаются в тех случаях, когда размер нормального окна инструмента, пристыкованного к боковой стороне оболочки, будет слишком ограниченным. Они также допускаются в тех случаях, когда пользователь, скорее всего, переведет диалог на вторичный монитор.
...
- Окна инструментов для конкретных документов должны быть автоматически видны, когда это возможно, чтобы они отображались только при активации родительского документа.
...
- **Окна с автоматической скрытой** орудием не закреплены. Окно может выскользнуть из поля зрения, оставляя вкладку (с названием окна инструмента и его значок) на краю области документа. Окно инструмента выскальзывает, когда пользователь парит над вкладкой.
 ...
 **Окно инструмента, включившее команду «Новое окно» при активной**
...
|Тип|Окно инструмента|Компонент|
|----------|-----------------|--------------|
|**Hierarchy**|Обозреватель решений|Иерархическое дерево, отображаев список документов, содержащихся в проектах, различных файлах и элементах решения. Отображение элементов в проектах определяется пакетом, владеющим типом проекта (например, на основе справочных, каталогных или смешанных типов).|
|**Hierarchy**|Представление классов|Иерархическое дерево классов и различные элементы в рабочем наборе документов, независимо от самих файлов.|
|**Hierarchy**|Обозреватель серверов|Иерархическое дерево, отображаевое все серверы и соединения данных в решении.|
|**Hierarchy**|Структура документа|Иерархическая структура активного документа.|
|**Сетка**|Свойства|Сетка, отображая список свойств для выбранного объекта, а также сборщики значений для отправления этих свойств.|
|**Сетка**|Список задач|Сетка, которая позволяет пользователю создавать/отсеивать/удалять задачи и комментарии.|
|**Содержимое**|Справка|Окно, которое позволяет пользователям получить доступ к различным методам получения помощи, от "Как я?" видео на форумах MSDN.|
|**Содержимое**|Динамическая справка|Окно инструмента, отображающие ссылки для помощи тем, применимых к текущему выбору.|
|**Содержимое**|Обозреватель объектов|Двухкомпонентный кадр со списком компонентов иерархического объекта в левом стене и свойствами и методами объекта в правой колонке.|
|**Диалог**|Найти, Расширенный Найти|Диалог, который позволяет пользователю найти или найти и заменить в различных файлах в рамках решения.|
|**Прочее**|Область элементов|Окно инструмента, используемое для хранения элементов, которые будут сброшены на поверхность дизайна, обеспечивая последовательный источник сопротивления для всех дизайнеров.|
|**Прочее**|Начальная страница|Портал пользователя в Visual Studio, с доступом к каналам новостей разработчика, Visual Studio помощь, и последние проекты. Пользователи также могут создавать пользовательские стартовые страницы, копируя файл StartPage.xaml из каталога файлов программы Visual Studio "Common7-IDE"StartPages" в папку StartPages в каталоге документов Visual Studio, а затем либо редактировать XAML вручную, либо открытие его в Visual Studio или другой редактор кода.|
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Автоматические||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Немедленно||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Выходные данные|Выходное окно может использоваться всякий раз, когда у вас есть текстовые события или статус для декларировать.|
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Память||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Точки останова||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Запущен||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Документы||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Стек вызовов||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Локальные||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Часы||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Дизассемблированный код||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Регистры||
|**Debugger:** группа окон, специфизаю для отладки задач и деятельности по мониторингу|Потоки||
...
- Команды меню соответствующим образом интегрированы в общие меню, такие как **edit,** **Format**и **View..** Если имеется значительное количество специализированных команд, то может быть создано новое меню, которое видно только тогда, когда документ имеет фокус.
 ...
 **Создать сильный ключ имя является примером простого диалога в Visual Studio.**
...
#### <a name="layered-dialogs"></a><a name="BKMK_LayeredDialogs"></a>Многоуровневые диалоги
 ...
 Диалог **«Инструменты > опций»** является примером многослойного диалога с использованием встроенного дерева:
 ...
 **Инструменты > опции является примером многоуровневого диалога в Visual Studio.**
...
#### <a name="position"></a>Положение
 Диалоги должны отображаться по центру IDE при первом запуске. Для неизлечимых диалогов не требуется, чтобы последняя позиция диалога сохранялась, поэтому она будет отображаться в центре последующих запусков. Для многоизооточенных диалогов размер должен сохраняться при последующих запусках. Для изменяемых диалогов, которые являются модальными, позиция не должна быть упорной. Отображение их по центру IDE предотвращает возможность появления диалога в непредсказуемом или непригодном для пользователя положении при изменении конфигурации дисплея пользователя. Для безрежимных диалогов, которые могут быть перемещены, позиция пользователя должна быть сохранена при последующих запусках, так как диалог может часто использоваться как неотъемлемая часть большего рабочего процесса.

 Когда диалоги должны породить другие диалоги, верхний диалог должен каскадировать сявд вправо и вниз от родителя, чтобы пользователю было очевидно, что они перешли на новое место.
...
- Кнопки помощи в заглавной панели были унижаются. Не добавляйте их в новые диалоги. Когда они существуют, они должны запустить тему справки, которая концептуально имеет отношение к задаче.
  ...
  **Руководящие спецификации для заголовков баров в диалогах Visual Studio.**

#### <a name="control-buttons"></a>Кнопки управления
 Как правило, кнопки **OK**/**Cancel**/**Help** должны быть расположены горизонтально в правом нижнем углу диалога. Альтернативный вертикальный стек разрешен, если диалог имеет несколько других кнопок в нижней части диалога, которые представляют визуальную путаницу с кнопками управления.
 ...
 **Приемлемые конфигурации для кнопок управления в диалогах Visual Studio**
...
#### <a name="access-keys"></a>Ключи доступа
 Не используйте ключи доступа для кнопок **OK**/**Cancel**/**Help.** Эти кнопки отображаются для клавиш исключаемых по умолчанию:
...
#### <a name="imagery"></a>Изображения
 Используйте изображения экономно в диалогах. Не используйте большие значки в диалогах только для того, чтобы использовать пространство. Используйте изображения только в том случае, если они являются важной частью передачи сообщения пользователю, например, значки предупреждений или анимации состояния.
 ...
 **Концепция прихожей для разоблачения дополнительного uI в Outlook**

##### <a name="adaptive-ui"></a>Адаптивный пользовательский интерфейс
 Отображение или сокрытие пользовательского интерфейса на основе использования или самостоятельного опыта пользователя является еще одним способом представления необходимого пользовательского интерфейса при сокрытии других частей. Это не рекомендуется в Visual Studio, как алгоритмы для принятия решения, когда показать или скрыть UI может быть сложно, и правила всегда будет неправильно для некоторых набор случаев.
...
- Системы справочных и смешанных проектов поддерживают идею добавления ссылки (или ссылки) на исходный элемент. Когда эти проекты являются целью операции перетаскивания (когда **Ctrl и Shift** удерживается), это должно привести к ссылке на элемент, добавленный в проект
  ...
  Также вводит в заблуждение делать вид, что она позволяет копировать операции между этими типами проектов, поскольку целевой справочный проект не должен делать независимую копию исходного элемента. Аналогичным образом, не допускается перетаскивание ctrl и Shift в целевой проект на основе каталога, поскольку проект на основе каталога не может сохранять ссылки. В тех случаях, когда операция «перетаскивание и падение» не поддерживается, IDE должен запретить падение и показать пользователю курсор без капли (показано в таблице указателей ниже).

  Для правильной реализации поведения перетаскивания исходный проект перетаскивания должен сообщить о его характере (например, основан о его характере или каталоге?) целевому проекту. Эта информация указана в формате буфера обмена, который предлагается источником. В качестве источника операции перетаскивания (или копирования буфера обмена) проект должен предлагать либо **CF_VSREFPROJECTITEM**S, либо **CF_VSSTGPROJECTITEMS** соответственно, в зависимости от того, основан ли проект на основе справочных или каталогов. Оба этих формата имеют одинаковое содержание данных, которое похоже на формат **CF_HDROP** Windows, за исключением того, что списки строк, вместо того, чтобы быть именами файлов, являются двойным-**NULL** прекращаемого списка строк **Projref** (как вернулся из **IVsSolution::GetProjrefOfItem** или **::GetProjrefOfProject** по мере необходимости).

  В качестве цели падения (или операции буфера обмена) проект должен принимать как **CF_VSREFPROJECTITEMS,** так и **CF_VSSTGPROJECTITEMS,** хотя точная обработка операции перетаскивания варьируется в зависимости от характера целевого проекта и исходного проекта. Проект-источник декларирует свою природу, независимо от того, предлагает ли он **CF_VSREFPROJECTITEMS** или **CF_VSSTGPROJECTITEMS.** Цель капли понимает свою собственную природу и, таким образом, имеет достаточно информации для принятия решений о том, следует ли выполнять перемещение, копирование или ссылку. Пользователь также изменяет, какие операции перетаскивания должны быть выполнены, нажав клавиши Ctrl, Shift или как Ctrl, так и Shift. Важно, чтобы цель падения правильно указала, какая операция будет выполнена заранее в методах **DragEnter** и **DragOver.** Solution **Explorer** автоматически знает, являются ли исходный проект и целевой проект одинаковым.
...
|-------------------|-------------|-----------------|
|![Значок мыши "Не бросать"](../../extensibility/ux-guidelines/media/0706-01-mousenodrop.png "0706-01_MouseNoDrop")|Нет капли|Элемент не может быть удален в указанное место.|
|![Значок мыши "Копировать"](../../extensibility/ux-guidelines/media/0706-02-mousecopy.png "0706-02_MouseCopy")|Копировать|Элемент будет скопирован в целевое место.|
|![Значок мыши "Переместить"](../../extensibility/ux-guidelines/media/0706-03-mousemove.png "0706-03_MouseMove")|Переместить|Элемент будет перемещен в целевое место.|
|![Значок мыши "Добавить ссылку" ](../../extensibility/ux-guidelines/media/0706-04-mouseaddref.png "0706-04_MouseAddRef")|Ссылка, которую нужно добавить|Ссылка на выбранный элемент будет добавлена к целевому местоположению.|
...
|||Источник: Ссылка / Ссылка|Элемент источника: Физический элемент или файловая система (CF_HDROP)|
|-|-|----------------------------------|-------------------------------------------------------------|
...
|Нет модификатора|Результат|**DROPEFFECT_MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_LINK** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|
...
|Сдвиг-Драг|Результат|**DROPEFFECT_MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|Нет капли|
...
|Ctrl-Drag|Результат|**DROPEFFECT_COPY** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|Нет капли|
...
|Ctrl-Shift-Drag|Результат|**DROPEFFECT_LINK** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_LINK** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|
...
|Нет модификатора|Результат|**DROPEFFECT_ MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_ MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|
...
|Сдвиг-Драг|Результат|**DROPEFFECT_ MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_ MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|
...
|Ctrl-Drag|Результат|**DROPEFFECT_ COPY** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_ COPY** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|
...
|Нет модификатора|Источник|Удаляет ссылку на исходный элемент|Удаляет ссылку на исходный элемент|
|Нет модификатора|Результат|**DROPEFFECT_ MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_ MOVE** возвращается как действие из **::Drop** и элемент удаляется из исходного местоположения в хранилище|
|Сдвиг-Драг|Действие|Переместить|Переместить|
|Сдвиг-Драг|Назначение|Добавлена ссылка на исходный элемент|Копирует элемент в целевое местоположение|
|Сдвиг-Драг|Источник|Удаляет ссылку на исходный элемент|Удаляет элемент из исходного местоположения|
|Сдвиг-Драг|Результат|**DROPEFFECT_ MOVE** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_ MOVE** возвращается как действие из **::Drop** и элемент удаляется из исходного местоположения в хранилище|
|Ctrl-Drag|Действие|Копировать|Копировать|
...
|Ctrl-Shift-Drag|Источник|Сохраняет ссылку на исходный элемент|Сохраняет исходный элемент|
|Ctrl-Shift-Drag|Результат|**DROPEFFECT_ LINK** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|**DROPEFFECT_ LINK** возвращается как действие от **::Drop** и элемент остается в исходном месте в хранилище|
|Вырезать /вставить|Действие|Переместить|Переместить|
|Вырезать /вставить|Назначение|Копирует элемент в целевое местоположение|Копирует элемент в целевое местоположение|
|Вырезать /вставить|Источник|Удаляет ссылку на исходный элемент|Удаляет элемент из исходного местоположения|
|Вырезать /вставить|Результат|Элемент остается в исходном месте в хранилище|Элемент удаляется из исходного местоположения в хранилище|
...
2. Если у открытого редактора/дизайнера есть несохраненные изменения, то источник перетаскивания должен ждать, пока произойдет падение, а затем попросить пользователя сохранить незафиксированные изменения в открытых документах перед закрытием окна с подсказкой, аналогичной следующему:
   ...
   Это дает пользователю возможность сохранить незавершенную работу до того, как цель сделает свои копии. Для включения этой обработки добавлен новый метод **IVsHierarchyDropDataSource2::OnBeforeDropNotify.**

   Цель будет копировать состояние элемента, как он находится в хранилище (не включая несохраненные изменения в редакторе, если пользователь выбрал **Нет).** После того, как цель завершила копирование (в **IVsHierarchyDropDataSource::Drop),** источникполучает возможность завершить удаленную часть операции перемещения (в **IVsHierarchyDropDataSource::OnDropNotify**).
