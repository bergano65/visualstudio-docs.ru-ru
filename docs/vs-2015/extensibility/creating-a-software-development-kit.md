...
- Пакеты SDK для платформы являются обязательными компонентами для разработки приложений для платформы. Например, [!INCLUDE[win81](../includes/win81-md.md)] пакет SDK необходим для разработки [!INCLUDE[win8_appname_long](../includes/win8-appname-long-md.md)] приложений.  
...
- [Пакеты SDK для платформ](#PlatformSDKs)  
  
- [Пакеты SDK расширения](#ExtensionSDKs)  
  
## <a name="platform-sdks"></a><a name="PlatformSDKs"></a> Пакеты SDK для платформы  
 Для разработки приложений для платформы требуются пакеты SDK для платформы. Например, [!INCLUDE[win81](../includes/win81-md.md)] пакет SDK необходим для разработки приложений для [!INCLUDE[win81](../includes/win81-md.md)] .  
  
### <a name="installation"></a>Установка  
 Все пакеты SDK для платформы будут установлены в пакетах SDK для HKLM\Software\Microsoft\Microsoft \\ [ТПИ] \v [ТПВ] \\ @InstallationFolder = [корень пакета SDK]. Соответственно, [!INCLUDE[win81](../includes/win81-md.md)] пакет SDK устанавливается на сайте HKLM\Software\Microsoft\Microsoft SDKs\Windows\v8.1.  
  
### <a name="layout"></a>Layout  
 Пакеты SDK для платформы будут иметь следующий вид:  
...
3. Папка Redist: файлы, необходимые для выполнения и отладки, которые должны быть упакованы в состав приложения пользователя. Все двоичные файлы должны размещаться под \Redist \\<config \> \\<Arch \> , а имена двоичных файлов должны иметь следующий формат для обеспечения уникальности: ** \<company> . \<product> . \<purpose> . \<extension> ** Например, Microsoft.Cpp.Build.dll. Все файлы с именами, которые могут конфликтовать с именами других пакетов SDK (например, файлы JavaScript, CSS, PRI, XAML, PNG и JPG), должны размещаться под \Redist \\<config \> \\<Arch \> \\<сдкнаме \> \, за исключением файлов, связанных с элементами управления XAML. Эти файлы должны размещаться под \Redist \\<config \> \\<Arch \> \\<ComponentName \> \\ .  
...
### <a name="sdkmanifestxml"></a>SDKManifest.xml  
 В этом файле описывается, как Visual Studio должен использовать пакет SDK. Пример приведен ниже.  
...
## <a name="specifying-the-location-of-toolbox-items"></a><a name="ToolboxItems"></a> Указание расположения элементов панели элементов  
