...
2. Откройте файл, содержащий определение VSPackage. (В проекте C#, он имеет  <em>\<имя_проекта ></em>Package.cs.)  
