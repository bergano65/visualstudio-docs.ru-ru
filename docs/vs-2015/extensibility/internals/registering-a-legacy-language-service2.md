...
|*\<ИДЕНТИФИКАТОР GUID >*|REG_SZ|""|Ключ для логического представления, поддерживаемые. Может иметь как многие из них, как требуется. Имя записи реестра — это том, что важно не значение, которое всегда является пустой строкой.|  
