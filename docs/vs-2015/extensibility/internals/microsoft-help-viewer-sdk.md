...
11. Запустите редактор реестра, выбрав **запуска** в **запустить** меню и введя `Regedit`. В редакторе реестра выберите **файл**, а затем **импорта**. Перейдите в папку проекта ContosoHelpShell. В ContosoHelpShell вложенную папку выберите ContosoHelpShell.reg.  
