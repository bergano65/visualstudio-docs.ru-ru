...
Использование классов языка Managed Package Framework (MPF) для реализации языковой службы в [!INCLUDE[csprcs](../../includes/csprcs-md.md)] — это просто. Для размещения языковой службы, самой языковой службы и средства синтаксического анализа для вашего языка требуется пакет VSPackage.  
...
3. Выберите в [!INCLUDE[csprcs](../../includes/csprcs-md.md)] качестве языка и параметр для создания нового файла ключа. Нажмите кнопку **Далее**.  
...
6. В файл MyLanguageService.cs добавьте следующие `using` инструкции.  
...
3. Добавьте в новый файл класса следующие `using` инструкции.  
