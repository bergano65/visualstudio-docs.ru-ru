...
ms.locfileid: "68196852"
---
# <a name="designing-xml-command-table-vsct-files"></a>Разработка файлов таблицы команд XML (VSCT-файлы)
[!INCLUDE[vs2017banner](../../includes/vs2017banner.md)]

Файл XML-таблицы команд (. vsct) описывает макет и внешний вид элементов команд для VSPackage. К элементам команды относятся кнопки, поля со списком, меню, панели инструментов и группы элементов команд. В этом разделе описываются XML-файлы командных таблиц, их влияние на элементы команд и меню, а также способы их создания.  
  
## <a name="commands-menus-groups-and-the-vsct-file"></a>Команды, меню, группы и файл. vsct  
 файлы. vsct организованы вокруг команд, меню и групп команд. XML-теги в файле. vsct представляют каждый из этих элементов вместе с другими связанными элементами, такими как кнопки команд, размещение команд и точечные рисунки.  
  
 Когда вы создаете новый пакет VSPackage, запустив [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] шаблон пакета, шаблон создает vsct-файл с необходимыми элементами для команды меню, окна инструментов или пользовательского редактора в зависимости от выбранных параметров. Этот vsct-файл можно изменить в соответствии с требованиями конкретного пакета VSPackage. Примеры изменения vsct-файла см. в примерах [расширения меню и команд](../../extensibility/extending-menus-and-commands.md).  
...
## <a name="differences-between-ctc-and-vsct-files"></a>Различия между файлами. CTC и. vsct  
 Хотя значение, заключенное в теги XML в файле. vsct, так же, как и в устаревшем формате файла. CTC, их реализация немного отличается.  
  
- Новый **\<extern>** тег — это место, где вы ссылаетесь на другие h-файлы для компиляции, например для [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] панели инструментов.  
  
- Хотя vsct-файлы поддерживают инструкцию **/include** , как и файлы CTC, она также содержит новый \<**import> элемент * *. Разница заключается в том, что **/include** переносит **всю** информацию, но \<**import> * * возвращает только имена.  
...
- Флаги команд можно хранить по отдельности или с пометкой.  Однако технология IntelliSense не работает для флагов команд с накоплением. Дополнительные сведения о флагах команд см. в разделе [элемент флага команды](../../extensibility/command-flag-element.md).  
...
- Родительский элемент является необязательным. Если этот параметр опущен, используется значение "Group Unknown".  
  
- Аргумент Icon является необязательным.  
  
- Раздел Bitmap — то же, что и CTC-файл, за исключением того, что теперь можно указать имя файла через href, которое будет запрошено компилятором vsct.exe во время компиляции.  
...
- HRef — новый метод, который позволяет указать имя файла для ресурса точечного рисунка. Предполагается, что используются все, поэтому можно опустить использованный раздел. Компилятор сначала ищет локальные ресурсы для файла, а затем на всех общих ресурсах и ресурсах, определенных параметром/I.  
...
- Кэйчорд--был удален. Новый формат: Key1, Mod1, Key2, Mod2.  Можно указать символьную, шестнадцатеричную или VK константу.  
  
  Новый компилятор, vsct.exe, компилирует файлы. CTC и. vsct. Однако старый компилятор ctc.exe не будет распознавать и компилировать файлы vsct.  
  
  Можно использовать компилятор vsct.exe для преобразования существующего ctoового файла в vsct файл. Дополнительные сведения об этом см [. в разделе инструкции. Создание. Vsct файл из существующего. Файл CTO](../../misc/how-to-create-a-dot-vsct-file-from-an-existing-dot-cto-file.md).  
...
## <a name="the-vsct-file-elements"></a>Элементы файла. vsct  
 ...
 [Элемент Bitmaps](../../extensibility/bitmaps-element.md) — представляет все растровые изображения для всех кнопок в VSPackage. Точечные рисунки — это изображения, отображаемые рядом с кнопками или на их кнопке в зависимости от контекста.  
 ...
 [Элемент висибилитиконстраинтс](../../extensibility/visibilityconstraints-element.md) — указывает, отображается ли команда в любое время или только в определенных контекстах, например при отображении определенного диалогового окна или окна. Меню и команды со значением для этого элемента будут отображаться только в том случае, если указан активный контекст. По умолчанию команда отображается всегда.  
  
 [Элемент сочетания клавиш](../../extensibility/keybindings-element.md) — задает все сочетания клавиш для команд. Это значит, что одно или несколько сочетаний клавиш, которые должны быть нажаты для выполнения команды, например **CTRL + S**.  
  
 [Элемент уседкоммандс](../../extensibility/usedcommands-element.md) — информирует [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] среду о том, что хотя указанная команда реализована другим кодом, когда текущий пакет VSPackage активен, он предоставляет реализацию команды.  
  
 `Symbols Element` — Содержит имена символов и идентификаторы GUID для всех команд в пакете.  
...
## <a name="vsct-file-notes"></a>. Заметки о файле vsct  
 Если вы вносите изменения в vsct-файл после компиляции и помещаете его во встроенную вспомогательную библиотеку DLL, следует запустить **devenv.exe/Setup/носетупвстемплатес**. Это приведет к принудительному повторному считыванию ресурсов VSPackage, указанных в экспериментальном реестре, и внутренней базы данных, которая описывает [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] Перестроение.  
 ...
 **% PROGRAMFILES (x86)% \ SDK\VisualStudioIntegration\Tools\Bin\CreateExpInstance.exeVisual Studio \<version>**  
 ...
 Запустите средство с помощью командной строки **CreateExpInstance/Reset**. Помните, что это средство удаляет из экспериментального Hive все зарегистрированные пакеты VSPackage, которые обычно не устанавливаются вместе с [!INCLUDE[vsprvs](../../includes/vsprvs-md.md)] .  
