...
Последнюю версию этого раздела можно найти в [средства синтаксического анализа службы языка для прежних версий и сканер](https://docs.microsoft.com/visualstudio/extensibility/internals/legacy-language-service-parser-and-scanner).  
...
## <a name="parsing-for-matching-braces"></a>Синтаксический анализ для соответствующих фигурных скобок  
 ...
 Несмотря на то, что в фигурные скобки используются в имен триггеров и синтаксический анализ причин, этот процесс не ограничивается фактическое фигурные скобки. Любая пара символов, которые определен соответствующий связываться поддерживается. Примеры (и), \< и >, и [и].  
