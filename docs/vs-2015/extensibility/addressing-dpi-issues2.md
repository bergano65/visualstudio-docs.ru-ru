...
- Диалоговые окна Win32/WinForms и Windows имеют средства для включения макета, который изменяет размер текста, например, через панели макета сетки, потока и таблицы. Они позволяют избежать жестко запрограммированных расположений пикселей, которые не масштабируются при увеличении размеров шрифтов.  
...
```  
 ...
 Масштабирование ImageList зависит от того, заполнено ли ImageList во время загрузки или добавляется во время выполнения. В случае завершения во время загрузки вызовите Логикалтодевицеунитс () с ImageList так же, как точечный рисунок. Когда коду необходимо загрузить отдельное растровое изображение, прежде чем создавать ImageList, необходимо масштабировать размер изображения ImageList:  
...
```  
 ...
 В следующей таблице приведены примеры масштабирования изображений по соответствующим коэффициентам масштабирования DPI. Изображения в зеленом виде обозначают нашу рекомендацию по Visual Studio 2013 (100%-200% DPI):  
...
## <a name="using-the-dpihelper-libraryclass-to-scale-images-and-layout"></a>Использование библиотеки или класса Дпихелпер для масштабирования изображений и макета  
 ...
 Чтобы использовать библиотеку, перейдите к [примерам расширяемости Visual Studio VSSDK](https://github.com/Microsoft/VSSDK-Extensibility-Samples) и выполните клонирование образца с высоким DPI_Images_Icons  
  
 В исходные файлы включите Всуидпихелпер. h и вызовите статические функции класса VsUI::D Пихелпер:  
...
- • Для крупных уровней масштабирования, не кратных 100% (например, 250% или 350%), масштабирование иконографи изображений с бикубический приводит к нечеткому пользовательскому интерфейсу. Лучший результат достигается путем первого масштабирования изображения с Неарестнеигхбор до самого большого числа, кратного 100% (например, 200% или 300%). и масштабирование с бикубический. Дополнительные сведения см. в разделе особый случай: предмасштабирование изображений WPF для крупных точек на дюйм.  
...
## <a name="enabling-hdpi-support-to-the-weboc"></a>Включение поддержки HDPI для Вебок  
 ...
 Реализуйте интерфейс Идочостуихандлер (см. статью MSDN в интерфейсе [идочостуихандлер](https://msdn.microsoft.com/library/aa753260.aspx) ):  
...
```  
 ...
 При необходимости реализуйте интерфейс Икустомдок (см. статью MSDN в интерфейсе [икустомдок](https://msdn.microsoft.com/library/aa753272.aspx) ):  
...
```  
 ...
 Если вы не реализовали интерфейс Икустомдок, то как только свойство документа Вебок является допустимым, необходимо привести его к Иолеобжект и вызвать метод Сетклиентсите, передав класс, реализующий Идочостуихандлер. Установите флаг DOCHOSTUIFLAG_DPI_AWARE для ДОЧОСТУИИНФО, передаваемого в вызов метода Жесостинфо:  
