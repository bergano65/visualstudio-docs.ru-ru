...
- уменьшать объем информации, не предполагающей действий, в данных кучи JavaScript.  
  ...
  Руководство по процедуре поиска утечек памяти в работающем приложении см. в статье [Пошаговое руководство. Поиск утечек памяти (JavaScript)](../profiling/walkthrough-find-a-memory-leak-javascript.md).  
  
  В этом разделе:  
  
  [Запуск анализатора памяти JavaScript](#Run)   
  [Проверка использования памяти](#Check)   
  [Изоляция утечки памяти](#Isolate)   
  [Просмотр сводки по использованию памяти в реальном времени](#LiveMemory)   
  [Просмотр сводки по снимкам](#SnapshotSummary)   
  [Просмотр сведений о снимке](#SnapshotDetails)   
  [Просмотр различий между снимками](#SnapshotDiff)   
  [Просмотр объектов по лидеру](#FoldObjects)   
  [Фильтрация данных по идентификатору](#Filter)   
  [Поиск объекта в дереве объектов](#ShowInRootsView)   
  [Просмотр ссылок на общие объекты](#References)   
  [Отображение встроенных объектов](#BuiltInValues)   
  [Сохранение файлов диагностических сеансов](#Save)   
  [Связывание исходного кода с данными об использовании памяти](#JSConsoleCommands)   
  [Советы по выявлению проблем памяти](#Tips)  
  
## <a name="run-the-javascript-memory-analyzer"></a><a name="Run"></a> Запуск анализатора памяти JavaScript  
...
## <a name="check-memory-usage"></a><a name="Check"></a> Проверка использования памяти  
...
12. Чтобы выявить проблемы с использованием памяти, выберите один из этих параметров фильтра **Область** :  
    ...
    > Отфильтрованное представление объектов, оставшихся после предыдущего снимка, можно использовать для изучения утечек памяти. Например, если разностное число объектов равно +205 / -195, в этом представлении будут показаны 10 оставшихся объектов, из-за которых, вероятно, и происходят утечки памяти.  
     ...
     ![Представление различий между снимками, отображающее типы](../profiling/media/js-mem-snapshot-diff.png "JS_Mem_Snapshot_Diff")  
...
## <a name="view-live-memory-usage-summary"></a><a name="LiveMemory"></a> Просмотр сводки по использованию памяти в реальном времени  
 ...
 ![График распределения памяти из анализатора памяти JavaScript](../profiling/media/js-mem-memory-graph.png "JS_Mem_Memory_Graph")  
...
## <a name="view-a-snapshot-summary"></a><a name="SnapshotSummary"></a> Просмотр сводки по снимкам  
...
## <a name="view-snapshot-details"></a><a name="SnapshotDetails"></a> Просмотр сведений о снимке  
...
## <a name="view-a-snapshot-diff"></a><a name="SnapshotDiff"></a> Просмотр различий между снимками  
...
- **Все объекты, оставшиеся после снимка №\<number>** . Настройки этого фильтра не отсеивают каких-либо объектов кучи.  
  ...
  Чтобы вывести на экран ссылки на объекты, которые не соответствуют текущему фильтру **Область**, выберите **Показать несовпадающие ссылки** в ![раскрывающемся списке параметров в анализаторе памяти](../profiling/media/js-mem-settings.png "JS_Mem_Settings") в правом верхнем углу области. При включении этого параметра несоответствующие ссылки отображаются серым текстом  
...
## <a name="view-objects-by-dominator"></a><a name="FoldObjects"></a> Просмотр объектов по лидеру  
...
## <a name="filter-data-by-identifier"></a><a name="Filter"></a> Фильтрация данных по идентификатору  
...
## <a name="find-an-object-in-the-object-tree"></a><a name="ShowInRootsView"></a> Поиск объекта в дереве объектов  
...
## <a name="view-shared-object-references"></a><a name="References"></a> Просмотр ссылок на общие объекты  
...
## <a name="show-built-in-objects"></a><a name="BuiltInValues"></a> Отображение встроенных объектов  
 ...
 Для вывода на экран этих объектов выберите **Показать встроенные объекты** в ![раскрывающемся списке параметров в анализаторе памяти](../profiling/media/js-mem-settings.png "JS_Mem_Settings") в правом верхнем углу области.  
  
## <a name="save-diagnostic-session-files"></a><a name="Save"></a> Сохранение файлов диагностических сеансов  
...
## <a name="associate-source-code-with-memory-usage-data"></a><a name="JSConsoleCommands"></a> Связывание исходного кода с данными об использовании памяти  
...
## <a name="tips-for-identifying-memory-issues"></a><a name="Tips"></a> Советы по выявлению проблем с памятью  
  
- Следуйте рабочему процессу, описанному в разделе [Локализация утечки памяти](#Isolate), и используйте фильтр **Объекты, оставшиеся после снимка №\<number>** в разностном представлении, чтобы выявить вероятных кандидатов, вызывающих утечки памяти.  
