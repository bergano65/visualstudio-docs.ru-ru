...
8. Тип **abc**.  
     ...
     **Abc** текст подчеркивается волнистой линией. Всплывающая подсказка отображает сообщение: Элемент «ID» имеет недопустимое значение для этого типа данных.  
...
10. Вернитесь к XML-файл и замените **abc** текст с **123**.  
     ...
     Волнистое подчеркивание и подсказка, относящиеся к элементу ID, удаляются. Подсказка для закрывающего тега элемента «employee» будет содержать сообщение: Содержимое элемента «сотрудник» неполно. «Предполагается наличие элемента "hire-date"».  
