...
- Создание [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)] , предоставляющее данные в EDM приложению WPF.
...
- Создание кнопки, сохраняющей изменения данных в элементах управления, в [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)] и в базовом источнике данных.
...
- Привязка данных WPF. Дополнительные сведения см. в разделе [Общие сведения о привязке данных](https://msdn.microsoft.com/library/c707c95f-7811-401d-956e-2fffd019a211).
...
5. В поле **Имя** введите `AdventureWorksService`, а затем нажмите кнопку **ОК**.
     ...
     Visual Studio создаст `AdventureWorksService` проект.
...
## <a name="create-an-entity-data-model-for-the-service"></a>Создание EDM для службы
 Чтобы предоставить данные приложению с помощью [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)], следует определить модель данных для этой службы. [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)]Поддерживает два типа моделей данных: модели EDM и пользовательские модели данных, которые определяются с помощью объектов среды CLR, реализующих <xref:System.Linq.IQueryable%601> интерфейс. В данном пошаговом руководстве вы создадите модель EDM для использования в качестве модели данных.
...
## <a name="create-the-service"></a>Создание службы
 Создайте, [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)] чтобы предоставить данные в EDM приложению WPF.
...
1. В `AdventureWorks.svc` файле кода замените `AdventureWorksService` объявление класса следующим кодом.
     ...
     Этот код обновляет `AdventureWorksService` класс, чтобы он наследовался от <xref:System.Data.Services.DataService%601> класса, который работает с `AdventureWorksLTEntities` классом контекста объекта в EDM. Он также обновляет метод `InitializeService`, чтобы предоставить клиентам службы полный доступ на чтение/запись к сущности `SalesOrderHeader`.
...
9. В поле **пространство имен** введите `AdventureWorksService` .

10. В поле **Службы** щелкните **AdventureWorksService.svc** и нажмите кнопку **ОК**.

     Visual Studio загрузит сведения о службе, а затем вернет его в мастер **настройки источника данных** .
...
4. Из окна **Источники данных** перетащите узел **SalesOrderHeaders** на строку сетки под строкой с кнопками.
    ...
    Visual Studio формирует XAML и код, который создает набор элементов управления, привязанных к данным в таблице **Продукт**. Дополнительные сведения о созданных XAML и коде см. в разделе [Привязка элементов управления WPF к данным в Visual Studio](../data-tools/bind-wpf-controls-to-data-in-visual-studio1.md).
...
## <a name="navigatesales-records"></a>Записи навигатесалес
...
## <a name="saving-changes-to-sales-records"></a>Сохранение изменений в записях о продажах
 Добавьте код, позволяющий пользователям просматривать и сохранять изменения в записях продаж с помощью кнопки **сохранить изменения** .
