...
2. В области имя введите `PhoneNumberBox` и нажмите кнопку **Добавить**.
...
- Перетащите главный узел **Customers** из окна **Источники данных** на форму и убедитесь, что элемент управления `PhoneNumberBox` используется для вывода данных в столбец `Phone`.
