---
title: Как добавить проверку в классы сущностей | Документация Майкрософт
ms.date: 11/15/2016
...
> [!NOTE]
> В этом разделе описаны основные этапы добавления проверки в классы сущностей с помощью [!INCLUDE[vs_ordesigner_short](../includes/vs-ordesigner-short-md.md)]. Поскольку выполнение этих универсальных действий может оказаться затруднительным, не ссылаясь на конкретный класс сущностей, пошаговое руководство, в котором используются фактические данные, предоставлено.
...
2. В конструкторе O/R щелкните правой кнопкой мыши класс, для которого требуется добавить проверку, и выберите пункт **Просмотреть код**.
...
4. Для проектов Visual Basic:
   ...
   2. Нахождение в**изменяемом** методе **в**_ColumnName_для столбца, в который необходимо добавить проверку.
...
2. Щелкните правой кнопкой мыши пустую область в конструкторе O/R и выберите пункт **Просмотреть код**.
...
4. Для проектов Visual Basic:
   ...
   2. Щелкните **Обновить**_ентитикласснаме_.

   3. @No__t_0 метод*ентитикласснаме* добавляется в разделяемый класс.
   ...
   5. Так C# как проекты не создают автоматически обработчики событий, можно использовать IntelliSense для создания разделяемого метода `Update`*CLASSNAME* .

   6. Введите `partial` и потом пространство для доступа к списку доступных разделяемых методов. Щелкните метод обновления для класса, для которого вы хотите добавить проверку. Следующий код напоминает код, который создается при выборе разделяемого метода `Update`*CLASSNAME* :
...
## <a name="see-also"></a>См. также раздел
 [Средства LINQ to SQL в Visual Studio](../data-tools/linq-to-sql-tools-in-visual-studio2.md) [LINQ to SQL](https://msdn.microsoft.com/library/73d13345-eece-471a-af40-4cc7a2f11655) [Проверка данных](https://msdn.microsoft.com/library/b3a9ee4e-5d4d-4411-9c56-c811f2b4ee7e)
