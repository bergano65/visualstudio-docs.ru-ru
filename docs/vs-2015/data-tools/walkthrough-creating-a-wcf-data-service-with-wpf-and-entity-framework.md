...
В этом пошаговом руководстве показано, как создать простое [!INCLUDE[ss_data_service](../includes/ss-data-service-md.md)] , размещенное в [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] веб-приложении, и получить к нему доступ из Windows Forms приложения.
...
3. Откройте контекстное меню для <xref:System.Windows.Forms.Button> элемента управления и выберите **Просмотреть код**, а затем добавьте следующий код в `Button1_Click` обработчик событий:
