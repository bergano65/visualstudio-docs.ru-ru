...
2. Создайте новый набор данных на основе таблицы Northwind `Customers` .

3. Создайте форму с <xref:System.Windows.Forms.DataGridView> отображением данных.

4. Заполнение набора данных данными из `Customers` таблицы в базе данных Northwind.

5. Используйте [визуальные инструменты для баз данных](https://msdn.microsoft.com/6b145922-2f00-47db-befc-bf351b4809a1) в Visual Studio для прямого доступа к `Customers` таблице данных и изменения записи.
...
> [!NOTE]
> Отображаемые диалоговые окна и команды меню могут отличаться от описанных в справке в зависимости от текущих параметров или используемого выпуска. Чтобы изменить параметры, выберите в меню **Сервис** пункт **Импорт и экспорт параметров** . Дополнительные сведения см. [в разделе Настройка параметров разработки в Visual Studio](https://msdn.microsoft.com/22c4debb-4e31-47a8-8f19-16f328d7dcd3).
...
3. На панели **Шаблоны** выберите пункт **Приложение Windows**.
...
1. В меню **данные** выберите команду **Добавить новый источник данных**.
     ...
     Откроется [Мастер настройки источника данных](https://msdn.microsoft.com/library/c4df7de5-5da0-4064-940c-761dd6d9e28f) .
...
5. Разверните узел **таблицы** и выберите `Customers` таблицу. Имя по умолчанию для набора данных должно быть `NorthwindDataSet` .
...
4. Перетащите таблицу на пустую область формы.
     ...
     <xref:System.Windows.Forms.DataGridView>Элемент управления с именем `CustomersDataGridView` и <xref:System.Windows.Forms.BindingNavigator> именем `CustomersBindingNavigator` добавляется в форму, привязанную к <xref:System.Windows.Forms.BindingSource> . Это в, в свою очередь, привязана к `Customers` таблице в `NorthwindDataSet` .
...
1. Нажмите **клавишу F5** , чтобы запустить приложение.
     ...
     Форма отображается с <xref:System.Windows.Forms.DataGridView> элементом управления, заполненным данными из `Customers` таблицы.
...
### <a name="addcode-to-handle-the-concurrency-exception"></a>Аддкоде для обработки исключения параллелизма
 ...
 В этом разделе вы добавите код, который пытается обновить базу данных. Вы также обрабатываете все <xref:System.Data.DBConcurrencyException> , что может быть вызвано, а также любые другие исключения.

> [!NOTE]
> `CreateMessage`Методы и `ProcessDialogResults` будут добавлены далее в этом пошаговом руководстве.
...
- Создайте сообщение, добавив следующий код в **Редактор кода**. Введите этот код под `UpdateDatabase` методом.
...
7. Переключитесь в `ConcurrencyWalkthrough` выполняемую форму.
