...
При создании приложения, которое работает с данными в базе данных, необходимо выполнить такие основные задачи, как определение строк подключения, вставка данных и выполнение хранимых процедур. В этом разделе вы узнаете, как взаимодействовать с базой данных из простого Windows Forms приложения "формы на основе данных" с помощью Visual C# или Visual Basic и ADO.NET.  Все технологии данных .NET, в том числе наборы данных, LINQ to SQL и Entity Framework, в конечном итоге выполняют шаги, которые очень похожи на те, которые приведены в этой статье.
...
1. Создайте проект для Windows Forms приложения и назовите его Симпледатаапп.
 ...
 **Форма NewCustomer**
 ...
 ![Добавление нового клиента и размещение заказа](../data-tools/media/simpleappnewcust.png "симплеаппневкуст")
 ...
 **Форма FillOrCancel**
 ...
 ![заказы на заполнение или отмену](../data-tools/media/simpleappcancelfill.png "симплеаппканцелфилл")
...
2. В строке меню выберите **проект**  > **Добавить класс** , чтобы добавить файл класса в проект, а затем назовите файл `Utility`.
...
3. В файле Utility замените код-заполнитель следующим кодом. Обратите внимание на нумерованные комментарии (с префиксом Util-), которые указывают разделы кода. В таблице под примером кода описаны ключевые моменты.
    ...
    |Добавление примечаний|Описание|
    |-------------|-----------------|
    ...
    |Util-3|Несмотря на то, что в окне **свойств** в качестве имени строки подключения введен `connString`, в коде необходимо указать `"SimpleDataApp.Properties.Settings.connString"` (C#) или `"SimpleDataApp.My.MySettings.connString"` (Visual Basic).|
...
#### <a name="make-the-navigation-form-the-startup-form"></a>Преобразование формы навигации в начальную форму
 Если вы используете C#, в **Обозреватель решений**откройте Program.cs и измените строку `Application.Run` на следующую: `Application.Run(new Navigation());`
...
|NC-7|Создание объекта `SqlCommand`, `cmdNewCustomer`.<br /><br /> — Укажите `Sales.uspNewCustomer` в качестве выполняемой хранимой процедуры.<br />— Используйте свойство `CommandType`, чтобы указать, что команда является хранимой процедурой.|
|NC-8|Добавление входного параметра `@CustomerName` из хранимой процедуры.<br /><br /> — Добавьте параметр в коллекцию `Parameters`.<br />— Используйте перечисление `SqlDbType`, чтобы указать тип параметра nvarchar (40).<br />— Укажите `txtCustomerName.Text` в качестве источника.|
|NC-9|Добавление выходного параметра из хранимой процедуры.<br /><br /> — Добавьте параметр в коллекцию `Parameters`.<br />— Используйте `ParameterDirection.Output`, чтобы указать параметр в качестве выходных данных.|
...
|NC-12|Используйте метод `ExecuteNonQuery` для `cmdNewCustomer`, чтобы запустить хранимую процедуру `Sales.uspNewCustomer`. Эта хранимая процедура выполняет инструкцию `INSERT`, а не запрос.|
|NC-13|Значение `@CustomerID` возвращается как значение идентификатора из базы данных. Так как это целое число, необходимо преобразовать его в строку, чтобы отобразить ее в текстовом поле **идентификатор клиента** .<br /><br /> -Вы объявили `parsedCustomerID` в NC-2.<br />— Сохранение значения `@CustomerID` в `parsedCustomerID` для последующего использования.<br />— Преобразуйте возвращенный идентификатор клиента в строку и вставьте его в `txtCustomerID.Text`.|
...
|NC-16|Определите метод для проверки наличия имени клиента.<br /><br /> — Если текстовое поле пусто, отобразится сообщение и возвращается `false`, поскольку для создания учетной записи требуется имя.<br />— Если текстовое поле не пустое, возвращается `true`.|
...
|FC-5|Эти задачи необходимо выполнить перед попыткой выполнения инструкции SQL или хранимой процедуры.<br /><br /> — Создание объекта `SqlConnection`.<br />— Определите инструкцию SQL или укажите имя хранимой процедуры. (В этом случае вам потребуется выполнить инструкцию `SELECT`.)<br />— Создание объекта `SqlCommand`.<br />— Определите любые параметры для инструкции SQL или хранимой процедуры.|
|FC-6|Этот код использует `SqlDataReader` и `DataTable` для получения и отображения результата запроса.<br /><br /> — Откройте подключение.<br />— Создайте объект `SqlDataReader` `rdr`, выполнив метод `ExecuteReader` для `cmdOrderID`.<br />— Создайте объект `DataTable` для хранения извлеченных данных.<br />— Загрузка данных из объекта `SqlDataReader` в объект `DataTable`.<br />— Отображение данных в представлении сетки данных путем указания `DataTable` как `DataSource` для представления сетки данных.<br />— Закройте `SqlDataReader`.|
...
|FC-9|Создайте метод, чтобы убедиться, что `OrderID` готово к отправке в качестве параметра в объект `SqlCommand`.<br /><br /> — Убедитесь, что в `txtOrderID` введен идентификатор.<br />— Используйте `Regex.IsMatch`, чтобы определить простую проверку для нецелочисленных символов.<br />— Вы объявили переменную `parsedOrderID` в FC-2.<br />— Если входные данные допустимы, преобразуйте текст в целое число и сохраните значение в переменной `parsedOrderID`.<br />— Заключите метод `isOrderID` вокруг `btnFindByOrderID`, `btnCancelOrder` и `btnFillOrder` щелкните обработчики событий.|

## <a name="BKMK_testyourapplication"></a>Тестирование приложения
 Нажмите клавишу F5, чтобы создать и протестировать приложение после создания кода каждого обработчика событий щелчка, а затем после завершения написания кода.
