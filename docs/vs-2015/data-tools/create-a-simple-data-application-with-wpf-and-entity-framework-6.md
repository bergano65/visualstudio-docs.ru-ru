...
1. В Visual Studio выберите **файл &#124; создать проект** , а затем создайте новое C# приложение WPF.
...
1. Щелкните правой кнопкой мыши узел проекта в обозреватель решений и выберите **команду &#124; добавить новый элемент**. В левой области в C# узле выберите **данные** и в средней области выберите **ADO.NET EDM**.
...
5. Мастер создает C# классы, представляющие модель Entity Framework. Это обычные старые C# классы, которые мы будем привязку к пользовательскому интерфейсу WPF. Файл EDMX описывает связи и другие метаданные, связывающие классы с объектами в базе данных.  TT — это шаблоны T4, которые создают код, который будет обрабатывать модель, и сохраняет изменения в базе данных. Все эти файлы можно просмотреть в обозреватель решений узле Northwind_model:
...
6. TT-файлы предназначены для общего назначения, и нам нужно настроить один из них для работы с DataBinding WPF, что требует Обсерваблеколлектионс.  В обозреватель решений разверните узел Northwind_model, чтобы найти Northwind_model. tt. (Убедитесь, что вы **не** используете *. Файл Context. TT, который находится непосредственно под EDMX-файлом).
...
2. Выберите клиент.  (Источники для заказов будут автоматически созданы из свойства навигации Orders в Customer.)
...
5. Теперь откройте файл MainWindow. XAML, чтобы просмотреть его в конструкторе. Это приведет к тому, что окно Источники данных будет отображаться в виде параметра в поле окна Visual Studio рядом с областью элементов. Щелкните вкладку, чтобы открыть окно, или нажмите клавиши **SHIFT + ALT + D** или выберите **Просмотр &#124; других источников данных Windows &#124;** . Мы будем отображать каждое свойство в классе Customers в отдельном текстовом поле. Сначала щелкните стрелку в поле со списком клиенты и выберите **сведения**. Затем перетащите узел в среднюю часть области конструктора, чтобы разработчик знал, что он должен находиться в средней строке.  В случае его невозможности вручную указать строку в XAML. По умолчанию элементы управления размещаются вертикально в элементе Grid, но на этом этапе их можно расположить в форме.  Например, может иметь смысл разместить текстовое поле Name сверху, над адресом. Пример приложения для этой статьи переупорядочивает поля и перемещает их в два столбца.
     ...
     В представлении кода теперь можно увидеть новый элемент `Grid` в строке 1 (средняя строка) родительской сетки. Родительская сетка имеет атрибут `DataContext`, который ссылается на CollectionViewSource, который был добавлен в элемент `Windows.Resources`. Учитывая этот контекст данных, когда первое текстовое поле, например, привязывается к "Address", которое сопоставляется с свойством `Address` в текущем объекте `Customer` в CollectionViewSource.
