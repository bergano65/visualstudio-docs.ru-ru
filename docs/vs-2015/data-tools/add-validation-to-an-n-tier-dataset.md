...
## <a name="validatechanges-to-individual-columns"></a>Валидатечанжес к отдельным столбцам
 Проверьте значения в отдельных столбцах, обрабатывая событие <xref:System.Data.DataTable.ColumnChanging>. Событие <xref:System.Data.DataTable.ColumnChanging> возникает при изменении значения в столбце. Создайте обработчик событий для <xref:System.Data.DataTable.ColumnChanging> события, дважды щелкнув нужный столбец в наборе данных.
...
3. Добавьте код, чтобы убедиться, что `e.ProposedValue` содержит данные, соответствующие требованиям приложения. Если предложенное значение недопустимо, установите столбец, чтобы указать, что он содержит ошибку.
     ...
     В следующем примере кода проверяется, что столбец **Quantity** содержит больше 0. Если **Quantity** меньше или равно 0, в столбце задается ошибка. Предложение `Else` очищает ошибку, если **Quantity** больше 0. Код в обработчике событий изменения столбца должен выглядеть следующим образом:
...
2. Дважды щелкните заголовок окна таблицы данных в конструкторе.
    ...
    > [!NOTE]
    > Конструктор наборов данных не создает автоматически обработчик событий для события <xref:System.Data.DataTable.RowChanging> в C# проектах. Необходимо создать метод для обработки события <xref:System.Data.DataTable.RowChanging> и выполнения кода для подключения события в методе инициализации таблицы.

3. Добавьте пользовательский код внутри объявления разделяемого класса.

4. В следующем коде показано, куда добавить пользовательский код для проверки во время события <xref:System.Data.DataTable.RowChanging> для Visual Basic:
...
5. В следующем коде показано, как создать обработчик событий `RowChanging` и добавить пользовательский код для проверки во время события <xref:System.Data.DataTable.RowChanging> для C#:
