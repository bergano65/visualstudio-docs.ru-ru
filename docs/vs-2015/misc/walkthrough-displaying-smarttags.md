...
ms.locfileid: "47569225"
---
# <a name="walkthrough-displaying-smarttags"></a>Пошаговое руководство. Отображение смарт-тегов
Смарт-теги устарели и были заменены меню лампочки. См. в разделе [Пошаговое руководство: отображение предложений лампочки](../extensibility/walkthrough-displaying-light-bulb-suggestions.md).  
 ...
 Несмотря на то, что смарт-теги являются частью реализации IntelliSense в редакторе, смарт-теги можно реализовать путем создания подклассов <xref:Microsoft.VisualStudio.Language.Intellisense.SmartTag>, а затем реализовать <xref:Microsoft.VisualStudio.Text.Tagging.ITagger%601> интерфейс и <xref:Microsoft.VisualStudio.Text.Tagging.IViewTaggerProvider> интерфейс.  
...
3.  Добавьте класс с именем `TestSmartTag` , наследуемый от <xref:Microsoft.VisualStudio.Language.Intellisense.SmartTag>.  
...
4.  Добавьте конструктор для этого класса, который вызывает базовый конструктор со <xref:Microsoft.VisualStudio.Language.Intellisense.SmartTagType> из <xref:Microsoft.VisualStudio.Language.Intellisense.SmartTagType>, что вызовет подчеркивание первого символа в слове синей линией. (Если вы используете <xref:Microsoft.VisualStudio.Language.Intellisense.SmartTagType>, будет подчеркиваться красной линией, а последним символом слова.)  
...
5.  Добавьте класс с именем `TestSmartTagger` , наследуемый от <xref:Microsoft.VisualStudio.Text.Tagging.ITagger%601> типа `TestSmartTag`и реализует <xref:System.IDisposable>.  
...
7.  Добавьте конструктор, который задает закрытые поля и подписывается на <xref:Microsoft.VisualStudio.Text.Editor.ITextView.LayoutChanged> событий.  
...
11. Реализуйте `OnLayoutChanged` обработчик событий, вызываемый `TagsChanged` событие, которое инициирует <xref:Microsoft.VisualStudio.Text.Tagging.ITagger%601.GetTags%2A> для вызова.  
...
12. Реализуйте <xref:System.IDisposable.Dispose%2A> метод, чтобы он отменял подписку из <xref:Microsoft.VisualStudio.Text.Editor.ITextView.LayoutChanged> событий.  
...
2.  Импорт <xref:Microsoft.VisualStudio.Text.Operations.ITextStructureNavigatorSelectorService> как свойство.  
...
4.  Реализуйте <xref:Microsoft.VisualStudio.Language.Intellisense.ISmartTagAction.Invoke%2A> метод, заменив текст в диапазоне на эквивалентный символ верхнего регистра.  
