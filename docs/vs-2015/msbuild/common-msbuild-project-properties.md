...
|ApplicationManifest|Задает путь к файлу, используемому для создания информации внешнего манифеста контроля учетных записей (UAC). Применяется только к проектам Visual Studio, нацеленным на [!INCLUDE[windowsver](../includes/windowsver-md.md)].<br /><br /> В большинстве случаев манифест является внедренным. Однако при использовании COM-компонентов без регистрации или развертывания [!INCLUDE[ndptecclick](../includes/ndptecclick-md.md)] манифест может представлять собой внешний файл, устанавливаемый вместе со сборками приложения. Дополнительные сведения см. в описании свойства NoWin32Manifest в данном разделе.|  
|AssemblyOriginatorKeyFile|Указывает файл, используемый для подписи сборки (SNK или PFX) и передаваемый [задаче ResolveKeySource](../msbuild/resolvekeysource-task.md) для создания фактического ключа, используемого для подписания сборки.|  
...
|BaseOutputPath|Задает базовый путь для выходного файла. Если это свойство задано, [!INCLUDE[vstecmsbuild](../includes/vstecmsbuild-md.md)] будет использовать `OutputPath = $(BaseOutputPath)\$(Configuration)\`. Пример синтаксиса:`<BaseOutputPath>c:\xyz\bin\</BaseOutputPath>`.|  
...
|DebugSymbols|Логическое значение, указывающее, создает ли сборка символы.<br /><br /> Параметр **/p: DebugSymbols = false** в командной строке отключает создание файлов символов базы данных программы (PDB).|  
...
|SubsystemVersion|Задает минимальную версию подсистемы, которую может использовать созданный исполняемый файл. Это свойство эквивалентно переключателю `/subsystemversion` компилятора. Сведения о значении этого свойства по умолчанию см. в разделе [/subsystemversion (Visual Basic)](https://msdn.microsoft.com/library/08be22b2-f447-4cd3-8203-120b1b920b54) или [/subsystemversion (параметры компилятора C#)](https://msdn.microsoft.com/library/a99fce81-9d92-4813-9874-bee777041445).|  
