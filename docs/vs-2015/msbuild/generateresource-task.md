...
```  
 ...
 `GenerateResource`Задача использует \<LogicalName> метаданные \<EmbeddedResource> элемента для именования ресурса, внедренного в сборку.  
...
```  
 ...
 Без \<LogicalName> метаданных ресурс будет называться myAssembly. myResource. Resources.  Этот пример применяется только к процессу сборки Visual Basic и Visual C#.  
