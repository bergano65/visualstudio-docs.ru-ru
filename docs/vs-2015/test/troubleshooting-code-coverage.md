...
### <a name="pdb-symbol-files-are-unavailable"></a>Недоступны PDB-файлы (файлы символов)
 ...
 Разрешение убедитесь, что в параметрах сборки создается PDB-файл. Если PDB-файлы не обновляются при сборке проекта, откройте свойства проекта, выберите страницу **Сборка**, щелкните **Дополнительно** и проверьте сведения в поле **Отладочная информация**.
...
### <a name="using-an-instrumented-or-optimized-binary"></a>Использование инструментированного или оптимизированного двоичного файла
 Анализ определяет, проявляется ли двоичный файл любой формой расширенной оптимизации, например профильной оптимизации, или инструментирован с помощью средства профилирования, такого как VSInstr. exe или VSPerfMon. exe.
...
### <a name="code-is-not-managed-net-or-native-c-code"></a>Неуправляемый код (.NET) или машинный код (C++)
 Анализ убедитесь, что выполняются некоторые тесты для управляемого кода C++ или.

 Объяснение анализ покрытия кода в Visual Studio доступен только в управляемом и машинномC++коде (). При работе со средствами сторонних разработчиков часть кода или весь код может выполняться на другой платформе.
...
### <a name="custom-runsettings-file-with-incorrect-exclusions"></a>Пользовательский RUNSETTINGS-файл с неверными исключениями
 ...
 Решение удалите все `Include` узлы из файла. runsettings, а затем удалите все узлы `Exclude`. Если это не решит проблему, последовательно верните их обратно.
