...
## <a name="BKMK_Unit_test_frameworks_and_test_projects"></a> Платформы модульных тестов и тестовых проектов
 ...
 Тесты в обозревателе тестов можно выполнять из нескольких тестовых проектов в решении, а также из тестовых классов, которые входят в производственные проекты. Тестовые проекты могут использовать различные платформы модульного тестирования. При написании тестируемого кода для платформы .NET Framework тестовый проект можно написать на любом языке, который также ориентирован на платформу .NET Framework, независимо от языка целевого кода. Проекты машинного кода C/C++ необходимо проверять с помощью платформы модульного тестирования C++.
...
## <a name="BKMK_Run_tests_in_Test_Explorer"></a> Выполнение тестов в обозревателе тестов
 [Выполнение тестов](#BKMK_Run_tests) **&#124;** [выполнение тестов после каждой сборки](#BKMK_Run_tests_after_every_build)

 Тесты появляются в обозревателе тестов после сборки тестового проекта. Если Обозреватель тестов не виден, выберите **Тест** в меню Visual Studio, выберите **Windows**, затем выберите **Обозреватель тестов**.
...
### <a name="BKMK_Run_tests"></a> Выполнить тесты
 Вы можете выполнить все тесты в решении, все тесты в группе или набор выбранных тестов. Выполните одно из следующих действий.
...
- Если у отдельных тестов нет зависимостей, предотвращающих выполнение этих тестов в любом порядке, включите параллельное тестирование с помощью переключателя ![UTE&#95;parallelicon&#45;small](../test/media/ute-parallelicon-small.png "UTE_parallelicon-малый") на панели инструментов. Это может заметно сократить время, необходимое для выполнения всех тестов.
  ...
  Выполнение тестов сопровождается анимацией индикатора прохождения или непрохождения тестов в верхней части окна обозревателя тестов. По завершении тестового запуска индикатор прохождения или непрохождения становится зеленым, если все тесты пройдены, или красным, если какой-либо тест не пройден.
...
|||
|-|-|
|![Запустить после сборки](../test/media/ute-runafterbuild-btn.png "UTE_RunAfterBuild_btn")|Чтобы запустить модульные тесты после каждой локальной сборки, в стандартном меню выберите **Тест** , а затем выберите **Выполнить тесты после сборки** в панели инструментов обозревателя тестов.|

 К [содержимому](#BKMK_Contents) ![сверху](../debugger/media/pcs-backtotop.png "PCS_BackToTop")

## <a name="BKMK_View_test_results"></a> Просматривать результаты тестов
 [Просмотр сведений о](#BKMK_View_test_details) **&#124;** тесте [Просмотр исходного кода метода теста](#BKMK_View_the_source_code_of_a_test_method)

 При выполнении, написании и повторном запуске тестов обозреватель тестов отображает результаты в группах **Неудачные тесты**, **Пройденные тесты**, **Пропущенные тесты** и **Не запущенные тесты**. В области сведений в нижней части обозревателя тестов приводится сводный обзор тестового запуска.
...
### <a name="BKMK_View_test_details"></a> Просмотр сведений о тесте
 ...
 ![Сведения о выполнении теста](../test/media/ute-testdetails.png "UTE_TestDetails")
...
- Время выполнения метода теста.

  Если тест не пройден, в области сведений отображаются также следующие данные:

- Сообщение, выданное платформой модульных тестов в отношении этого теста.

- Данные трассировки стека на момент неудачного завершения теста.
...
## <a name="BKMK_Group_and_filter_the_test_list"></a> Группировка и фильтрация списка тестов
 [Группирование группы списка](#BKMK_Grouping_the_test_list) **&#124;** тестов [по характеристикам](#BKMK_Group_by_traits) **&#124;** [Поиск и фильтрация списка тестов](#BKMK_Search_and_filter_the_test_list)
...
### <a name="BKMK_Grouping_the_test_list"></a> Группирование списка тестов
 Чтобы изменить способ организации тестов, щелкните стрелку "вниз" рядом с кнопкой **Группировать**![Кнопка "Группировать" в обозревателе тестов](../test/media/ute-groupby-btn.png "UTE_GroupBy_btn") и выберите новые условия группировки.

 ![Группирование тестов по категориям в обозревателе тестов](../test/media/ute-groupbycategory.png "UTE_GroupByCategory")
...
|**Project**|Группировка тестов по именам проектов.|
...
3. Введите значение фильтра в кавычках.
   ...
   ![Фильтрация тестов в обозревателе тестов](../test/media/ute-filtertestlist.png "UTE_FilterTestList")
...
## <a name="BKMK_Create_custom_playlists"></a> Создание настраиваемых списков воспроизведения
 ...
 ![Выбор списка воспроизведения](../test/media/ute-playlist.png "UTE_Playlist")
...
### <a name="BKMK_Debug_unit_tests"></a> Отладка модульных тестов
 Обозреватель тестов можно использовать для запуска сеанса отладки тестов. Пошагово выполняя код, отладчик Visual Studio плавно переключается назад и вперед между модульными тестами и проектом для тестирования. Чтобы начать отладку, выполните указанные ниже действия.

1. В редакторе Visual Studio задайте точку останова в одном или нескольких методах теста, которые необходимо отладить.

   > [!NOTE]
   > Поскольку методы теста могут выполняться в любом порядке, задайте точки останова в методах теста, которые необходимо отладить.
...
## <a name="see-also"></a>См. также раздел
 [Модульное тестирование кода](../test/unit-test-your-code.md) [Выполнение модульного теста как 64-разрядного процесса](../test/run-a-unit-test-as-a-64-bit-process.md)
