...
Типы оболочек** — одна из двух технологий Microsoft Fakes Framework, используемых для простой изоляции тестируемых компонентов от окружения. Прокладки перенаправляют вызовы отдельных методов в код, написанный в рамках теста. Множество методов возвращает различные результаты в зависимости от внешних условиях, но оболочка находится под контролем теста и может возвращать последовательные результаты при каждом вызове. Это делает написание тестов намного проще.

 Используйте прокладки, чтобы изолировать код из сборок, не являющихся частью решения. Для изоляции компонентов решения друг от друга рекомендуется использовать заглушки.
