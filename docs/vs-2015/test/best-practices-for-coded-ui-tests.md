...
- Создавайте тест как последовательность записанных методов. Дополнительные сведения о том, как записать метод, см. в разделе [Создание закодированных тестов пользовательского интерфейса](../test/use-ui-automation-to-test-your-code.md#VerifyingCodeUsingCUITCreate).
...
- Создайте отдельный файл [UIMap](/previous-versions/dd580454(v=vs.140)) для каждого модуля в тестируемом приложении. Дополнительные сведения см. [в разделе Тестирование большого приложения с несколькими картами пользовательского интерфейса](../test/testing-a-large-application-with-multiple-ui-maps.md).
