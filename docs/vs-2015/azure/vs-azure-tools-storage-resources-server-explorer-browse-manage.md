...
Обозреватель серверов автоматически показывает список больших двоичных объектов, очередей и таблиц в учетной записи эмулятора хранилища. Учетная запись эмулятора хранения отображается в обозревателе сервера в узле **Хранилище** как узел **Разработка**.
...
1. В диалоговом окне **Отправить новый файл** нажмите кнопку **Обзор**, чтобы указать необходимый файл, а затем введите имя папки в поле **Папка (необязательно)** .
   ...
   ![Отправка файла в папку большого двоичного объекта](./media/vs-azure-tools-storage-resources-server-explorer-browse-manage/IC766037.png)
...
Можно выбрать один или несколько файлов для отправки и отправлять файлы любого типа. В **журнале действий Azure** отображается ход отправки. Дополнительные сведения о работе с данными больших двоичных объектов см. в статье [Приступая к работе с хранилищем BLOB-объектов Azure с помощью .NET](https://www.windowsazure.com/develop/net/how-to-guides/blob-storage/).
...
Очереди служб хранилища размещаются в учетной записи хранения Azure. Их можно использовать, чтобы разрешать ролям облачных служб взаимодействовать друг с другом и другими службами с помощью механизма передачи сообщений. Доступ к очередям можно получить программным путем через облачную службу и веб-службу для внешних клиентов. Можно также обращаться к очереди напрямую с помощью обозревателя серверов в Visual Studio.
...
В обозревателе серверов можно просматривать очереди в учетной записи хранилища, создавать и удалять очереди, открывать очередь для просмотра сообщений в ней и добавлять сообщения в очередь. Если очередь открывается для просмотра, можно просматривать отдельные сообщения, а с помощью кнопок в левом верхнем углу можно выполнять следующие действия в очереди:
...
Дополнительные сведения об очередях служб хранилища см. в статье [Приступая к работе с хранилищем очередей Azure с помощью .NET](https://www.windowsazure.com/develop/net/how-to-guides/queue-service/). Сведения о веб-службе для очередей служб хранилища см. в статье [Queue Service Concepts](https://msdn.microsoft.com/library/windowsazure/dd179353.aspx) (Основные понятия службы очередей). Сведения об отправке сообщения в очередь служб хранилища с помощью Visual Studio см. в статье об [отправке сообщений в очередь служб хранилища](/azure/visual-studio/vs-storage-cloud-services-getting-started-queues).
...
Существует два способа добавления учетной записи хранения с помощью обозревателя серверов. Можно создать или присоединить имеющеюся учетную запись хранения в подписке Azure.
...
1. В диалоговом окне **Создание учетной записи хранилища** выберите или укажите следующие сведения:
   ...
   * Протокол для подключения к учетной записи хранения, например, HTTP, HTTPS или настраиваемая конечная точка. Дополнительные сведения о настраиваемых конечных точках см. в статье [Настройка строк подключения службы хранилища Azure](https://msdn.microsoft.com/library/azure/ee758697.aspx).
...
Дополнительные сведения об использовании служб хранилища Azure см. в статье Общие сведения о службе хранилища [Azure](/azure/storage/common/storage-introduction).
