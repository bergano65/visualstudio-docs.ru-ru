...
* [Портал Azure](https://go.microsoft.com/fwlink/p/?LinkID=525040) использует строку подключения в CSCFG-файле для доступа к данным диагностики. Данные отображаются на вкладке **мониторинг** . Строка подключения требуется для настройки службы на отображение подробных данных мониторинга на портале.
...
5. Чтобы просмотреть диалоговое окно **конфигурации диагностики**, выберите **Настройка**. Каждая вкладка (кроме вкладок **Общие** и **Каталоги журналов**) соответствует определенному источнику диагностических данных, которые можно собирать. Вкладка **Общие** содержит следующие параметры сбора данных диагностики: **только ошибки**, **все сведения**и **пользовательский план**. По умолчанию установлен режим **Только ошибки**, для которого требуется наименьший объем хранилища, поскольку в этом режиме предупреждения и сообщения трассировки не передаются. Режим **Все сведения** передает больше всего информации и использует самый большой объем хранилища, поэтому он наиболее затратный.
...
Дополнительные сведения см. [в статьях Управление ведением журнала и трассировка в Microsoft Azure](https://msdn.microsoft.com/magazine/ff714589.aspx) и [система диагностики Microsoft Azure. часть 4. пользовательские компоненты ведения журнала и система диагностики Azure 1,3 изменений](https://www.red-gate.com/simple-talk/cloud/platform-as-a-service/microsoft-azure-diagnostics-part-4-custom-logging-components-and-azure-diagnostics-1.3-changes/).
...
* **PreciseTimeStamp**: метка времени ETW события. Это время регистрации события клиентом.
* **Timestamp**: значение для **PreciseTimeStamp** округляется вниз до границы частоты передачи. Например, если вы используете период передачи 5 минут, а событие регистрируется в 00:17:12, в поле TIMETAMP установлено значение 00:15:00.
* **Timestamp**: метка времени, в которой сущность была создана в таблице Azure.
