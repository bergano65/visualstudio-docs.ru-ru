...
- Анализ кадров позволяет получать новые сводки производительности на основе ранее захваченных данных, например, если драйверы GPU были оптимизированы или предоставляют дополнительные возможности отладки.  
  ...
  Для демонстрации того, что можно сделать анализ кадров для вашего приложения, вы можете посмотреть [анализ кадров графики Visual Studio](http://channel9.msdn.com/Shows/C9-GoingNative/GoingNative-25-Offline-Analysis-Graphics-Tool) видео на канале Channel 9.  
...
## <a name="using-frame-analysis"></a>Использование анализа кадров  
 ...
 ![Выберите вкладку "Анализ кадров"](../debugger/media/pix-frame-analysis-select-tab.png "pix_frame_analysis_select_tab")  
...
#### <a name="timeline"></a>Временная шкала  
 ...
 ![На временной шкале отображаются действия&#45;вызывать расходы. ](../debugger/media/pix-frame-analysis-timeline.png "pix_frame_analysis_timeline")  
...
#### <a name="hot-draw-calls"></a>"Горячие" вызовы Draw  
 ...
 ![Этот вызов DrawIndexed имеет горячего и холодного пассивные варианты. ](../debugger/media/pix-frame-analysis-hot-calls.png "pix_frame_analysis_hot_calls")  
...
#### <a name="statistical-significance"></a>Статистическая значимость  
 ...
 ![Статистическая релевантность варианта вызова рисования](../debugger/media/pix-frame-analysis-summary-stats.png "pix_frame_analysis_summary_stats")  
...
#### <a name="platforms-that-support-hardware-counters"></a>Платформы, поддерживающие аппаратные счетчики  
 ...
 ![Аппаратные счетчики отображаются в том случае, когда это возможно. ](../debugger/media/pix-frame.png "pix_frame")  
...
|**Анизотропная фильтрация текстур**|Для всех соответствующих образцов текстур устанавливается режим фильтрации `DXD11_FILTER_ANISOTROPIC`, а свойству `MaxAnisotropy` присваивается значение `16` (16-кратная анизотропная фильтрация текстур).<br /><br /> Дополнительные сведения см. в разделе [точки, билинейной, Трилинейной и текстуры анизотропной фильтрации](../debugger/point-bilinear-trilinear-and-anisotropic-texture-filtering-variants.md).|  
