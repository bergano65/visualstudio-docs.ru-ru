 ...
 **_IGNORE_BLOCK**  
 ...
 Чтобы определить тип и подтип данного блока, используются функция [_CrtReportBlockType](http://msdn.microsoft.com/library/0f4b9da7-bebb-4956-9541-b2581640ec6b) и макросы **_BLOCK_TYPE** и **_BLOCK_SUBTYPE**. Макросы определяются (в crtdbg.h) следующим образом:  
...
|Функция|Описание|  
|--------------|-----------------|  
|[_CrtMemCheckpoint](http://msdn.microsoft.com/library/f1bacbaa-5a0c-498a-ac7a-b6131d83dfbc)|Сохраняет снимок кучи в структуре **_CrtMemState**, предоставляемой приложением.|  
...
|[_CrtMemDumpStatistics](http://msdn.microsoft.com/library/27b9d731-3184-4a2d-b9a7-6566ab28a9fe)|Формирует дамп предоставленной структуры **_CrtMemState**. Структура может содержать снимок состояния отладочной кучи на данный момент или различие между двумя состояниями.|  
|[_CrtMemDumpAllObjectsSince](http://msdn.microsoft.com/library/c48a447a-e6bb-475c-9271-a3021182a0dc)|Выводит сведения обо всех объектах, выделенных с момента получения данного снимка кучи или с начала выполнения. При каждом выводе блока **_CLIENT_BLOCK** вызывается предоставляемая приложением функция-ловушка, если она была установлена с помощью **_CrtSetDumpClient**.|  
 ...
 **Уникальные номера запросов выделения и _crtBreakAlloc**  
 ...
 Узнав номер запроса выделения некорректно выделенного блока, можно передать его в [_CrtSetBreakAlloc](http://msdn.microsoft.com/library/33bfc6af-a9ea-405b-a29f-1c2d4d9880a1) и создать точку останова. Перед выделением блока выполнение прервется, и можно вернуться и определить, какая процедура отвечает за плохой вызов. Во избежание перекомпиляции эти же действия можно проделать в отладчике, присвоив **_crtBreakAlloc** номер запроса интересующего выделения.  
