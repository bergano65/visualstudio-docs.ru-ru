---
title: Запустите сеанс отладки для приложения Магазина (VB, C,, C и XAML) Документы Майкрософт
ms.date: 11/15/2016
...
> [!NOTE]
> Для приложений, написанных в JavaScript и HTML, смотрите [Начало сеанса отладки (JavaScript).](../debugger/start-a-debugging-session-for-store-apps-in-visual-studio-javascript.md)
...
- [(Необязательно) Переустановите приложение при начале отладки](#BKMK__Optional__Reinstall_the_app_when_you_start_debugging)

- [(Необязательно) Требование о удалении аутентификации для запуска удаленного отладчика](#BKMK__Optional__Disable_authentication_requirement_to_start_the_remote_debugger)
...
2. Нажмите F5.
   ...
   Visual Studio создает и запускает приложение с прикрепленным отладчиком. Выполнение продолжается до достижения точки останова, приостановления выполнения вручную, необработанного исключения или завершения приложения. Для получения дополнительной информации [см. Перейдите сеанс отладки (Xaml и C)](../debugger/navigate-a-debugging-session-in-visual-studio-xaml-and-csharp.md) .
...
2. Выполните это, чтобы открыть страницу свойств отладки для проекта.
    ...
    - В Visual C# и Visual Basic выберите **Отладка**.
         ...
         ![C&#35; &#47; VB проект отладка собственности страницы](../debugger/media/dbg-csvb-debugpropertypage.png "DBG_CsVb_DebugPropertyPage")

    - В Visual C++ разверните узел **Свойства конфигурации**  и выберите **Отладка**.

         ![C&#43;&#43; страница отладки свойств Windows Store](../debugger/media/dbg-cpp-debugpropertypage.png "DBG_CPP_DebugPropertyPage")
...
- Выберите удаленный компьютер из диалогового окна **Выбрать подключение к удаленному отладчику** .
  ...
  - Для приложений СЗ, выберите вниз стрелка в поле **имя машины** и выбрать ** \<Locate ... >**.
  ...
  > [!NOTE]
  > В диалоговом окне **Select Remote Debugger Connection** отображаются машины, находящиеся на локальной подсети, и машины, непосредственно подключенные к аппарату Visual Studio, по кабелю Ethernet. Чтобы указать другой компьютер, введите имя в поле **Имя компьютера** .
...
### <a name="optional-delay-starting-the-debug-session"></a><a name="BKMK__Optional__Delay_starting_the_debug_session"></a>(Необязательно) Задержка начала сеанса отладки
...
### <a name="start-debugging-f5"></a><a name="BKMK_Start_debugging__F5_"></a> Начать отладку (F5)
 При выборе **Start Debugging** (Keyboard: F5) в меню **Debug** Visual Studio запускает приложение с прикрепленным отладчиком. Выполнение продолжается до достижения точки останова, приостановления выполнения вручную, необработанного исключения или завершения приложения.
...
- Выберите **«Старт отладку»** в меню **Debug** (клавиатура: F5).
...
## <a name="see-also"></a>См. также:
 [Приложения для отладки в Visual Studio](../debugger/debug-store-apps-in-visual-studio.md) [Перейдите сеанс отладки (Xaml и C)](../debugger/navigate-a-debugging-session-in-visual-studio-xaml-and-csharp.md)
