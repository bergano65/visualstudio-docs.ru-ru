...
- Первые 16 байт данных в блоке, в шестнадцатеричном формате.  
  ...
  Эти способы работают для памяти, выделенной с помощью стандартной функции `malloc` библиотеки CRT. Однако если программа выделяет память с помощью оператора C++ `new` , то вы можете видеть только файл и номер строки, где реализация глобальных `operator new` вызовов `_malloc_dbg` в отчете об утечке памяти. Так как это поведение не очень полезно, его можно изменить, чтобы сообщить о строке, в которой было выполнено выделение, с помощью макроса, который выглядит следующим образом: 
...
Теперь можно заменить оператор `new` с помощью макроса `DBG_NEW` в коде. В отладочных сборках используется перегрузка Global `operator new` , которая принимает дополнительные параметры для типа блока, файла и номера строки. Эта перегрузка `new` вызовов `_malloc_dbg` для записи дополнительных сведений. При использовании `DBG_NEW` отчеты об утечке памяти показывают имя файла и номер строки, в которой были выделены утечки объектов. В розничных сборках используется значение по умолчанию `new` . (Не рекомендуется создавать макрос препроцессора с именем `new` или любое другое ключевое слово языка.) Ниже приведен пример метода.  
...
При запуске этого кода в отладчике в Visual Studio вызов метода `_CrtDumpMemoryLeaks` создает отчет в окне **вывода** , который выглядит примерно так:  
...
Это говорит о том, что утечка памяти находилась в строке 20 debug_new. cpp.  
...
5. В столбце **Значение** замените отображаемое значение номером выделения памяти, на котором нужно приостановить выполнение.  
   ...
   Иногда может быть полезно задать точку останова по данным на самом объекте. Дополнительные сведения см. [в разделе Использование точек останова](../debugger/using-breakpoints.md).  
