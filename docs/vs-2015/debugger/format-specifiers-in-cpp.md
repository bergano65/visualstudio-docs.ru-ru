---
title: Описатели формата в C++ | Документация Майкрософт
ms.date: 11/15/2016
...
```  
 ...
 Добавьте `my_var1` переменную в окно **Контрольные значения** (при отладке, **отладке/Windows/контрольное значение 1**) и задайте для параметра вывод шестнадцатеричное значение (в окне **Контрольные значения** щелкните переменную правой кнопкой мыши и выберите **шестнадцатеричное отображение**). Теперь в окне "Контрольные значения" показано значение 0x0065. Чтобы представить это значение в символьном, а не целочисленном формате, в столбце "Имя" после имени переменной добавьте описатель символьного формата **, c**. В столбце **Значение** теперь отображается значение **101 'e'**.  
...
|s|const char* string|\<location> "Hello World"|"Здравствуй, мир!"|  
|**sb**|const char* string|\<location> "Hello World"|Здравствуй, мир!|  
|s8|const char* string|\<location> "Hello World"|"Здравствуй, мир!"|  
|**s8b**|const char* string|\<location> "Hello World"|"Здравствуй, мир!"|  
|su|const wchar_t * const<br /><br /> \*строка char16_t|\<location> L "Hello World"|L"Здравствуй, мир!"<br /><br /> u"Здравствуй, мир!"|  
|sub|const wchar_t * const<br /><br /> \*строка char16_t|\<location> L "Hello World"|Здравствуй, мир!|  
|bstr|Строка BSTR|\<location> L "Hello World"|L"Здравствуй, мир!"|  
|**s32**|Строка UTF-32|\<location> U "Hello World"|U"Здравствуй, мир!"|  
...
|**na**|Подавляет адрес указателя на объект в памяти.|\<location>, {Member = значение...}|{элемент=значение…}|  
...
|Описатель|Формат|Исходное контрольное значение|Отображаемое значение|  
|---------------|------------|--------------------------|---------------------|  
|**г, я**|Десятичное целое число со знаком|0xF000F065|-268373915|  
...
### <a name="format-specifiers-memory-locations-in-interop-debugging-with-ccli"></a><a name="BKMK_Format_specifiers_memory_locations_in_interop_debugging_and_C___edit_and_continue"></a> Описатели формата адресов памяти при отладке взаимодействия с использованием C++/CLI  
...
### <a name="size-specifier-for-pointers-as-arrays-in-interop-debugging-with-cclit"></a><a name="BKMK_Size_specifier_for_pointers_as_arrays_in_interop_debugging_and_C___edit_and_continue"></a> Описатель размера для указателей как массивов при отладке взаимодействия с использованием C++/CLI  
