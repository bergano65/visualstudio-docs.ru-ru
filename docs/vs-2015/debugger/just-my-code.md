...
## <a name="BKMK_Enable_or_disable_Just_My_Code"></a> Включение или отключение режима "Только мой код"  
 Чтобы включить или отключить Только мой код, выберите **Параметры и настройки** в меню **Отладка** . В узле **отладка** / **Общие** выберите или снимите флажок **включить только мой код**.  
  
 ![Включение Только мой код в диалоговом окне «Параметры»](../debugger/media/dbg-justmycode-options.png "DBG_JustMyCode_Options")  
...
### <a name="BKMK_Override_call_stack_filtering"></a>Переопределение фильтрации стека вызовов  
...
## <a name="BKMK__NET_Framework_Just_My_Code"></a>.NET Framework Только мой код  
  
### <a name="BKMK_NET_User_and_non_user_code"></a>Пользовательский и не пользовательский код  
...
### <a name="BKMK_NET_Stepping_behavior"></a>Пошаговое поведение  
...
### <a name="BKMK_CPP_User_and_non_user_code"></a>Пользовательский и не пользовательский код  
...
- Функции, определенные в файлах `*.natjmc`, находящихся в папке `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers`.  
  ...
  **Пошаговое выполнение**  
  ...
  Для настройки пошагового выполнения и поведения окна стека вызовов в `.natstepfilter` вы можете создавать собственные `.natjmc` и `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers`.  
  
### <a name="BKMK_CPP_Stepping_behavior"></a>Пошаговое поведение  
...
### <a name="BKMK_CPP_Customize_stepping_behavior"></a>Настройка пошагового поведения  
...
- Чтобы указать код, не являющийся пользователем, для всех пользователей компьютера Visual Studio, добавьте файл. натстепфилтер в папку `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers`.  
  
- Чтобы указать непользовательский код для отдельного пользователя, добавьте натстепфилтер-файл в папку `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers`.  
...
|`Name`|Обязательный элемент. Регулярное выражение в формате ECMA-262, содержащее полное имя функции для сопоставления. Пример:<br /><br /> `<Name>MyNS::MyClass.*</Name>`<br /><br /> сообщает отладчику, что все методы в `MyNS::MyClass`, должны считаться кодом, не написанным пользователем. Соответствие учитывает регистр символов.|  
...
|`Action`|Обязательный элемент. Одно из следующих чувствительных к регистру значений:<br /><br /> -   `NoStepInto` — указывает отладчику выполнить шаг с обходом соответствующей функции.<br />-   `StepInto` — указывает отладчику выполнить шаг с заходом в соответствующие функции, переопределив все остальные `NoStepInto` для соответствующих функций.|  
...
- Чтобы указать код, не являющийся пользователем, для всех пользователей компьютера Visual Studio, добавьте файл. натжмк в папку `%VsInstallDirectory%\Common7\Packages\Debugger\Visualizers`.  
  
- Чтобы указать непользовательский код для отдельного пользователя, добавьте натжмк-файл в папку `%USERPROFILE%\My Documents\Visual Studio 2015\Visualizers`.  
...
### <a name="BKMK_JS_User_and_non_user_code"></a>Пользовательский и не пользовательский код  
 **Классификации кода**  
...
- Скрипт, который выполняется путем передачи строки в предоставленную ведущим `eval` функцию, классифицируется как **myCode**.  
...
- Скрипт, выполняемый путем передачи строки в `setTimeout`, `setImmediate`или функции `setInterval` классифицируется как **UnrelatedCode**.  
...
### <a name="BKMK_JS_Stepping_behavior"></a>Пошаговое поведение  
...
|**Компонент**|Скрипт, который выполняется путем передачи строки конструктору `Function`. По умолчанию скрипт Function классифицируется как **LibraryCode**.|  
