---
title: Использование брейк-пойнтов Документы Майкрософт
ms.date: 11/15/2016
...
## <a name="setting-a-function-breakpoint-in-source-code"></a><a name="BKMK_Overview"></a> Задание точки останова в функции в исходном коде  
 ...
 ![Установка точки разрыва](../debugger/media/basicbreakpoint.png "Базоваяточка")  
...
## <a name="setting-a-data-breakpoint-native-c-only"></a><a name="BKMK_set_a_data_breakpoint_native_cplusplus_only"></a>Настройка точки разрыва данных (только для родных СЗ)  
...
## <a name="managing-breakpoints"></a><a name="BKMK_Specify_advanced_properties_of_a_breakpoint_"></a> Управление точками останова  
 ...
 ![Окно «Точки разрыва»](../debugger/media/breakpointswindow.png "BreakpointsWindow")  
...
2. В контекстном меню выберите **Условия**. Откроется окно **Параметры точки останова** :  
   ...
   **Условные выражения.** При выборе условного выражения можно выбрать два условия: **Является true** и **При изменении**. Выберите значение **Является true** , если требуется прервать выполнение при истинности выражения, или значение **При изменении** , если требуется прервать выполнение при изменении значения выражения.  
   ...
   ![Точка останова при изменении](../debugger/media/breakpointwhenchanged.png "Точка разрываКогдаИзменена")  
   ...
   Условие может быть любым допустимым выражением, которое распознает отладчик. Для получения дополнительной информации о допустимых выражениях [см.](../debugger/expressions-in-the-debugger.md)  
...
## <a name="using-object-ids-in-breakpoint-conditions-c-and-f"></a>Использование идентификаторов объектов в условиях точек останова (C# и F#)  
 Бывают случаи, когда необходимо посмотреть поведение конкретного объекта; например, может потребоваться узнать, почему объект был вставлен в коллекцию больше одного раза. В C и F-версиях можно создавать идентификаторы объектов для определенных [типов ссылок](https://msdn.microsoft.com/library/801cf030-6e2d-4a0d-9daf-1431b0c31f47) и использовать их в условиях точки разрыва. Идентификатор объекта создается службами отладки среды CLR и связан с объектом.  Чтобы создать идентификатор объекта, выполните следующее.  
...
## <a name="breakpoint-actions-and-tracepoints"></a><a name="BKMK_Print_to_the_Output_window_with_tracepoints"></a> Действия точки останова и точки отслеживания  
