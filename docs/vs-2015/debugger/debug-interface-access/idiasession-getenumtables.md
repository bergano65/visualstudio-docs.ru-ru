...
## <a name="example"></a>Пример  
 В этом примере представлена общая функция, использующая `getEnumTables` метод для получения конкретного объекта перечислителя. Если перечислитель найден, функция возвращает указатель, который может быть приведен к требуемому интерфейсу. в противном случае функция возвращает значение `NULL` .  
