...
1. В следующем консольном приложении C# метод Main создает исключение **AccessViolationException** внутри `try/catch` блока:  
...
### <a name="setting-the-debugger-to-continue-on-user-unhandled-exceptions"></a><a name="BKMK_UserUnhandled"></a> Настройка отладчика на продолжение при необработанных пользователем исключениях  
...
3. Этот параметр можно изменить либо для конкретного исключения (выберите исключение, щелкните его правой кнопкой мыши и выберите или отмените выбор параметра **Продолжать, если не обрабатывается в пользовательском коде**), либо для всей категории исключений (например, все исключения CLR).  
   ...
   ![прерывание на&#45;пользователя исключение унханлдед](../debugger/media/exceptionunhandledbyuser.png "ExceptionUnhandledByUser")  
...
> [!TIP]
> Проверьте правильность написания! В окне **параметры исключений** не проверяется наличие добавленного исключения. Поэтому при вводе **Sytem. UriTemplateMatchException**вы получите запись для этого исключения (а не для **System. UriTemplateMatchException**).  
...
## <a name="see-also"></a>См. также:  
 ...
 [Использование проверок во время выполнения без библиотеки времени выполнения языка C](../debugger/using-run-time-checks-without-the-c-run-time-library.md)   
