...
## <a name="create-the-sample-app"></a><a name="BKMK_CreateTheApplication"></a> Создание примера приложения
 ...
 **Замените конструктор MainPage образцом кода.** Удалите метод MainPage(). Перейдите по этой ссылке: [пример кода навигации отладчика (XAML и C#)](../debugger/debugger-navigation-sample-code-xaml-and-csharp.md), а затем скопируйте код, указанный в разделе C#, в буфер обмена. (Нажмите кнопку **назад** в браузере или окне справки, чтобы вернуться на эту страницу быстрого запуска.) В редакторе Visual Studio вставьте код в `partial class MainPage` блок. Нажмите сочетание клавиш CTRL + s, чтобы сохранить файл.

 После этого можно выполнить примеры, приведенные в этом разделе.

## <a name="set-and-run-to-a-breakpoint-step-into-a-method-and-examine-program-data"></a><a name="BKMK_StepInto"></a> Установка и выполнение в точке останова, шаг с заходом в метод и изучение данных программы
 ...
 **Выполните код до точки останова.** Запустите сеанс отладки, выбрав **начать отладку** в меню **Отладка** (клавиатура: F5).
...
- Если следующий оператор является вызовом функции в вашем решении, отладчик переходит к точке входа вызываемой функции, а затем приостанавливает выполнение.
  ...
  **Просмотр значений переменных в подсказках по данным.** При наведении указателя мыши на имя переменной в подсказке отображается имя, значение и тип переменной.
  ...
  ![Подсказка отладчика по данным](../debugger/media/dbg-basics-datatip.png "DBG_Basics_DataTip")
  ...
  **Проверьте значения переменных в окне «Локальные».** On the **Отладки** выберите пункт **Окна**и затем щелкните **Локальные**. (Клавиатура: Alt + 4).

  ![Окно локальных переменных](../debugger/media/dbg-basics-localswindow.png "DBG_Basics_LocalsWindow")
...
## <a name="step-into-over-and-out-of-methods"></a><a name="BKMK_StepIntoOverOut"></a> Шаг с заходом, обход и выход из методов
 ...
 **Выполните код до точки останова.** Запустите сеанс отладки, выбрав **начать отладку** в меню **Отладка** (клавиатура: F5). Отладчик приостанавливает выполнение в точке останова.
 ...
 **Останавливает отладку.** Выберите «Остановить отладку» в меню «Отладка» (клавиатура: Shift + F5). Это приведет к завершению сеанса отладки.

## <a name="set-a-conditional-breakpoint-run-to-the-cursor-and-visualize-a-variable"></a><a name="BKMK_ConditionCursorVisualize"></a> Задание условной точки останова, выполнение до курсора и визуализация переменной
 ...
 **Выполните код до точки останова.** Запустите сеанс отладки, выбрав **начать отладку** в меню **Отладка** (клавиатура: F5). Отладчик приостанавливает выполнение в точке останова в методе MainPage.
 ...
 **Задайте условную точку останова.** В левом поле окна кода щелкните правой кнопкой мыши строку `x += i;` и выберите **Условие**. Установите флажок **Условие** , а затем введите `i == 500;` в текстовом поле. Выберите параметр **Верно** и выберите **ОК**. Точка останова позволяет проверить значение в 500-й итерации цикла `for` .
 ...
 ![Диалоговое окно "Условие для точки останова"](../debugger/media/dbg-basics-breakpointcondition.png "DBG_Basics_BreakpointCondition")

 Для определения значка условной точки останова используется белое перекрестие.

 ![Условная точка останова](../debugger/media/dbg-basics-conditionalbreakpoint.png "DBG_Basics_ConditionalBreakpoint")
...
## <a name="edit-and-continue-recover-from-an-exception"></a><a name="BKMK_EditContinueRecoverExceptions"></a> Изменить и продолжить, восстановить из исключения
 ...
 **Выполните код до исключения.** Запустите сеанс отладки, выбрав **начать отладку** в меню **Отладка** (клавиатура: F5). Нажмите клавишу F5, чтобы возобновить выполнение. Отладчик приостанавливает выполнение в точке исключения в методе Example4 и выводит диалоговое окно исключения.

 ![Диалоговое окно "Исключение"](../debugger/media/dbg-basics-exceptiondlg.png "DBG_Basics_ExceptionDlg")
 ...
 **Измените логику программы.** Очевидно, что ошибка находится в условии `if` : значение `x` должно быть изменено при `x` , равном 0, а не при `x` , не равном нулю. Выберите **Прервать** , чтобы исправить логику метода. При попытке изменения строки отображается другое диалоговое окно.
 ...
 ![Диалоговое окно "изменить и продолжить"](../debugger/media/dbg-basics-editandcontinuedlg.png "DBG_Basics_EditAndContinueDlg")
 ...
 **Останавливает отладку.** On the **Отладки** выберите команду **Отладка** (клавиатура: Shift + F5). Это приведет к завершению сеанса отладки.

## <a name="see-also"></a>См. также:
 [Запуск сеанса отладки (VB, C#, C++ и XAML)](../debugger/start-a-debugging-session-for-a-store-app-in-visual-studio-vb-csharp-cpp-and-xaml.md) [триггеров приостановки, возобновления и фоновых событий для Магазина Windows для](../debugger/how-to-trigger-suspend-resume-and-background-events-for-windows-store-apps-in-visual-studio.md) [отладки приложений в Visual Studio](../debugger/debug-store-apps-in-visual-studio.md)
