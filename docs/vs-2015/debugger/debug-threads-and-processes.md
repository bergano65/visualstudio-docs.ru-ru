...
Потоки * и *процессы* — это связанные понятия в компьютерной науки. Оба представляют из себя последовательность инструкций, которые должны выполняться в определенном порядке. Инструкции в отдельных потоках или процессах, однако, могут выполняться параллельно.  
...
## <a name="tools-for-debugging-threads-and-processes-in-visual-studio"></a>Средства отладки потоков и процессов в Visual Studio  
 Основные средства для работы с процессами в [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] — это диалоговое окно **Присоединение к процессу** , окно **процессов** и панель инструментов " **Расположение отладки** ". Основными средствами для отладки потоков являются окно **потоки** , маркеры потоков в окнах исходного кода и панель инструментов **место отладки** .  
  
 Основные средства для отладки многопоточных приложений — это **Параллельные стеки** и **Параллельные задачи**, параллельные **часы**и **потоки GPU** .  
