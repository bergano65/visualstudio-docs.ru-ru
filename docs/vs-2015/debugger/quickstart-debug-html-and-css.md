...
- [Проверка действующей модели DOM](#InspectingDOM)  
...
- [Отладка элемента управления WebView](../debugger/debug-a-webview-control.md)  
  ...
  Сведения о других функциях отладки JavaScript, таких как использование окна консоли JavaScript и задание точек останова, см. в [кратком руководстве по отладка JavaScript](../debugger/quickstart-debug-javascript-using-the-console.md) и статье об [отладке приложений в Visual Studio](../debugger/debug-store-apps-in-visual-studio.md).  
...
2. Выберите **JavaScript**  >  **магазин**JavaScript, выберите **приложения Windows** или **Windows Phone приложения**, а затем выберите **пустое приложение**.  
...
6. Замените код в файле default.js следующим кодом:  
   ...
   ```  
    ...
    ![Приложение FlipView с отображением ожидаемых результатов](../debugger/media/js-dom-appfixed.png "JS_DOM_AppFixed")  
...
13. Откройте правило CSS для высоты. Обратите внимание на наличие встроенного стиля со значением 100 пикс., который не соответствует значению высоты 100 %, установленному для селектора CSS `#fView` . Зачеркнутый текст для селектора `#fView` означает, что встроенный стиль обладает приоритетом перед этим стилем.  
     ...
     ![Вкладка "Вычисляемые" в проводнике DOM](../debugger/media/js-dom-explorer-computed.png "JS_DOM_Explorer_Computed")  
...
20. Чтобы исследовать его, переключитесь в Visual Studio и перейдите на вкладку **Макет** , чтобы просмотреть модель поля элемента.  
    ...
    - Для эмулятора телефона: 100 пикс. (смещение) и 100 пикс. (поле).  
      ...
      ![Вкладка "Макет" проводника DOM](../debugger/media/js-dom-explorer-layout.png "JS_DOM_Explorer_Layout")  
...
3. Нажмите кнопку **Обновить приложение Windows** на панели инструментов "Отладка" (или нажмите F4). Эта кнопка выглядит следующим образом: ![Кнопка обновления приложения Windows](../debugger/media/js-refresh.png "JS_Refresh").  
...
- с помощью кнопки `select` , входящей в [JavaScript Console commands](../debugger/javascript-console-commands.md).  
  ...
  ![Кнопка выбора элемента в проводнике DOM](../debugger/media/js-dom-select-element-button.png "JS_DOM_Select_Element_Button")  
  ...
  ![Выделенная кнопка "Отобразить веб-страницу"](../debugger/media/js-dom-display-highlights-button.png "JS_DOM_Display_Highlights_Button")  
...
> В эмуляторе Windows Phone выделение элементов с помощью наведения поддерживается лишь частично.  
 ...
 Пример, демонстрирующий выбор элементов с помощью кнопки **Выбор элемента** , см. в разделе [Отладка стилей CSS с помощью проводника DOM](../debugger/debug-css-styles-using-dom-explorer.md).  
