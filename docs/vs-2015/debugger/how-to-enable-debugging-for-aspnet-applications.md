...
> [!NOTE]  
> Отображаемые диалоговые окна и команды меню могут отличаться от описанных в справке в зависимости от текущих параметров или выпуска. Чтобы изменить параметры, выберите в меню **Сервис** пункт **Импорт и экспорт параметров** . Дополнительные сведения см. [в разделе Настройка параметров разработки в Visual Studio](/previous-versions/zbhkx167(v=vs.140)).  
...
Например, можно указать `debug="true"` в `www.microsoft.com/aaa/Web.config` , а любое приложение в папке AAA или в любой вложенной папке AAA будет наследовать этот параметр. Поэтому, если [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] приложение находится в `www.microsoft.com/aaa/bbb` , оно будет наследовать этот параметр, как [!INCLUDE[vstecasp](../includes/vstecasp-md.md)] и любые приложения в `www.microsoft.com/aaa/ccc` , `www.microsoft.com/aaa/ddd` и т. д. Единственное исключение — случай, когда одно из приложений переопределяет настройку посредством собственного файла Web.config, находящегося ниже по иерархии.  
