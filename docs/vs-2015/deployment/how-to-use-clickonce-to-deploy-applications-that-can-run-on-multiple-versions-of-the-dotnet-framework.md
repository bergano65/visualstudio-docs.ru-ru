---
title: Как использовать ClickOnce для развертывания приложений, которые могут выполняться в нескольких версиях .NET Framework | Документация Майкрософт
ms.date: 11/15/2016
...
> [!NOTE]
> Перед изменением приложения для использования нескольких версий .NET Framework следует убедиться, что приложение работает с несколькими версиями .NET Framework. Общая языковая среда выполнения отличается от версии до [!INCLUDE[net_v40_short](../includes/net-v40-short-md.md)] .NET Framework 2,0, .NET Framework 3,0 и .NET Framework 3,5.  
...
3. Измените номер версии `<assemblyIdentity>` элемента для Microsoft. Windows. коммонлангуажерунтиме на номер версии для .NET Framework, который является наименьшим общим знаменателем. Например, если приложение предназначено для .NET Framework 3,5 и [!INCLUDE[net_v40_short](../includes/net-v40-short-md.md)] , используйте номер версии 2.0.50727.0 и XML-код должен выглядеть следующим образом:  
...
- Обновите и повторно подпишите манифесты приложения и развертывания. Для получения дополнительной информации см. [How to: Re-sign Application and Deployment Manifests](../deployment/how-to-re-sign-application-and-deployment-manifests.md).  
