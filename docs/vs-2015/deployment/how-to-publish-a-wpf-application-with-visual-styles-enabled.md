---
title: Как опубликовать приложение WPF с включенными стилями оформления | Документация Майкрософт
ms.date: 11/15/2016
...
## <a name="publish-the-solution-without-visual-styles-enabled"></a><a name="BKMK_publishsolwovs"></a> Публикация решения без включенных стилей оформления
...
1. Убедитесь, что в проекте не включены визуальные стили. Сначала проверьте файл манифеста проекта на наличие следующего XML-кода. Затем, если имеется XML-код, заключите его в тег комментария.
    ...
    **Открытие файла манифеста в проекте Visual Basic**
    ...
    1. В строке меню выберите **проект**,**Свойства** _имяПроекта_, где *имя_проекта* — это имя проекта WPF.
    ...
    **Открытие файла манифеста в проекте C#**
    ...
    3. В **Обозреватель решений**выберите пункт " **отобразить все файлы** " ().
...
## <a name="create-a-manifest-file"></a><a name="BKMK_CreateManifest"></a> Создание файла манифеста
...
## <a name="embed-the-manifest-file-into-the-executable-file-of-the-published-solution"></a><a name="BKMK_embedmanifest"></a> Внедрение файла манифеста в исполняемый файл опубликованного решения

1. Откройте **командную строку Visual Studio**.

    Дополнительные сведения о том, как открыть **командную строку Visual Studio**, см. в разделе [Командные запросы](https://msdn.microsoft.com/library/94fcf524-9045-4993-bfb2-e2d8bad44219).
...
## <a name="sign-the-application-and-deployment-manifests"></a><a name="BKMK_signappdeplyman"></a> Подписывание манифестов приложения и развертывания

1. В командной строке выполните следующую команду, чтобы удалить `.deploy` расширение из исполняемого файла в текущем каталоге.
   ...
   > [!NOTE]
   > В этом примере предполагается, что только один файл имеет `.deploy` расширение. Убедитесь, что все файлы в этом каталоге переименованы, имеющие `.deploy` расширение файла.
...
3. В командной строке выполните следующую команду, чтобы добавить `.deploy` расширение к имени файла, переименованного на предыдущем шаге этой процедуры.
   ...
   > [!NOTE]
   > В этом примере предполагается, что расширение файла имеет только один файл `.deploy` . Убедитесь, что переименованы все файлы в этом каталоге, для которых ранее было было `.deploy` расширение имени файла.
...
[Устранение конкретных ошибок в развертываниях ClickOnce](../deployment/troubleshooting-specific-errors-in-clickonce-deployments.md) 
 [Общие сведения о](https://msdn.microsoft.com/5b5d7bb6-684f-478d-bf5f-b8d18bbcff2e) 
 визуальных стилях [Командные приглашения](https://msdn.microsoft.com/library/94fcf524-9045-4993-bfb2-e2d8bad44219)
