...
|`HomeSite`|Необязательный параметр. Расположение пакета на удаленном сервере, если он не входит в состав установщика.|  
|`CopyOnBuild`|Необязательный параметр. Указывает, следует ли загрузчик скопируйте файл пакета на диск во время сборки. Значение по умолчанию — true.|  
