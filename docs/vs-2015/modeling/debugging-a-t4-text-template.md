...
|-------------------|-----------------|--------------|
|Не удалось загрузить базовый класс "{0}", от которого наследуется класс преобразования.|Возникает, если не удается найти базовый класс, указанный в параметре `inherits` в директиве template. Сообщение содержит номер строки директивы шаблона.|Убедитесь, что указанный класс существует и что сборка, в которой он существует, указана в директиве Assembly.|
...
|При инициализации объекта преобразования были сформированы ошибки. Преобразование не будет выполняться.|Происходит при сбое "Initialize ()" класса преобразования или возвращении значения false.|Код в функции Initialize () берется из базового класса преобразования, указанного в директиве \< # @template # > и из обработчиков директив. Ошибка, вызвавшая сбой инициализации, вероятно, находится в списке ошибок. Выясните причину сбоя. Вы можете просмотреть фактически созданный код для Initialize (), выполнив процедуры для отладки шаблона.|
...
|---------------------|-----------------|--------------|
|При загрузке включаемого файла "{0}" возвращена пустая или неопределенная строка.|Происходит, если включенный файл текстового шаблона пуст. Сообщение содержит имя файла включаемого файла.|Удалите директиву include или убедитесь, что файл имеет некоторое содержимое.|
...
|В директиве template указано недопустимое значение отладки "{0}". Значение отладки должно быть либо "true", либо "false". Будет использоваться значение по умолчанию "false".|Происходит при неправильном указании параметра `debug` в директиве шаблона. Сообщение содержит номер строки директивы шаблона.|Задайте для параметра отладки значение "true" или "false".|
|В директиве template указано недопустимое значение HostSpecific "{0}". Значение HostSpecific должно быть либо "true", либо "false". Будет использоваться значение по умолчанию "false".|Происходит, когда параметр конкретного узла в директиве `template` указан неправильно. Сообщение содержит номер строки директивы шаблона.|Задайте для параметра конкретного узла значение "true" или "false".|
|В директиве template указан недопустимый язык "{0}". Язык должен быть либо "C#", либо "VB". Будет использоваться значение по умолчанию "C#".|Происходит, когда в директиве `template` указан неподдерживаемый язык. РазрешеныC#только "" или "VB" (без учета регистра). Сообщение содержит номер строки директивы шаблона.|Присвойте параметру `language` в директиве template значениеC#«» или «VB».|
|В шаблоне найдено несколько выходных директив. Все, кроме первого, будет игнорироваться.|Происходит при указании нескольких директив `output` в файле шаблона. Сообщение содержит номер строки для повторяющейся директивы Output.|Удалите дублирующиеся директивы `output`.|
|В шаблоне найдено несколько директив шаблона. Все, кроме первого, будет игнорироваться. Несколько параметров директивы template должны быть указаны в одной директиве шаблона.|Происходит при указании нескольких директив `template` в файле текстового шаблона (включая включенные файлы). Сообщение содержит номер строки для директивы повторяющегося шаблона.|Объединить различные директивы `template` в одну директиву `template`.|
|Не указан процессор для директивы с именем "{0}". Директива будет пропущена.|Происходит при указании директивы `custom`, но не предоставляет атрибут `processor`. Сообщение содержит имя директивы и номер строки.|Укажите `processor` атрибут с именем процессора `directive` для директивы.|
|Не удалось найти процессор с именем "{0}" для директивы с именем "{1}". Директива будет пропущена.|Возникает, когда системе не удается найти процессор `directive`, указанный в директиве `custom`. Сообщение содержит имя директивы, имя процессора и номер строки директивы.|Присвойте атрибуту `processor` в директиве имя обработчика директив.|
...
|Возникла проблема при загрузке сборки "{0}"|Происходит при возникновении проблемы при загрузке сборки. Сообщение содержит имя сборки.|Можно указать сборки для загрузки в директивах \< @ #assembly # > и процессорах директив. Сообщение об ошибке, следующее за этой строкой, должно предоставить дополнительные данные о том, почему не удалось загрузить сборку.|
