...
|"Слишком сложный выражение" при обработке шаблона времени разработки или компиляции шаблона среды выполнения (предварительно обработанного).<br /><br /> [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] прекращает работу при попытке проверки кода, созданного шаблоном среды выполнения.|Слишком длинный текстовый блок. T4 преобразует текстовые блоки в выражение объединения строк с одним строковым литералом для каждой строки шаблона. Слишком длинные текстовые блоки могут переделать ограничения на размер компилятора.|Разбейте блок Long Text блоком выражения, например:<br /><br /> `<#= "" #>`|
