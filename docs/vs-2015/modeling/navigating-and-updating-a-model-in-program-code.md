...
## <a name="in-this-topic"></a>В этом разделе
 ...
 Фигуры, соединители и схемы, а также их связи с элементами модели описаны в отдельном разделе. Дополнительные сведения см. в разделе [Практическое руководство. Перемещение по схеме](../misc/how-to-navigate-and-update-a-diagram.md)и ее обновление.
...
### <a name="properties"></a>Свойства
 ...
 Если в определении DSL вычисляется **тип** свойства, его нельзязадать. Дополнительные сведения см. в разделе [вычисляемые и пользовательские свойства хранилища](../modeling/calculated-and-custom-storage-properties.md).
 ...
 **Елементлинкс**. Связь также представлена элементом модели с именем *Link*, который является экземпляром типа отношения домен. Ссылка всегда имеет один исходный элемент и один целевой элемент. Исходный элемент и целевой элемент могут быть одинаковыми.
 ...
 По умолчанию ни один экземпляр связи не может связать любую пару элементов модели. Но если в определении `Allow Duplicates` DSL флаг имеет значение true для связи, то может существовать несколько ссылок, и необходимо использовать: `GetLinks`
...
## <a name="transaction"></a>Выполнение изменений внутри транзакции
 ...
 Наиболее удобный способ управления транзакцией заключается в использовании `using` оператора, заключенного `try...catch` в инструкцию:
...
```
 ...
 Чтобы изменения были постоянными, `Commit` необходимо выполнить транзакцию до ее удаления. Если возникает исключение, которое не перехватывается внутри транзакции, хранилище будет сброшено до состояния до внесения изменений.
...
- Элементы в ролях, для которых `PropagatesDelete` флаг имеет значение true. Например, фигура, которая отображает элемент, будет удалена.
  ...
  По умолчанию каждое отношение внедрения имеет `PropagatesDelete` значение true в целевой роли. Удаление `henry` не приводит к `familyTree`удалению, `familyTree.Delete()` но приведет к `Persons`удалению всех. Дополнительные сведения см. в разделе [Настройка поведения при удалении](../modeling/customizing-deletion-behavior.md).
  ...
  (В этом конкретном примере это не повлияет `PropagatesDelete` , `false` поскольку `ParentsHaveChildren` для ролей связи.)
...
## <a name="deletelinks"></a>Удаление связей связей
 ...
 Если роль имеет 0.. 1 или 1.. 1 Кратность, можно задать для `null`нее или другое значение:
...
```
 ...
 `Merge ()`может принимать `PresentationElement` значение `ModelElement`или. Если вы присвоить `PresentationElement`ему значение, можно также указать расположение на целевой диаграмме в качестве третьего параметра.
...
- Следит за всеми директивами слияния элементов, указанными в определении DSL.
  ...
  В этом примере создается фигура в позиции мыши, когда пользователь дважды щелкает диаграмму. В определении DSL для этого образца `FillColor` было предоставлено свойство объекта. `ExampleShape`
...
```
 ...
 Если вы предоставляете более одной фигуры, задайте их относительное положение с `AbsoluteBounds`помощью.
...
## <a name="see-also"></a>См. также
 <xref:Microsoft.VisualStudio.Modeling.ModelElement>[Проверка на предметно-ориентированном языке](../modeling/validation-in-a-domain-specific-language.md) [Создание кода на основе предметно-ориентированного языка](../modeling/generating-code-from-a-domain-specific-language.md) [Как: Использование транзакций для обновления моделей интеграции](../modeling/how-to-use-transactions-to-update-the-model.md) моделей с [помощью Visual Studio ModelBus](../modeling/integrating-models-by-using-visual-studio-modelbus.md) [для реагирования на изменения и распространения изменений](../modeling/responding-to-and-propagating-changes.md)
