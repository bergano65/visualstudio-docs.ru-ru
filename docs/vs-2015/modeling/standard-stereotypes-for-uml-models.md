...
### <a name="uml-standard-profile-l2"></a><a name="L2"></a> Стандартный профиль UML L2
...
|Стереотип|Применяется к|Значение|
|----------------|----------------|-------------|
...
|вызывает|Зависимость|Класс клиента вызывает операции поставщика.|
|create|Зависимость|Класс клиента создает экземпляры поставщика.|
|create|Сообщение|Отправитель создает приемник.|
|create|Операция|Эта операция является конструктором.|
...
|Отправить|Зависимость|Исходная операция отправляет целевой сигнал.|
|служба|Компонент|Компонент без состояния.|
|source|Артефакт|Компилируемый "файл".|
|спецификация|Класс, интерфейс, компонент|Определяет поведение компонента или объекта без определения его внутренней работы.|
...
|энергетическая или коммунальная компания|Класс|Коллекция статических функций. Класс не имеет экземпляров.|

### <a name="uml-standard-profile-l3"></a><a name="L3"></a> Стандартный профиль UML L3
...
|Стереотип|Применяется к|Описание|
|----------------|----------------|-----------------|
...
## <a name="c-profile"></a><a name="NetProfile"></a> Профиль C#
...
|Стереотип|Применяется к|Свойства|
|----------------|----------------|----------------|
|**Класс C#**|Класс UML<br /><br /> Компонент|**Атрибуты среды CLR**<br /><br /> **Является разделенным**<br /><br /> **Запечатан**<br /><br /> **Является статическим**<br /><br /> **Является небезопасным**<br /><br /> **Видимость пакета**|
|**C# struct**|Класс UML<br /><br /> Компонент|**Атрибуты среды CLR**<br /><br /> **Является разделенным**<br /><br /> **Является небезопасным**<br /><br /> **Видимость пакета**|
|**Глобальные члены C#**|Класс UML<br /><br /> Компонент|**Атрибуты среды CLR**|
|**Интерфейс C#**|Интерфейс UML|**Атрибуты среды CLR**<br /><br /> **Является разделенным**<br /><br /> **Видимость пакета**|
|**C# enum**|UML-перечисление|**ClrAttributes**<br /><br /> **Базовый тип**|
