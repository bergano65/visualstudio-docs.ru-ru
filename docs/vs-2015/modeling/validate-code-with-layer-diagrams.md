...
- Решение, включающее проект моделирования со схемой слоев. Эта схема слоев должна быть связана с артефактами в проектах Visual C# .NET или Visual Basic .NET, которые необходимо проверить. См. раздел [Создание схем слоев из кода](../modeling/create-layer-diagrams-from-your-code.md).  
  ...
  Можно проверить код вручную из открытой схемы слоев в Visual Studio или из командной строки. Также код можно проверить автоматически при выполнении локальных сборок или Team Foundation Build. См [. видео Channel 9: Разработка и проверка архитектуры с помощью схем](http://go.microsoft.com/fwlink/?LinkID=252073)слоев.  
...
|----------------------|---------------------------|  
|AV0001: Недопустимая зависимость: *Артефакт1* (*ArtifactType1*) — > *Артефакт2*(*ArtifactType2*)<br /><br /> Верхн *LayerName1*, *LayerName2* &#124; зависимости: *депенденцитипе*|Для функции *Артефакт1* в *LayerName1* не должно быть зависимости от *Артефакт2* в *LayerName2* , так как *LayerName1* не имеет прямой зависимости от *LayerName2*.|  
|AV1001: Недопустимое пространство имен: *Артефакта*<br /><br /> Уровневые *Имя_слоя* &#124; Требуемое пространство имен: *NamespaceName1* &#124; Текущее пространство имен: *NamespaceName2*|Для *Имя_слоя* необходимо, чтобы связанные с ним артефакты принадлежали *NamespaceName1*. *Артефакт* находится в *NamespaceName2*, а не *NamespaceName1*.|  
|AV1002: Зависит от запрещенного пространства имен: *Артефакт1* (*ArtifactType1*) &#124; *Артефакт2*(*ArtifactType2*)<br /><br /> Уровневые *Имя_слоя* &#124; Запрещенное пространство имен: Самое некоторая &#124; Зависимости: *депенденцитипе*|Для *Имя_слоя* необходимо, чтобы связанные с ним артефактыне зависели от самого себя. В системе *Артефакт1* не может зависеть от *Артефакт2* , поскольку *Артефакт2* находится в некотором ходе.|  
|AV1003: В запрещенном пространстве имен: *Артефакт* (*ArtifactType*)<br /><br /> Уровневые *Имя_слоя* &#124; Запрещенное пространство имен: *NamespaceName*|Для *Имя_слоя* требуется, чтобы связанные с ним артефакты не применялись к этому. *Артефакт* принадлежит к«самое самое».|  
...
|AV9001: При анализе архитектуры обнаружены внутренние ошибки. Результат может быть неполным. Для получения дополнительных сведений см. подробный журнал событий построения или окно вывода.|Для получения дополнительных сведений см. журнал событий сборки или окно вывода.|  
