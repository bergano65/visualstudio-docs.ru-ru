...
## <a name="validate-code-automatically"></a><a name="ValidateAuto"></a> Автоматически проверять код
...
## <a name="troubleshoot-layer-validation-issues"></a><a name="TroubleshootingValidation"></a> Устранение проблем при проверке слоев
...
## <a name="understanding-and-resolving-layer-validation-errors"></a><a name="UnderstandingValidationErrors"></a> Основные сведения и разрешение ошибок проверки слоев
...
|*DependencyType*|Тип отношения зависимости между *Артефакт1* и *Артефакт2*. Например, метод *Артефакт1* имеет связь **Calls** с *Артефакт2*.|

|**Синтаксис ошибки**|**Описание ошибки**|
|----------------------|---------------------------|
|AV0001: Недопустимая зависимость: *Артефакт1*(*ArtifactType1*) — > *Артефакт2*(*ArtifactType2*)<br /><br /> Слои: *LayerName1*, *LayerName2* &#124; зависимости: *депенденцитипе*|Для функции *Артефакт1* в *LayerName1* не должно быть зависимости от *Артефакт2* в *LayerName2* , так как *LayerName1* не имеет прямой зависимости от *LayerName2*.|
|AV1001: Недопустимое пространство имен: *артефакт*<br /><br /> Слой: *имя_слоя* &#124; требуемое пространство имен: *NamespaceName1* &#124; текущее пространство имен: *NamespaceName2*|Для *Имя_слоя* необходимо, чтобы связанные с ним артефакты принадлежали *NamespaceName1*. *Артефакт* находится в *NamespaceName2*, а не *NamespaceName1*.|
|AV1002: зависит от запрещенного пространства имен: *Артефакт1*(*ArtifactType1*) &#124; *Артефакт2*(*ArtifactType2*)<br /><br /> Слой: *имя_слоя* &#124; Запрещенное пространство имен: *более &#124;ные зависимости:* *депенденцитипе*|Для *Имя_слоя* необходимо, чтобы связанные с ним артефакты не зависели *от самого себя.* В системе *Артефакт1* не может зависеть от *Артефакт2* , поскольку *Артефакт2* находится в некотором *ходе.*|
|AV1003: в запрещенном пространстве имен: *артефакт*(*ArtifactType*)<br /><br /> Слой: *имя_слоя* &#124; Запрещенное пространство *имен: самое* переданное|Для *Имя_слоя* требуется, чтобы *связанные с ним*артефакты не применялись к этому. *Артефакт* принадлежит к *«самое*самое».|
