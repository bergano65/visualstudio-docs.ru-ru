...
3. Назначьте проект VSIX запускаемым проектом решения.
    ...
    - В обозреватель решений в контекстном меню проекта VSIX выберите **Назначить запускаемым проектом**.
...
1. Нажмите клавишу **F5**или выберите команду **Начать отладку** в меню **Отладка**.
    ...
    - В обозревателе решений в контекстном меню запускаемого или единственного проекта выберите пункт **Свойства**. В редакторе свойств проекта перейдите на вкладку **Отладка** . Убедитесь, что строка в поле **Запуск внешней программы** является полным путем [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , как правило:
...
- `elementsWithError` определяет элементы в модели. При двойном щелчке по отчету об ошибках выбирается фигура, представляющая этот элемент.
  ...
  `LogError(),``LogWarning()`и `LogMessage()` размещайте сообщения в разных разделах списка ошибок.
...
1. На компьютере найдите файл **.vsix** , который был создан проектом VSIX.
    ...
    2. Нахождение файла **bin \\ \* \\ **_йоурпрожект_**. VSIX**
