...
## <a name="Executing"></a>Исполнение ограничения проверки
...
6. Дважды щелкните отчет об ошибках. Если на экране отображаются элементы, упомянутые в отчете, они будут выделены.
    ...
    - `ValidationCategories.Menu` включен в аргумент для атрибута `ValidationMethod` и состоит из других значений с помощью логического или (&#124;).
...
- `elementsWithError` определяет элементы в модели. При двойном щелчке по отчету об ошибках выбирается фигура, представляющая этот элемент.
  ...
  `LogError(),` `LogWarning()` и `LogMessage()` размещения сообщений в разных разделах списка ошибок.
...
## <a name="how-validation-methods-are-applied"></a>Применение методов проверки
 ...
 Иерархия типов обобщена в [типах элементов модели UML](../modeling/uml-model-element-types.md).
...
### <a name="ContextCache"></a>Координация нескольких проверок
