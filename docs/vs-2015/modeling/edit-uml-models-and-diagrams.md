 ...
 **Каждый проект моделирования имеет один стек изменений.** Все вносимые в модель и схемы изменения хранятся в этом стеке. Стек также включает в себя изменения фокуса с одной схемы на другую. Команда отмены отменяет изменения в этом стеке.  
 ...
 Например предположим, что выполняются следующие операции: Измените Diagram1; Перевод фокуса на схеме 2; Изменение схемы 2. При отмене изменений первая команда отмены отменит последнее изменение, следующая вернет фокус на схему 1, а третья отменит изменение схемы 1.  
