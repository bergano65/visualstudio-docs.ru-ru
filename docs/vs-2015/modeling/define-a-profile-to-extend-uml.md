...
## <a name="how-to-define-a-profile"></a><a name="DefineProfile"></a> Определение профиля
...
## <a name="how-to-add-a-profile-to-a-visual-studio-extension"></a><a name="AddProfile"></a> Добавление профиля в расширение Visual Studio
 Чтобы установить профиль и разрешить его отправку другим пользователям, нужно добавить профиль в расширение Visual Studio. Дополнительные сведения см. в разделе [развертывание расширений Visual Studio](https://msdn.microsoft.com/library/dd393694(VS.100).aspx).
...
1. Создайте проект расширения Visual Studio.
   ...
   2. В диалоговом окне **Новый проект** в разделе **Установленные шаблоны**разверните узел **Visual C#**, щелкните **расширяемость**, а затем щелкните **проект VSIX**. Задайте имя проекта и нажмите кнопку **ОК**.
...
5. На странице **активы** добавьте строку, описывающую профиль:
   ...
   - Нажмите кнопку **Создать**. Задайте поля в диалоговом окне **Добавление нового ресурса** следующим образом.
...
8. **Извлечение файла VSIX для развертывания**
   ...
   2. Чтобы установить расширение, сделайте следующее.
...
1. Создайте каталог Windows, содержащий следующие три файла:
    ...
    - `[Content_Types].xml` — Введите это имя, как показано здесь, с квадратными скобками
...
## <a name="how-to-define-localized-profiles"></a><a name="Localized"></a> Определение локализованных профилей
...
4. Добавьте копию профиля по умолчанию в новый каталог. Не изменяйте имя файла.
     ...
     Пример [!INCLUDE[vs_current_short](../includes/vs-current-short-md.md)] папки расширения перед ее построением или сжатием в `.vsix` файл будет содержать следующие папки и файлы:
...
## <a name="the-structure-of-a-profile"></a><a name="Schema"></a> Структура профиля
...
- `<propertyTypes>` — список типов, используемых для свойств, определенных в разделе стереотипы.

- `<metaclasses>` — список типов элементов модели, к которым применяются стереотипы в этом профиле, например Икласс, Иинтерфаце, IOperation, Идепенденци.
...
#### <a name="property-types"></a>Типы свойств
 В `<propertyTypes>` разделе объявляется список типов, используемых для свойств в `<stereotypes>` разделе. Существует две разновидности типов свойств: внешние типы и перечисление.
...
> Имя моникера должно начинаться с `/yourProfileName/`, где `yourProfileName` определяется в атрибуте `name` профиля (CSharpProfile в этом примере). Моникер заканчивается именем одной из записей в разделе метаклассов.
 ...
 В каждом стереотипе можно перечислить сколько угодно свойств (или ни одного), которые он добавляет в любой элемент модели, к которому применяется. Объект `<propertyType>` содержит ссылку на один из типов, определенных в `<propertyTypes>` разделе. Это должна быть ссылка `<externalTypeMoniker>`, указывающая на `<externalType>,`, или `<enumerationTypeMoniker>`, указывающая на `<enumerationType>`. Ссылка начинается с имени профиля.
...
## <a name="model-element-types"></a><a name="Elements"></a> Типы элементов модели
...
5. Выполните повторную сборку файла VSIX и откройте его в проводнике для повторной установки. Перезапустите [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
   ...
   - *LocalAppData* обычно находится в C:\Users \\ *username*\аппдата\локал\
