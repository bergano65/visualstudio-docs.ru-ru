...
ms.locfileid: "75851506"
---
# <a name="integrating-models-by-using-visual-studio-modelbus"></a>Интеграция моделей с помощью Visual Studio Modelbus
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

[!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus предоставляет метод для создания ссылок между моделями и из других средств в модели. Например, можно связать модели доменного языка (DSL) с моделями UML или создать интегрированный набор DSL.
...
### <a name="making-sure-that-elements-can-be-referenced"></a>Проверка возможности ссылки на элементы
 [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] В адаптерах ModelBus по умолчанию используется идентификатор GUID элемента. Это значит, что такие идентификаторы должны храниться в файле модели.
...
3. В проект "Отладка" в экспериментальном экземпляре [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] добавьте файлы, являющиеся экземплярами каждого DSL.
    ...
    > [!NOTE]
    > [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] ModelBus может разрешать ссылки только на модели, которые являются элементами одного [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] решения. Например, создать ссылку на файл модели, расположенный в другой части файловой системы, нельзя.
