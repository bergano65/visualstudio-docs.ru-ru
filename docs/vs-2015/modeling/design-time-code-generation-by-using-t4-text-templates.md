...
2. Сохраните TT-файл и снова проверьте созданный TXT-файл. В нем перечисляются квадратные корни чисел от 0 до 10.
   ...
   Если вы записываете созданный код в [!INCLUDE[vbprvb](../includes/vbprvb-md.md)], директива `template` должна содержать атрибут `language="VB"`. `"C#"` используется по умолчанию.
