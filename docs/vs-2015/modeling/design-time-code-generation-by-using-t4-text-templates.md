...
1. Создайте проект [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] или откройте существующий.
...
## <a name="Converting"></a>Преобразование существующего файла в шаблон
 Полезной возможностью шаблонов является то, что они очень похожи на создаваемые им файлы и включают некоторую часть программного кода. Это предполагает удобный способ создания шаблона. Сначала создайте обычный файл в качестве прототипа, например файл [!INCLUDE[csprcs](../includes/csprcs-md.md)], а затем постепенно попытайтесь ввести код поколения, который изменяет получившийся файл.
...
4. Проверьте следующие свойства файла **. TT** :
    ...
    |||
    |-|-|
    |**Настраиваемое средство =**|**тексттемплатингфилеженератор**|
    |**Действие сборки =**|**None**|
