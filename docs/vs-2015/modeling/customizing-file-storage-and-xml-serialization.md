...
1. Убедитесь, что **ключ является ключом моникера** `false` для каждого свойства домена в классе и его базовых классах.
    ...
    1. В обозревателе DSL разверните узел **XML-сериализация бехавиор\класс \\ данные** _\<the domain class>_ **\елемент**.
...
- Set **является ключом моникера** для свойства предметной области существующего доменного класса. Свойство должно иметь тип `string` .
    ...
    1. В обозревателе DSL разверните узел **XML-сериализация бехавиор\класс \\ данные** _\<the domain class>_ **\елемент**, а затем выберите свойство Domain.
...
- Задайте **Representation**  =  для**элемента** представления свойство домена, сохраненное как элемент, а не как значение атрибута.
