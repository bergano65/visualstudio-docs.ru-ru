...
API неизменности [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] пакета SDK визуализации и моделирования позволяет программе заблокировать часть или всю модель доменного языка (DSL), чтобы ее можно было читать, но не изменять. Этот параметр доступен только для чтения, например, чтобы пользователь мог попросить коллег закомментировать и проверить модель DSL, но может запретить им изменять исходное значение.
...
> Политику блокировки можно обойти с помощью отражения. Она предоставляет четкие границы для сторонних разработчиков, но не обеспечивает надежную защиту.
 ...
 Дополнительные сведения можно найти на [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] веб-сайте [SDK визуализации и моделирования](https://www.microsoft.com/download/details.aspx?id=48148) .
