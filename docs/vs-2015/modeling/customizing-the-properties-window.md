...
- Свойства домена, определенные в доменном классе элемента модели, включая те, которые определены в базовых классах. Исключением являются свойства домена, для которых вы настроили возможность **просмотра для `False`** .
...
1. Создайте [!INCLUDE[dsl](../includes/dsl-md.md)] решение, которое содержит по крайней мере два класса, которые в этом примере называются **Book** и **Author**. Между **книгой** и **автором**должна быть связь любого вида.
...
3. Добавьте свойство домена в доменный класс обычным способом.
   ...
   На этом этапе пользователи могут просматривать значения свойства, но не могут изменять их. Отображаемые значения получаются из функции `ToString()`. Можно написать программный код, который задает значение свойства, например в команде или правиле.
...
```
 ...
 Тип `AnEditor` должен быть производным от типа, указанного во втором параметре. Второй параметр должен быть либо <xref:System.Drawing.Design.UITypeEditor>, либо <xref:System.ComponentModel.ComponentEditor>. Для получения дополнительной информации см. <xref:System.ComponentModel.EditorAttribute>.

 Можно указать либо собственный редактор, либо редактор, предоставляемый в [!INCLUDE[dnprdnshort](../includes/dnprdnshort-md.md)], например <xref:System.Windows.Forms.Design.FileNameEditor> или <xref:System.Drawing.Design.ImageEditor>. Например, используйте следующую процедуру, чтобы иметь свойство, в котором пользователь может ввести имя файла.
