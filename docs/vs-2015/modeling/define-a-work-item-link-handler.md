...
1. Создайте проект библиотеки классов, добавив его в существующее решение VSIX или создав новое решение.
    ...
    2. В разделе **Установленные шаблоны**разверните узел **Visual C#** или **Visual Basic**, а затем в среднем столбце щелкните **Библиотека классов**.
...
### <a name="import-the-work-item-namespace"></a>Импорт пространства имен рабочего элемента
 В [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] **ссылки**проекта добавьте ссылки на следующие сборки:
...
1. Нажмите клавишу **F5**или выберите команду **Начать отладку** в меню **Отладка**.
     ...
     **Устранение неполадок**. Если новая [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] версия не запускается, убедитесь, что проект VSIX установлен в качестве запускаемого проекта решения.
...
### <a name="listening-for-new-work-items"></a>Ожидание вызова при создании рабочих элементов
 `OnWorkItemCreated` вызывается, когда пользователь выбирает создание нового рабочего элемента для связывания с элементами модели. Код может инициализировать поля рабочего элемента. Затем рабочий элемент представляется пользователю, который может изменить значения полей и сохранить рабочий элемент. Ссылка на элемент модели не создается, пока рабочий элемент не будет успешно сохранен.
...
### <a name="listening-for-link-removal"></a>Ожидание вызова при удалении ссылок
 `OnWorkItemRemoved` вызывается один раз непосредственно перед удалением каждой удаленной ссылки на рабочий элемент. При удалении элемента модели удаляются все связи.
