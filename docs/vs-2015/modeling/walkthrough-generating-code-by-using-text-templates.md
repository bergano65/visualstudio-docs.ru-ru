...
3. В директиве template файла измените значение атрибута `hostspecific` на `true`.  
    ...
    Это изменение позволит коду шаблона получить доступ к службам [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].  
...
### <a name="accessing-the-visual-studio-api"></a>Доступ к API Visual Studio  
 Задание атрибута `hostspecific` директивы `<#@template#>` позволяет шаблону получить доступ к API [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Благодаря этому шаблон может получить сведения о расположении файлов проекта, чтобы избежать использования абсолютного пути к файлу в коде шаблона.  
...
### <a name="running-the-test-program"></a>Выполнение тестовой программы  
