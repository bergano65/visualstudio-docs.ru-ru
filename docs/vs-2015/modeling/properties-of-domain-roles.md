...
|Распространяет удаление|`True` Удаление элемента, который воспроизводит эту роль при удалении связанной ссылки.|`True` для целевого объекта роли внедрения.<br /><br /> `False` для других ролей.<br /><br /> Дополнительные сведения см. в разделе [Настройка поведения при удалении](../modeling/customizing-deletion-behavior.md).|
