...
ms.locfileid: "72663132"
---
# <a name="customize-code-maps-by-editing-the-dgml-files"></a>Customize code maps by editing the DGML files
[!INCLUDE[vs2017banner](../includes/vs2017banner.md)]

Чтобы настроить карту кода, можно изменить ее файл DGML. Например, можно изменить элементы, чтобы указать пользовательские стили, назначить свойства и категории кодовым точкам и связям либо связать узлы с документы или URL-адреса с кодовыми точками или связями. Дополнительные сведения об элементах DGML см. в разделе [Справочник по языку для направляемых графов (DGML)](../modeling/directed-graph-markup-language-dgml-reference.md).
...
## <a name="OrganizeNodes"></a>Элементы кода группы
...
4. В разделе `<Links>` убедитесь, что элемент `<Link/>` со следующими атрибутами существует для каждой связи между кодовой точкой группы и дочерними кодовыми точками:
   ...
   ```
    ...
    Дополнительные сведения об атрибуте `Category` см. [в разделе Назначение категорий элементам кода и ссылкам](#AssignCategories).

## <a name="ChangeGraphStyle"></a>Изменение стиля схемы
...
## <a name="Highlight"></a>Изменение стиля элементов кода и ссылок
...
1. Если `Coverage` имеет значение > 80, установите для свойства `Background` значение зеленый.

2. В противном случае, если `Coverage` > 50, задайте для свойства `Background` оттенок оранжевый в зависимости от значения свойства `Coverage`.
...
## <a name="AssignProperties"></a>Назначение свойств элементам кода и ссылкам
...
## <a name="AssignCategories"></a>Назначение категорий элементам кода и ссылкам
...
## <a name="AddReferences"></a>Связывание документов или URL-адресов с элементами кода и ссылками
...
- Схему слоев можно связать с кодовым элементом группы, который представляет слой в логической архитектуре программного обеспечения.
...
- Свяжите элемент кода с Team Foundation Server рабочим элементом или ошибкой или другими сведениями, относящимися к элементу кода.
...
3. Выполните одну из задач, представленных в следующей таблице:
   ...
   2. В разделе `<Properties>`:
      ...
      3. Добавьте атрибут `IsReference` и задайте для него значение `True`, чтобы ссылка отображалась в контекстном меню **перехода** к элементу кода.
