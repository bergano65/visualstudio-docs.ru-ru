...
3. Задайте свойство **значок панели элементов** , чтобы оно ссылалось на точечный рисунок 16x16.
     ...
     Если вы хотите определить новый значок, создайте файл точечного рисунка в обозреватель решений в папке **дсл\ресаурцес** Файл должен иметь следующие значения свойств: содержимое **действия сборки**  =  **Content**. **Копировать в выходной каталог**  =  Не **копируйте**.
 ...
 **Подключение входит в тип OutPort вложенного компонента**
 ...
 ![Мастер подключения](../modeling/media/connectionbuilder-3.png "ConnectionBuilder_3")
