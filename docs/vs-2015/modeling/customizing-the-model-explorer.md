...
## <a name="changing-the-tab-icon"></a>Изменение значка вкладки
 Чтобы изменить значок вкладки для обозревателя, используйте значок размером 16x16 пикселей в BMP-файле. Вставьте файл значка в папку \Дслпаккаже\ресаурцес\, а затем измените имя файла на **моделексплорертулвиндовбитмапс. bmp**. Например, можно изменить [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] файл значка Setup. ICO на формат. bmp и переименовать его в **дсллангуаженаме\дслпаккаже\ресаурцес\моделексплорертулвиндовбитмапс.БМП**. Созданный конструктор отобразит этот значок на вкладке обозревателя, когда он закрепляется вместе с **Обозреватель решений**.
