...
## <a name="error-and-warning-methods"></a>Методы error и Warning
 Для добавления сообщений в Список ошибок можно использовать служебные методы ошибок и предупреждений [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] . Например, следующий код добавит сообщение об ошибке в Список ошибок.
...
## <a name="access-to-host-and-service-provider"></a>Доступ к узлу и поставщику услуг
 ...
 Тип `this.Host` зависит от типа узла, в котором выполняется шаблон. В шаблоне, который выполняется в [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] , можно выполнить приведение к, чтобы `this.Host` `IServiceProvider` получить доступ к службам, таким как интегрированная среда разработки. Пример:
