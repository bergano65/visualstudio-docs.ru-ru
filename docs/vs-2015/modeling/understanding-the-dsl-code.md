...
> Шаблон "схемы компонентов" DSL не связан с UML-схемами компонентов, которые можно создать с помощью меню "Архитектура" в Visual Studio. В диалоговом окне **Новый проект** разверните узел **другой проект типес\екстенсибилити** и нажмите кнопку **конструктор предметно-ориентированных языков**.
 ...
 ![Компоненты и взаимосвязанных портов](../modeling/media/componentsample.png "компонентсампле")
...
### <a name="the-code-directories"></a>Каталоги кода
 ...
 Файлы \*. TT содержат очень небольшой код. Вместо него они используют директивы `<#include>`, которые включают общие файлы шаблонов. Общие файлы можно найти в **папке \Program Files\Microsoft Visual Studio 10.0 \ COMMON7\IDE\EXTENSIONS\MICROSOFT\DSL Сдк\дсл Designer\11.0\TextTemplates**
