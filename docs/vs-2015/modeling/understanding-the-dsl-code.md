---
title: Основные сведения о коде доменных ЯЗЫКОВ | Документация Майкрософт
ms.date: 11/15/2016
 ...
 **Dsl** проекта определяет API для вашего DSL. **DslPackage** проекта определяет, как он интегрируется с [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Вы можете также добавить собственные проекты, которые могут содержать код, созданный из модели.  
