...
Windows Forms можно использовать для отображения состояния модели доменного языка (DSL) вместо использования схемы DSL. В этом разделе описано, как привязать форму Windows к DSL с помощью пакета SDK для визуализации и моделирования [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
...
2. Поэкспериментируйте с исходным примером, который предоставляет шаблон:
   ...
   В основном экземпляре [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] Обратите внимание на следующие моменты о решении DSL:
...
- Проект `UI` содержит рабочий пример элемента управления формы, привязанного к DSL. Однако он не будет работать после изменения определения DSL. Проект `UI` содержит:
...
3. Переименуйте класс домена **пространстве ExampleModel** в `Farm`.

     Предоставьте ей дополнительные свойства домена с именем `Size` типа **Int32**и `IsOrganic` типа **Boolean**.
...
6. Используйте средство **наследования** , чтобы сделать `Goat` и `Sheep` наследовать от `Animal`.
...
5. Удалите **фармбиндингнавигатор**. Это также автоматически создается в конструкторе `FarmControl`, но не используется для этого приложения.
...
8. С помощью панели элементов создайте два экземпляра **моделингбиндингсаурце**и назовите их `AnimalBinding` и `FieldBinding`.
...
- Это гарантирует, что когда пользователь выбирает строку, окно свойств отображает свойства соответствующего элемента модели вместо строки сетки данных.
  ...
  ![DslWpf4](../modeling/media/dslwpf4.png "DslWpf4") Схема связей между источниками данных и представлениями.
