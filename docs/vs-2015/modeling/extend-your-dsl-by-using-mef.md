---
title: Расширение DSL с помощью MEF | Документация Майкрософт
ms.date: 11/15/2016
...
6. В окно свойств убедитесь, что по крайней мере одно из свойств с именем **использует...** имеет `true`.
...
4. Создайте проект VSIX в том же решении. Для этого в диалоговом окне **Новый проект** разверните узел **Visual Basic** или **C#Visual**, щелкните **расширяемость**, а затем выберите **проект VSIX**.

5. В обозреватель решений щелкните правой кнопкой мыши проект VSIX и выберите пункт **Назначить запускаемым проектом**.
...
9. Проверьте и обновите другие поля **source. extension. vsixmanifest**. Щелкните **Выбрать выпуски** и убедитесь, что установлены правильные выпуски [!INCLUDE[vsprvs](../includes/vsprvs-md.md)].
...
11. Чтобы проверить расширение, нажмите клавишу **F5**. В экспериментальном экземпляре [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] создать или открыть пример файла DSL.
...
### <a name="gesture-handlers"></a>Обработчики жестов
 Обработчик жестов может обрабатывать объекты, перетаскиваемые на схему из любого места, внутри или снаружи [!INCLUDE[vsprvs](../includes/vsprvs-md.md)]. Следующий пример позволяет пользователю перетаскивать файлы из проводника Windows на схему. Он создает элементы, которые содержат имена файлов.
...
## <a name="see-also"></a>См. также раздел
 [Доставка расширений Visual Studio](../extensibility/shipping-visual-studio-extensions.md) [Managed Extensibility Framework (MEF)](https://msdn.microsoft.com/library/6c61b4ec-c6df-4651-80f1-4854f8b14dde) [как добавить проверку обработчиков перетаскивания](../modeling/how-to-add-a-drag-and-drop-handler.md) [в доменном языке](../modeling/validation-in-a-domain-specific-language.md) .
