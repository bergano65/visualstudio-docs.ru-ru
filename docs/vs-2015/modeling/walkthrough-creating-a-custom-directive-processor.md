...
1. В среде Visual Studio создайте проект библиотеки классов C# или Visual Basic с именем CustomDP.  
    ...
    > [!NOTE]
    > Если требуется установить процессор директив на более чем один компьютер, лучше использовать проект расширения (VSIX) [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] и включить в это расширение pkgdef-файл. Дополнительные сведения см. в разделе [развертывание пользовательского процессора директив](../modeling/deploying-a-custom-directive-processor.md).  
...
## <a name="registering-the-directive-processor"></a>Регистрация процессора директив  
 Прежде чем директиву можно вызывать из текстового шаблона в [!INCLUDE[vsprvs](../includes/vsprvs-md.md)], необходимо добавить раздел реестра для процессора директив.  

> [!NOTE]
> Если требуется установить процессор директив на более чем один компьютер, лучше определить расширение [!INCLUDE[vsprvs](../includes/vsprvs-md.md)] (VSIX), включающее pkgdef-файл и вашу сборку. Дополнительные сведения см. в разделе [развертывание пользовательского процессора директив](../modeling/deploying-a-custom-directive-processor.md).  
...
5. Добавьте новое строковое значение с именем CodeBase и значением, равным пути к файлу CustomDP.dll, созданному ранее в этом пошаговом руководстве.  
   ...
   | CodeBase  | REG_SZ | <strong>\<Путь к решению ></strong>CustomDP\bin\Debug\CustomDP.dll |
...
1. Создайте текстовый файл с именем `DocFile.xml` , используя любой текстовый редактор (например, Блокнота).  
...
4. Замените содержимое файла TestDP.tt следующим текстом.  
    ...
    > [!NOTE]
    > Обязательно замените строки <`YOUR PATH>` с путем к файлу DocFile.xml.  
...
1. В **обозревателе решений**, щелкните файл TestDP.tt правой кнопкой мыши и выберите команду **пользовательское средство**.  
     ...
     Для [!INCLUDE[vbprvb](../includes/vbprvb-md.md)] пользователей, TestDP.txt может не отображаться в **обозревателе решений** по умолчанию. Чтобы отобразить все файлы, назначенные проекту, откройте **проекта** меню и выберите пункт **Показать все файлы**.  
