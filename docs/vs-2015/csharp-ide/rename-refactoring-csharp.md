...
|Компонент|Поведение рефакторинга в интегрированной среде разработки|
|-------------|----------------------------------------|
...
2. В объявлении для `MethodB` введите или BACKSPACE над идентификатором метода. Под этим идентификатором появится запрос смарт-тега.
...
## <a name="renaming-implemented-or-overridden-members"></a>Переименование реализованных или переопределенных членов
 ...
 В предыдущем примере переименование `C.Method()` также переименовывается, `Ibase.Method()` поскольку `C.Method()` реализует `Ibase.Method()` . Затем подсистема рефакторинга рекурсивно видит, что `Ibase.Method()` реализовано `Derived.Method()` и переименовывает `Derived.Method()` . Модуль реструктуризации не переименовывает `Base.Method()` , так как не `Derived.Method()` переопределяет `Base.Method()` . Подсистема рефакторинга останавливается, если в диалоговом окне **Переименование** не установлены **переименованные перегрузки** .

 Если флажок **Переименовать перегрузки** установлен, подсистема реструктуризации переименовывается из- `Derived.Method(int i)` за перегрузки, так как она `Derived.Method()` `Base.Method(int i)` переопределяется `Derived.Method(int i)` и `Base.Method()` является перегрузкой `Base.Method(int i)` .
...
```
 ...
 В приведенном выше коде переименование изменится `ID` `ID` в обеих инструкциях, так как они имеют один и тот же базовый анонимный тип.
...
```
 ...
 В приведенном выше коде переименование `ID` приведет к переименованию только одного экземпляра, `ID` поскольку `companyIDs` `orderIDs` Свойства и не имеют тех же свойств.

## <a name="see-also"></a>См. также:
 Рефакторинг [анонимных типов](https://msdn.microsoft.com/library/59c9d7a4-3b0e-475e-b620-0ab86c088e9b) [(C#)](../csharp-ide/refactoring-csharp.md)