---
title: Добавление заметок к параметрам и возвращаемым значениям функции | Документация Майкрософт
ms.date: 11/15/2016
...
- `_Out_writes_to_(s,c)`  
     ...
     Указатель на массив `s` элементов.  Элементы не обязательно должны быть допустимыми в предварительном состоянии.  В состоянии после состояния элементы вплоть до `c` элемента-th должны быть допустимыми.  Если размер известен в байтах, выполните масштабирование `s` и `c` размер элемента или используйте `_bytes_` вариант, который определен как:  
...
- `_In_reads_to_ptr_(p)`  
     ...
     Указатель на массив, для которого выражение (то `p` `_Curr_` есть `p` минус `_Curr_` ) определяется соответствующим стандартом языка.  Элементы, предшествующие, `p` должны быть допустимыми в предварительном состоянии.  
...
- `_Out_writes_to_ptr_(p)`  
     ...
     Указатель на массив, для которого выражение (то `p` `_Curr_` есть `p` минус `_Curr_` ) определяется соответствующим стандартом языка.  Элементы до не должны `p` быть допустимыми в предварительном состоянии и должны быть допустимы в состоянии после.  
  
- `_Out_writes_to_ptr_z_(p)`  
  
     Указатель на массив, заканчивающийся нулем, для которого выражение `p` `_Curr_` (то есть `p` минус `_Curr_` ) определяется соответствующим стандартом языка.  Элементы до не должны `p` быть допустимыми в предварительном состоянии и должны быть допустимы в состоянии после.  
  
## <a name="optional-pointer-parameters"></a>Необязательные параметры указателя  
 Если заметка параметра указателя включает `_opt_` , то она указывает, что параметр может иметь значение null. В противном случае аннотация выполняется так же, как и версия, которая не включает `_opt_` . Ниже приведен список `_opt_` вариантов заметок к параметрам указателя:  
