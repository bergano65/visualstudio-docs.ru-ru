...
- Убедитесь, что соблюдены минимальные требования к оборудованию и применяются корректные параметры BIOS.
  ...
  4. Если в BIOS имеются следующие параметры, отключите их.
     ...
     - Отключить Trusted Execution
       ...
       Дополнительные сведения см. в статье: TechNet Hyper-V Как устранить ошибки BIOS Включение Hyper-V
...
##  <a name="XamarinPlayer"></a> Эмулятор зависает, так как ему не удалось настроить UDP-порт
 Эта проблема может возникнуть из-за несовместимости с Xamarin Player. Если эмулятор отображается зависание, или если вы видите это сообщение об ошибке «не удалось подключиться к операционной системе устройства эмулятора: Не удалось настроить UDP-порт.  Некоторые функциональные возможности могут быть недоступны". Выполните следующие действия.
