...
3. В форму можно добавить такие элементы управления, как кнопки, флажки и метки. Дважды щелкните значок элемента управления `TableLayoutPanel` на панели элементов. (Или можно перетащить элемент управления с панели элементов на форму.) После этого интегрированная среда разработки добавляет в `TableLayoutPanel` форму элемент управления, как показано на следующем рисунке.
...
10. Выберите задачу **Изменить строки и столбцы**, чтобы открыть окно **Стили столбцов и строк**. Выберите **Столбец1**, убедитесь, что нажата кнопка **Проценты**, установите его размер равным 15 процентам — введите `15` в поле **Проценты**. (Это `NumericUpDown` элемент управления, который будет использоваться в последующем учебнике.) Выберите **Столбец2** и задайте для него значение 85 процентов. Пока не нажимайте кнопку **ОК**, так как окно будет закрыто. (но если это было сделано, его можно открыть повторно с помощью списка задач).
