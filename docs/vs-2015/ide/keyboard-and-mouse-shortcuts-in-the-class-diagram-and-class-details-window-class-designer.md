...
|\<space>|Перемещает курсор к следующему полю, сначала двигаясь слева направо и затем сверху вниз. Если курсор находится в пустом поле, например, в поле \<add parameter>, он перемещается в первое поле следующей строки. Обратите внимание, что \<space>, введенный сразу после запятой, игнорируется.<br /><br /> Если курсор находится в поле «Сводка», ввод пробела добавляет символ пробела.<br /><br /> Если курсор находится в столбце «Скрыть» данной строки, ввод пробела меняет состояние флажка «Скрыть».|
