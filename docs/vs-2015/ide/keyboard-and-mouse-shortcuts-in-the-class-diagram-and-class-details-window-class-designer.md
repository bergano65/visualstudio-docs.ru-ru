...
- [Использование мыши в окно подробного представления класса](../ide/keyboard-and-mouse-shortcuts-in-the-class-diagram-and-class-details-window-class-designer.md#MouseClassDetails)
...
- [Использование клавиатуры в окно подробного представления класса](../ide/keyboard-and-mouse-shortcuts-in-the-class-diagram-and-class-details-window-class-designer.md#KeyboardClassDetails)

## <a name="using-the-mouse-in-class-designer"></a><a name="MouseClassDesigner"></a>Использование мыши в конструктор классов
...
## <a name="using-the-mouse-in-the-class-details-window"></a><a name="MouseClassDetails"></a> Использование мыши в окне "Сведения о классах"
...
## <a name="using-the-keyboard-in-class-designer"></a><a name="KeyboardClassDesigner"></a>Использование клавиатуры в конструктор классов
 В схемах классов поддерживаются следующие действия клавиатуры.

|Клавиши|Контекст|Описание|
|---------|-------------|-----------------|
...
|HOME|Внутри фигур типа|Переход к заголовку фигуры верхнего уровня.|
...
|SPACE|Все контексты|Переключает выбор в текущем элементе.|

## <a name="using-the-keyboard-in-the-class-details-window"></a><a name="KeyboardClassDetails"></a> Использование клавиатуры в окне "Сведения о классах"
...
|Клавиши|Результат|
|-|-|
|, (запятая)|Если курсор находится в строке параметров, введите запятую, чтобы переместить курсор в поле «Имя» следующего параметра. Если курсор находится в последней строке параметра метода, он перемещает курсор в \<add parameter> поле, которое можно использовать для создания нового параметра.<br /><br /> Если курсор находится в другом месте в окне сведений о классе, ввод запятой приводит к добавлению запятой в текущее поле.|
...
|Вкладка|Перемещает курсор к следующему полю, сначала двигаясь слева направо и затем сверху вниз. Если курсор перемещается из поля, в котором введен текст, окно сведений о классе обрабатывает этот текст и сохраняет его, если это не вызывает ошибку.<br /><br /> Если курсор находится в пустом поле, таком как \<add parameter> , вкладка перемещает его в первое поле следующей строки.|
|\<space>|Перемещает курсор к следующему полю, сначала двигаясь слева направо и затем сверху вниз. Если курсор находится в пустом поле \<add parameter> , например, он перемещается в первое поле следующей строки. Обратите внимание, что \<space> тип, указанный сразу после запятой, игнорируется.<br /><br /> Если курсор находится в поле «Сводка», ввод пробела добавляет символ пробела.<br /><br /> Если курсор находится в столбце «Скрыть» данной строки, ввод пробела меняет состояние флажка «Скрыть».|
