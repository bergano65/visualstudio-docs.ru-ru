...
### <a name="update-and-performance-issues"></a>Проблемы с обновлением и производительностью
 Для проектов Visual C++ для отображения изменения в исходном файле на схеме классов может потребоваться от 30 до 60 секунд. Из-за такой задержки конструктор классов может выдать ошибку **В выбранном блоке не найдены типы**. Если появится сообщение об ошибке, например, нажмите кнопку **Отмена** в сообщении об ошибке и дождитесь появления элемента code в представление классов. После этого конструктор классов должен быть в состоянии отобразить этот тип.
...
- Тип находится в библиотеке, на которую ссылается директива #import. В качестве возможного обходного пути можно вручную добавить сформированный код (TLH-файл) в директиву #include в файле заголовка.
  ...
  Наиболее вероятной причиной проблемы с разрешением типа является невозможность **найти код для одной или нескольких фигур в диаграмме классов " \<element> "**. Это сообщение об ошибке не обязательно означает, что код содержит ошибку. Оно указывает лишь на то, что конструктору классов не удалось отобразить код. Попробуйте сделать следующее.
...
### <a name="troubleshooting-other-error-messages"></a>Диагностика других сообщений об ошибках
 На открытых форумах Microsoft Developer Network (MSDN) можно получить помощь в устранении ошибок и предупреждений. См. [Форум по конструкторам классов Visual Studio](https://social.msdn.microsoft.com/Forums/en-US/vsclassdesigner/threads?page=1).

## <a name="limitations-for-c-code-elements"></a><a name="limitations"></a> Ограничения для элементов кода C++
...
## <a name="see-also"></a>См. также:
 [Проектирование и Просмотр классов и типов](../ide/designing-and-viewing-classes-and-types.md) [, работающих с классами и другими типами (конструктор классов)](../ide/working-with-classes-and-other-types-class-designer.md) [Работа с схемами классов (конструктор классов)](../ide/working-with-class-diagrams-class-designer.md) [Конструирование классов и типов (конструктор классов)](../ide/designing-classes-and-types-class-designer.md) [Дополнительные сведения об ошибках конструктор классов](../ide/additional-information-about-class-designer-errors.md) [Visual C++ классов](../ide/visual-cpp-classes-in-class-designer.md) [в конструктор классов Visual C++](../ide/visual-cpp-structures-in-class-designer.md) конструктор классов [в Visual C++ конструктор классов](../ide/visual-cpp-enumerations-in-class-designer.md) [typedefs](../ide/visual-cpp-typedefs-in-class-designer.md) в Visual C++
