...
- Весь тип доступен только для чтения в случаях, когда, например, тип находится в сборке, на которую существует ссылка (см. раздел [Отображение информации только для чтения](http://msdn.microsoft.com/33e2d3a9-1668-4d10-ae56-fa09b3156e0a)). При выборе фигуры в конструкторе классов окно "Сведения о классе" отображает сведения о типе в состоянии только для чтения.  
