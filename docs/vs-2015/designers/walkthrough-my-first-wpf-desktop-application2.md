...
2. Добавьте следующий код XAML между тегами `<Application.Resources>` и `</Application.Resources>` :  
    ...
    - `labelStyle`: Для форматирования `Label` элементов управления.  
  
    - `columnHeaderStyle`: Для форматирования `DataGridColumnHeader`.  
  
    - `listHeaderStyle`: Форматирование заголовка списка `Border` элементов управления.  
  
    - `listHeaderTextStyle`: Форматирование заголовка списка **метка**.  
  
    - `buttonStyle`: Для форматирования `Button` на **ExpenseItHome.xaml** элемента управления.  
