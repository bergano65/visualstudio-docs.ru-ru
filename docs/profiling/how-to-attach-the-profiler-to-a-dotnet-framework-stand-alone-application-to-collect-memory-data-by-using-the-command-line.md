...
В этом разделе описывается, как использовать Средства профилирования [!INCLUDE[vsPreShort](../code-quality/includes/vspreshort_md.md)] из командной строки для подключения профилировщика к запущенному автономному (клиентскому) приложению .NET Framework и сбора данных по использованию памяти.  
...
5.  Присоедините профилировщик к целевому приложению. Тип:  
    ...
    -   **/targetclr:** `Version` указывает версию профилируемой среды CLR, если в приложении загружено несколько версий среды выполнения. Необязательно.  
