...
|**Module**|Имя модуля, содержащего функцию.
...
- `MainPage::GetMaxNumberAsyncButton_Click` просто управляет списком значений задач, вычисляет максимальное значение результатов и отображает выходные данные.
...
- `MainPage::<GetNumberAsync>b__b` показывает действия задач, которые вызывают `GetNumber`.
