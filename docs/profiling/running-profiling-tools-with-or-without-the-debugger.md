...
Средство загрузки ЦП можно запустить на удаленном компьютере с помощью программ командной строки.
...
![Средства диагностики](../profiling/media/diagnostictools-update1.png "Диагностические средства")

- Используйте значок **Параметры** на панели инструментов, чтобы указать, требуется ли просматривать показатели **Использование памяти** или **Загрузка ЦП**.
...
![Фильтр событий диагностики](../profiling/media/diagnosticeventfilter.png "Фильтр событий диагностики")
...
![Поиск событий диагностики](../profiling/media/diagnosticseventsearch.png "Поиск событий диагностики")
...
Кроме запуска приложения из проекта Visual Studio, диагностические сеансы также можно запускать на альтернативных целевых объектах. Например, может потребоваться диагностика проблем производительности для приложения, установленного из Магазина приложений Windows. В Профилировщике производительности в раскрывающемся списке **Изменение целевого объекта** выберите нужный вариант.
...
Можно запускать приложения, которые уже установлены, или подключать средства диагностики к приложениям и процессам, которые уже работают.

Если в качестве целевого объекта анализа выбран **Исполняемый файл**, можно ввести путь к *EXE*-файлу на локальном или удаленном компьютере. В любом случае *EXE*-файл запустится локально. Однако рекомендуется профилировать приложение, открыв решение в Visual Studio.

При выборе для приложения UWP режима **выполняемого приложения** или **установленного приложения** выберите приложение в списке приложений, найденных в указанном целевом объекте развертывания. Этот целевой объект может быть локальным или удаленным компьютером. Для профилирования приложения UWP на удаленном компьютере необходимо выбрать **Универсальный (незашифрованный протокол)** в диалоговом окне **Удаленные подключения**.
...
> [!NOTE]
> Другие сценарии, требующие удаленного использования средств профилирования, см. в статье [Измерение производительности приложения из командной строки](../profiling/profile-apps-from-command-line.md). Программы командной строки можно использовать со средством "Загрузка ЦП" и инструментом выделения объектов .NET.
