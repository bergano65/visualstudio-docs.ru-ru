---
title: "Скорость реагирования ИП HTML | Microsoft Docs"
ms.custom: ""
ms.date: "12/05/2016"
...
dev_langs: 
  ...
  - "C++"
helpviewer_keywords: 
  - "производительность, JavaScript [приложения Магазина Windows]"
  - "средства производительности, JavaScript [приложения Магазина Windows]"
  - "Профилировщик скорости отклика ИП [JavaScript]"
  - "профилировщик, скорость реагирования ИП [JavaScript]"
  - "профилировщик, JavaScript [приложения Магазина Windows]"
...
2.  В меню **Отладка** выберите пункт **Профилировщик производительности...**  
     ...
     ![Изменение целевого объекта анализа](../profiling/media/js_tools_target.png "JS\_Tools\_Target")  
...
4.  При запуске профилировщика скорости реагирования пользовательского интерфейса в окне "Контроль учетных записей" может быть запрошено разрешение на запуск файла Collector.exe трассировки событий Windows Visual Studio. Выберите **Да**.  
     ...
     Взаимодействуйте с приложением для тестирования соответствующего сценария оценки производительности. Подробный рабочий процесс см. в разделах [Локализация проблемы скорости реагирования ИП](#Workflow) и [Локализация проблемы пропускной способности визуализации](#IsolateVisualThroughput).  
...
8.  Если добавлены пользовательские отметки, они будут отображаться на [Просмотр временной шкалы диагностического сеанса](#Ruler) профилировщика. На следующем рисунке показана отдельная пользовательская отметка, используемая для задания конкретной операции в коде.  
     ...
     ![Линейка диагностики с пользовательской отметкой](../profiling/media/js_htmlvizprofiler_usermark.png "JS\_HTMLVizProfiler\_UserMark")  
...
10. Выберите интересующую область на одной из диаграмм, щелкнув часть диаграммы и перетащив указатель \(или выберите ее с помощью клавиши TAB и клавиш со стрелками\). При выборе периода времени путем выделения диаграмма сведений о временной шкале в нижней области профилировщика изменяется для отображения только выбранного периода.  
     ...
     ![График использования ЦП](../profiling/media/js_htmlvizprof_cpu_util.png "JS\_HTMLVizProf\_CPU\_Util")  
...
11. Используйте [Просмотр сведений о временной шкале](#TimelineDetails) , чтобы получить подробную информацию о событиях, выполнение которых либо занимает слишком много времени, либо производится слишком часто. Например, обратите внимание на следующее:  
    ...
    -   Другие события указаны в разделе [Справочник по событиям профилировщика](#ProfilerEvents).  
...
###  <a name="IsolateVisualThroughput"></a> Локализация проблемы пропускной способности визуализации  
...
```  
 ...
 ![Событие оценки пользователя в представлении сведений о шкале времени](../profiling/media/js_htmlvizprofiler_user_measure.png "JS\_HTMLVizProfiler\_User\_Measure")  
...
###  <a name="Ruler"></a> Просмотр временной шкалы диагностического сеанса  
 ...
 ![Линейка диагностического сеанса](../profiling/media/js_htmlvizprof_ruler.png "JS\_HTMLVizProf\_Ruler")  
...
-   получения более подробного представления выбранного периода времени путем нажатия кнопки **Увеличить**.  
 ...
 Дополнительные сведения об использовании диаграммы см. в подразделе [Локализация проблемы скорости реагирования ИП](#Workflow) далее в этом разделе.  
...
-   Если кадры отбрасываются, на диаграмме отображается нулевое значение.  
 ...
 ![График пропускной способности визуализации](../profiling/media/js_htmlvizprof_vizthru.png "JS\_HTMLVizProf\_VizThru")  
...
>  Когда в фоновом потоке происходит какое\-либо событие, рядом с именем этого события в квадратных скобках отображается идентификатор потока.  
 ...
 ![График сведений о временной шкале](../profiling/media/js_htmlvizprof_timelinedet.png "JS\_HTMLVizProf\_TimelineDet")  
 ...
 События на диаграмме сведений о временной шкале имеют цветовую маркировку для представления тех же категорий действий, которые показываются на диаграмме использования ЦП. Дополнительные сведения о категориях событий и конкретных событиях см. в подразделе [Справочник по событиям профилировщика](#ProfilerEvents) этого раздела.  
...
###  <a name="FilterTimelineDetails"></a> Фильтр сведений о временной шкале  
 ...
 ![Фильтрация шкалы времени по событию](../profiling/media/js_htmlvizprofiler_filtertoevent.png "JS\_HTMLVizProfiler\_FilterToEvent")  
...
###  <a name="FilterEvents"></a> Фильтр событий  
 ...
 ![Фильтрация событий на шкале времени](../profiling/media/js_htmlvizprofiler_event_filter.png "JS\_HTMLVizProfiler\_Event\_Filter")  
...
###  <a name="GroupFrames"></a> События группы по кадрам  
 ...
 ![Группирование событий верхнего уровня по кадру](../profiling/media/js_htmlvizprofiler_frame_grouping_button.png "JS\_HTMLVizProfiler\_Frame\_Grouping\_Button")  
 ...
 ![События шкалы времени, сгруппированные по кадру](../profiling/media/js_htmlvizprofiler_frame_grouping.png "JS\_HTMLVizProfiler\_Frame\_Grouping")  
...
##  <a name="ProfilerEvents"></a> Справочник по событиям профилировщика  
...
|Сборка мусора|GC|Было потрачено время на сбор в памяти объектов, которые больше не использовались.|  
...
## См. также  
 [Анализ производительности приложений](../Topic/Analyze%20the%20performance%20of%20Windows%20Store%20apps%20using%20Visual%20Studio%20diagnostic%20tools.md)