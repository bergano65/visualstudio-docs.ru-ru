...
2. Запустите анализатор памяти JavaScript. Дополнительные сведения см. в разделе [Запуск анализатора памяти JavaScript](#run-the-JavaScript-memory-analyzer).  
...
-   Следуйте рабочему процессу, описанному в разделе [Локализация утечки памяти](#isolate-a-memory-leak), и используйте фильтр **Объекты, оставшиеся после снимка №\<номер>** в разностном представлении, чтобы выявить вероятных кандидатов, вызывающих утечки памяти.  
...
-   Попробуйте найти объекты, которые случайно сохраняются в памяти после перехода пользователя на новую страницу, что является распространенной причиной проблем с памятью. Например:  
    ...
    -   Возможно, потребуется удалить один или несколько прослушивателей событий. Дополнительные сведения см. в разделе [View DOM event listeners](/visualstudio/debugger/quickstart-debug-html-and-css).  
