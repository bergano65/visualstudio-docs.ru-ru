...
Чтобы добавить поддержку NPM в проект, вам потребуется **рабочая нагрузка разработки Node.js** и установленная среда выполнения Node.js. Подробные инструкции см. в разделе [Создание проекта Node.js](/visualstudio/ide/quickstart-nodejs?toc=/visualstudio/javascript/toc.json).
...
* **Обновите пакеты NPM** Обновляет пакеты до последних версий в соответствии с диапазоном semver, указанным в *package.json*. (Запустить `npm update --save`.) Диапазоны Semver обычно задаются с помощью "~" или "^".
