...
> [!Important]
> NPM ожидает найти папку *node_modules* и *package.json* в корневом каталоге проекта. Если структура папок вашего приложения отличается, можно [открыть проект как папку](npm-package-management.md) или обновить структуру папок, если вы хотите управлять пакетами npm с помощью Visual Studio.
