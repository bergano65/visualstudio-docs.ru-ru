...
* У вас должна быть установлена среда выполнения Node.js.
    ...
    Этот учебник был протестирован с версией 10.16.0.
...
1. Выберите Microsoft Edge или Chrome в качестве текущего целевого объекта отладки.
    ...
    ::: moniker range=">=vs-2019"
    Если Chrome установлен на вашем компьютере, но отсутствует в списке, выберите **Веб-браузер (название браузера)** > **Выбрать веб-браузер** в раскрывающемся списке целевых объектов отладки, а затем установите **Chrome** в качестве целевого браузера по умолчанию.
    ::: moniker-end
    ::: moniker range="vs-2017"
    Если Chrome установлен на вашем компьютере, но отсутствует в списке, выберите **Веб-браузер (название браузера)** >**Google Chrome** в раскрывающемся списке целевых объектов отладки, а затем установите **Chrome** в качестве целевого браузера по умолчанию.
    ::: moniker-end
...
1. Переключитесь в Visual Studio, а затем задайте точку останова в исходном коде, *app-bundle.js* или *app.tsx*.
    ...
    ![Установка точки останова](../javascript/media/tutorial-nodejs-react-set-breakpoint-in-tsx-file.png)

2. Если точка останова устанавливается в файле *.tsx* (а не *app-bundle.js*), необходимо обновить файл *webpack-config.js*. Замените код
