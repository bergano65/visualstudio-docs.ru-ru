...
Чтобы языковая служба анализировала только ваши исходные файлы в папке `tsconfig.json`, но при этом получала и использовала файлы `js` для библиотек в папке `.d.ts`, вы можете использовать следующую конфигурацию `lib`.
