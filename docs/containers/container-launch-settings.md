...
| launchUrl            | "launchUrl": "<scheme>://<serviceHost>:<servicePort>" | Этот URL-адрес используется при запуске браузера. Поддерживаемые токены замены для этой строки:                          |
|                      |                                                       | - <scheme>: заменяется на http или https в зависимости от того, используется ли протокол SSL.                                   |
|                      |                                                       | - <serviceHost>: обычно заменяется на localhost.                                                                    |
...
|                      |                                                       | - <servicePort>: обычно заменяется на sslPort или httpPort в зависимости от того, используется ли протокол SSL.                   |
