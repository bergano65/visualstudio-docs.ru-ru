...
ms.locfileid: "74549958"
---
# <a name="build-and-debug-containerized-apps-using-visual-studio-or-the-command-line"></a>Создание и отладка контейнерных приложений с помощью Visual Studio или командной строки
...
Сначала в файле Dockerfile берется образ Nano Server из реестра контейнеров Майкрософт (mcr.microsoft.com) и создается промежуточный образ `base`, для которого открываются порты 80 и 443 и задается рабочий каталог `/app`.
...
Прогрев выполняется только в **быстром** режиме. Благодаря этому для запущенного контейнера подключается том папки приложения, и любые изменения в приложении не приведут к переходу контейнера в недопустимое состояние. Таким образом, значительно повышается эффективность отладки и сокращается время ожидания для длительных задач, таких как извлечение крупных образов.

## <a name="volume-mapping"></a>Сопоставление томов

Чтобы реализовать отладку в контейнерах, Visual Studio использует сопоставление томов для сопоставления отладчика и папок NuGet с хост-компьютера. Ниже перечислены тома, которые подключаются в контейнере:
...
Инструменты для контейнеров в Visual Studio поддерживают отладку в приложениях ASP.NET Core с поддержкой SSL с использованием сертификата разработки так же, как и при работе без контейнеров. Для этого Visual Studio выполняет несколько дополнительных действий для экспорта сертификата и предоставления контейнеру доступа к нему. Этот процесс выглядит следующим образом:

1. С помощью инструмента `dev-certs` убедитесь, что сертификат разработки присутствует на хост-компьютере и является доверенным.
2. Экспортируйте сертификат в папку %APPDATA%\ASP.NET\Https с использованием надежного пароля, который находится в хранилище секретных ключей пользователей для этого приложения.
...
Дополнительные сведения о поддержке SSL в приложениях ASP.NET Core в контейнерах см. в разделе [Размещение образов ASP.NET Core с Docker через HTTPS](https://docs.microsoft.com/aspnet/core/security/docker-https).
