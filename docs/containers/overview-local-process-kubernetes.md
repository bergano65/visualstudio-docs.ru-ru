...
> [!WARNING]
> Функция "Локальный процесс с Kubernetes" предназначена для использования только в сценариях разработки и тестирования. Он не предназначен для выполнения с рабочими кластерами или службами Live в активном использовании.
...
Кроме того, Локальный процесс с Kubernetes позволяет реплицировать переменные среды и подключенные файлы, доступные группам pod в кластере, на компьютере разработки через файл `KubernetesLocalProcessConfig.yaml`. Этот файл также можно использовать для создания новых переменных среды и подключений томов.

> [!NOTE]
> На протяжении всего сеанса подключения к кластеру (и еще 15 минут) функция "Локальный процесс с Kubernetes" выполняет на локальном компьютере процесс *EndpointManager* с разрешениями администратора.
...
Файл `KubernetesLocalProcessConfig.yaml` позволяет реплицировать переменные среды и подключенные файлы, доступные для групп pod в кластере. Дополнительные сведения об дополнительных параметрах конфигурации см. в статье [Настройка Локального процесса с Kubernetes][using-config-yaml].
...
По умолчанию локальный процесс с Kubernetes перенаправляет на компьютер разработки весь трафик службы. Кроме того, можно использовать возможности маршрутизации, чтобы перенаправлять запросы к только службе, которая выполняется из поддомена на компьютере разработчика. Эти возможности маршрутизации позволяют использовать локальный процесс с Kubernetes для разработки в изоляции и предотвращения помех для другого трафика в кластере.

В следующей анимации показаны два разработчика, работающие над одним кластером в изоляции:

![Анимированный GIF, иллюстрирующий изоляцию](media/local-process-kubernetes/lpk-graphic-isolated.gif)

При работе в изоляции локальный процесс с Kubernetes выполняет следующие действия в дополнение к подключению к кластеру Kubernetes:
...
Если локальный процесс с Kubernetes обнаруживает, что в кластере Kubernetes включена служба Azure Dev Spaces, вам будет предложено отключить Azure Dev Spaces, прежде чем можно будет использовать локальный процесс с Kubernetes.
...
При отключении от кластера по умолчанию локальный процесс с Kubernetes удалит все модули pod – представители и дублирующую службу. 

> [ПРИМЕЧАНИЕ]. Развертывание и служба диспетчера маршрутизации будут продолжать работать в вашем пространстве имен. Чтобы удалить развертывание и службу, выполните следующие команды для пространства имен.
>
> ```azurecli
...
При использовании функции "Локальный процесс с Kubernetes" для подключения к кластеру журналы диагностики из кластера записываются в каталог *TEMP* в папке *Локальный процесс с Kubernetes* на компьютере разработки.
...
* Локальный процесс с Kubernetes нельзя использовать в кластерах с включенной службой Azure Dev Spaces.

### <a name="local-process-with-kubernetes-and-clusters-with-azure-dev-spaces-enabled"></a>Локальный процесс с Kubernetes и кластеры с включенной службой Azure Dev Spaces

Нельзя использовать локальный процесс с Kubernetes в кластерах с включенной службой Azure Dev Spaces. Если вы хотите использовать локальный процесс с Kubernetes в кластере с включенной службой Azure Dev Spaces, необходимо отключить Azure Dev Spaces перед подключением к кластеру.
