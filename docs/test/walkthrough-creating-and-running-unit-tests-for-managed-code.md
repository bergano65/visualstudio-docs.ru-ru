...
Результат теста содержит сообщение, описывающее возникшую ошибку. Для метода `AreEquals` сообщение отражает ожидаемый результат (параметр **Ожидается\<*значение*>**) и фактически полученный (параметр **Фактическое\<*значение*>**). Ожидалось, что баланс уменьшится, а вместо этого он фактически увеличился на сумму списания.
