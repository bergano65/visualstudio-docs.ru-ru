...
Для работы с учебником нужно установить Visual Studio Enterprise Edition с рабочей нагрузкой .NET Core 2.0.
...
3. Выберите шаблон **Библиотека классов (.NET Standard)** в правой области и введите `StringLibrary` в текстовом поле **Имя**, как показано на следующем рисунке:
...
3. Задайте для проекта имя `StringLibrary`.
...
6. Выберите **Сборка** > **Сборка решения** в меню верхнего уровня Visual Studio. Система Visual Studio должна успешно создать вашу библиотеку.
...
2. В диалоговом окне **Добавление нового проекта** выберите узел Visual Basic, а затем **.NET Standard**.
...
3. Выберите шаблон **Проект модульного теста (.NET Core)** в правой области и введите `StringLibraryTests` в текстовом поле **Имя**, как показано на следующем рисунке:
...
3. Задайте для проекта имя `StringLibraryTests`.
...
9. В раскрывающемся списке **Кодировка** диалогового окна **Дополнительные параметры сохранения** выберите параметр **Юникод (UTF-8, без сигнатуры), кодовая страница 65001**, как показано на следующем рисунке:
...
2. В диалоговом окне **Добавление нового проекта** выберите узел Visual Basic, а затем **.NET Core**.
