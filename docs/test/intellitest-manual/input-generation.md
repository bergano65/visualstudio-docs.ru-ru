...
IntelliTest создает входные данные для [параметризованных модульных тестов](test-generation.md#parameterized-unit-testing) путем анализа условий ветви в программе. Тестовые входные данные выбираются на основе того, могут ли они активировать новые поведения ветвления программы. Такой анализ является последовательным. Он уточняет предикат `q: I -> {true, false}` по формальным входным параметрам теста `I`. `q` Представляет набор поведений, которые IntelliTest уже наблюдал. Изначально `q := false`, так как ничего не наблюдалась еще.

Ниже указаны этапы цикла:

1. IntelliTest определяет входные параметры `i` таким образом, чтобы `q(i)=false` с помощью [ограничений](#constraint-solver). По своему назначению входные данные `i` займет раньше не пути выполнения. Изначально это означает, что `i` может быть любым входным, так как ни один путь выполнения еще не был обнаружен.

1. IntelliTest выполняет тест с выбранным входным `i`и отслеживает выполнение теста и тестируемую программу.

1. Во время выполнения программа принимает определенный путь, определяемый всеми условными ветвями программы. Набор всех условий, определяющих выполнение называется *условие пути*, записывается в виде предиката `p: I -> {true, false}` по формальным входным параметрам. IntelliTest вычисляет представление этого предиката.
...
.NET имеет тщательно проработанную модель видимости: типы, методы, поля и другие элементы могут быть **частного**, **открытый**, **внутренней**и многое другое.
...
Как протестировать метод, имеющий параметр типа интерфейса? Или незапечатанный класс? IntelliTest не знает, какие реализации будут использоваться позднее при вызове этого метода. И просто отсутствовать фактической реализации доступными во время тестирования.

В общем случае рекомендуется использовать *макеты объектов* с явным поведением.

Макет объекта реализует интерфейс (или расширяет незапечатанный класс). Он не представляет фактическую реализацию, а является просто ярлыком, обеспечивающим выполнение тестов с помощью макета объекта. Его поведение определяется вручную в рамках каждого тестового случая, где он используется. Существует множество инструментов, которые позволяют легко определить макеты объектов и их ожидаемое поведение, однако подобное поведение нужно по-прежнему задавать вручную.
