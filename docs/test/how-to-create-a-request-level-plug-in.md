...
С помощью следующего кода можно создать настраиваемый подключаемый модуль веб-теста производительности, который отображается в двух диалоговых окнах. В первом диалоговом окне отображается URL-адрес, связанный с запросов, к которому добавляется подключаемый модуль запросов. Во втором окне отображается имя компьютера агента.
