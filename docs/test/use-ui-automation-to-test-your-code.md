...
Автоматизированные тесты пользовательского интерфейса приложений называются *закодированными тестами пользовательского интерфейса*. Эти тесты обеспечивают функциональное тестирование элементов управления ИП. Они позволяют убедиться, что все приложение, в том числе интерфейс, работает правильно. Закодированные тесты ИП, в частности, полезны, если интерфейс содержит логику проверки или другую логику, например на веб-странице. Они также часто используются для автоматизации существующих ручных тестов.  
 ...
 ![Тестирование во время разработки приложения](~/docs/test/media/cuit_overview.png "CUIT_Overview")  
...
2.  **Добавьте файл закодированного теста пользовательского интерфейса.**  
     ...
     ![Создание закодированного теста пользовательского интерфейса](~/docs/test/media/codedui_create.png "CodedUI_Create")  
     ...
     ![Выбор действия для записей](~/docs/test/media/codedui_codegendialogb.png "CodedUI_CodeGenDialogB")  
  
     Откроется построитель кодированных тестов, а окно Visual Studio свернется.  
  
     ![Построитель закодированных тестов пользовательского интерфейса](~/docs/test/media/codedui_testbuilder.png "CodedUI_TestBuilder")  
...
4.  **Проверьте значения в полях пользовательского интерфейса, например в текстовых полях**.  
     ...
     ![Целевой элемент закодированного теста пользовательского интерфейса](~/docs/test/media/codedui_1.png "CodedUI_1")  
...
7.  **Изменение действий и утверждений теста**.  
     ...
     ![Редактор закодированных тестов пользовательского интерфейса](~/docs/test/media/cuit_editor_edit.png "CUIT_Editor_edit")  
...
-   ![Предварительное требование](~/docs/test/media/prereq.png "Prereq") Если вы не хотите записывать действия, чтобы запускать приложение во время тестирования, необходимо запустить приложение до нажатия значка **Запись**.  
  
-   ![Предварительное требование](~/docs/test/media/prereq.png "Prereq")В конце теста процесс, в котором тест выполняется, завершается. Если вы запустили приложение во время теста, оно обычно закрывается.  Если вы не хотите закрывать приложение, добавьте RUNSETTINGS-файл в решение и используйте параметр `KeepExecutorAliveAfterLegacyRun`. Дополнительные сведения см. в разделе [Настройка модульных тестов с помощью файла .runsettings](../test/configure-unit-tests-by-using-a-dot-runsettings-file.md).  
  
-   ![Предварительное требование](~/docs/test/media/prereq.png "Prereq") Вы можете добавить метод инициализации теста, который определяется атрибутом [TestInitialize], для запуска кода в начале каждого метода теста. Например, в методе TestInitialize можно запустить приложение.  
  
-   ![Предварительное требование](~/docs/test/media/prereq.png "Prereq") Вы можете добавить метод очистки теста, который определяется атрибутом [TestCleanup], для запуска кода в конце каждого метода теста. Например, метод для закрытия приложения можно вызвать в методе TestCleanup.  
...
###  <a name="VerifyingCodeUsingCUITGenerateAssertions"></a> Проверка свойства элементов управления пользовательского интерфейса  
 ...
 ![Свойства закодированного теста пользовательского интерфейса](~/docs/test/media/codedui_2.png "CodedUI_2")  
...
-   *Я не вижу свойств при выборе элемента управления в приложении или не вижу элемент управления в карте элементов управления пользовательского интерфейса.*  
 ...
 ![Утверждения закодированного теста пользовательского интерфейса](~/docs/test/media/codedui_3.png "CodedUI_3")  
...
-   Добавляет метод `AssertForAddTwoNumbers` на карту ИП (UIMap.uitest). Этот файл открывается в редакторе закодированных тестов ИП, где можно изменить утверждения.  
     ...
     ![Редактирование утверждения с помощью редактора закодированных тестов пользовательского интерфейса](~/docs/test/media/cuit_editor_assert.png "CUIT_Editor_assert")  
