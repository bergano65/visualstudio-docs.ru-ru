...
Если у вас есть тестовый случай, записанный в Microsoft Test Manager, вы можете создать код на его основе. Дополнительные сведения см. в разделе [Запись и воспроизведение ручных тестов](/vsts/manual-test/getting-started/record-play-back-manual-tests).
...
## <a name="creating-coded-ui-tests"></a>Создание закодированных тестов пользовательского интерфейса
...
### <a name="starting-and-stopping-the-application-under-test"></a>Запуск и остановка тестируемого приложения

*Я не хочу запускать и останавливать приложение, браузер или базу данных отдельно для каждого теста. Как этого избежать?*  
  
-   ![Предварительное требование](../test/media/prereq.png "Prereq") Если вы не хотите записывать действия, чтобы запускать приложение во время тестирования, необходимо запустить приложение до выбора значка **Запись**.  
  
-   ![Предварительное требование](../test/media/prereq.png "Prereq")В конце теста процесс, в котором выполняется тест, завершается. Если вы запустили приложение во время теста, оно обычно закрывается.  Если вы не хотите закрывать приложение, добавьте RUNSETTINGS-файл в решение и используйте параметр `KeepExecutorAliveAfterLegacyRun`. Дополнительные сведения см. в разделе [Настройка модульных тестов с помощью файла .runsettings](../test/configure-unit-tests-by-using-a-dot-runsettings-file.md).  
...
### <a name="validating-the-properties-of-ui-controls"></a>Проверка свойства элементов управления пользовательского интерфейса
...
## <a name="customizing-your-coded-ui-test"></a>Настройка закодированного теста пользовательского интерфейса
...
### <a name="coding-ui-control-actions-and-properties"></a>Программирование действий и свойств элементов управления пользовательского интерфейса
...
Закодированные тесты ИП часто используются для автоматизации ручных тестов. Дополнительные сведения о ручных тестах см. в разделе [Выполнение ручных тестов с помощью Microsoft Test Manager](/vsts/manual-test/mtm/run-manual-tests-with-microsoft-test-manager). Дополнительные сведения об автоматизированных тестах см. в [описании средств тестирования в Visual Studio](../test/improve-code-quality.md).  
...
[Вопросы и ответы о закодированных тестах пользовательского интерфейса](http://go.microsoft.com/fwlink/?LinkID=230576)
  
## <a name="see-also"></a>См. также

<xref:Microsoft.VisualStudio.TestTools.UITest.Common.UIMap.UIMap> <xref:Microsoft.VisualStudio.TestTools.UnitTesting.Assert> [Повышение качества кода](../test/improve-code-quality.md) 
