...
|-|-|-|
|**Запрос**|URL-адрес запроса. Например, *home.html* или *orange-arrow.gif*.|Yes|
|**Сценарий**|Имя сценария.|Yes|
|**Тест**|Имя теста.|Yes|
|**Итого**|Общее число запросов этого веб-теста производительности, выданных во время выполнения нагрузочного теста. В это число входят успешные и неудачные запросы. Кэшированные запросы сюда не входят, поскольку они не передаются на веб-сервер.|Yes|
|**Выполнено**|Количество раз вызовов и передач запроса.|Нет|
|**Сбой**|Количество раз вызовов и сбоев при выполнении запроса. Записи в этом столбце представлены в виде гиперссылок. Чтобы просмотреть список отдельных ошибок в диалоговом окне **Ошибки при загрузке теста**, выберите любую гиперссылку. Дополнительные сведения см. в разделе [Анализ результатов нагрузочных тестов](../test/analyze-load-test-results-using-the-load-test-analyzer.md).|Yes|
...
|**Время отклика**|Среднее время получения полного ответа на запрос, измеренное с момента отправки запроса на веб-сервер. Единицей измерения является секунда.|Yes|
|**Длина содержимого**|Средняя длину содержимого ответа на запрос. Единицей измерения является байт.|Yes|
...
|**Тест**|Имя теста.|Yes|
|**Сценарий**|Имя сценария.|Yes|
|**Итого**|Общее количество раз выполнения теста в сценарии. В это число входит количество раз успешного и неудачного выполнения теста.|Yes|
|**Выполнено**|Количество раз успешного выполнения теста в сценарии.|Yes|
|**Сбой**|Количество раз неудачного выполнения теста в сценарии. Записи в этом столбце представлены в виде гиперссылок. Чтобы просмотреть список отдельных ошибок в диалоговом окне **Ошибки при загрузке теста**, выберите любую гиперссылку. Дополнительные сведения см. в разделе [Анализ результатов нагрузочных тестов](../test/analyze-load-test-results-using-the-load-test-analyzer.md).|Yes|
|**Тестов в секунду**|Скорость теста во время выполнения нагрузочного теста.|Yes|
...
|**Время теста**|Среднее время выполнения теста во время нагрузочного теста. Единицей измерения является секунда.|Yes|
|**90 % времени теста**|Значение процентиля 90 для времени выполнения теста.|Нет|
|**95 % времени теста**|Значение процентиля 95 для времени выполнения теста.|Yes|
...
|-|-|-|
|**Транзакция**|Имя транзакции.|Yes|
|**Сценарий**|Имя сценария.|Yes|
|**Тест**|Имя теста.|Yes|
|**Итого**|Общее число транзакций, выданных во время выполнения нагрузочного теста.|Yes|
...
|Столбец|ОПИСАНИЕ|Отображается по умолчанию|
|-|-|-|
|Тип|Тип ошибки. Например, HttpError.|Yes|
|Подтип|Подтип ошибки. Например, LoadTestException.|Yes|
|Количество|Число ошибок данного типа, возникших во время нагрузочного теста. Записи в этом столбце представлены в виде гиперссылок. Для просмотра списка отдельных ошибок нужно выбрать гиперссылку.|Yes|
|Последнее сообщение|Сообщение, описывающее ошибку. Например, 404 - NotFound.|Yes|
