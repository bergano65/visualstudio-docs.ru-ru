...
|Модель тестового набора|Определяет вероятность, с которой виртуальный пользователь работает с заданным тестом в сценарии нагрузочного теста. Пример: с вероятностью 20 % выполняется TestA и с вероятностью 80 % — TestB. Модель тестового набора должна отражать цели теста для определенного сценария.|
