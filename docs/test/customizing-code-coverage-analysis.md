...
3. Чтобы выбрать файл параметров запуска, в **обозревателе тестов** щелкните стрелку на кнопке **Параметры**, а затем выберите пункт **Выбрать файл параметров**. Чтобы указать файл параметров запуска для запуска тестов из командной строки или в рабочем процессе сборки, см. раздел [Настройка модульных тестов с помощью файла *.runsettings*](../test/configure-unit-tests-by-using-a-dot-runsettings-file.md#specify-a-run-settings-file).
...
Чтобы включить или отключить пользовательские параметры, выберите файл или отмените его выбор в меню **Параметры** в **обозревателе тестов**.
