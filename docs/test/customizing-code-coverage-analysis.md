...
3. Чтобы выбрать файл параметров запуска, в **обозревателе тестов** щелкните стрелку на кнопке **Параметры**, а затем выберите пункт **Выбрать файл параметров**. Чтобы указать файл параметров запуска для запуска тестов из командной строки, см. раздел [Настройка модульных тестов](../test/configure-unit-tests-by-using-a-dot-runsettings-file.md#command-line).
