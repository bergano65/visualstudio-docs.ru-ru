...
Последовательно выберите **Тест** > **Параметры тестирования** > **Выбрать файл параметров теста** и выберите RUNSETTINGS-файл. В меню "Параметры тестирования" отобразится файл, который можно выбрать или отменить. Если RUNSETTINGS-файл выбран, он применяется при каждом выполнении команды **Анализ покрытия кода**.
...
3.  На странице **Процесс** разверните **Автоматизированные тесты** > **Исходный код теста** > **Параметры запуска**. Выберите файл **.runsettings**.
    ...
    -   *Параметр **Тестовая сборка** появляется вместо параметра **Исходный код теста**. При попытке настроить поле **Параметры запуска** для выбора доступны только TESTSETTINGS-файлы.*  
