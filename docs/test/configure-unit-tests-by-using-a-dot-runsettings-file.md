...
Модульные тесты в Visual Studio можно настроить с помощью файла \*.runsettings. (Имя файла не имеет значения, если вы используете расширение ".runsettings".) Например, можно изменить версию платформы .NET Framework, на которой выполняются тесты, каталог, в который отправляются результаты теста, и данные, собранные во время тестового запуска.

Если вам не требуется специальная конфигурация, файл \*.runsettings не нужен. Чаще всего файл \*.runsettings используют для настройки [покрытия кода](../test/customizing-code-coverage-analysis.md).
...
## <a name="example-runsettings-file"></a>Пример RUNSETTINGS-файла
