---
title: "Формирование и компиляция кода, а также соглашения об именовании в Microsoft Fakes | Microsoft Docs"
ms.custom: ""
ms.date: "12/13/2016"
...
##  <a name="BKMK_Code_generation_and_compilation"></a> Создание и компиляция кода  
  
###  <a name="BKMK_Configuring_code_generation_of_stubs"></a> Настройка создания кода заглушек  
...
###  <a name="BKMK_Stubbing_concrete_classes_and_virtual_methods"></a> Использование заглушек для конкретных классов и виртуальных методов  
...
-   Необходимо добавить открытые ключи сборки Fakes и тестовой сборки в атрибуты **InternalsVisibleToAttribute** в сборках с оболочкой совместимости.  Вот как будут выглядеть наши образцы атрибутов в коде сборки с оболочкой совместимости, когда сборка с оболочкой совместимости имеет строгое имя.  
 ...
 Если сборка с оболочкой совместимости имеет строгое имя, платформа Fakes автоматически назначает созданной сборке Fakes строгую подпись.  Тестовой сборке следует назначить строгую подпись.  См. раздел [Создание и использование сборок со строгими именами](../Topic/Creating%20and%20Using%20Strong-Named%20Assemblies.md).  
  
 Платформа Fakes использует один и тот же ключ для подписи всех созданных сборок, чтобы можно было использовать этот фрагмент кода в качестве отправной точки для добавления атрибута **InternalsVisibleTo** для сборки Fakes в код сборки с оболочкой совместимости.  
  
```c#  
[assembly: InternalsVisibleTo("FileSystem.Fakes, PublicKey=")]  
```  
  
 Можно указать другой открытый ключ для сборки Fakes, например ключ, созданный для сборки с оболочкой совместимости, указав полный путь к файлу **.snk**, который содержит альтернативный ключ в виде значения атрибута `KeyFile` в элементе `Fakes`\\`Compilation` файла **.fakes**.  Пример:  
...
```  
 ...
 Затем необходимо использовать открытый ключ из альтернативного файла **.snk** в качестве второго параметра атрибута InternalVisibleTo для сборки Fakes в коде сборки с оболочкой совместимости:  
...
|**Метод доступа**, имя которого состоит из двух частей, разделенных символом «\_» \(как у методов получения свойства\)|*kind\_name* \(распространенный вариант, однако не является обязательным согласно ECMA\)|*NameKind*, где обе части начинаются с прописных букв и поменяны местами|  
...
-   **Методы получения и задания индексаторов** обрабатываются аналогично случаю со свойствами.  Для индексаторов имя по умолчанию имеет значение `Item`.  
  
-   Имена **типов параметров** преобразуются и объединяются.  
  
-   **Тип возвращаемого значения** игнорируется при наличии неоднозначности перегрузки.  В этом случае тип возвращаемого значения добавляется в конец имени.  
