...
Автоматически созданная коллекция в <xref:System.Windows.Forms.BindingSource> коллекции <xref:System.ComponentModel.BindingList%601> использует для своей типизированной коллекции. Если приложению не требуются дополнительные функции, вы можете поддерживать коллекцию в <xref:System.Windows.Forms.BindingSource>. Дополнительные сведения см. в <xref:System.Windows.Forms.BindingSource.List%2A> описании свойства <xref:System.Windows.Forms.BindingSource> класса.

> [!NOTE]
> Если для вашей коллекции требуются функции <xref:System.ComponentModel.BindingList%601>, не предоставляемые базовой реализацией, следует создать пользовательскую коллекцию, чтобы при необходимости можно было добавить в класс.
...
> [!NOTE]
> Метод предоставляется автоматически для пользовательской коллекции при наследовании от <xref:System.ComponentModel.BindingList%601>. `Add`
...
Объекты из коллекции удаляются путем вызова `Remove` метода или `RemoveAt` <xref:System.Windows.Forms.BindingSource>пользовательского класса коллекции или.

> [!NOTE]
> Методы `Remove` <xref:System.ComponentModel.BindingList%601>и `RemoveAt` автоматически предоставляются для пользовательской коллекции при наследовании от.

В следующем коде показано, как выполнять обнаружение и удаление объектов из типизированной коллекции в <xref:System.Windows.Forms.BindingSource> <xref:System.Windows.Forms.BindingSource.RemoveAt%2A> с помощью метода:
...
Если приложение требует отслеживания изменений и отката предложенных изменений их исходных значений, необходимо реализовать эту функциональность в объектной модели. Примеры того, как таблицы данных отслеживают предлагаемые изменения, см <xref:System.Data.DataRowState>. <xref:System.Data.DataSet.HasChanges%2A>в статьях, и <xref:System.Data.DataTable.GetChanges%2A>.
...
|`TableAdapter.Update`|Обновляет существующие записи в базе данных. Метод Update принимает исходные и новые значения столбцов в качестве параметров метода. Исходные значения используются для нахождение исходной записи, а для обновления этой записи используются новые значения.<br /><br /> <xref:System.Data.DataTable> <xref:System.Data.DataSet> <xref:System.Data.DataRow> <xref:System.Data.DataRow>Метод также используется для согласования изменений в наборе данных с базой данных с помощью массива,, или из s в качестве параметров метода. `TableAdapter.Update`|
