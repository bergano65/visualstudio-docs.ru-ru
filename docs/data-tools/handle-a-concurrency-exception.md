...
5.  Используйте **Показать таблицу данных** компонент в **обозревателя серверов** для доступа к `Customers` данных и изменение записи таблицы.
...
1.  На **данных** меню, выберите **добавить новый источник данных**.
     ...
     [Мастер настройки источника данных](../data-tools/media/data-source-configuration-wizard.png) открывает.
...
### <a name="add-code-to-handle-the-concurrency-exception"></a>Добавьте код для обработки исключения параллельности
 ...
 В этом разделе добавьте код, который пытается обновить базу данных. Можно также обработать какие-либо <xref:System.Data.DBConcurrencyException> , может быть вызвано, а также любые другие исключения.
