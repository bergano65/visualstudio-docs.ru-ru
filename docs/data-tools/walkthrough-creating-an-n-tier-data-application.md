...
![Ссылка на видео](../data-tools/media/playvideo.gif) с видео-версией этого раздела см [. в видео как: Создание n-уровневого приложения](http://go.microsoft.com/fwlink/?LinkId=115188)для данных.
...
Следующим шагом является создание типизированного набора данных. Типизированные наборы данных создаются с помощью класса DataSet (включая `DataTables` классы) `TableAdapter` и классов в одном проекте. (Все классы формируются в одном файле.) Если набор данных и адаптеры TableAdapter разделяются на разные проекты, то это класс набора данных, который перемещается в другой проект, `TableAdapter` освобождая классы в исходном проекте. Поэтому создайте набор данных в проекте, который в конечном итоге будет содержать адаптеры таблиц (проект DataAccessTier). Набор данных создается с помощью **мастера настройки источника данных**.

> [!NOTE]
> Для создания подключения необходимо иметь доступ к учебной базе данных "Борей". Дополнительные сведения о настройке образца базы данных Northwind см. в разделе [как Установите образцы баз](../data-tools/installing-database-systems-tools-and-samples.md)данных.
...
Служба данных должна вызывать два метода на уровне доступа к данным: `GetCustomers` и. `GetOrders` Эти методы возвращают Northwind `Customers` и `Orders` Tables. Создайте методы `GetCustomers` и `GetOrders` в`DataAccessTier` проекте.
...
Значение по умолчанию `maxReceivedMessageSize` для недостаточно велико для хранения данных, полученных `Customers` из таблиц и `Orders` . В следующих шагах вы увеличите значение до 6553600. Вы изменяете значение на клиенте, которое автоматически обновляет ссылку на службу.
...
Запустите приложение, нажав клавишу **F5**. Данные из `Customers` таблиц и `Orders` извлекаются из службы данных и отображаются в форме.
