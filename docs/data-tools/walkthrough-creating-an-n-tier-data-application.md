...
В этом пошаговом руководстве показано, как разделить набор данных и `TableAdapter` код в отдельным проектам библиотеки классов с помощью **конструктор наборов данных**. После разделения набора данных и кода адаптера таблицы, можно создать [службы Windows Communication Foundation и WCF Data Services в Visual Studio](../data-tools/windows-communication-foundation-services-and-wcf-data-services-in-visual-studio.md) служба для вызова в уровне доступа к данным. Наконец создается в приложении Windows Forms как уровень представления. Этот уровень осуществляет доступ к данным из службы данных.
...
![ссылка на видео](../data-tools/media/playvideo.gif) видеоверсию этого раздела, см. в разделе [Video How to: создание n-уровневое приложение](http://go.microsoft.com/fwlink/?LinkId=115188).
...
> [!NOTE]
>  Для создания подключения необходимо иметь доступ к учебной базе данных "Борей". Сведения о настройке учебной базе данных Northwind, см. в разделе [как: установить образцы баз данных](../data-tools/installing-database-systems-tools-and-samples.md).
...
3.  В **источников данных** выберите **добавить новый источник данных** запустить **мастер настройки источника данных**.
...
1.  Щелкните правой кнопкой мыши **OrdersTableAdapter** и нажмите кнопку **добавить запрос**.
...
## <a name="add-a-reference-to-the-data-entity-and-data-access-tiers-to-the-data-service"></a>Добавьте ссылки на объекты данных и уровень доступа к данным в службу данных
 Так как служба данных требует данные из набора данных и адаптеров таблиц TableAdapter, добавьте ссылки на **DataEntityTier** и **DataAccessTier** проектов.
...
1.  В **DataService** проекта, дважды щелкните **IService1.vb** или **IService1.cs**.
...
4.  Добавьте следующий код, чтобы **Service1** класса:
...
### <a name="to-set-the-new-presentation-tier-project-as-the-startup-project"></a>Чтобы задать нового проекта уровня представления в качестве запускаемого проекта
...
1.  В **обозревателе решений**выберите **PresentationTier** проекта.
...
1.  В **обозревателе решений**, дважды щелкните **app.config** файл **PresentationTier** проекта.
