...
>  Важно понимать, что порядок обновления является полным. То есть при выполнении обновлений, вставок и удалений выполняются для всех таблиц в наборе данных.
 ...
 Чтобы задать `UpdateOrder` свойства, после перетаскивания элементов из [окна "Источники данных"](add-new-data-sources.md) в форму, выберите `TableAdapterManager` в область компонентов, а затем задайте `UpdateOrder` свойство в **свойства** окна.
