...
Определении функции или методы, предоставляемые службой WCF, маркировав их <xref:System.ServiceModel.OperationContractAttribute> атрибута.

[!code-csharp[WCFWalkthrough#1](../data-tools/codesnippet/CSharp/windows-communication-foundation-services-and-wcf-data-services-in-visual-studio_2.cs)]
[!code-vb[WCFWalkthrough#1](../data-tools/codesnippet/VisualBasic/windows-communication-foundation-services-and-wcf-data-services-in-visual-studio_2.vb)]

Кроме того, можно предоставить сериализованные данные, пометив составной тип <xref:System.Runtime.Serialization.DataContractAttribute> атрибута. Это позволяет привязку данных в клиенте.
...
Дополнительные сведения о шаблонах WCF, см. в разделе [шаблонов WCF Visual Studio](/dotnet/framework/wcf/wcf-vs-templates).

#### <a name="wcf-service-host"></a>Узел службы WCF

При запуске отладчика Visual Studio (нажав **F5**) для проекта службы WCF, узел службы WCF автоматически запускается средство для размещения службы локально. Узел службы WCF перечисление служб в проекте службы WCF, загружает конфигурацию проекта и создает экземпляр узла для каждой найденной службы.
...
1.  Добавьте ссылку на службу WCF, щелкните правой кнопкой мыши узел проекта в **обозревателе решений** и выбрав **Добавление ссылки на службу**.
...
3.  Отображает список IntelliSense, который включает перегрузки конструктора. Выберите `endpointConfigurationName As String` перегрузки.

4.  Перегрузке, введите `=` *ConfigurationName*, где *ConfigurationName* имя конечной точки, которую вы хотите использовать.

    > [!NOTE]
    > Если вы не знаете имена доступных конечных точек, вы найдете их в *app.config* файл.

### <a name="to-find-the-available-endpoints-for-a-wcf-service"></a>Чтобы найти доступные конечные точки для службы WCF

1.  В **обозревателе решений**, щелкните правой кнопкой мыши **app.config** файла для проекта, который содержит ссылку на службу и нажмите кнопку **откройте**. Файл отображается в редакторе кода.
...
2.  В **источников данных** окне разверните узел для ссылки на службу. Все составные типы, возвращаемые отображение службы.

3.  Разверните узел для типа. Поля данных для этого типа отображаются.
...
5.  Щелкните тип элемента управления, к которому требуется привязать.
...
1.  На **данных** меню, выберите **Показать источники данных**. Открывается окно **Источники данных**.
