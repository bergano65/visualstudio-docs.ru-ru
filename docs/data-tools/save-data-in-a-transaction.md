...
На этом шаге **Мастер настройки источника данных** используется для создания источника данных на основе `Customers` и `Orders` таблиц в образце базы данных Northwind.
...
3. Перетащите связанный узел **Orders** (не главный узел **заказы** , а связанный узел дочерней таблицы ниже столбца **факса** ) в форму под элементом **кустомерсдатагридвиев**.
   ...
   На форме появляется <xref:System.Windows.Forms.DataGridView>. В области компонентов появятся `OrdersTableAdapter` и <xref:System.Windows.Forms.BindingSource>.
