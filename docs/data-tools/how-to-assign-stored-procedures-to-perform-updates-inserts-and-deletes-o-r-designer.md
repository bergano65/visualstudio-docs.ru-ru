...
1.  Откройте **LINQ to SQL** файл в конструкторе. (Дважды щелкните **.dbml** файл **обозревателе решений**.)

2.  В **обозревателя серверов** или **обозреватель баз данных**, разверните **хранимые процедуры** и найдите сохраненные процедуры, которые вы хотите использовать для Insert, Update или Delete команды класса сущностей.
...
9. Проверьте список **аргументы метода** и **свойства класса** чтобы убедиться, что **аргументы метода** сопоставляются соответствующим **свойства класса**. Сопоставьте аргументы исходного метода (`Original_<ArgumentName>`) исходным свойствам (`<PropertyName> (Original)`) для `Update` и `Delete` команд.
...
Чтобы вернуться к использованию логики среды выполнения по умолчанию наличие обновлений, щелкните кнопку с многоточием рядом **вставить**, **обновление**, или **удалить** в команду **свойства**  окна, а затем выберите **использовать среду выполнения** в **Конфигурирование поведения** диалоговое окно.
