---
title: Создание простого приложения для обработки данных с помощью WPF и Entity Framework 6
ms.date: 08/22/2017
ms.topic: conceptual
dev_langs:
- CSharp
...
1. В Visual Studio создайте новое C# **приложение WPF** проекта.

2. Добавьте пакет NuGet для платформы Entity Framework 6. В **обозревателе решений**, выберите узел проекта. В главном меню выберите **проекта** > **управление пакетами NuGet**.
...
1. Щелкните правой кнопкой мыши узел проекта в **обозревателе решений** и выберите **добавить** > **новый элемент**. В области слева в разделе C# узел, выберите **данных** и в средней области выберите **ADO.NET Entity Data Model**.

   ![Entity Framework модель нового элемента](../data-tools/media/raddata-ef-new-project-item.png)

2. Вызов ее `Northwind_model` и выберите **ОК**. Открывается **Мастер моделей EDM**. Выберите **конструктор EF из базы данных** и нажмите кнопку **Далее**.
...
5. Мастер формирует C# классы, представляющие модель Entity Framework. Классы, plain old C# классы и они являются какие мы databind для пользовательского интерфейса WPF. *.Edmx* файл описывает связи и другие метаданные, который связывает классы с объектами в базе данных. *.Tt* файлы являются шаблоны T4, которые создают код, который работает на модели и сохраните изменения в базе данных. Вы увидите все эти файлы в **обозревателе решений** узле Northwind_model:
