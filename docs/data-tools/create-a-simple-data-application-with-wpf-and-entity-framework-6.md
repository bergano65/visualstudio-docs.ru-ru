...
В этом пошаговом руководстве показано, как создать базовое приложение "формы для данных" в Visual Studio. Приложение использует SQL Server LocalDB, базу данных Northwind, Entity Framework 6 (не Entity Framework Core) и Windows Presentation Foundation для .NET Framework (не .NET Core). В нем показано, как выполнить базовую привязку данных с помощью представления «основной/подробности», а также с пользовательским навигатором привязки с кнопками для **перехода далее** , **Переместить назад** , **Перейти к началу** , **Перейти к концу** , **Обновить** и **Удалить**.
