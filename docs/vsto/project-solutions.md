...
## <a name="automate-project-by-using-the-project-object-model"></a>Автоматизация проекта с помощью объектной модели проекта
 ...
 Чтобы получить доступ к объектной модели проекта из надстройки VSTO, используйте `Application` поле `ThisAddIn` класса в проекте. `Application` Поле`Microsoft.Office.Interop.MsProject.Application` возвращает объект, представляющий текущий экземпляр Project. Дополнительные сведения см. в разделе [программирование VSTO Add-ins](../vsto/programming-vsto-add-ins.md).
...
- [Project 2010 и Project Server 2010 при разработке решений для Office](http://go.microsoft.com/fwlink/?LinkId=199016)
