...
## <a name="automate-project-by-using-the-project-object-model"></a>Автоматизация project с помощью объектной модели project  
 ...
 Для доступа к объектной модели Project из надстройки VSTO используйте поле `Application` класса `ThisAddIn` в своем проекте. `Application` Возвращает `Microsoft.Office.Interop.MsProject.Application` , представляющий текущий экземпляр Project. Дополнительные сведения см. в разделе [надстроек VSTO программы](../vsto/programming-vsto-add-ins.md).  
