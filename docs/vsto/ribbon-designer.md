...
|**Кнопка**|Кнопка, которая запускает действие. Можно добавить кнопку в группу, группу кнопок, раскрывающийся список, коллекцию, меню или разворачивающуюся кнопку.|
...
|**DropDown**|Список элементов, которые может выбрать пользователь. Пользователь не может ввести новый элемент в раскрывающемся списке.<br /><br /> Используйте <xref:Microsoft.Office.Tools.Ribbon.RibbonDropDown.Items%2A> свойство, чтобы добавить элементы в список. Элементы можно добавлять и удалять во время выполнения.<br /><br /> Используйте <xref:Microsoft.Office.Tools.Ribbon.RibbonDropDown.Buttons%2A> свойство, чтобы добавить кнопки в список. Однако нельзя добавлять и удалять кнопки во время выполнения после загрузки ленты в приложение Office.|
|**Поле**|Поле, в котором пользователь может ввести текст.|
|**Галерея**|Меню, представляющее собой массив или сетку визуальных вариантов выбора, из которых пользователи могут выбирать. Можно управлять макетом выбора в меню. Используйте <xref:Microsoft.Office.Tools.Ribbon.RibbonGallery.ColumnCount%2A> Свойства и, <xref:Microsoft.Office.Tools.Ribbon.RibbonGallery.RowCount%2A> чтобы указать количество строк и столбцов, в которых будут отображаться элементы и кнопки коллекции.|
...
|**Двоеточи**|Тонкая полоса, используемая для разделения элементов в списке. При добавлении в группу полоса является вертикальной. При добавлении в меню полоса является горизонтальной.|
...
|Выполните итерацию вперед по всем элементам управления.|**Вкладка**|
...
|Откройте раскрывающийся список.|**Правильно**|
|Выберите первый элемент управления на раскрывающейся панели, если раскрывающийся список открыт.|**Правильно**|
