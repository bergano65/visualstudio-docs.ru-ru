...
---
# Вызов кода в надстройках VSTO из других решений Office
...
## Предоставление объектов для других решений  
...
### Определение класса, который требуется предоставлять другим решениям  
 Как минимум, предоставляемый класс должен быть общим, для атрибута этого класса <xref:System.Runtime.InteropServices.ComVisibleAttribute> должно быть задано значение **true** и он должен предоставлять интерфейс [IDispatch](http://msdn.microsoft.com/ru-ru/ebbff4bc-36b2-4861-9efa-ffa45e013eb5).  
  
 Предоставление интерфейса [IDispatch](http://msdn.microsoft.com/ru-ru/ebbff4bc-36b2-4861-9efa-ffa45e013eb5) рекомендуется выполнять следующим образом.  
...
4.  Примените к классу атрибут <xref:System.Runtime.InteropServices.ClassInterfaceAttribute> и задайте для атрибута значение None перечисления <xref:System.Runtime.InteropServices.ClassInterfaceType>.  
  
5.  Чтобы предоставить класс внепроцессным клиентам, может также потребоваться выполнить следующие действия.  
 ...
 Следующие примеры кода демонстрируют класс `AddInUtilities` с методом `ImportData`, который может вызываться другими решениями. Пример этого кода в контексте более крупного пошагового руководства см. в разделе [Пошаговое руководство. Вызов кода из VBA в надстройках VSTO](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md).  
...
### Предоставление классов VBA  
 ...
 Кроме того, вы можете предоставить интерфейс [IDispatch](http://msdn.microsoft.com/ru-ru/ebbff4bc-36b2-4861-9efa-ffa45e013eb5), задав для атрибута <xref:System.Runtime.InteropServices.ClassInterfaceAttribute> значение AutoDispatch или AutoDual перечисления <xref:System.Runtime.InteropServices.ClassInterfaceType>. В этом случае вам не потребуется объявлять методы в отдельном интерфейсе. Тем не менее код VBA сможет вызывать все общие и нестатические методы в классе, включая полученные из базовых классов, таких как <xref:System.Object>. Кроме того, внепроцессные клиенты, использующие раннее связывание, не смогут вызывать ваш класс.  
...
## Получение доступа к объектам из других решений  
...
### Получение доступа к объектам из решений VBA  
 В следующем примере кода показано использование VBA для вызова метода, который предоставляется надстройкой VSTO. Этот макрос VBA вызывает метод с именем `ImportData`, определенный в надстройке VSTO **ExcelImportData**. Пример этого кода в контексте более крупного пошагового руководства см. в разделе [Пошаговое руководство. Вызов кода из VBA в надстройках VSTO](../vsto/walkthrough-calling-code-in-a-vsto-add-in-from-vba.md).  
...
### Получение доступа к объектам из решений, не основанных на VBA  
