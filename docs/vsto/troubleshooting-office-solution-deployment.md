---
title: Устранение неполадок развертывания решения Office
ms.custom: ''
ms.date: 02/02/2017
ms.technology:
- office-development
...
ms.locfileid: "34693212"
---
# <a name="troubleshoot-office-solution-deployment"></a>Устранение неполадок развертывания решения Office
...
## <a name="change-the-assembly-name-causes-conflicts"></a>Изменение имени сборки приводит к конфликтам  
 При изменении **имя сборки** значение в **приложения** страница **конструктора проектов** после уже развертывания решения средства публикации изменяют Пакет установки, необходимо применять один *Setup.exe* файла и два манифеста развертывания. При развертывании двух файлов манифеста могут возникнуть следующие обстоятельства.  
...
## <a name="check-for-updates-takes-a-long-time"></a>Проверьте наличие обновлений занимает много времени  
 Visual Studio 2010 Tools для среды выполнения Office предоставляет запись реестра, администраторы могут использовать для установки значения времени ожидания для загрузки манифестов и решения.  
...
## <a name="cant-update-or-publish-to-a-network-file-share"></a>Не удается обновить или опубликовать в сетевую общую папку  
 Решения Office, которые находятся в общей сетевой папке могут отображать вводящее в заблуждение сообщение во время обновления, если решение *Setup.exe* файл заблокирован в процессе при публикации обновления. Сообщение может говорить следующее: "Не удается добавить ’setup.exe’ на веб-сайт. Файл ’setup.exe’ уже существует на этом веб-сайте".  
...
## <a name="prerequisites-for-microsoft-office-arent-installed"></a>Необходимые условия для Microsoft Office не установлены  
 Вы можете добавить .NET Framework, [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)]и основные сборки взаимодействия Office в пакет установки в качестве необходимых компонентов, развертываемых вместе с решением Office. Сведения о том, как установка основных сборок взаимодействия см. в разделе [Настройка компьютера для разработки решений Office](../vsto/configuring-a-computer-to-develop-office-solutions.md) и [как: установить основные сборки взаимодействия](../vsto/how-to-install-office-primary-interop-assemblies.md).  
...
## <a name="installation-fails-when-the-uri-has-characters-that-arent-us-ascii"></a>Если в URI входят символы, не входящие в набор US-ASCII, происходит сбой установки  
...
## <a name="uncaught-exception-or-method-not-found-error-when-you-install-a-solution"></a>Неперехваченное исключение или метода не найдена ошибка при установке решения  
...
## <a name="manifest-registry-keys-for-add-ins-change-after-an-installshield-limited-edition-project-is-built"></a>Разделы реестра для надстроек изменения манифеста, после построения проекта InstallShield Limited Edition  
 Раздел реестра манифеста, который является частью установки надстройки VSTO программы иногда изменяется с *.vsto* для *. dll.manifest* при сборке проекта InstallShield Limited Edition.  
...
## <a name="cant-install-a-clickonce-solution-by-opening-the-deployment-manifest-from-the-web"></a>Не удается установить решение Office путем открытия манифеста развертывания из Интернета  
 Пользователи могут установить решение Office путем открытия манифеста развертывания из Интернета. Однако в некоторых установках Internet Information Services (IIS) блоков *.vsto* расширение имени файла. Прежде чем использовать его для развертывания решения Office, необходимо определить тип MIME в IIS.  
