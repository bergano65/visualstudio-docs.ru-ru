...
### <a name="access-and-delete-controls"></a>Элементы управления доступом и delete  
 ...
 Не удается удалить статические элементы управления во время выполнения. Если вы попытаетесь использовать `Delete` или `Remove` метод, чтобы удалить статический элемент управления, <xref:Microsoft.Office.Tools.CannotRemoveControlException> будет создано.  
...
-   В Word следует использовать один из методов <xref:Microsoft.Office.Tools.Word.ControlCollection.AddControl%2A> объекта <xref:Microsoft.Office.Tools.Word.ControlCollection> .  
 ...
 Чтобы добавить элемент управления, передайте <xref:System.Windows.Forms.Control>, расположение элемента управления и имя, которое однозначно определяет элемент управления `AddControl` метод. `AddControl` Метод возвращает объект, который определяет, как элемент управления взаимодействует с листом или документом. `AddControl` Возвращает <xref:Microsoft.Office.Tools.Excel.ControlSite> (для Excel) или <xref:Microsoft.Office.Tools.Word.ControlSite> (для Word).  
...
### <a name="use-members-of-custom-controls"></a>Использование членов пользовательских элементов управления  
 После применения одного из `AddControl` методы, чтобы добавить элемент управления в лист или документ, вы теперь имеют два разных объекта элемента управления:  
...
-   Для доступа к членам, которые являются общими для элементов управления, используйте `ControlSite`, `OLEObject`, или `OLEControl` объекта.  
 ...
 При доступе к общему члену из <xref:System.Windows.Forms.Control>может возникнуть сбой без предупреждения или уведомления либо может быть получен недопустимый результат. Всегда использовать методы и свойства `ControlSite`, `OLEObject`, или `OLEControl` объект Если требуемые метод или свойство не существует; только следует ссылаться <xref:System.Windows.Forms.Control>.  

 Например, оба <xref:Microsoft.Office.Tools.Excel.ControlSite> класс и <xref:System.Windows.Forms.Control> класс иметь `Top` свойство. Чтобы получить или задать расстояние между верхней границей элемента управления и верхней частью документа, используйте свойство <xref:Microsoft.Office.Tools.Excel.ControlSite.Top%2A> объекта <xref:Microsoft.Office.Tools.Excel.ControlSite>, а не свойство <xref:System.Windows.Forms.Control.Top%2A> объекта <xref:System.Windows.Forms.Control>.  
