...
В данном примере данные извлекаются из примера базы данных Northwind (Борей) для заполнения поля со списком и меню в Microsoft Office Outlook. Элементы, выбираемые в этих элементах управления, автоматически заполнять поля, такие как **для** и **субъекта** в сообщении электронной почты.

В данном пошаговом руководстве рассмотрены следующие задачи:

- Создайте новый проект надстройки Outlook VSTO.
...
Лента для этого примера будет отображаться при составлении пользователем нового почтового сообщения. Чтобы создать пользовательскую группу для ленты, сначала добавьте элемент ленты в проект и затем создайте группу в конструкторе лент. Эта пользовательская группа поможет вам создавать сообщения сообщение электронной почты для пользователей путем подстановки имен и журналов заказов из базы данных.
...
3. Измените имя новой ленты на **CustomerRibbon**, а затем нажмите кнопку **добавить**.
     ...
     *CustomerRibbon.cs* или *CustomerRibbon.vb* файл открывается в конструкторе лент, отобразятся вкладка по умолчанию и группы.

4. Щелкните конструктор лент, чтобы выбрать его.

5. В **свойства** окно, щелкните стрелку раскрывающегося списка рядом с полем **RibbonType** , а затем щелкните **Microsoft.Outlook.Mail.Compose**.

     Это позволяет лента должна отображаться, когда пользователь создает новое почтовое сообщение в Outlook.

6. В конструкторе лент щелкните **Group1** чтобы выбрать его.

7. В **свойства** окне **метка** для **покупки клиента**.

8. Из **элементы управления ленты Office** вкладке **элементов**, перетащите **ComboBox** на **покупки клиента** группы.

9. Нажмите кнопку **ComboBox1** чтобы выбрать его.

10. В **свойства** окне **метка** для **клиентов**.

11. Из **элементы управления ленты Office** вкладке **элементов**, перетащите **меню** на **покупки клиента** группы.

12. В **свойства** окне **метка** для **приобретенный продукт**.

13. Задайте **динамическое** для **true**.
...
## <a name="add-the-custom-group-to-a-built-in-tab"></a>Добавление пользовательской группы на встроенную вкладку
...
2. В **свойства** окне разверните **ControlId** свойства, а затем задайте **OfficeId** для **TabNewMailMessage**.

     Это добавляет **покупки клиента** группа **сообщений** вкладки ленты, которая появляется в новое сообщение.

3. Нажмите кнопку **покупки клиента** группы, чтобы выбрать его.

4. В **свойства** окне разверните **позиции** свойство, щелкните стрелку раскрывающегося списка рядом с полем **меню** , а затем щелкните  **BeforeOfficeId**.

5. Задайте **OfficeId** свойства **установите значение GroupClipboard**.

     Это размещает **покупки клиента** группе перед **буфер обмена** группе **сообщений** вкладки.
...
3. Выберите **набора данных**, а затем нажмите кнопку **Далее**.

4. Выберите подключение данных к базе данных Microsoft SQL Server Compact 4.0 Northwind или добавьте новое подключение с помощью **новое подключение** кнопки.

5. После выбора или создания подключения нажмите кнопку **Далее**.

6. Нажмите кнопку **Далее** сохранить строку подключения.

7. На **Choose Your Database Objects** странице, разверните узел **таблиц**.
...
## <a name="update-controls-in-the-custom-group-at-runtime"></a>Обновление элементов управления в пользовательской группе во время выполнения

Используйте объектную модель ленты для выполнения следующих задач.

- Добавление имен клиентов к **клиентов** поле со списком.
...
- Заполнение To, тему и текст поля новых почтовых сообщений с помощью данных из **клиентов** поле со списком и **приобретенные продукты** меню.
...
Чтобы создать сообщение, сообщение электронной почты клиента, выберите клиента и выберите продукты, приобретенные им. Элементы управления в **покупки клиента** группы обновляются во время выполнения с данными из базы данных "Борей".
...
2. В Outlook на **файл** последовательно выберите пункты **New**, а затем нажмите кнопку **почтовое сообщение**.
    ...
    - На **сообщение** вкладку на ленте, **покупки клиента** группа появляется перед **буфер обмена** группы.

    - **Клиентов** поле со списком в группе обновляется имена заказчиков в базе данных "Борей".

3. На **сообщение** вкладке ленты в **покупки клиента** выберите клиента из **клиентов** поле со списком.

     Происходят следующие действия:

    - **Приобретенные продукты** меню обновляется для отображения каждого заказа на продажу для выбранного клиента.

    - Каждое подменю заказов на продажу обновится и будет показывать продукты, приобретенные в рамках данного заказа.

    - Адрес электронной почты выбранного заказчика добавляется в **для** строки сообщения и тему и текст сообщения электронной почты будет подставлен текст.

4. Нажмите кнопку **покупки продуктов** меню, выберите любой заказ на продажу и щелкните продукт из заказа на продажу.
