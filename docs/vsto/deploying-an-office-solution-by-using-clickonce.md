...
9. Если решение предназначено для .NET Framework 4.5, также выберите **Visual Studio 2010 Tools для Office Runtime** флажок.  
    ...
    > [!NOTE]  
    >  По умолчанию этот флажок не отображается. Чтобы отобразить этот флажок, необходимо создать пакет начального загрузчика. В разделе [создайте пакет начального загрузчика для Office 2013 VSTO надстройки с помощью Visual Studio 2012](http://blogs.msdn.com/b/vsto/archive/2012/12/21/creating-a-bootstrapper-package-for-an-office-2013-vsto-add-in-with-visual-studio-2012.aspx).  
...
13. Выберите **Опубликовать сейчас** кнопки.  
    ...
    > [!NOTE]  
    >  ClickOnce добавляет *.deploy* расширения сборок, чтобы защищенная установка служб Internet Information Services (IIS) не блокировала файлы из-за небезопасного расширения. Когда пользователь установит решение, ClickOnce удалит *.deploy* расширения.  
...
##  <a name="Trust"></a> Выбрать способ предоставления доверия решению  
 Прежде чем решение можно будет запускать на компьютерах пользователей, необходимо предоставить доверие. В противном случае пользователи должны будут ответить на запрос о доверии при установке решения. Чтобы предоставить доверие решению, подпишите манифест с помощью сертификата, который определяет известного и надежного издателя. В разделе [доверие, подписание манифестов приложения и развертывания решения](../vsto/granting-trust-to-office-solutions.md#Signing).  
  
 При развертывании настройки уровня документа и размещении документа в папку на компьютере пользователя или предоставлении доступа к документу на сайте SharePoint, убедитесь, что Office доверяет расположению документа. В разделе [предоставить доверие к документам](../vsto/granting-trust-to-documents.md).  
...
### <a name="install-the-solution-by-opening-the-deployment-manifest-from-an-iis-website"></a>Установка решения путем открытия манифеста развертывания с веб-сайта IIS  
 Пользователи могут установить решение Office путем открытия манифеста развертывания из Интернета. Однако защищенная установка служб Internet Information Services (IIS) будет блокировать файлы имеют *.vsto* расширения. Тип MIME должен быть определен в IIS перед развертыванием решения Office с использованием IIS.  
...
6.  В **тип MIME** окно, введите **.vsto** в качестве расширения и введите **приложения/x-ms-vsto** как MIME введите, а затем применить новые параметры.  
    ...
    > [!NOTE]  
    >  Чтобы изменения вступили в силу, необходимо перезапустить службу веб-публикаций или подождать, пока рабочий процесс будет перезапущен. Необходимо затем очистить кэш диска браузера и снова попытаться открыть *.vsto* файл еще раз.  
...
6.  В строке меню выберите **проекта** > **добавить ссылку**.  
...
7.  Ввод **данные**, а затем выберите **ввод** ключа.  
...
2.  Воспользуйтесь одним из следующих подходов, выполнив соответствующие инструкции.  
    ...
    -   Убедитесь, что каждый пользователь выполнил следующие действия.  
        ...
        4.  В **путь** введите URL-адрес библиотеки документов SharePoint, которая содержит загруженный документ (например, *http://SharePointServerName/TeamName/ProjectName/DocumentLibraryName*).  
             ...
             Не следует добавлять имя веб-страницы по умолчанию, таких как *default.aspx* или *AllItems.aspx*.  
...
##  <a name="Custom"></a> Создание пользовательского установщика  
 ...
 В составе пользовательского процесса установки вызовите средство установки решений Office (*VSTOInstaller.exe*), который устанавливается в следующий каталог по умолчанию:  
  
 *%CommonProgramFiles%\Microsoft shared\VSTO\10.0\VSTOInstaller.exe*  
  
 Если это средство находится в этом расположении, можно использовать **HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\VSTO Runtime Setup\v4\InstallerPath** или **Setup\v4 HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\VSTO среды выполнения \InstallerPath** раздел реестра, чтобы найти путь к нему.  
...
|-100|Один или несколько параметров командной строки недопустимы или заданы более одного раза. Дополнительные сведения, введите «vstoinstaller /?» или в разделе [Создание пользовательского установщика для решения ClickOnce Office](http://msdn.microsoft.com/en-us/3e5887ed-155f-485d-b8f6-3c02c074085e).|  
...
|-300|Не удалось установить решение, так как произошло исключение безопасности. В разделе [решений Office, защита](../vsto/securing-office-solutions.md).|  
...
## <a name="see-also"></a>См. также  
 ...
 [Создание пользовательского установщика для решения office ClickOnce](http://msdn.microsoft.com/en-us/3e5887ed-155f-485d-b8f6-3c02c074085e)  
