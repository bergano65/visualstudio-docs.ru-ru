---
title: Рекомендации по обеспечению безопасности для решений Office
ms.custom: ''
ms.date: 02/02/2017
ms.technology:
- office-development
...
## <a name="trusted-code-is-repurposed-in-a-new-malicious-document"></a>Доверенный код перенаправляется в новый, вредоносный документ  
...
## <a name="code-is-blocked-by-the-outlook-object-model-guard"></a>Код, блокируемый системой безопасности объектной модели Outlook  
 Microsoft Office может запретить коду использовать определенные свойства, методы и объекты в объектной модели. Ограничивая доступ к этим объектам, Outlook помогает предотвратить использование объектной модели в злонамеренных целях червями электронной почты и вирусов. Эта функция безопасности называется системой безопасности объектной модели Outlook. Если надстройка VSTO пытается использовать запрещенное свойство или метод, при включенном объектной модели, Outlook отображает предупреждение системы безопасности, который позволяет остановить операцию или позволяет пользователю предоставить доступ к свойству или методу на ограниченный период t IME. Если пользователь останавливает операцию, надстройки VSTO Outlook, созданные с помощью решений Office в Visual Studio, будут создавать исключение <xref:System.Runtime.InteropServices.COMException>.  
...
-   Если Outlook используется с Exchange, администратор может включать или отключать систему безопасности объектной модели для всех надстроек VSTO на компьютере, а также может указать, что определенные надстройки VSTO могут выполняться без задействования системы безопасности объектной модели. Администраторы также могут изменять поведение системы безопасности объектной модели в некоторых областях объектной модели. Например Администраторы могут автоматически разрешать надстроек VSTO для отправки электронной почты программными средствами, даже если объектной модели включена.  
  
 Начиная с Outlook 2007 поведение объектной модели было изменено, чтобы улучшить взаимодействие с пользователем и разработчиком, продолжая поддерживать безопасность Outlook. Дополнительные сведения см. в разделе [кода изменения системы безопасности в Outlook 2007](http://go.microsoft.com/fwlink/?LinkId=73429).  
  
### <a name="minimize-object-model-guard-warnings"></a>Свести к минимуму предупреждений системы безопасности объектной модели объектов  
 Чтобы избежать излишних предупреждений системы безопасности при использовании ограниченных свойств и методов, убедитесь, что ваша надстройка VSTO получает объекты Outlook из поля `Application` класса `ThisAddIn` в проекте. Дополнительные сведения об этом поле см. в разделе [программы VSTO Add-ins](../vsto/programming-vsto-add-ins.md).  
  
 Система безопасности объектной модели считает надежными только объекты Outlook, полученные из этого объекта. Напротив, объекты, полученные из нового `Microsoft.Office.Interop.Outlook.Application` объекта не являются доверенными и Ограниченные свойства и методы будут вызывать предупреждения безопасности, если включен объектной модели.  
  
 Следующий пример кода отображает предупреждение системы безопасности, если система безопасности объектной модели включена. `To` Свойство `Microsoft.Office.Interop.Outlook.MailItem` класса ограничен в объектной модели. `Microsoft.Office.Interop.Outlook.MailItem` Объект не является доверенным, поскольку код получает его из `Microsoft.Office.Interop.Outlook.Application` , созданного при помощи **новый** оператор, вместо получения его из `Application` поля.  
  
 [!code-csharp[Trin_VstcoreOutlookSecurity#1](../vsto/codesnippet/CSharp/Trin_VstcoreOutlookSecurity/ThisAddIn.cs#1)]
 [!code-vb[Trin_VstcoreOutlookSecurity#1](../vsto/codesnippet/VisualBasic/Trin_VstcoreOutlookSecurity/ThisAddIn.vb#1)]  
  
 В следующем примере кода демонстрируется использование ограниченного свойства `Microsoft.Office.Interop.Outlook.MailItem` объект, который является доверенным для объектной модели. Код использует доверенное `Application` для получения `Microsoft.Office.Interop.Outlook.MailItem`.  
...
### <a name="specify-which-add-ins-to-trust-when-using-exchange"></a>Укажите, какие надстройки должны доверять при использовании Exchange  
 ...
 Outlook доверяет надстройке VSTO, исходя из хэш-кода DLL точки входа надстройки VSTO. Все Outlook VSTO Add-ins, предназначенных для [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] использовать же DLL точки входа (*VSTOLoader.dll*). Это означает, что если администратор доверяет любой надстройки VSTO, предназначенного [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] разрешается выполняться без системы безопасности объектной модели, а затем все другие надстройки VSTO, предназначенных [!INCLUDE[vsto_runtime](../vsto/includes/vsto-runtime-md.md)] , также являются доверенными. Дополнительные сведения об указании конкретных доверенных надстроек VSTO, которым разрешается выполняться без системы безопасности объектной модели, см. в разделе [Выбор метода, используемого Outlook для управления функциями антивирусной защиты](http://go.microsoft.com/fwlink/?LinkId=128773).  
...
## <a name="trust-center-settings-in-the-microsoft-office-system-do-not-affect-add-ins-or-document-level-customizations"></a>Параметры центра управления безопасностью в системе Microsoft Office не влияют на надстройки или настройки уровня документа  
