...
- Файл кода ThisAddIn. Этот файл содержит созданный класс `ThisAddIn` для надстройки VSTO. Дополнительные сведения об этом классе см. в разделе [программы VSTO Add-Ins](../vsto/programming-vsto-add-ins.md).  
...
### <a name="excel-worksheets-and-workbooks-in-the-designer"></a>Листы Excel и книгах в конструкторе  
 Когда лист открыт в конструкторе, его можно изменять точно так же, как если бы он был открыт напрямую в приложении Excel. При двойном щелчке в ячейке листа она переключается в режим редактирования. Если дважды щелкнуть ячейку, которая содержит элемент управления ведущего приложения, открывается редактор кода и Visual Studio создает обработчик событий по умолчанию для элемента управления. Для перехода на другие листы можно использовать вкладки листов, отображаемые в нижней части конструктора.  
...
### <a name="design-mode-vs-runtime-mode"></a>Режим конструктора и режим среды выполнения  
 ...
 Чтобы просмотреть документ в *режим среды*, необходимо открыть приложение и вне среды Visual Studio. Также можно построить и выполнить проект. В этом случае документ автоматически открывается в приложении вне среды Visual Studio.  
...
## <a name="code-editor"></a>Редактор кода  
 ...
 Дополнительные сведения о редакторе кода см. в разделе [писать код в редакторе кода и текста](/visualstudio/ide/writing-code-in-the-code-and-text-editor). Дополнительные сведения о написании кода в проектах Office см. в разделе [написания кода в решениях Office](../vsto/writing-code-in-office-solutions.md).  
