...
3.  На **параметры конфиденциальности** вкладке Очистить **удалять личные сведения из свойств файла при сохранении** "флажок".
...
### <a name="controls-on-a-word-template-are-not-visible-in-visual-studio"></a>Элементы управления в шаблоне Word не видны в Visual Studio
 При открытии шаблона Word в конструкторе Visual Studio элементы управления шаблона, расположенные не в тексте, могут не отображаться. Это обусловлено тем Visual Studio открывает шаблон Word в **обычный** представления. Чтобы увидеть эти элементы управления, нажмите кнопку **представление** последовательно выберите пункты **вид Microsoft Office Word** и нажмите кнопку **разметка страницы**.
...
### <a name="insert-clip-art-command-does-nothing-in-the-visual-studio-designer"></a>Вставка картинки из коллекции картинок не выполняет никаких действий в конструкторе Visual Studio
 Если Excel или Word открыт в конструкторе Visual Studio, при нажатии **картинки** , нажмите кнопку **иллюстрации** не открывается вкладка на ленте **картинки** области задач. Чтобы добавить коллекции картинок, необходимо открыть копию книги или документа из главной папки проекта (не к копии, который находится в *\bin* папку) вне Visual Studio, добавить картинки из коллекции картинок, а затем сохраните книгу или документ.
...
### <a name="some-events-of-office-objects-are-not-accessible-when-using-c"></a>Некоторые события объектов Office недоступны при использовании C#
 ...
 Эта ошибка означает, что вы пытаетесь получить доступ к событию, имя которого совпадает с именем другого свойства или метода объекта. Для доступа к событию, необходимо привести объект к его *интерфейса событий*.

 Типы основных сборок взаимодействия Office с событиями реализуют два интерфейса: основной интерфейс со свойствами и методами и интерфейс событий, содержащий события, предоставляемые объектом. Этих интерфейсах событий используется соглашение об именовании *objectname*события*n*_Event, например <xref:Microsoft.Office.Interop.Excel.AppEvents_Event> и <xref:Microsoft.Office.Interop.Word.ApplicationEvents2_Event>. Если вам не удается получить доступ к событию, которое должно быть в объекте, приведите объект к типу соответствующего интерфейса событий.
...
```
 ...
 В проекты, предназначенные для [!INCLUDE[net_v40_short](../sharepoint/includes/net-v40-short-md.md)] или [!INCLUDE[net_v45](../vsto/includes/net-v45-md.md)], по умолчанию внедряются все типы из основных сборок взаимодействия Office. Эта ошибка компиляции возникает потому, что типы внедренных сборок взаимодействия работают только с интерфейсами, а не классами. Дополнительные сведения об интерфейсах и классах сборок взаимодействия Office см. в разделе [Общие сведения о классах и интерфейсах в основных сборках взаимодействия Office](http://go.microsoft.com/fwlink/?LinkId=189592). Дополнительные сведения о функции внедренные типы взаимодействия в проектах Office см. в разделе [проектирования и создания решений Office](../vsto/designing-and-creating-office-solutions.md).

### <a name="references-to-office-classes-are-not-recognized"></a>Ссылки на классы Office не распознаются.
 Некоторые имена классов, например приложения, находятся в нескольких пространств имен, таких как <xref:Microsoft.Office.Interop.Word> и <xref:System.Windows.Forms>. По этой причине **Imports**/**с помощью** инструкция в верхней части шаблонов проекта содержит краткую уточняющую константу, например:

 [!code-csharp[Trin_VstcoreTroubleshootingWord#2](../vsto/codesnippet/CSharp/Trin_VstcoreTroubleshootingWordCS/ThisDocument.cs#2)]
 [!code-vb[Trin_VstcoreTroubleshootingWord#2](../vsto/codesnippet/VisualBasic/Trin_VstcoreTroubleshootingWordVB/ThisDocument.vb#2)]

 Это использование **Imports**/**с помощью** необходимо разграничивать ссылки на классы Office с помощью квалификатора Word или Excel, например:
 ...
 Несмотря на то, что пространство имен Word или Excel было импортировано и имеют доступ ко всем классам внутри него, необходимо полностью указать все типы в Word или Excel, для устранения неоднозначности имен.

##  <a name="building"></a> Построение проектов
 При сборке проектов Office могут возникнуть следующие ошибки.

### <a name="cannot-build-a-document-level-project-that-is-based-on-a-document-with-restricted-permissions"></a>Не удается создать проект уровня документа, основанного на документе с ограниченными разрешениями
...
### <a name="error-when-stopping-the-debugger-in-a-document-level-project-after-using-edit-and-continue"></a>Ошибка при остановке отладчика в проекте уровня документа после использования, изменить и продолжить
 Если вы используете **изменить** и **Продолжить** для внесения изменений в код в проекте уровня документа для Excel или Word, когда проект находится в режиме приостановки выполнения, может появиться диалоговое окно с сообщение об ошибке при вы впоследствии остановите отладчик.
