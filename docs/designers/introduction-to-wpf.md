...
##  <a name="Controls"></a> Элементы управления  
 ...
 Дополнительные сведения см. в статье [Элементы управления](http://msdn.microsoft.com/Library/3f255a8a-35a8-4712-9065-472ff7d75599).  
...
-   **Отображение и выделение данных:** <xref:System.Windows.Controls.Calendar> и <xref:System.Windows.Controls.DatePicker>.  
...
-   **Выделение:** <xref:System.Windows.Controls.CheckBox>, <xref:System.Windows.Controls.ComboBox>, <xref:System.Windows.Controls.ListBox>, <xref:System.Windows.Controls.RadioButton> и <xref:System.Windows.Controls.Slider>.  
  
-   **Сведения о пользователе:** <xref:System.Windows.Controls.AccessText>, <xref:System.Windows.Controls.Label>, <xref:System.Windows.Controls.Primitives.Popup>, <xref:System.Windows.Controls.ProgressBar>, <xref:System.Windows.Controls.Primitives.StatusBar>, <xref:System.Windows.Controls.TextBlock> и <xref:System.Windows.Controls.ToolTip>.  
...
-   <xref:System.Windows.Controls.DockPanel>: дочерние элементы управления выравнены по краям области.  
  
-   <xref:System.Windows.Controls.Grid>: дочерние элементы управления размещаются по строкам и столбцам.  
...
-   <xref:System.Windows.Controls.VirtualizingStackPanel>: дочерние элементы управления являются виртуализированными и располагаются в одной строке (горизонтальной или вертикальной).  
  
-   <xref:System.Windows.Controls.WrapPanel>: дочерние элементы управления расположены слева направо и переносятся на следующую строку, если не помещаются в текущей.  
  
 В примере ниже используется <xref:System.Windows.Controls.DockPanel>, чтобы представить несколько элементов управления <xref:System.Windows.Controls.TextBox>.  
  
 [!code-xml[IntroToWPFSnippets#LayoutMARKUP](../designers/codesnippet/Xaml/introduction-to-wpf_1.xaml)]  
  
 <xref:System.Windows.Controls.DockPanel> позволяет дочерним элементам управления <xref:System.Windows.Controls.TextBox> задавать способ их размещения. Для этого в <xref:System.Windows.Controls.DockPanel> реализуется свойство <xref:System.Windows.Controls.DockPanel.Dock%2A>, представляемое дочерним элементам управления, чтобы каждый из них указывал стиль закрепления.  
...
2.  обеспечение копирования изменений, внесенных в данные с помощью элементов управления, обратно в управляемые объекты.  
 ...
 Чтобы упростить разработку приложений, платформа WPF предоставляет механизм привязки данных для автоматического выполнения этих действий. Основным элементом механизма привязки данных является класс <xref:System.Windows.Data.Binding>, который имеет задачу привязать элемент управления (целевой объект привязки) к объекту данных (источнику привязки). Эта связь показана на рисунке ниже.  
  
 ![Основная схема привязки данных](../designers/media/databindingmostbasic.png "DataBindingMostBasic")  
  
 В примере ниже показано, как привязать <xref:System.Windows.Controls.TextBox> к экземпляру настраиваемого объекта `Person`. Реализация `Person` показана в следующем коде:  
 ...
 Разметка ниже привязывает <xref:System.Windows.Controls.TextBox> к экземпляру настраиваемого объекта `Person`.  
...
[!code-xml[SimpleDataBindingSnippets#DataBindingMARKUP3](../designers/codesnippet/Xaml/introduction-to-wpf_5.xaml)]  
 ...
 В этом примере экземпляр класса `Person` создается в коде программной части и устанавливается в качестве контекста данных для `DataBindingWindow`. В разметке свойство <xref:System.Windows.Controls.TextBox.Text%2A> элемента <xref:System.Windows.Controls.TextBox> привязывается к свойству `Person.Name` (с помощью синтаксиса XAML "`{Binding ... }`"). Данный XAML дает указание WPF связать элемент управления <xref:System.Windows.Controls.TextBox> с объектом `Person`, который хранится в свойстве окна <xref:System.Windows.FrameworkElement.DataContext%2A>.  
...
### <a name="2-d-shapes"></a>Двумерные фигуры  
 ...
 Интересной особенностью фигур является то, что они предназначены не только для отображения. В них реализованы многие возможности элементов управления, включая ввод с клавиатуры и с помощью мыши. В примере ниже показана обработка события <xref:System.Windows.UIElement.MouseUp> в <xref:System.Windows.Shapes.Ellipse>.  
...
### <a name="2-d-geometries"></a>Двумерные геометрические объекты  
 ...
 Вы можете использовать объекты <xref:System.Windows.Shapes.Path> для рисования замкнутых и незамкнутых фигур, нескольких фигур и даже криволинейных.  
  
 Вы можете использовать объекты <xref:System.Windows.Media.Geometry> для обрезки, проверки попадания, а также для отрисовки двумерных графических данных.  
...
### <a name="video-and-audio"></a>Видео и звук  
 Элемент управления <xref:System.Windows.Controls.MediaElement> позволяет воспроизводить как видео, так и звук. Он достаточно гибок, чтобы служить основой для пользовательского проигрывателя мультимедиа. Приведенная ниже разметка XAML реализует мультимедиапроигрыватель.  
  
 [!code-xml[IntroToWPFSnippets#MediaElementMARKUP](../designers/codesnippet/Xaml/introduction-to-wpf_9.xaml)]  
  
 В окне в примере ниже отображается элемент управления <xref:System.Windows.Controls.MediaElement> в действии.  
...
##  <a name="Text_and_Typography"></a> Текст и оформление  
...
[!code-xml[IntroToWPFSnippets#TextBoxContentMARKUP3](../designers/codesnippet/Xaml/introduction-to-wpf_12.xaml)]  
 ...
 Другие же элементы управления могут содержать несколько элементов с различным типом содержимого. Содержимое элемента <xref:System.Windows.Controls.Button>, определенного свойством <xref:System.Windows.Controls.ContentControl.Content%2A>, может содержать целый ряд элементов, включая элементы управления макетом, текст, изображения и фигуры. В примере ниже показан элемент <xref:System.Windows.Controls.Button> с содержимым, которое включает элементы <xref:System.Windows.Controls.DockPanel>, <xref:System.Windows.Controls.Label>, <xref:System.Windows.Controls.Border> и <xref:System.Windows.Controls.MediaElement>.  
...
### <a name="control-templates"></a>Шаблоны элементов управления  
 Пользовательские интерфейсы по умолчанию для элементов управления WPF обычно формируются на основе других элементов управления и фигур. Например, элемент управления <xref:System.Windows.Controls.Button> состоит из элементов <xref:Microsoft.Windows.Themes.ButtonChrome> и <xref:System.Windows.Controls.ContentPresenter>. Элемент <xref:Microsoft.Windows.Themes.ButtonChrome> обеспечивает стандартный вид кнопки, а элемент <xref:System.Windows.Controls.ContentPresenter> отображает содержимое кнопки, заданное свойством <xref:System.Windows.Controls.ContentControl.Content%2A>.  
  
 Иногда внешний вид элемента управления по умолчанию может не согласовываться с общим оформлением приложения. В этом случае вы можете использовать элемент <xref:System.Windows.Controls.ControlTemplate>, чтобы изменить внешний вид интерфейса элемента управления, не меняя его содержимое и поведение.  
  
 В примере ниже показано, как изменить внешний вид элемента <xref:System.Windows.Controls.Button> с помощью <xref:System.Windows.Controls.ControlTemplate>.  
 ...
 В этом примере кнопка пользовательского интерфейса по умолчанию была заменена элементом <xref:System.Windows.Shapes.Ellipse>, который имеет темно-синюю границу и заполнен с помощью элемента <xref:System.Windows.Media.RadialGradientBrush>. Элемент управления <xref:System.Windows.Controls.ContentPresenter> отображает содержимое элемента <xref:System.Windows.Controls.Button> — "Click Me!" Если вы щелкнете элемент <xref:System.Windows.Controls.Button>, событие <xref:System.Windows.Controls.Primitives.ButtonBase.Click> по-прежнему вызывается как часть поведения по умолчанию элемента <xref:System.Windows.Controls.Button>. Результат показан на примере ниже.  
...
### <a name="data-templates"></a>Шаблоны данных  
 В то время как шаблон элемента управления позволяет определять внешний вид элемента управления, шаблон данных дает возможность настраивать оформление его содержимого. Шаблоны данных часто используются для оптимизации отображения привязанных данных. На рисунке ниже показано стандартное оформление элемента управления <xref:System.Windows.Controls.ListBox>, привязанного к коллекции объектов `Task`, в которой у каждой задачи есть имя, описание и приоритет.  
  
 ![Список с оформлением по умолчанию](../designers/media/wpfintrofigure18.png "WPFIntroFigure18")  
  
 Оформление по умолчанию является стандартным для элемента управления <xref:System.Windows.Controls.ListBox>. Однако оно предполагает, что для каждой задачи отображается только ее название. Чтобы отобразить имя, описание и приоритет задачи, необходимо изменить оформление по умолчанию для элементов списка, привязанных к элементу управления <xref:System.Windows.Controls.ListBox>, с помощью <xref:System.Windows.DataTemplate>. Приведенный ниже код XAML определяет такой шаблон <xref:System.Windows.DataTemplate>, который применяется к каждой задаче с помощью атрибута <xref:System.Windows.Controls.ItemsControl.ItemTemplate%2A>.  
...
[!code-xml[IntroToWPFSnippets#DataTemplateMARKUP4](../designers/codesnippet/Xaml/introduction-to-wpf_21.xaml)]  
 ...
 Обратите внимание, что элемент управления <xref:System.Windows.Controls.ListBox> сохранил свое поведение и общий внешний вид. Изменилось только оформление содержимого, отображаемого в списке.  
...
### <a name="resources"></a>Ресурсы  
 ...
 В примере ниже определяется общий цвет фона для элементов <xref:System.Windows.Controls.Button> и <xref:System.Windows.Controls.Label>.  
...
-   **Модель пользовательского элемента управления**. Пользовательский элемент управления наследуется от элемента <xref:System.Windows.Controls.UserControl> и состоит из одного или нескольких элементов управления.  
  
-   **Модель элемента управления**. Пользовательский элемент управления наследуется из элемента <xref:System.Windows.Controls.Control> и используется для создания реализаций, в которых поведение и внешний вид разделяются с помощью шаблонов, как в большинстве элементов управления WPF. Наследование от элемента <xref:System.Windows.Controls.Control> обеспечивает большую гибкость при создании собственного пользовательского интерфейса, чем пользовательские элементы управления, но может потребовать больших усилий.  
  
-   **Модель элемента платформы**. Пользовательский элемент управления наследуется от элемента <xref:System.Windows.FrameworkElement>, если его внешний вид определяется пользовательской логикой отрисовки, а не шаблонами.  
  
 В примере ниже показан пользовательский элемент управления для увеличения или уменьшения числового значения, наследуемый от элемента <xref:System.Windows.Controls.UserControl>.  
 ...
 В примере ниже показан код XAML, необходимый для включения пользовательского элемента управления в элемент <xref:System.Windows.Window>.  
  
 [!code-xml[IntroToWPFSnippets#UserControlWindowMARKUP1](../designers/codesnippet/Xaml/introduction-to-wpf_37.xaml)]  
  
 На рисунке ниже показан элемент управления `NumericUpDown`, размещенный в <xref:System.Windows.Window>.  
...
-   [Безопасность (WPF)](https://msdn.microsoft.com/en-us/library/aa970906\(v=vs.100\).aspx)  
