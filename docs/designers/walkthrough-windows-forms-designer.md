...
1. Дважды щелкните значок элемента управления `TextBox` на **панели элементов**.
...
7. Дважды щелкните значок `Button` на **панели элементов**.

   Элемент управления `Button` разместится в следующей свободной ячейке элемента управления `TableLayoutPanel`.

8. На **панели элементов** дважды щелкните значок `Button` еще четыре раза, чтобы заполнить вторую строку элемента управления `TableLayoutPanel`.
...
Кнопки в элементе управления DemoCalculator имеют обработчики событий, которые можно использовать для реализации большей части логики калькулятора. Конструктор Windows Forms позволяет реализовать заглушки всех обработчиков событий для всех кнопок одним двойным щелчком.
...
3. Перейдите на вкладку **Проекты** и дважды щелкните проект DemoCalculatorLib, чтобы добавить ссылку на тестовый проект.
