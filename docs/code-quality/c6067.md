...
```
 ...
 В следующем коде создается это предупреждение, так как при передаче параметра buffer в `scanf` был указан неверный уровень косвенного обращения:
