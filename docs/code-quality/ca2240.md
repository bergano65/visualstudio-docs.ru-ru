...
- Тип наследует, но не переопределяет метод <xref:System.Runtime.Serialization.ISerializable.GetObjectData%2A?displayProperty=fullName>, а тип объявляет поля экземпляра, не помеченные атрибутом <xref:System.NonSerializedAttribute?displayProperty=fullName>.
...
## <a name="example"></a>Пример
Следующий пример устраняет два предыдущих нарушения, предоставляя переопределяемую реализацию <xref:System.Runtime.Serialization.ISerializable.GetObjectData> в классе Book и предоставляя реализацию `GetObjectData` для класса Library.
