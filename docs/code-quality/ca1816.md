...
|CheckId|CA1816|
|Категория|Майкрософт (Microsoft). Использование|
...
Метод <xref:System.IDisposable.Dispose%2A?displayProperty=nameWithType> позволяет пользователям освобождать ресурсы в любое время, прежде чем объект станет доступным для сборки мусора. Если вызывается метод <xref:System.IDisposable.Dispose%2A?displayProperty=nameWithType>, он освобождает ресурсы объекта. Это делает завершение ненужным. <xref:System.IDisposable.Dispose%2A?displayProperty=nameWithType> должен вызывать <xref:System.GC.SuppressFinalize%2A?displayProperty=nameWithType>, чтобы сборщик мусора не вызывал метод завершения объекта.

Чтобы в производных типах с методами завершения не пришлось повторно реализовывать <xref:System.IDisposable> и вызывать его, незапечатанные типы без методов завершения должны по-прежнему вызывать <xref:System.GC.SuppressFinalize%2A?displayProperty=nameWithType>.
...
- Измените все вызовы на <xref:System.GC.SuppressFinalize%2A?displayProperty=nameWithType> для передачи [этого (C#)](/dotnet/csharp/language-reference/keywords/this) или [Me (Visual Basic)](/dotnet/visual-basic/programming-guide/program-structure/me-my-mybase-and-myclass#me).
...
- @NO__T 0CA2215: Методы Dispose должны вызывать базовый класс Dispose @ no__t-0
