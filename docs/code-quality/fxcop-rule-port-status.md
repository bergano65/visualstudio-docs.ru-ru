---
title: Статус порта правила FxCop
ms.date: 05/21/2019
ms.topic: reference
helpviewer_keywords:
- fxcop rules
...
ms.locfileid: "81649358"
---
# <a name="fxcop-rule-port-status"></a>Статус порта правила Fxcop

Если вы ранее использовали статический анализ кода в Visual Studio, вы можете быть удивлены, какие из этих правил доступны в текущей реализации, как [анализаторы FxCop.](install-fxcop-analyzers.md) На этой странице перечислены правила, которые портируются, а также те, которые не были портированы и есть ли планы их переноса.

## <a name="ported-rules"></a>Перенесенные правила

На [странице автоматической документации](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md) в репо рослин-анализаторов содержится самый актуальный список правил, которые были портированы в анализаторы FxCop. Эта страница также содержит дополнительную информацию, например, включено ли правило по умолчанию и имеет ли оно *связанное исправление кода.* ([Исправления кода](../ide/quick-actions.md) являются одним щелчком мыши исправления доступны в меню значок лампочки в Visual Studio.)

На дату на этой странице, список правил FxCop, которые были портированы на [анализаторы FxCop](install-fxcop-analyzers.md) включает в себя:
...
[CA1014](ca1014-mark-assemblies-with-clscompliantattribute.md) | Марк сборки с CLSСССмульант
[CA1016](ca1016-mark-assemblies-with-assemblyversionattribute.md) | Сборки марок с сборочной версией
[CA1017](ca1017-mark-assemblies-with-comvisibleattribute.md) | Марк сборки с ComVisible
...
[CA1028](ca1028-enum-storage-should-be-int32.md) | Enum Хранение должно быть Int32
...
[CA1043](ca1043-use-integral-or-string-argument-for-indexers.md) | Используйте интегральный или строковый аргумент для индексаторов
...
[CA1052](ca1052-static-holder-types-should-be-sealed.md) | Статические типы держателей должны быть статичными или ненаследуется
[CA1053](ca1053-static-holder-types-should-not-have-constructors.md) | Статические типы держателей не должны иметь конструкторов (CA1053 является частью [CA1052](ca1052-static-holder-types-should-be-sealed.md) для анализаторов FxCop)
[CA1054](ca1054-uri-parameters-should-not-be-strings.md) | Параметры Uri не должны быть строками
[CA1055](ca1055-uri-return-values-should-not-be-strings.md) | Uri возврат значения не должны быть строки
[CA1056](ca1056-uri-properties-should-not-be-strings.md) | Uri свойства не должны быть строки
[CA1058](ca1058-types-should-not-extend-certain-base-types.md) | Типы не должны расширять определенные базовые типы
[CA1060](ca1060-move-p-invokes-to-nativemethods-class.md) | Перемещение pinvokes к классу родных методов
...
[CA1063](ca1063-implement-idisposable-correctly.md) | Правильно реализовать IОдноразовые
...
CA1066 | Тип {0} должен реализовать IEquatable\<T>, поскольку он переопределяет Равные
CA1067 | Переопределение Object.Equals(объект) при реализации\<IEquatable T>
...
[CA1309](ca1309-use-ordinal-stringcomparison.md) | Использование ординаторного сравнения строк
...
[CA1507](ca1507.md) | Используйте имя для выражения имен символов
CA1508 | Избегайте мертвого условного кода
CA1509 | Недействительная запись в файле спецификации спецификации кодовых метрик
...
[CA1724](ca1724-type-names-should-not-match-namespaces.md) | Имена типов не должны совпадать с пространствами имен
...
[CA1802](ca1802.md) | Используйте буквальные буквы, когда это уместно
...
[CA1816](ca1816.md) | Методы утилизации должны вызывать SuppressFinalize
...
[CA1821](ca1821.md) | Удалить пустые финализаторы
...
CA1825 | Избегайте выделений массивов с нулевой длиной.
CA1826 | Не используйте перечисленные методы в индексируемых коллекциях. Вместо этого используйте коллекцию напрямую
...
[CA2007](ca2007.md) | Рассмотрите возможность вызова ConfigureAWait на ожидаемую задачу
CA2008 | Не создавайте задачи без прохождения TaskScheduler
CA2009 | Не звоните в ToImmutableCollection по значению ImmutableCollection
CA2010 | Всегда потребляйте значение, возвращенное методами, отмеченными PreserveSigAttribute
...
[CA2153](ca2153.md) | Не ловите испорченные исключения государства
[CA2200](ca2200.md) | Перебросьте, чтобы сохранить детали стека.
...
[CA2219](ca2219.md) | Не выними исключений в, наконец, положениях
[CA2224](ca2224.md) | Переопределение Равные при перегрузке оператора равно
...
[CA2231](ca2231.md) | Оператор перегрузки равен по переопределения типу значения Равные
[CA2234](ca2234.md) | Система передачи uri объектов вместо строк
...
CA2244 | Не дублируйте индексированные элементы инициализации
...
CA3061 | Не добавляйте схему по URL
[CA3075](ca3075.md) | Небезопасная обработка DTD в формате XML
[CA3076](ca3076.md) | Небезопасная обработка скриптов XSLT.
[CA3077](ca3077.md) | Небезопасная обработка в Дизайне API, XmlDocument и XmlTextReader
[CA3147](ca3147.md) | Марк Глагол Обработчики с проверкой антиподделки токен
[CA5350](ca5350.md) | Не используйте ненадежные алгоритмы шифрования
[CA5351](ca5351.md) | Не используйте сломанные криптографические алгоритмы
CA5358 | Не используйте небезопасные режимы шифрования
CA5359 | Не отменяйте проверку сертификата
CA5360 | Не называйте опасные методы десериализации
CA5361 | Не отменяйте SChannel Использование сильного Crypto
CA5362 | Не ссылайтесь на себя в серийном классе
CA5363 | Не отменяйте проверку запроса
CA5364 | Не используйте протоколы безопасности с удручаемыми
CA5365 | Не отменяйте проверку заголовка HTTP
CA5366 | Используйте XmlReader для набора данных Читать Xml
CA5367 | Не серизировать типы с полями указателей
CA5368 | Установите ViewStateUserKey для классов, полученных из страницы
CA5369 | Используйте XmlReader для десериализации
CA5370 | Используйте XmlReader для проверки чтения
CA5371 | Использование XmlReader для схемы Читать
CA5372 | Используйте XmlReader для xpathDocument
CA5373 | Не использовать устаревшую функцию формирования ключа
CA5374 | Не используйте XslTransform
CA5375 | Не использовать общую подпись доступа к учетной записи
CA5376 | Используйте SharedAccessProtocol HttpsOnly
CA5377 | Использование политики доступа к уровню контейнеров
CA5378 | Не отключайте ServicePointManagerSecurityProtocols
CA5379 | Не используйте слабый алгоритм функции произвхотливости ключа
CA9999 | Несоответствие версии анализатора

## <a name="unported-rules"></a>Непортированные правила

Набор правил, которые не были портированы в [анализаторы FxCop,](install-fxcop-analyzers.md) состоит из правил, которые еще не были, но все еще [могут быть портированы,](#rules-that-may-be-ported)и тех, которые амортизированы и [не будут портированы.](#deprecated-rules)

### <a name="rules-that-may-be-ported"></a>Правила, которые могут быть портированы

Следующие правила анализа наследия FxCop еще не реализованы в качестве анализаторов, но все же могут быть реализованы. Это может быть из-за блокировки технической причины или просто, что правило является более низким приоритетом. Для получения дополнительной информации о статусе переноса каждого правила нажмите на ссылку в столбце **проблемы отслеживания.**

Идентификатор правила | Проблема отслеживания
--- | ---
...
### <a name="deprecated-rules"></a>Униженные правила

Следующие правила анализа наследия FxCop унижаются и не будут реализованы в качестве анализаторов. Для получения дополнительной информации можно искать по идентификатору правил (например, **CA1009)** на [странице проблем Слина-аналитиков GitHub.](https://github.com/dotnet/roslyn-analyzers/issues?utf8=%E2%9C%93&q=is:issue+label:FxCop-Port)
...
- [CA2220](ca2220.md)
- [CA2221](ca2221.md)
- [CA2222](ca2222.md) [(оправдание](https://github.com/dotnet/roslyn-analyzers/issues/1378))
- [CA2223](ca2223.md)
- [CA2228](ca2228.md)
- [CA2230](ca2230.md)
- [CA2233](ca2233.md)
...
- [Правила Microsoft.CodeAnalysis.FxCopAnalyzers](https://github.com/dotnet/roslyn-analyzers/blob/master/src/Microsoft.CodeAnalysis.FxCopAnalyzers/Microsoft.CodeAnalysis.FxCopAnalyzers.md)
