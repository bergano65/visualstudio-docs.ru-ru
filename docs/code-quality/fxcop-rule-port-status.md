...
Начиная с даты, на этой странице, список FxCop правил, который были перенесены [анализаторы FxCop](install-fxcop-analyzers.md) включает в себя:
...
[CA1016](ca1016-mark-assemblies-with-assemblyversionattribute.md) | Пометьте сборки с помощью версии сборки
[CA1017](ca1017-mark-assemblies-with-comvisibleattribute.md) | Пометьте сборки с помощью ComVisible
...
[CA1043](ca1043-use-integral-or-string-argument-for-indexers.md) | Используйте целый или строковый аргумент для индексаторов
...
[CA1052](ca1052-static-holder-types-should-be-sealed.md) | Типы со статическими заполнителями должны быть статическими или NotInheritable (см. Примечание после этой таблицы)
[CA1053](ca1053-static-holder-types-should-not-have-constructors.md) | Типы со статическими заполнителями не должны иметь конструкторы, (см. Примечание после этой таблицы)
[CA1054](ca1054-uri-parameters-should-not-be-strings.md) | Параметры URI не должны быть строками
[CA1055](ca1055-uri-return-values-should-not-be-strings.md) | URI возвращать значения не должны быть строками
...
[CA1060](ca1060-move-p-invokes-to-nativemethods-class.md) | Переместите вызовы PInvoke класс собственных методов
...
[CA1309](ca1309-use-ordinal-stringcomparison.md) | Использовать сравнение строк по порядковому номеру
...
[CA1507](ca1507.md) | Использовать nameof для выражения имена символов
CA1508 | Избегайте неиспользуемых условного кода
CA1509 | Недопустимая запись в файле спецификации правило метрики кода
...
[CA1802](ca1802-use-literals-where-appropriate.md) | При необходимости использовать литералы
...
[CA1821](ca1821-remove-empty-finalizers.md) | Удалите пустые завершающие методы
...
CA1825 | Не выделять память под массив нулевой длины.
CA1826 | Не используйте методов класса Enumerable индексирования коллекций. Вместо них используйте непосредственно коллекции
...
[CA2007](ca2007-do-not-directly-await-task.md) | Рассмотрите возможность вызова ConfigureAwait ожидаемой задачи
CA2008 | Не создавайте задачи без передачи TaskScheduler
CA2009 | Не вызывайте ToImmutableCollection ImmutableCollection значение
CA2010 | Всегда использовать значение, возвращенное методы, отмеченные PreserveSigAttribute
...
[CA2153](ca2153-avoid-handling-corrupted-state-exceptions.md) | Не перехватывайте исключений поврежденного состояния
[CA2200](ca2200-rethrow-to-preserve-stack-details.md) | Исключение для сохранения сведений о стеке.
...
[CA2224](ca2224-override-equals-on-overloading-operator-equals.md) | Переопределяйте Equals при перегрузке оператора equals
...
[CA2231](ca2231-overload-operator-equals-on-overriding-valuetype-equals.md) | Перегружайте оператор равенства на переопределяющем значение введите Equals
[CA2234](ca2234-pass-system-uri-objects-instead-of-strings.md) | Передайте системы объекты uri вместо строк
...
CA2244 | Не дублируют инициализаций индексированный элемент
...
CA3061 | Не добавляйте схемы по URL-АДРЕСУ
[CA3075](ca3075-insecure-dtd-processing.md) | Небезопасная обработка DTD в формате XML
[CA3076](ca3076-insecure-xslt-script-execution.md) | Небезопасная обработка скрипт XSLT.
[CA3077](ca3077-insecure-processing-in-api-design-xml-document-and-xml-text-reader.md) | Небезопасная обработка в структуре API, XmlDocument и XmlTextReader
[CA3147](ca3147-mark-verb-handlers-with-validateantiforgerytoken.md) | Обработчики команды Mark с проверить токен против подделки
[CA5350](ca5350-do-not-use-weak-cryptographic-algorithms.md) | Не используйте ненадежные алгоритмы шифрования
[CA5351](ca5351-do-not-use-broken-cryptographic-algorithms.md) | Не используйте ослабленные алгоритмы шифрования
CA5358 | Не используйте режимы шифрования Unsafe
CA5359 | Не отключить проверку сертификатов
CA5360 | Не вызывайте опасных методов в десериализации
CA5361 | Не отключайте SChannel использование стойкого шифрования
CA5362 | Не ссылаются Self в сериализуемого класса
CA5363 | Не отключайте проверка запросов
CA5364 | Не используйте протоколы безопасности не рекомендуется
CA5365 | Не отключайте проверка заголовка HTTP
CA5366 | Использовать XmlReader для чтения набора данных Xml
CA5367 | Не сериализовать типы с полями указателя
CA5368 | Задать ViewStateUserKey для классов, производных от страницы
CA5369 | Используйте для Deserialize XmlReader
CA5370 | Использовать для проверки чтения XmlReader
CA5371 | Использовать XmlReader для чтения схемы
CA5372 | Использовать XmlReader для XPathDocument
CA5373 | Не используйте устаревшие производного ключа-функция
CA5374 | Не используйте XslTransform
CA5375 | Не используйте учетную запись подписанные URL-адреса
CA5376 | Используйте SharedAccessProtocol HttpsOnly
CA5377 | Используйте политики доступа на уровне контейнера
CA5378 | Не отключайте ServicePointManagerSecurityProtocols
CA5379 | Не используйте алгоритм функции слабые создания производных ключей
CA9999 | Несовпадение версий анализатора

> [!NOTE]
> CA1052 правила и CA1053 из исходной реализации FxCop, объединяются в одно правило, CA1052, в анализаторы FxCop.

## <a name="unported-rules"></a>Unported правила

Набор правил, которые еще не были перенесены в [анализаторы FxCop](install-fxcop-analyzers.md) состоит из правил, которые еще не выполнили, но по-прежнему [расширится](#rules-that-may-be-ported)и те, которые являются устаревшими и [не будет перенесенном](#deprecated-rules).

### <a name="rules-that-may-be-ported"></a>Правила, которые можно перенести

Следующие правила анализа статического кода FxCop еще не реализована как анализаторы, но по-прежнему возможно. Это может быть из-за блокировки технические причины или просто, что правило имеет более низкий приоритет. Дополнительные сведения о состоянии переноса каждого правила, щелкните ссылку в **отслеживания проблемы** столбца.
...
Следующие правила анализа статического кода FxCop устарели и не быть реализованы как анализаторы. Для получения дополнительных сведений можно выполнить поиск, идентификатора правила (например, **CA1009**) на [странице проблем на GitHub анализаторов roslyn](https://github.com/dotnet/roslyn-analyzers/issues?utf8=%E2%9C%93&q=is:issue+label:FxCop-Port).
