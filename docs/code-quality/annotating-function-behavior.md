...
|----------------|-----------------|
|`_Called_from_function_class_(name)`|Не является автономной; это предикат, который должен использоваться с примечанием `_When_`. Дополнительные сведения см. в разделе [Указание времени и места применения заметки](../code-quality/specifying-when-and-where-an-annotation-applies.md).<br /><br /> Параметр — произвольная строка, которая также отображается `_Function_class_` в заметке в объявлении некоторых функций. `name`  `_Called_from_function_class_`Возвращает ненулевое `name`значение, если анализируемая функция объявляется с помощью инструкции `_Function_class_` с тем же значением; в противном случае возвращается ноль.|
