...
Эта проверка поддерживает правило *I. 13: Не передавайте массив как один указатель @ no__t-0. При использовании необработанных указателей в арифметических операциях их следует заменить более безопасными типами буферов, такими как `span<T>` или `vector<T>`.
