...
Это предупреждение активируется при перехвате CSE с помощью общего обработчика, который перехватывает все исключения `catch (System.Exception e)` , `catch` например или без параметра исключения.
...
<xref:System.Runtime.ExceptionServices.HandleProcessCorruptedStateExceptionsAttribute> Удаление атрибута гарантирует, что исключения поврежденного состояния не будут обрабатываться методом.
