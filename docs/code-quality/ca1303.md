...
Можно настроить, какие типы, включая производные типы, следует исключить из анализа. Например, чтобы указать, что правило не должно выполняться в каких-либо методах в типах с именем `MyType` и его производных типах, добавьте следующую пару "ключ-значение" в editorconfig-файл в проекте:
...
Разрешенные форматы имен символов в значении параметра (разделенном "|"):
  ...
  - Полные имена в [формате идентификатора документации](https://github.com/dotnet/csharplang/blob/master/spec/documentation-comments.md#id-string-format)символа с необязательным префиксом `T:`.
