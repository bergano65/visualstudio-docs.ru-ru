...
Чтобы устранить нарушение этого правила, вызовите <xref:System.IDisposable.Dispose%2A> для полей, которые имеют типы, реализующие <xref:System.IDisposable>.
...
В следующем фрагменте кода показан тип `TypeB`, который нарушает правило CA2213, объявляя поле `aFieldOfADisposableType` в качестве высвобождаемого типа (`TypeA`) и не вызывая <xref:System.IDisposable.Dispose%2A> в поле.
