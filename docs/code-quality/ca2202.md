...
Правильно реализованный метод <xref:System.IDisposable.Dispose%2A> можно вызывать несколько раз, не вызывая исключение. Однако это не гарантируется, и во избежание создания <xref:System.ObjectDisposedException?displayProperty=fullName> не следует вызывать <xref:System.IDisposable.Dispose%2A> более одного раза для объекта.
...
Чтобы устранить эту проблему, используйте блок `try` @ no__t-1 @ no__t-2 вместо внешней инструкции `using`. В блоке `finally` убедитесь, что ресурс `stream` не имеет значение null.
...
> [!TIP]
> Приведенный выше синтаксис `?.` представляет собой [Условный оператор NULL](/dotnet/csharp/language-reference/operators/member-access-operators#null-conditional-operators--and-).
