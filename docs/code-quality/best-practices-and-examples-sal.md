...
## <a name="_pre_defensive_-and-_post_defensive_"></a>\_Pre @ no__t-1defensive @ no__t-2 и \_Post @ no__t-4defensive @ no__t-5
...
Аннотация `_Out_writes_` означает, что у вас есть буфер. Выделяется @no__t 0 байт, при этом первый байт инициализируется при выходе. Эта аннотация не является строго неправильной и полезна для выражения выделенного размера. Однако он не указывает, сколько элементов инициализируется функцией.
...
## <a name="_out_-pstr"></a>@no__t 0Out @ no__t-1 ПСТР
...
Аннотация, например `_In_ PCSTR`, является распространенной и полезной. Он указывает на входную строку с завершающим НУЛЕМ, поскольку предварительное условие `_In_` разрешает распознавание строки, завершающейся нулем.

## <a name="_in_-wchar-p"></a>\_In @ no__t-1 WCHAR * p

`_In_ WCHAR* p` говорит о том, что имеется входной указатель `p`, указывающий на один символ. Однако в большинстве случаев это не является требуемой спецификацией. Вместо этого, скорее всего, используется спецификация массива, заканчивающегося нулем; для этого используйте `_In_ PWSTR`.
...
## <a name="wrong-context-in-_when_"></a>Неправильный контекст в \_When @ no__t-1
