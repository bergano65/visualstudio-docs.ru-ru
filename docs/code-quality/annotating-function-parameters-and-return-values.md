...
- `_In_z_`
     ...
     Указатель на строку, завершающуюся нулем, которая используется в качестве входных данных.  Строка должна быть допустимой в предварительном состоянии.  Предпочтительными являются варианты `PSTR`, которые уже имеют правильные заметки.
...
- `_In_reads_(s)`
     ...
     Вариант `_bytes_` задает размер в байтах вместо элементов. Используйте этот параметр только в том случае, если размер не может быть выражен как элемент.  Например, строки `char` будут использовать вариант `_bytes_` только в том случае, если аналогичная функция, использующая `wchar_t`, будет.

- `_In_reads_z_(s)`

     Указатель на массив, заканчивающийся нулем и имеющий известный размер. Элементы вплоть до нулевого терминатора — или `s`, если Терминатор NULL отсутствует — должен быть допустимым в предварительном состоянии.  Если размер известен в байтах, масштабирование `s` по размеру элемента.

- `_In_reads_or_z_(s)`

     Указатель на массив, заканчивающийся нулем или имеющий известный размер или оба. Элементы вплоть до нулевого терминатора — или `s`, если Терминатор NULL отсутствует — должен быть допустимым в предварительном состоянии.  Если размер известен в байтах, масштабирование `s` по размеру элемента.  (Используется для семейства `strn`).

- `_Out_writes_(s)`

     `_Out_writes_bytes_(s)`

     Указатель на массив элементов `s` (отв. bytes), который будет записан функцией.  Элементы массива не обязательно должны быть допустимы в предварительном состоянии, а количество элементов, допустимых в состоянии после, не указано.  Если в типе параметра есть аннотации, они применяются в состоянии после. Например, рассмотрим следующий код.

     `typedef _Null_terminated_ wchar_t *PWSTR; void MyStringCopy(_Out_writes_ (size) PWSTR p1,    _In_ size_t size,    _In_ PWSTR p2);`

     В этом примере вызывающий объект предоставляет буфер для элементов `size` для `p1`.  `MyStringCopy` делает некоторые из этих элементов допустимыми. Что более важно, заметка `_Null_terminated_` в `PWSTR` означает, что `p1` завершается нулем в состоянии после.  Таким образом, количество допустимых элементов по-прежнему четко определено, но определенный счетчик элементов не требуется.
...
- `_Out_writes_to_(s,c)`
     ...
     Указатель на массив элементов `s`.  Элементы не обязательно должны быть допустимыми в предварительном состоянии.  В состоянии после состояния элементы @no__t -0-TH должны быть допустимыми.  Если размер известен в байтах, масштабируйте `s` и `c` по размеру элемента или используйте вариант `_bytes_`, который определяется следующим образом:
...
- `_Inout_updates_to_(s,c)`
     ...
     Указатель на массив, который считывается и записывается функцией.  Он имеет размер `s` элементов, все из которых должны быть допустимыми в предварительном состоянии, а `c` элементы должны быть допустимыми в состоянии после.
...
- `_Inout_updates_all_(s)`
     ...
     Указатель на массив, который считывается и записывается функцией size `s` элементов. Определяется как эквивалент:
...
- `_In_reads_to_ptr_(p)`
     ...
     Указатель на массив, для которого выражение `p` @ no__t-1 @ no__t-2 (то есть `p` минус `_Curr_`) определяется соответствующим стандартом языка.  Элементы до `p` должны быть допустимы в предварительном состоянии.

- `_In_reads_to_ptr_z_(p)`

     Указатель на массив, заканчивающийся нулем, для которого выражение `p` @ no__t-1 @ no__t-2 (то есть `p` минус `_Curr_`) определяется соответствующим стандартом языка.  Элементы до `p` должны быть допустимы в предварительном состоянии.

- `_Out_writes_to_ptr_(p)`

     Указатель на массив, для которого выражение `p` @ no__t-1 @ no__t-2 (то есть `p` минус `_Curr_`) определяется соответствующим стандартом языка.  Элементы, предшествующие `p`, не обязательно должны быть допустимыми в предварительном состоянии и должны быть допустимы в состоянии после.

- `_Out_writes_to_ptr_z_(p)`

     Указатель на массив, заканчивающийся нулем, для которого выражение `p` @ no__t-1 @ no__t-2 (то есть `p` минус `_Curr_`) определяется соответствующим стандартом языка.  Элементы, предшествующие `p`, не обязательно должны быть допустимыми в предварительном состоянии и должны быть допустимы в состоянии после.

## <a name="optional-pointer-parameters"></a>Необязательные параметры указателя

Если заметка параметра указателя включает `_opt_`, это означает, что параметр может иметь значение null. В противном случае аннотация выполняется так же, как версия, не включающая `_opt_`. Ниже приведен список вариантов `_opt_` заметок к параметрам указателя:
...
- `_Outptr_result_z_`
   ...
   Возвращаемый указатель имеет заметку `_Null_terminated_`.
...
- `_Outref_result_bytebuffer_(s)`
     ...
     Результат должен быть допустимым в состоянии POST и не может иметь значение null. Указывает на допустимый буфер размера `s` байт.

- `_Outref_result_buffer_to_(s, c)`

     Результат должен быть допустимым в состоянии POST и не может иметь значение null. Указывает на буфер для элементов `s`, для которых первый `c` является допустимым.

- `_Outref_result_bytebuffer_to_(s, c)`

     Результат должен быть допустимым в состоянии POST и не может иметь значение null. Указывает на буфер `s` байт, в котором первая `c` является допустимой.

- `_Outref_result_buffer_all_(s)`

     Результат должен быть допустимым в состоянии POST и не может иметь значение null. Указывает на допустимый буфер размера `s` допустимых элементов.

- `_Outref_result_bytebuffer_all_(s)`

     Результат должен быть допустимым в состоянии POST и не может иметь значение null. Указывает на допустимый буфер `s` байт допустимых элементов.
...
- `_Outref_result_bytebuffer_maybenull_(s)`
     ...
     Результат должен быть допустимым в состоянии после состояния, но может иметь значение NULL в состоянии после. Указывает на допустимый буфер размера `s` байт.

- `_Outref_result_buffer_to_maybenull_(s, c)`

     Результат должен быть допустимым в состоянии после состояния, но может иметь значение NULL в состоянии после. Указывает на буфер для элементов `s`, для которых первый `c` является допустимым.

- `_Outref_result_bytebuffer_to_maybenull_(s,c)`

     Результат должен быть допустимым в состоянии после состояния, но может иметь значение NULL в состоянии POST. Указывает на буфер `s` байт, в котором первая `c` является допустимой.

- `_Outref_result_buffer_all_maybenull_(s)`

     Результат должен быть допустимым в состоянии после состояния, но может иметь значение NULL в состоянии POST. Указывает на допустимый буфер размера `s` допустимых элементов.

- `_Outref_result_bytebuffer_all_maybenull_(s)`

     Результат должен быть допустимым в состоянии после состояния, но может иметь значение NULL в состоянии POST. Указывает на допустимый буфер `s` байт допустимых элементов.

## <a name="return-values"></a>Возвращаемые значения

Возвращаемое значение функции напоминает параметр `_Out_`, но находится на другом уровне разыменования, и вам не нужно рассматривать понятие указателя на результат.  Для следующих заметок возвращаемое значение является объектом с заметками — скаляром, указателем на структуру или указателем на буфер. Эти заметки имеют ту же семантику, что и соответствующая Аннотация `_Out_`.
...
- `_In_range_(low, hi)`
    ...
    > [!IMPORTANT]
    > Хотя имена содержат "in" и "out", семантика `_In_` и `_Out_` **не** применяется к этим заметкам.
...
- `_Struct_size_bytes_(size)`
     ...
     Применяется к объявлению структуры или класса.  Указывает, что допустимый объект этого типа может быть больше, чем объявленный тип, с числом байтов, присваиваемым `size`.  Пример:
