...
|[CA2249: рекомендуется использовать String. Contains вместо String. IndexOf](../code-quality/ca2249.md)|Вызовы, `string.IndexOf` где результат используется для проверки наличия или отсутствия подстроки, можно заменить на `string.Contains` .|
