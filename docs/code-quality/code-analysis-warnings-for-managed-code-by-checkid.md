...
| CA1834 |[CA1834: используйте StringBuilder. append (char) для отдельных символьных строк](../code-quality/ca1834.md) | <xref:System.Text.StringBuilder> имеет `Append` перегрузку, которая принимает в `char` качестве аргумента. Предпочитать вызов `char` перегрузки для повышения производительности. |
