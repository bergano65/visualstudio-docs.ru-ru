...
В приведенных выше примерах используются двухбайтовые символы в 2 байта ( `CharSet.Unicode` ). Если собственная функция использует 1-байтные символы ( `CharSet.Ansi` ), `byte` вместо буфера можно использовать буфер `char` . Например:
