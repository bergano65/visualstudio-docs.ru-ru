...
Метод в следующем примере управляет целым числом, нарушающим это правило. для срабатывания параметра [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] **требуется отключить параметр переполнение целочисленных** значений.
