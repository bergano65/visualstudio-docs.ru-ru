...
|`_Lock_level_order_(name1, name2)`|Инструкция, которая обеспечивает порядок блокировки между `name1` и `name2`.  Блокировки, имеющие `name1` уровня, должны быть получены перед блокировками, имеющими `name2`уровня.|
