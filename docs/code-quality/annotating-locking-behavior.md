...
Во избежание ошибок параллелизма в многопоточных программах, всегда выполняйте соответствующую дисциплину блокировок и используйте заметки SAL.
 ...
 К сожалению, простым на первый взгляд правилам блокировки удивительно сложно следовать на практике. В современных языках программирования и компиляторов фундаментальные ограничение заключается в том, что они не поддерживают непосредственно спецификации и анализ для требования параллелизма. Программисты должны полагаться на неофициальные комментарии к коду для выражения своих намерений о том, как они используют блокировки.
...
## <a name="general-guidelines"></a>Общие рекомендации
 ...
 SAL поддерживает множество различных типов примитивов блокирования, таких как критические секции, мьютексы, спин-блокировки и другие объекты ресурсов. Многие заметки параллелизма принимать блокировки выражение в качестве параметра. По соглашению блокировку обозначается выражение пути базового объекта блокировки.
...
## <a name="locking-annotations"></a>Блокировки заметок
 В следующей таблице перечислены блокировки заметок.
...
|`_Has_lock_kind_(kind)`|Отмечает любого объекта, чтобы уточнить сведения о типе объекта ресурса. Иногда используется тип данных для разных типов ресурсов и перегруженные типа недостаточно для различения семантических требований среди различных ресурсов. Ниже представлен список предварительно определенных параметров `kind`:<br /><br /> `_Lock_kind_mutex_`<br /> Код типа блокировки для мьютексов.<br /><br /> `_Lock_kind_event_`<br /> Код типа блокировки для событий.<br /><br /> `_Lock_kind_semaphore_`<br /> Код типа блокировки для семафоров.<br /><br /> `_Lock_kind_spin_lock_`<br /> Код типа блокировки для спин-блокировки.<br /><br /> `_Lock_kind_critical_section_`<br /> Код типа блокировки для критических разделов.|
|`_Has_lock_level_(name)`|Аннотирует объект блокировки и присваивает ему уровень блокировки `name`.|
|`_Lock_level_order_(name1, name2)`|Оператор, который предоставляет блокировку упорядочению `name1` и `name2`.|
...
## <a name="sal-intrinsics-for-unexposed-locking-objects"></a>Встроенные SAL для непредоставленных явно объектов блокировки
 Некоторые объекты блокировки не предоставляются с помощью реализации связанных с ним функций блокировки.  В следующей таблице перечислены встроенные переменные SAL, которые содержат заметки для функций, действующих на эти защищенные объекты блокировки.
...
|`_Interlocked_operand_`|Параметр заметками функции является целевой операнд одного Interlocked различные функции.  Эти операнды должны иметь дополнительные свойства.|
