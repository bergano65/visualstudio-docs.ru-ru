...
ms.locfileid: "90036379"
---
# <a name="configure-code-quality-analysis"></a>Настройка анализа качества кода

Начиная с .NET 5,0 анализаторы качества кода входят в состав пакета SDK для .NET. (Ранее эти анализаторы были установлены как пакет NuGet.) По умолчанию анализ кода включен для проектов, предназначенных для .NET 5,0 или более поздней версии. Вы можете включить анализ кода для проектов, предназначенных для более ранних версий .NET, задав для свойства [енабленетанализерс](/dotnet/core/project-sdk/msbuild-props#enablenetanalyzers) значение `true` . Можно также отключить анализ кода для проекта, задав для значение `EnableNETAnalyzers` `false` .

Каждое *диагностическое* средство анализатора качества кода или правило имеет серьезность по умолчанию и состояние подавления, которые могут быть перезаписаны и настроены для проекта. В этой статье рассматривается настройка серьезности анализатора качества кода и подавление нарушений анализатора.
...
|-|-|-|
| Ошибка | `error` | Нарушения отображаются как *ошибки* в список ошибок и в выходных данных сборки в командной строке, что приводит к сбою сборки.| Код, вызвавший ошибку, подчеркнут красной волнистой линией и отмечен небольшой красной рамкой в полосе прокрутки. |
...
| Сведения | `suggestion` | Нарушения отображаются как *сообщения* в список ошибок, а не все в выходных данных сборки в командной строке. | Код, вызвавший ошибку, подчеркивается серой волнистой линией и отмечается небольшим серым прямоугольником в полосе прокрутки. |
| Скрытый | `silent` | Не является видимым для пользователя. | Не является видимым для пользователя. Однако в модуль диагностики интегрированной среды разработки сообщается о диагностике. |
| None | `none` | Полностью подавлен. | Полностью подавлен. |
| По умолчанию | `default` | Соответствует уровню серьезности по умолчанию для правила. Чтобы определить, какое значение используется для правила по умолчанию, просмотрите окно свойств. | Соответствует уровню серьезности по умолчанию для правила. |
...
Нарушения анализатора, указанные в списке ошибок, соответствуют [параметрам уровня серьезности](../code-quality/use-roslyn-analyzers.md#configure-severity-levels) правила. Нарушения анализатора также отображаются в редакторе кода в виде волнистых линий в коде, вызвавшем ошибку. На следующем рисунке показаны три нарушения &mdash; одной ошибки (красная волнистая линия), одно предупреждение (Зеленая волнистая линия) и одно предложение (три серые точки):
...
Многие правила анализатора или *диагностики*содержат один или несколько *исправлений кода* , которые можно применить для исправления нарушения правил. Исправления кода отображаются в меню со значком лампочки вместе с другими типами [быстрых действий](../ide/quick-actions.md). Сведения об этих исправлениях кода см. в разделе [Распространенные быстрые действия](../ide/quick-actions.md).
...
Рассмотрим следующий пример EditorConfig, где [CA1822](./ca1822.md) имеет категорию "производительность":
...
2. Добавьте запись для каждого правила, которое требуется настроить, с соответствующим расширением файла. Например, чтобы задать для [CA1822](ca1822.md) уровень серьезности для `error` файлов C#, запись будет выглядеть следующим образом:
...
1. В Обозреватель решений разверните узел **References**  >  **анализаторы** ссылок (или **Dependencies**  >  **анализаторы** зависимостей для проектов .NET Core).
...
- В **Обозреватель решений**дважды щелкните файл, щелкните правой кнопкой мыши узел **References**  >  **анализаторы** ссылок и выберите команду **Открыть активный набор правил**.
