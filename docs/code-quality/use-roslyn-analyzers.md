...
Свойства диагностики, включая ее описание и серьезность по умолчанию, можно просмотреть в окне **Свойства** . Чтобы просмотреть свойства, щелкните правило правой кнопкой мыши и выберите пункт **Свойства**или выберите правило, а затем нажмите клавиши **ALT**+**Enter**.
...
|-|-|-|
| Ошибка | `error` | Нарушения отображаются как *ошибки* в список ошибок и в выходных данных сборки в командной строке, что приводит к сбою сборки.| Код, вызвавший ошибку, подчеркнут красной волнистой линией и отмечен небольшим красным прямоугольником на полосе прокрутки. |
| Предупреждение | `warning` | Нарушения отображаются как *предупреждения* в список ошибок и в выходных данных сборки в командной строке, но не вызывают сбой сборок. | Код, вызвавший ошибку, подчеркнут зеленой волнистой линией и отмечен маленьким зеленым прямоугольником на полосе прокрутки. |
...
2. Добавьте запись для каждого правила, которое требуется настроить, с соответствующим расширением файла. Например, чтобы задать для [CA1822](ca1822-mark-members-as-static.md) уровень серьезности `error` для C# файлов, запись будет выглядеть следующим образом:
...
1. После возникновения нарушения наведите указатель мыши на нарушение в редакторе и откройте меню лампочки. Или установите курсор в строке и нажмите клавиши **Ctrl**+ **.** (точка).

2. В меню лампочки выберите **Настройка или отключение проблем** > **Configure \<rule ID > Severity**.
...
1. В **Обозреватель решений**разверните **ссылки** > **анализаторы** (или **зависимости** > **анализаторов** для проектов .NET Core).
...
1. Откройте файл активного [набора правил](analyzer-rule-sets.md) , дважды щелкнув его в **Обозреватель решений**, выберите **Открыть активный набор правил** в контекстном меню в узле **ссылки** > **анализаторы** или щелкните **Открыть** в окнеСтраница свойств "анализ кода" для проекта.
...
- В меню **анализ**
  ...
  Выберите **анализ** > **Запуск анализа кода и подавление активных проблем** в строке меню, чтобы отключить все текущие нарушения. Иногда это называется "задания базовых показателей".
...
- В **редакторе кода**
  ...
  Поместите курсор в строку кода с нарушением и нажмите **Ctrl**+**period (.)** , чтобы открыть меню **быстрые действия** . Выберите **ПОДАВЛЯТЬ какскскскс** > **в файле источника/в подавлении**.

  ![Подавлять диагностику в меню быстрых действий](media/suppress-diagnostic-from-editor.png)

- Из **Список ошибок**

  Выберите правила, которые требуется отключить, и щелкните правой кнопкой мыши и выберите пункт **подавлять** > **в файле источника/в подавлении**.
  ...
  - Если выбрать параметр **в файле подавления**, откроется диалоговое окно **Предварительный просмотр изменений** , в котором будет показан предварительный просмотр атрибута <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute>, добавленного в глобальный файл подавления.
