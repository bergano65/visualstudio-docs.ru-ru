...
2. Добавьте запись для каждого правила, которое требуется настроить, с соответствующим расширением файла. Например, чтобы задать для [CA1822](ca1822-mark-members-as-static.md) `error` уровень серьезности для C# файлов, запись выглядит следующим образом:
...
> [!NOTE]
> Для анализаторов кода интегрированной среды разработки можно также настроить их в файле EditorConfig, используя другой синтаксис, `dotnet_style_qualification_for_field = false:suggestion`например. Однако если задать серьезность с помощью `dotnet_diagnostic` синтаксиса, он имеет приоритет. Дополнительные сведения см. в разделе [языковые соглашения для EditorConfig](../ide/editorconfig-language-conventions.md).
...
2. В меню лампочки выберите **настроить или отключить проблемы** > **Настройка \<идентификатора правила > серьезности**.
...
1. Откройте файл активного [набора правил](analyzer-rule-sets.md) , дважды щелкнув его в **Обозреватель решений**, выбрав пункт **Открыть активный набор правил** в контекстном меню узла**анализаторы** **ссылок** > или щелкнув **Открыть** в Страница свойств " **анализ кода** " для проекта.

   Если вы изменяете набор правил в первый раз, Visual Studio создает копию файла набора правил по умолчанию, называет его  *\<именем имя_проекта >. RuleSet*и добавляет его в проект. Этот настраиваемый набор правил также станет активным набором правил для вашего проекта.
...
- В **файле EditorConfig**
  ...
  Задайте уровень серьезности `none`, `dotnet_diagnostic.CA1822.severity = none`например.
