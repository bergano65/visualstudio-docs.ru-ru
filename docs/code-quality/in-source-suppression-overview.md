---
title: Отключение предупреждений анализа кода в Visual Studio
ms.date: 01/29/2018
...
> [!NOTE]
> Если миграция проекта в Visual Studio 2017, вы можете столкнуться внезапно с огромное количество предупреждений анализа кода. Если вы не готовы для устранения предупреждений и хотите временно отключить анализ кода, откройте страницы свойств проекта (**проекта** > **\<проект > свойства**) и перейдите в раздел **анализа кода** вкладки. Отмените выбор **включить анализ кода в построении**, а затем перестройте проект. Кроме того можно выбрать другой, меньший набор правил в выполняться код. Не забудьте включить анализ кода обратно в когда вы будете готовы устранить предупреждения.
...
- **Правило категории** -категорию, в котором определено правило. Дополнительные сведения о категориях правил анализа кода, см. в разделе [управляемый код предупреждения](../code-quality/code-analysis-for-managed-code-warnings.md).
...
> [!NOTE]
> `Target` всегда содержит элемент полное доменное имя.
