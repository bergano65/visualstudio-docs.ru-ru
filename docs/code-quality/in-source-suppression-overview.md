...
В C++/CLI для добавления атрибута используйте цс \_SUPPRESS \_MESSAGE или ca \_GLOBAL \_SUPPRESS_MESSAGE в файле заголовка.
...
- **Scope** — целевой объект, в котором предупреждение подавляется. Если целевой объект не указан, ему присваивается значение целевого объекта атрибута. В число поддерживаемых [областей](xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute.Scope) входят следующие.
  ...
  - `namespaceanddescendants`-(New для Visual Studio 2019) Эта область подавляет предупреждения в пространстве имен и всех его потомках. Значение `namespaceanddescendants` игнорируется устаревшим анализом.
...
Чтобы отключить определенное нарушение правил, укажите имя символа для свойства `MessageId` атрибута <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute>. В следующем примере показан код с двумя нарушениями [CA1500: вариабленамесшаулднотматчфиелднамес](../code-quality/ca1500.md) &mdash;one для переменной `name` и одной для `age` переменной. Подавляется только нарушение `age` символа.
