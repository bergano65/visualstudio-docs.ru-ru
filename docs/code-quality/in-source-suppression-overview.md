...
Атрибут <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute> является условным атрибутом, включенным в метаданные IL сборки управляемого кода, только если символ компиляции CODE_ANALYSIS определен во время компиляции.

В C++/CLI Используйте макросы CA @ NO__T-1SUPPRESS @ NO__T-2MESSAGE или CA @ NO__T-3GLOBAL @ NO__T-4SUPPRESS_MESSAGE в файле заголовка, чтобы добавить атрибут.
...
- **Scope** — целевой объект, в котором предупреждение подавляется. Если целевой объект не указан, ему присваивается значение целевого объекта атрибута. В число поддерживаемых [областей](xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute.Scope) входят следующие.
  ...
  - `namespaceanddescendants` — (новое для Visual Studio 2019) Эта область подавляет предупреждения в пространстве имен и всех его потомках. Значение `namespaceanddescendants` игнорируется устаревшим анализом.
...
В некоторых случаях может потребоваться отключить определенный экземпляр нарушения, например таким образом, чтобы будущий код не выводился автоматически из правила анализа кода. Некоторые правила анализа кода позволяют сделать это с помощью свойства `MessageId` атрибута <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute>. В общем случае устаревшие правила для нарушений в конкретном символе (локальная переменная или параметр) учитывают свойство `MessageId`. [CA1500: вариабленамесшаулднотматчфиелднамес](../code-quality/ca1500-variable-names-should-not-match-field-names.md) — это пример такого правила. Однако устаревшие правила нарушения работы исполняемого кода (не символа) не учитывают свойство `MessageId`. Кроме того, .NET Compiler Platform ("Roslyn") анализаторы не учитывают свойство `MessageId`.

Чтобы отключить определенное нарушение правила, укажите имя символа для свойства `MessageId` атрибута <xref:System.Diagnostics.CodeAnalysis.SuppressMessageAttribute>. В следующем примере показан код с двумя нарушениями [CA1500: вариабленамесшаулднотматчфиелднамес](../code-quality/ca1500-variable-names-should-not-match-field-names.md)&mdash;one для переменной `name` и одной для переменной `age`. Подавляется только нарушение для символа `age`.
...
Компиляторы управляемого кода и некоторые сторонние средства создают код для упрощения разработки кода. Код, созданный компилятором, который отображается в исходных файлах, обычно помечается атрибутом `GeneratedCodeAttribute`.

Можно выбрать, следует ли подавлять предупреждения и ошибки анализа кода для созданного кода. Сведения о подавлении таких предупреждений и ошибок см. в разделе [How to: Отключить предупреждения для созданного кода @ no__t-0.

> [!NOTE]
> Анализ кода игнорирует `GeneratedCodeAttribute` при применении к целой сборке или одному параметру.
...
> [!NOTE]
> При подавлении предупреждения с областью `namespace` она подавляет предупреждение относительно пространства имен. Предупреждение не подавляется по типам в пространстве имен.
