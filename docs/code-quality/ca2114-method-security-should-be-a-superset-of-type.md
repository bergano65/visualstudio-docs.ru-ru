...
Проверьте код, чтобы убедиться, что оба действия требуются. Если требуются оба действия, убедитесь, что действие уровня метода включает безопасность, указанную на уровне типа. Например, если `X`тип требует разрешения, а его метод должен также требовать разрешения `Y`, метод должен явно запрашивать `X` и `Y`.
