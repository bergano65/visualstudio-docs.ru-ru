...
## Пример  
 Данное предупреждение создается в следующем коде путем двукратного вызова функции аннотирования [ReadFile](http://msdn2.microsoft.com/library/aa365467.aspx).  После первого вызова свойство атрибута Post помечает значение второго параметра функции как ненадежное.  Поэтому при передаче ненадежного значения во втором вызове `ReadFile` создается это предупреждение, как показано в следующем коде:  
