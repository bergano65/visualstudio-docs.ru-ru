...
|Комментарий|Описание|
|----------------|-----------------|
|`_String_length_(param)`|`param` — это количество элементов в строке вплоть до нуля, но не включительно. Эта заметка зарезервирована для символьных типов строк.|
|`strlen(param)`|`param` — это количество элементов в строке вплоть до нуля, но не включительно. Эта заметка зарезервирована для использования в массивах символов и напоминает функцию среды выполнения C [strlen ()](/cpp/c-runtime-library/reference/strlen-wcslen-mbslen-mbslen-l-mbstrlen-mbstrlen-l).|
