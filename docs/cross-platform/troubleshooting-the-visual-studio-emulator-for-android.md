...
## <a name="emulator-fails-to-install"></a><a name="NoInstall"></a> Не удается запустить эмулятор
 ...
 ![Снимок экрана: сообщение Visual Studio о блокировке установки эмулятора Android для Microsoft Visual Studio, поскольку компьютер не поддерживает Hyper-V.](../cross-platform/media/android_emu_install_issue.png "Android_Emu_Install_Issue")
...
- В диспетчере Hyper-V откройте диспетчер виртуальных коммутаторов и проверьте наличие двух сетевых коммутаторов. Убедитесь, один из них является внутренним, а второй — внешним.
     ...
     ![Снимок экрана: диспетчер виртуальных машин в диспетчере Hyper-V. Выделен новый виртуальный коммутатор, и из его свойств видно, что он является комутатором внешней сети.](../cross-platform/media/android_emu_v_switch_man.png "Android_Emu_V_Switch_Man")
