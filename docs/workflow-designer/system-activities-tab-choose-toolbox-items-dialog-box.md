...
Фильтрация выполняется на каждой вкладке, и невозможно добавить действие рабочего процесса через **компонента .NET** вкладки. Добавить их с помощью **System.Activities** самой вкладке.
...
Namespace\
Отображает иерархию пространства имен .NET, которая определяет структуру действия.

Имя сборки
Отображает имя и версию сборки .NET, которая содержит действия.

Directory\
Отображает расположение сборки .NET, которая содержит действия рабочих процессов. По умолчанию все сборки находятся в глобальном кэше сборок.
