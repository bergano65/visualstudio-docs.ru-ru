...
| <xref:System.ServiceModel.Activities.Send.OperationName%2A> | Верно | Имя операции службы, вызываемой этим действием <xref:System.ServiceModel.Activities.Send>. Это свойство используется для создания значения по умолчанию для свойства **Action** , если свойство **Action** не задано явно. |
| <xref:System.ServiceModel.Activities.Send.ServiceContractName%2A> | Верно | Имя контракта службы, который реализуется вызываемой службой. |
