---
title: Практическое руководство. Указание событий сборки (C#) | Документы Майкрософт
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- vs-ide-general
...
2.  В диалоговом окне **Новый проект** разверните узел **Visual C#**, выберите **Классический рабочий стол Windows**, а затем шаблон **Приложение Windows Forms**. Задайте для проекта имя `CSWinApp`.  
...
5.  Опубликуйте проект, щелкнув **Опубликовать сейчас**.  
     ...
     Будет выполнена сборка файла манифеста, и он будет помещен в каталог *C:\TEMP\CSWinApp_1_0_0_0\CSWinApp.exe.manifest*. Чтобы просмотреть манифест, щелкните правой кнопкой мыши файл и выберите пункт **Открыть с помощью**, затем выберите **Выбрать программу из списка** и щелкните **Блокнот**.  
...
7.  В **командной строке события после сборки** введите следующую команду:  
     ...
     Так как макрос `$(TargetPath)` выражает полный путь к создаваемому исполняемому файлу, файл `$(TargetPath)`*.manifest* будет указывать манифест приложения, созданный в каталоге *bin*. При публикации этот манифест будет скопирован в расположение публикаций, которое было задано ранее.  
...
## <a name="see-also"></a>См. также  
 [Страница "События сборки" в конструкторе проектов (C#)](../ide/reference/build-events-page-project-designer-csharp.md)   
