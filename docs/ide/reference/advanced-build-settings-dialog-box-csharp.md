 ...
 **Отладочная информация**. Определяет тип отладочной информации, создаваемой компилятором. Сведения о настройке производительности отладки для приложения см. в разделе [Упрощение отладки образов](/dotnet/framework/debug-trace-profile/making-an-image-easier-to-debug). Включает следующие значения:
