...
**Повторное открытие документов при загрузке решения (Visual Studio 2017 версии 15.8 (предварительная версия 2) и более поздние версии)**
 
При загрузке решения автоматически повторно открываются документы, открытые во время предыдущего сеанса. При выборе документы, открытые в предыдущий раз после закрытия этого решения, автоматически открываются при его загрузке.
