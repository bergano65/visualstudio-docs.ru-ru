...
- Если флажок рядом с компонентом снят, соответствующий значок удаляется с **панели элементов.**
    ...
    > [!NOTE]
    > Компоненты .NET, установленной на компьютере, остаются доступными, ли их значки отображаются в **элементов**.
...
Отображает иерархию пространства [.NET API](/dotnet/api/?view=netframework-4.7) пространства имен, которая определяет структуру компонента. Сортировку по этому столбцу, чтобы получить список доступных компонентов в каждом пространстве имен .NET, установленной на компьютере.

**Имя сборки**

Отображает имя сборки .NET, которая включает в себя пространство имен для каждого компонента. Выполните сортировку по этому столбцу, чтобы список пространств имен в каждой сборке .NET, установленной на компьютере.
