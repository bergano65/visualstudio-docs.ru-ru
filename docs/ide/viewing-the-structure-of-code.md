---
title: Просмотр структуры кода в окне "Представление классов", "Иерархия вызовов", "Обозреватель объектов" и окне определения кода
ms.date: 05/18/2018
...
Вы можете отфильтровать компоненты, которые нужно просмотреть, с помощью раскрывающегося списка в верхней части окна. К пользовательским компонентам относятся исполняемые файлы управляемого кода, сборки библиотек, библиотеки типов и *OCX*-файлы. Пользовательские компоненты C++ добавить нельзя. Пользовательские параметры сохраняются в каталоге приложения пользователя Visual Studio — *%APPDATA%\Microsoft\VisualStudio\15.0\ObjBrowEX.dat*.
...
Контекстное меню в **обозревателе объектов** может содержать приведенные ниже команды (в зависимости от выбранного вида элемента).
...
Контекстное меню в окне **Определение кода** может содержать следующие команды:
