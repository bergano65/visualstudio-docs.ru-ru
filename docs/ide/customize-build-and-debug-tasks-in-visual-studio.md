---
title: "Настройка задач сборки и отладки в Visual Studio с помощью файлов tasks.vs.json и launch.vs.json | Документация Майкрософт"
ms.date: 02/21/2018
...
Этот файл содержит целевые объекты сборки, очистки и перестроения, и вы можете определить для него указанный ниже файл *tasks.vs.json*. Он содержит три задачи сборки (для сборки, перестроения и очистки), для которых применяется средство сборки NMAKE.
...
Когда вы определите в *tasks.vs.json* задачи сборки, в **обозревателе решений** для соответствующих файлов появятся дополнительные пункты контекстного меню. В нашем примере для всех файлов *makefile* в контекстное меню добавляются действия **Собрать**, **Перестроить** и **Очистить**.
