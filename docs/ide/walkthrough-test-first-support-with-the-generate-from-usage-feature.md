---
title: "Пошаговое руководство. Основанная на тестировании разработка с помощью возможности \"Создание в результате использования\" | Документация Майкрософт"
ms.custom: 
ms.date: 10/09/2017
ms.reviewer: 
ms.suite: 
...
### <a name="to-add-a-reference-to-the-class-library-project"></a>Добавление ссылки на проект библиотеки классов
...
7.  В **обозревателе решений**обратитесь к узлам проекта GFUDemo_VB или GFUDemo_CS, чтобы убедиться в том, что новый файл Automobile.vb или Automobile.cs содержится в проекте. В редакторе кода фокус по-прежнему находится в `AutomobileTest.DefaultAutomobileIsInitializedCorrectly`, что позволяет продолжить написание теста максимально быстро.  
...
Предположим, что согласно спецификации новый объект `Automobile` может быть переведен в состояние Running, если его свойства `Model` и `TopSpeed` содержат значения, отличные от значений по умолчанию.  
