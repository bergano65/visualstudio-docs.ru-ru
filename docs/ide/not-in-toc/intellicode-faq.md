...
## В. Предполагается ли добавить в расширение Visual Studio IntelliCode другие функции? Как насчет формирования кода EditorConfig?

Мы активно работаем над рядом возможностей, которые обязательно представим широкой общественности, как только подготовим. Мы уже продемонстрировали раннюю, внутреннюю версию средства, которое определяет параметры EditorConfig для стиля и форматирования кода на языке C# на конференции Microsoft Build 2018. Это расширение пока находится на стадии тестирования, однако мы планируем выпустить его совсем скоро. Вы можете зарегистрироваться для получения новостей на сайте [https://aka.ms/intellicode](https://aka.ms/intellicode), чтобы первыми узнать о выпуске новых возможностей.
...
Предложения IntelliCode отображаются в стандартном интерфейсе IntelliSense в Visual Studio для C#. Расширения, которые переопределяют эту среду, блокируют вывод предложений IntelliCode в верхней части списка. Чтобы убедиться, что это поведение вызвано расширениями, отключите их, а затем попробуйте использовать IntelliSense еще раз.
