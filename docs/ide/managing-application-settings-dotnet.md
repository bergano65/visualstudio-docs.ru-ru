...
## <a name="using-customized-settings-files"></a>Использование настраиваемых файлов параметров
...
## <a name="accessing-or-changing-application-settings-at-run-time-in-c"></a>Доступ или изменение параметров приложения во время выполнения в C# #
...
Также необходимо явным образом вызывать метод `Save` этого оберточного класса, чтобы сохранить настройки пользователя. Обычно это выполняется в обработчике событий `Closing` главной формы. В приведенном ниже примере C# показан вызов метода `Save`.
