---
title: "Создание шаблонов проектов для Visual Studio | Документы Майкрософт"
ms.custom: 
ms.date: 01/02/2018
ms.reviewer: 
ms.suite: 
...
>[!NOTE]
> Чтобы найти шаблон в диалоговом окне **Новый проект**, разверните узел **Установленные**, а затем разверните категорию, которая соответствует элементу `ProjectType` в VSTEMPLATE-файле. Например, VSTEMPLATE-файл, содержащий `<ProjectType>CSharp</ProjectType>`, по умолчанию отображается в разделе **Установленные** > **Visual C#**. Шаблон можно расположить во вложенном каталоге типа проекта. Для этого просто создайте папку в этом каталоге и поместите в нее ZIP-файл проекта. Дополнительные сведения см. в разделе [Практическое руководство. Размещение и упорядочение шаблонов и элементов](../ide/how-to-locate-and-organize-project-and-item-templates.md).

## <a name="other-ways-to-create-project-templates"></a>Другие способы создания шаблонов проектов

Шаблоны проектов можно создать вручную путем сбора файлов, образующих проект, в папку, а затем создав VSTEMPLATE-файл XML-кода с соответствующими метаданными. Дополнительные сведения см. в статье [Практическое руководство. Создание веб-шаблонов вручную](../ide/how-to-manually-create-web-templates.md).
...
[Приступая к работе с использованием шаблона проекта VSIX](../extensibility/getting-started-with-the-vsix-project-template.md)
