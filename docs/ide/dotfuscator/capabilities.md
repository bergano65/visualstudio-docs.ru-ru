...
При обнаружении некорректного состояния приложения код подтверждения может [вызвать код приложения, чтобы исправить ситуацию соответствующим образом][check-app].
