...
1.  Добавьте в функцию [\<signature\>](../Topic/%3Csignature%3E%20\(JavaScript\).md) элемента для каждой перегрузки.  В этих элементах добавлять другие элементы, такие как `<summary>`, `<param>`, и `<returns>`, предшествующего каждый элемент с тремя косой черты \(\/ \/ \/\).  
