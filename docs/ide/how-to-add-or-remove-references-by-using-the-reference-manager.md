...
Все проекты классических приложений содержат неявную ссылку на библиотеку mscorlib. Проекты [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] содержат неявную ссылку на Microsoft.VisualBasic. Все проекты содержат неявную ссылку на System.Core, даже если она была удалена из списка ссылок.
...
2. В разделе **Расширения** перечислены все сборки, разработанные внешними поставщиками компонентов и элементов управления для расширения целевой версии .NET Framework. В зависимости от назначения пользовательского приложения эти сборки могут понадобиться ему.
   ...
   - HKEY_CURRENT_USER\SOFTWARE\Microsoft\[Идентификатор требуемой версии .NET Framework]\v[Требуемая версия .NET Framework]\AssemblyFoldersEx\[Имя_пользовательского_компонента]\@default=[Расположение сборок на диске]
   - HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\[Идентификатор требуемой версии .NET Framework]\v[Требуемая версия .NET Framework]\AssemblyFoldersEx\[Имя_пользовательского_компонента]\@default=[Расположение сборок на диске]

   64-разрядный компьютер:
   - HKEY_CURRENT_USER\SOFTWARE\Wow6432Node\Microsoft\[Идентификатор требуемой версии .NET Framework]\v[Требуемая версия .NET Framework]\AssemblyFoldersEx\[Имя_пользовательского_компонента]\@default=[Расположение сборок на диске]
   - HKEY_LOCAL_MACHINE\SOFTWARE\Wow6432Node\Microsoft\[Идентификатор требуемой версии .NET Framework]\v[Требуемая версия .NET Framework]\AssemblyFoldersEx\[Имя_пользовательского_компонента]\@default=[Расположение сборок на диске] И более старые версии [Идентификатор требуемой версии .NET Framework]
...
В разделе "Расширения" перечислены пользовательские пакеты SDK, расширяющие целевую платформу Windows. Эта вкладка отображается только для проектов приложений для Магазина Windows 8.x. Для проектов классических приложений эта вкладка не отображаются, так как они могут использовать только основные файлы WinMD.
