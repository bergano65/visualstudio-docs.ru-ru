...
1. В разделе **Платформа** перечислены все сборки, из которых состоит целевая платформа.
   ...
   Для проектов, не предназначенных для .NET Core и универсальной платформы Windows **Framework** вкладке перечислены сборки из целевой версии .NET framework. Пользователь должен добавить ссылки, которые требуются приложению.

   Универсальных проектов Windows по умолчанию содержат ссылки на все сборки в целевой версии .NET framework. В управляемых проектах доступный только для чтения узел в папке **Ссылки** в **Обозревателе решений** содержит ссылку на всю платформу .NET Framework. Соответственно **Framework** вкладки не указаны никакие сборки из .NET framework, а вместо них отображается следующее сообщение: "Ссылки на все сборки .NET Framework уже заданы. Используйте обозревателя объектов рассмотрите ссылки в Framework».
...
- Компонент, который использует последние версии платформы несовместим с проектом, нацеленным на более ранней версии.

   Сведения о том, как изменить версию целевой платформы для проекта, см. в разделе [как: Целевая версия framework](../ide/how-to-target-a-version-of-the-dotnet-framework.md).

- Компонент, который использует .NET Framework 4 не совместим с проекте, ориентированном на .NET Framework 4.5.
...
> [!NOTE]
> В Visual Studio 2015 или более поздней версии ссылку на файл вместо ссылки на проект создается в том случае, если целевая версия framework одного проекта — .NET Framework 4.5 или более поздней версии, а целевая версия другого проекта — .NET Framework 2, 3, 3.5 или 4.0.
...
- Установите раздел реестра, определяющий положение отображаемых сборок.
   ...
   *\<VersionMinimum\>*  является минимальной версией платформы, применяется. Если *\<VersionMinimum\>*  — v3.0, папки, указанные в *AssemblyFoldersEx*, относятся к проектам, предназначенным для платформы .NET Framework 3.0 и более поздних версий.
...
Проект может ссылаться на другой проект, предназначенный для версии другая платформа. Например можно создать проект, ориентированном на .NET Framework 4, но ссылается на сборку, собранную для .NET Framework 2. Однако проект .NET Framework 2 не может ссылаться на проект .NET Framework 4. Дополнительные сведения см. в разделе [Framework targeting Обзор](../ide/visual-studio-multi-targeting-overview.md).

> [!NOTE]
> Проект, предназначенный для .NET Framework 4 не совместим с проектом, нацеленным клиентский профиль .NET Framework 4.
...
Проект может ссылаться на компонент, предназначенный для версии другая платформа. Например можно создать приложение, которое предназначено .NET Framework 4.7, но ссылается на компонент, который нацелен на .NET Framework 4. Дополнительные сведения см. в разделе [Framework targeting Обзор](../ide/visual-studio-multi-targeting-overview.md).

Избегайте добавлять ссылки на выходные файлы другого проекта в том же решении, так как это может привести к ошибкам компиляции. Вместо этого следует создавать ссылки между проектами, используя вкладку **Решение** диалогового окна **Диспетчер ссылок**. Это позволит упростить коллективную разработку за счет более эффективного управления библиотеками классов, создаваемыми в проектах. Дополнительные сведения см. в статье [Устранение неполадок неработающих ссылок](../ide/troubleshooting-broken-references.md).
