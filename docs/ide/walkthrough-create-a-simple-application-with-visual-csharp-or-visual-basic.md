---
title: "Пошаговое руководство. Создание простого приложения с помощью Visual C# или Visual Basic | Microsoft Docs"
ms.custom: ""
ms.date: "12/14/2016"
...
3.  Сохраните изменения, нажав клавиши Ctrl\-s или с помощью меню **Файл**.  
 ...
 Следующий шаг — добавить в форму два элемента управления [RadioButton](../Topic/RadioButton.md).  
...
2.  Откройте контекстное меню элемента управления RadioButton2 \(выбрав его и нажав правую кнопку мыши\), выберите команду **Изменить текст**, а затем введите `Goodbye`.  
 ...
 Последний элемент пользовательского интерфейса, который вам предстоит добавить, — это [Button](../Topic/Button.md).  
...
##  <a name="BKMK_DebugTest"></a> Отладка и тестирование приложения  
 После этого вам предстоит отладить приложение для выявления ошибок и тестирования того, что оба окна сообщений отображаются правильно. Приведенные ниже инструкции описывают, как выполнить сборку и запустить отладчик \(дополнительные сведения см. в разделах [Building a WPF Application \(WPF\)](../Topic/Building%20a%20WPF%20Application%20\(WPF\).md) \(Построение приложения WPF\) и [Отладка WPF](../debugger/debugging-wpf.md)\).  
...
1.  Выберите в главном меню **Сборка** и **Очистить решение** для удаления промежуточных файлов и выходных файлов, которые были созданы в ходе предыдущих сборок.  Это не является обязательным, но очищает результаты отладочной сборки.  
     ...
     ![Команда "Очистить решение" в меню "Построение"](../ide/media/exploreide-cleansolution.png "ExploreIDE\-CleanSolution")  
...
3.  Постройте решение, выбрав **Сборка**, затем **Собрать решение** или нажмите клавишу F6.  
 ...
 Поздравляем с завершением этого пошагового руководства\! Построенный EXE\-файл находится в каталоге решения и проекта \(... \\HelloWPFApp\\HelloWPFApp\\bin\\Release\\\). Чтобы изучить больше примеров, см. раздел [Примеры Visual Studio](../ide/visual-studio-samples.md).  
  
## См. также  
 [Новые возможности Visual Studio 2015](../ide/what-s-new-in-visual-studio-2015.md)   
 [Начало работы](../ide/get-started-developing-with-visual-studio.md)   
