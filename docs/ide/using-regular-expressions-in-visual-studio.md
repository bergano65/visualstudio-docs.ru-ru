...
В [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] регулярные выражения платформы .NET Framework используются для поиска и замены текста. Подробнее о регулярных выражениях .NET см. в разделе [Регулярные выражения в .NET Framework](http://msdn.microsoft.com/Library/521b3f6d-f869-42e1-93e5-158c54a6895d).  
...
|Соответствует границе слова.|`\b` (вне класса символов \b определяет границу слова, а внутри класса символов определяет стирание назад).|`\bin` совпадает с in в inside, но не в pinto.|  
