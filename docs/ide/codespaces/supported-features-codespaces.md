...
> [!NOTE]
> Если при использовании codespace с Visual Studio вам не хватает какой-либо функции, сообщите нам об этом, открыв проблему в https://developercommunity.visualstudio.com/. Это позволит нам упорядочить функции по приоритету.
...
* Редактор JavaScript* — частичная поддержка.
...
* Навигация по коду* — большинство команд, в частности "Перейти к базовому" и "Найти в файлах" с указанием пути, пока не поддерживается.
...
* Редактор JavaScript* — блоки скриптов (например, содержимое JavaScript в файлах HTML и CSHTML) и семантическое выделение пока не поддерживаются. Известные проблемы с функциями "лампочки" и анализом кода.
...
* Приложения C++ для Linux — в основном поддерживаются не для графического пользовательского интерфейса. Возможность установки и подготовки WSL, технология IntelliSense для конкретной платформы и сборка.
...
* Конструкторы пользовательского интерфейса — конструкторы WinForms и WPF
