...
![Рабочая нагрузка "Кроссплатформенная разработка .NET Core" в Visual Studio Installer](../ide/media/dotnet-core-cross-platform-workload.png)

При первом запуске Visual Studio вы можете выполнить [вход](signing-in-to-visual-studio.md) с использованием либо учетной записи Майкрософт, либо рабочей или учебной учетной записи.

## <a name="tour-of-the-ide"></a>Обзор IDE

Обобщенный визуальный обзор Visual Studio приведен на снимке экрана ниже. На нем показана среда Visual Studio с открытым проектом и несколькими окнами основных инструментов, которые вы, скорее всего, будете использовать.

![Интегрированная среда разработки Visual Studio](../ide/media/visualstudioide.png)

- [**Обозреватель решений**](../ide/solutions-and-projects-in-visual-studio.md) (правый верхний угол) позволяет просматривать файлы кода, а также перемещаться по ним и управлять ими. **Обозреватель решений** позволяет упорядочить код путем объединения файлов в [решения и проекты](quickstart-projects-solutions.md).
...
- [Рефакторинг](../ide/refactoring-in-visual-studio.md)
   ...
   ![Рефакторинг в Visual Studio](../ide/media/refactoring-menu.png)
...
- [IntelliSense](../ide/using-intellisense.md)
   ...
   ![Список элементов Visual Studio](../ide/media/intellisense-list-members.png)
...
- [Быстрый запуск](../ide/reference/quick-launch-environment-options-dialog-box.md)
   ...
   ![Поле поиска в быстром запуске в Visual Studio](../ide/media/quick-launch-nuget.png)
...
- Волнистые линии и [быстрые действия](../ide/quick-actions.md)
   ...
   ![Волнистые линии в Visual Studio](../ide/media/squiggles-error.png)
...
- [Перейти к определению](../ide/go-to-and-peek-definition.md)
   ...
   ![Перейти к определению](../ide/media/go-to-definition-menu.png)
...
- [Показать определение](../ide/how-to-view-and-edit-code-by-using-peek-definition-alt-plus-f12.md)
   ...
   ![Показать определения](../ide/media/peek-definition.png)
...
1. Запустите Visual Studio. В строке меню выберите **Файл** > **Создать** > **Проект**.
   ...
   ![Параметры меню "Файл" > "Новый" > "Проект".](../ide/media/file-new-project-menu.png)
...
3. Теперь запустите приложение. Это можно сделать, выбрав **Запуск без отладки** в меню **Отладка** в строке меню. Можно также нажать клавиши **CTRL**+**F5**.
...
5. Давайте добавим дополнительный код в приложение. Перед строкой `Console.WriteLine("Hello World!");` добавьте следующий код C#:
...
7. Снова запустите приложение, выбрав пункты **Отладка** > **Запуск без отладки** или нажав клавиши **CTRL**+**F5**.
...
3. Выберите значок лампочки для отображения доступных [быстрых действий](quick-actions.md). Выберите **Переименовать name в username**.
...
4. Теперь рассмотрим возможности IntelliSense. Под строкой с текстом `Console.WriteLine($"\nHello {username}!");` введите **DateTime now = DateTime.**.
...
5. Выберите член с именем **Now**, который является свойством класса, дважды щелкнув его или нажав клавишу **TAB**. Завершите строку кода, добавив точку с запятой **;**.

6. Под ней введите или скопируйте следующие строки кода:
...
8. Щелкните значок отвертки, чтобы увидеть предложения Visual Studio. В этом случае отображается рефакторинг [Встроенная временная переменная](reference/inline-temporary-variable.md) для удаления строки кода без изменения общего поведения:
