...
## <a name="use-msbuild-to-reproduce-clickonce-application-deployments"></a>Воспроизведение развертываний приложений ClickOnce с помощью MSBuild
 ...
 Сведения о создании полного [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] развертывания с помощью программы командной строки Mage. exe для [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] создания манифеста см. в разделе [пошаговое руководство. Развертывание приложения](../deployment/walkthrough-manually-deploying-a-clickonce-application.md)ClickOnce вручную.
...
4. Чтобы удалить существующие файлы, созданные в "Создание и публикация [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] проекта", введите. `rmdir /s publish`
...
5. Введите `msbuild /target:publish`.
   ...
   Описанные выше действия приведут к [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] полному развертыванию приложения во вложенной папке проекта с именем **Publish**. *Кмдлинедемо. Application* — это [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] манифест развертывания. Папка *cmdlinedemo_ 1.0.0.0* содержит файлы *кмдлинедемо. exe* и [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] *кмдлинедемо. exe. manifest*, манифест приложения. *Setup. exe* — это начальный загрузчик, который по умолчанию настроен для установки .NET Framework. Папка DotNetFX содержит распространяемые компоненты для .NET Framework. Это полный набор файлов, необходимых для развертывания приложения через Интернет или через UNC или компакт-диск или DVD-диск.
   
> [!NOTE]
> Система MSBuild использует параметр **PublishDir** для указания расположения выходных данных, `msbuild /t:publish /p:PublishDir="<specific location>"`например.

## <a name="publish-properties"></a>Параметры публикации
 При публикации приложения в описанных выше процедурах в файл проекта вставляются следующие свойства с помощью мастера публикации. Эти свойства непосредственно влияют на то [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] , как создается приложение.
...
```
 ...
 Управление [!INCLUDE[vsprvs](../code-quality/includes/vsprvs_md.md)] свойствами публикации осуществляется на страницах свойств **Публикация**, **Безопасность**и **подпись** **конструктора проектов**. Ниже приведено описание свойств публикации, а также сведения о том, как они заданы на различных страницах свойств конструктора приложений.
...
- **Включить параметры безопасности ClickOnce** определяет, [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] создаются ли манифесты. При первоначальном создании [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] проекта создание манифеста по умолчанию отключено. Мастер автоматически установит этот флаг при публикации в первый раз.

- **Таржетзоне** определяет уровень доверия, который будет выдаваться в [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] манифест приложения. Возможные значения: "Internet", "LocalIntranet" и "Custom". В Интернете и интрасети набор разрешений по умолчанию создается в [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] манифесте приложения. Локальная интрасеть используется по умолчанию, и она по сути означает полное доверие. Custom Указывает, что только разрешения, явно указанные в файле *app. manifest* , должны быть [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] переданы в манифест приложения. Файл *app. manifest* является частичным файлом манифеста, который содержит только определения сведений о доверии. Это скрытый файл, который автоматически добавляется в проект при настройке разрешений на странице **Безопасность** .

  На странице **Публикация** задаются следующие свойства:

- `PublishUrl`— Это расположение, в котором приложение будет опубликовано в интегрированной среде разработки. Он вставляется в [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] манифест приложения, если не указано `InstallUrl` ни `UpdateUrl` свойство, ни.

- `ApplicationVersion`Указывает версию [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] приложения. Это номер версии из четырех цифр. Если последняя цифра является "*", то вместо значения `ApplicationRevision` , вставляемого в манифест, во время сборки заменяется.
...
- `InstallUrl`(не показано) — это расположение, откуда пользователи будут устанавливать приложение. Если этот параметр задан, это значение будет записано в загрузчик *Setup. exe* , `IsWebBootstrapper` если свойство включено. Он также вставляется в манифест приложения, если параметр `UpdateUrl` не указан.
...
- `UpdateIntervalUnits`Указывает, находится `UpdateInterval` ли значение в единицах времени, днях или неделях.
...
|`SupportURL`|Необязательный параметр. Задайте этот параметр URL-адреса, если сайт поддержки отличается от `PublishURL`. Например, можно установить `SupportURL` на веб-сайт поддержки клиентов вашей компании.|
