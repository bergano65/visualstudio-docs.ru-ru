...
| Атрибут | Описание |
|--------------------------| - |
| `install` | Обязательный. Указывает, определяет ли приложение присутствие в меню " **Пуск** " Windows и на панели управления " **Установка и удаление программ** ". Допустимые значения: `true` и `false`. Если `false` значение равно, [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] приложение всегда будет запускать последнюю версию этого приложения из сети и не будет распознать `subscription` элемент. |
