...
| Атрибут | Описание |
|--------------------------| - |
| `install` | Обязательный. Указывает, определяет ли это приложение свое присутствие на Windows **запустить** меню и панели управления **Установка и удаление программ** приложения. Допустимые значения: `true` и `false`. Если `false`, [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] всегда будет выполняться последнюю версию этого приложения из сети, а не распознает `subscription` элемент. |
...
| Атрибут | Описание |
|------------| - |
| `codebase` | Обязательный. Указывает расположение, как универсальный код ресурса (URI), манифест развертывания, который используется для обновления [!INCLUDE[ndptecclick](../deployment/includes/ndptecclick_md.md)] приложения. Этот элемент также позволяет переадресовывать расположение обновлений для установок на основе компакт-диска. Должен быть допустимым URI. |
