...
## <a name="create-customer-deployments-by-using-application-manifest-for-trust"></a>Создание клиентских развертываний с помощью манифеста приложения для доверия
 ...
 Пошаговые инструкции по созданию развертывания ClickOnce с включенным параметром `<useManifestForTrust>` см. в разделе [пошаговое руководство. Вручную разверните приложение ClickOnce, которое не требует повторного подписания и сохраняет сведения о](../deployment/walkthrough-manually-deploying-a-clickonce-app-no-re-signing-required.md)фирменной символике.
