...
* Манифест продукта *Product. XML*содержит любые независимые от языка метаданные для пакета. Здесь находятся общие метаданные для всех локализованных версий распространяемого компонента.  Чтобы создать этот файл, см [. раздел как Создайте манифест](../deployment/how-to-create-a-product-manifest.md)продукта.

* Манифест пакета *Package. XML*содержит метаданные для конкретного языка; Обычно они содержат локализованные сообщения об ошибках. Компонент должен содержать хотя бы один манифест пакета для каждой локализованной версии. Чтобы создать этот файл, см [. раздел как Создайте манифест](../deployment/how-to-create-a-package-manifest.md)пакета.
...
Затем скопируйте распространяемые файлы в расположение папки начального загрузчика. Дополнительные сведения см. в разделе [Практическое руководство. создать пакет локализованного начального загрузчика](../deployment/how-to-create-a-localized-bootstrapper-package.md).
...
|[VersionMSI](/windows/desktop/Msi/versionmsi)|Версия сборки установщик Windows (MSI. dll), которая должна быть запущена во время установки.|
...
- [Практическое руководство. Установка необходимых компонентов с помощью приложения ClickOnce](../deployment/how-to-install-prerequisites-with-a-clickonce-application.md)
