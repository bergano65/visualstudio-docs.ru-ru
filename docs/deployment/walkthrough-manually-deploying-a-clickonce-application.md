...
6. Подпишите манифест приложения с помощью сертификата Authenticode. Замените *MyCert. pfx* на путь к файлу сертификата. Замените *passwd* паролем для файла сертификата.
   ...
   Начиная с пакета SDK .NET Framework 4.6.2, который распространяется в Visual Studio и с Windows SDK, *mage.exe* подписывает манифесты с помощью CNG, а также с сертификатами Authenticode. Используйте те же параметры командной строки, что и для сертификатов Authenticode.
