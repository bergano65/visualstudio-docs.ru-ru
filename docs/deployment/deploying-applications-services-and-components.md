...
В разделе учебников пошаговые инструкции по развертыванию. Если вы развертываете веб-приложения и требуется более подробные сведения, чтобы определить наилучшее решение развертывания из Visual Studio, см. раздел [какие параметры публикации наиболее подходящие для меня?](../ide/not-in-toc/web-publish-options.md).

## <a name="deploy-to-local-folder"></a>Развернуть локальную папку

Развертывание в локальную папку обычно используется для тестирования или начать поэтапного развертывания, в котором другое средство будет использоваться для окончательного развертывания.
...
## <a name="azure"></a> Опубликовать в Azure

- **ASP.NET**, **ASP.NET Core**, **Python**, и **Node.js**: можно использовать средство публикации для быстрого развертывания приложений для службы приложений Azure или Azure виртуальный Компьютер. В обозревателе решений щелкните проект правой кнопкой мыши и выберите пункт **Опубликовать**. (Если вы ранее настроили все профили публикации, нажмите кнопку **Создание нового профиля**.) В диалоговом окне «публикация» выберите **службы приложений** или **виртуальных машинах Azure**, а затем выполните действия по настройке.

    ![Выберите службы приложений Azure](../deployment/media/quickstart-publish-azure.png "выберите службы приложений Azure")

    В Visual Studio 2017 г. версия 15,7, можно развертывать приложения ASP.NET Core для **службы приложений для Linux**.
...
## <a name="web"></a> Публикация в Интернете или развернуть на сетевом ресурсе

- **ASP.NET**, **ASP.NET Core**, **Node.js**, и **Python**: можно использовать средство публикации для развертывания на веб-сайт через FTP или веб-развертывания. Дополнительные сведения см. в разделе [развертывание на веб-сайт](quickstart-deploy-to-a-web-site.md).
...
## <a name="microsoft_store"></a> Публикация в Microsoft Store
...
## <a name="deploy-to-a-device-uwp"></a>Развертывания на устройстве (UWP)

Если выполняется развертывание приложения UWP для тестирования на устройстве, см. раздел [приложений UWP, запустите на удаленном компьютере в Visual Studio](../debugger/run-windows-store-apps-on-a-remote-machine.md).

## <a name="installer"></a> Создание пакета установщика (клиент Windows)
...
В этом учебнике заняло кратко рассмотрим параметры развертывания для разных приложений. При развертывании веб-приложения, такие как ASP.NET считывать углубленные о некоторых параметров развертывания, доступных в Visual Studio.

> [!div class="nextstepaction"]
> [Какие параметры публикации наиболее подходящие для меня?](../ide/not-in-toc/web-publish-options.md)
