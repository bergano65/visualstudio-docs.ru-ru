---
title: Общие сведения о развертывании - Visual Studio | Документация Майкрософт
ms.custom: ''
ms.date: 06/22/2018
...
Для многих распространенных типов приложений вы можете развернуть ваше право приложения из обозревателя решений в Visual Studio. Краткий обзор этой возможности, см. в разделе [Первое знакомство с развертыванием](../deployment/deploying-applications-services-and-components.md).

![Выберите вариант публикации](../deployment/media/quickstart-publish-azure.png)

## <a name="what-publishing-options-are-right-for-me"></a>Выбор подходящих вариантов публикации

Из среды Visual Studio приложения можно публиковать непосредственно в следующие целевые объекты:
...
Служба приложений выполняется на облачных виртуальных машинах в Azure, но эти виртуальные машины являются управляемыми. Каждое приложение в службе приложений назначается уникальный \*. URL-адреса azurewebsites.net; все ценовые категории Кроме бесплатный позволяют назначать сайту пользовательские имена доменов.
...
[Виртуальные машины Azure](https://azure.microsoft.com/documentation/services/virtual-machines/) позволяют создавать любое количество вычислительных ресурсов в облаке и управлять ими. Принимая на себя ответственность за все программное обеспечение и обновления на виртуальных машинах, можно настроить их соответствии согласно требованиям приложения. Вы можете обращаться к виртуальным машинам напрямую через удаленный рабочий стол, и для каждой из них назначенный IP-адрес будет существовать столько, сколько нужно.

Масштабирование приложения, размещенного на виртуальных машинах включает в себя запуск дополнительных виртуальных машин в соответствии с требованиями и затем развернуть необходимое программное обеспечение. Этот дополнительный уровень управления позволяет вам применять разные режимы масштабирования в разных глобальных регионах. Например, если ваше приложение обслуживает сотрудников в различных региональных офисах, вы можете масштабировать ваши виртуальные машины в зависимости от количества сотрудников в этих регионах, что способствует сокращению затрат.
...
Развертывание в файловой системе означает просто скопировать файлы приложения в определенную папку на своем компьютере. Чаще всего используется в целях тестирования или для развертывания приложения для использования с ограниченным количеством пользователей, если на компьютере также запущена на сервере. Если целевая папка является общей для сети, при развертывании в файловой системе файлы веб-приложения станут доступны другим пользователям, которые затем смогут развернуть их на конкретных серверах.

Все локальные компьютеры под управлением сервера может сделать приложение доступным через Интернет или интрасеть в зависимости от конфигурации сети, к которым он подключен. (При подключении компьютера к Интернету напрямую обязательно защитите его от внешних угроз.) Поскольку вы управляете этими компьютерами, вы полностью контролируете конфигурации оборудования и программного обеспечения.
...
Дополнительные сведения см. в разделе [краткое руководство. развертывание в локальную папку](quickstart-deploy-to-local-folder.md)

## <a name="custom-targets-iis-ftp"></a>Пользовательские целевые объекты (IIS, FTP)

Пользовательский целевой объект позволяет развертывать приложения в целевой объект, отличный от службы приложений Azure, виртуальных машинах или в локальной файловой системе. Его можно развернуть в файловой системе или на любом другом сервере (в Интернете или интрасети), к которому у вас есть доступ, включая области в других облачных службах. Он может работать с веб-развертыванием (файлы или ZIP) и FTP.
...
Дополнительные сведения см. в разделе [краткое руководство. развертывание веб-сайте](quickstart-deploy-to-a-web-site.md)

## <a name="next-steps"></a>Следующие шаги

Учебники:

- [Развертывание приложения на .NET Core с помощью средства публикации](/dotnet/core/deploying/deploy-with-vs?toc=/visualstudio/deployment/toc.json&bc=/visualstudio/deployment/_breadcrumb/toc.json)
...
- [Развертывание приложений UWP](/windows/uwp/packaging/packaging-uwp-apps?toc=/visualstudio/deployment/toc.json&bc=/visualstudio/deployment/_breadcrumb/toc.json)
...
- [Публикация приложения Python в службе приложений Azure](/visualstudio/python/publishing-python-web-applications-to-azure-from-visual-studio?toc=/visualstudio/deployment/toc.json&bc=/visualstudio/deployment/_breadcrumb/toc.json)
