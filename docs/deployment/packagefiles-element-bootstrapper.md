...
| `HomeSite` | Необязательный параметр. Расположение пакета на удаленном сервере, если оно не включено в установщик. |
| `CopyOnBuild` | Необязательный параметр. Указывает, должен ли загрузчик копировать файл пакета на диск во время сборки. Значение по умолчанию — true. |
...
| `Hash` | Необязательный параметр. Хэш SHA1 файла пакета. Используется для проверки целостности файла во время установки. Если идентичный хэш не может быть вычислен из файла пакета, то пакет не будет установлен. |

## <a name="example"></a>Пример
 В следующем примере кода определяются пакеты для распространяемого пакета .NET Framework и его зависимостей, таких как установщик Windows.
