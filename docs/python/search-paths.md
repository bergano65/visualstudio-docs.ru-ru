...
Чтобы добавить путь поиска, в обозревателе решений щелкните в **обозревателе решений** правой кнопкой мыши **Пути поиска**, выберите **Добавить папку для пути поиска** и выберите нужную папку. Этот путь используется для любой среды, связанной с проектом. (Если окружение создано на основе Python 3, может произойти ошибка при попытке добавить путь поиска для модулей Python 2.7.)

Файлы с расширением *.zip* или *.egg* также можно добавить в качестве путей поиска. Для этого нужно выбрать **Добавить ZIP-архив к пути поиска**. Как и при использовании папок, содержимое этих файлов проверяется и предоставляется для IntelliSense.
