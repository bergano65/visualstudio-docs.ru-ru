...
Чтобы превратить библиотеку DLL на C++ в расширение для Python, сначала нужно изменить экспортированные методы для взаимодействия с типами Python. После этого нужно добавить функцию, экспортирующую модуль, а также определения методов модуля. Общие сведения о приведенном здесь примере см. в [справочном руководстве по API Python/C](https://docs.python.org/3/c-api/index.html) и в особенности в разделе об [объектах модуля](https://docs.python.org/3/c-api/module.html) на сайте python.org. (Не забудьте выбрать свою версию Python в раскрывающемся списке в правом верхнем углу.)

> [!Note]
> Эти инструкции предназначены для Python 3.x. Если вы работаете с Python 2.7, см. статьи о [расширении Python 2.7 с использованием C или C++](https://docs.python.org/2.7/extending/extending.html) и [переносе модулей расширения в Python 3](https://docs.python.org/2.7/howto/cporting.html) (python.org).
...
1. В начало файла C++ включите `Python.h`:
    ...
    > [!Tip]
    > Если отображается ошибка *E1696: Не удается открыть исходный файл "Python.h"* и (или) *C1083: Не удается открыть включаемый файл: "Python.h": такой файл или каталог отсутствует*, убедитесь, что вы задали для параметра **C/C++ > Общие > Дополнительные каталоги включаемых файлов** в свойствах проекта папку `include` установки Python, как описано на шаге 6 в разделе [Создание основного проекта C++](#create-the-core-c-project).
...
1. Задайте целевой конфигурации значение "Выпуск" и повторно создайте проект C++, чтобы проверить код. Если возникают ошибки, проверьте следующие варианты:
    - Не удается найти Python.h. Убедитесь, что путь **C/C++ > Общие > Дополнительные каталоги включаемых файлов** в свойствах проекта указывает на папку `include` установки Python.
    - Не удается найти библиотеки Python. Убедитесь, что путь **C/C++ > Общие > Дополнительные каталоги библиотеки** в свойствах проекта указывает на папку `libs` установки Python.
    - Ошибки компоновщика, связанные с целевой архитектурой. Измените архитектуру целевого проекта C++ в соответствии с вашей установкой Python.
...
Альтернативный метод, описанный далее, предполагает установку модуля в глобальной среде Python и предоставление к нему доступа в других проектах Python. (При этом обычно требуется обновить базу данных завершения IntelliSense для этой среды. Обновление также требуется и при извлечении модуля из среды.)
...
1. Запустите программу Python (**Отладка > Запуск без отладки** или CTRL+F5) и обратите внимание, что подпрограмма C++ выполняется в 5–20 раз быстрее, чем реализация Python. Попробуйте увеличить значение переменной `COUNT`, чтобы разница стала еще очевиднее. Кроме того, обратите внимание, что отладочная сборка модуля C++ выполняется медленнее, чем сборка выпуска, так как отладочная сборка менее оптимизирована и включает в себя различные проверки ошибок. Вы можете свободно переключаться между этими конфигурациями для их сравнения.
...
| Cython | 2007 | [gevent](http://www.gevent.org/), [kivy](https://kivy.org/) | Аналогичен Python. Высокая степень зрелости. Высокая производительность. | Компиляция, новый синтаксис, новая цепочка инструментов. |
