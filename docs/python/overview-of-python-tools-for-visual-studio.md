...
Visual Studio предоставляет первоклассный редактор Python, включающий цветовую разметку синтаксиса, автозаполнение во всем коде и библиотеках, форматирование кода, справку по сигнатурам, рефакторинг, анализ кода на наличие потенциальных ошибок (изображен ниже) и подсказки по типам. Visual Studio также предоставляет уникальные функции, такие как представление классов, переход к определению, поиск всех ссылок и фрагменты кода. Непосредственная интеграция с [интерактивным окном](#interactive-window) помогает быстро разрабатывать код на Python, который уже сохранен в файле.
