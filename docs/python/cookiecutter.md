---
title: "Использование расширения CookieCutter с Инструментами Python для Visual Studio | Документация Майкрософт"
ms.custom: 
ms.date: 3/7/2017
...
Работа с Cookiecutter заключается в следующем: просмотр и выбор шаблона, клонирование его на локальный компьютер, установка параметров, а затем создание кода из клонированного шаблона. Весь этот процесс описывается в разделах ниже.
...
После настройки параметров выберите **Создать**, чтобы создать код. Если папка выходных данных не пуста, появится предупреждение. Если вы уже сталкивались с выходными данными шаблона и не против перезаписи файлов, вы можете проигнорировать предупреждение. В противном случае щелкните **Отмена**, укажите пустую папку, а затем вручную скопируйте созданные файлы в папку выходных данных с определенным содержимым.
...
Чтобы ознакомиться с основами разработки шаблона Cookiecutter, обратитесь к [документации Cookiecutter](https://cookiecutter.readthedocs.io/en/latest/first_steps.html). Обратите внимание, что расширение Cookiecutter для Visual Studio поддерживает шаблоны, созданные для Cookiecutter версии 1.4.
...
Обратите внимание, что команда `Python.InstallProjectRequirements` работает только при добавлении файлов в существующий проект. Она обрабатывается проектом Python в обозревателе решений и не работает в представлении папок в обозревателе решений, так как в таком случае отсутствует проект. Мы планируем устранить это ограничение в последующих выпусках (и улучшить поддержку представления папок в целом).

## <a name="troubleshooting"></a>Устранение неполадок

### <a name="error-loading-template"></a>Ошибка при загрузке шаблона

Некоторые шаблоны могут использовать недопустимые типы данных, например логические значения, в файлах `cookiecutter.json`. Авторы шаблонов должны быть уведомлены об этом. На информационной панели шаблона щелкните ссылку **Проблемы**.

### <a name="hook-script-failed"></a>Не удалось выполнить сценарий обработчика

Некоторые шаблоны могут использовать сценарии задач после создания, несовместимые с пользовательским интерфейсом Cookiecutter. Например, сценарии на запрос ввода данных пользователем завершатся сбоем из-за отсутствия консоли терминала.

### <a name="hook-script-not-supported-on-windows"></a>Сценарий перехватчика не поддерживается в Windows

Если значение постсценария — `.sh`, его невозможно будет связать с приложением на компьютере с Windows. Может появиться всплывающее диалоговое окно с предложением найти совместимое приложение в Магазине Windows.
