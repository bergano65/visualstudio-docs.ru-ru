...
| Всегда выполнять pip от имени администратора | Off | Всегда повышает операции `pip install` для всех окружений. Если при установке пакетов окружение находится в защищенной области файловой системы, такой как `c:\Program Files`, Visual Studio запрашивает права администратора. В этом запросе можно разрешить всегда повышать `pip install` только для этого отдельного окружения. См. раздел [Окружения Python — вкладка pip](managing-python-environments-in-visual-studio.md#pip-tab). |
...
| Игнорировать системные переменные PYTHONPATH | включить | По умолчанию PYTHONPATH игнорируется, так как в Visual Studio есть более уместные способы указать пути поиска в окружениях и проектах. Дополнительные сведения см. в статье [Окружения Python — пути поиска](managing-python-environments-in-visual-studio.md#search-paths). |
| Обновлять пути поиска при добавлении связанных файлов | включить | Если параметр задан, добавление [связанного файла](managing-python-projects-in-visual-studio.md#linked-files) в проект обновляет [пути поиска](managing-python-environments-in-visual-studio.md#search-paths), чтобы система IntelliSense могла включить содержимое папки связанного файла в базу данных завершения. Снимите этот флажок, чтобы исключить подобное содержимое из базы данных завершения. |
...
| Параметр | По умолчанию | Описание: |
| --- | --- | --- |
| Сценарии | Н/Д | Указывает общую папку для скриптов запуска, применяемых ко всем интерактивным окнам для всех окружений. См. раздел [Скрипты запуска](managing-python-environments-in-visual-studio.md#startup-scripts). Однако следует отметить, что сейчас эта функция не работает. |
