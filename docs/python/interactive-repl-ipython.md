...
1. Откройте Visual Studio, переключитесь в окно среды Python (**Просмотр > Другие окна > Окружения Python**) и выберите среду Anaconda.

1. Изучите вкладку **Пакеты (Conda)** (которая может отображаться как **pip** или **Пакеты**) для этой среды, чтобы убедиться, что `ipython` и `matplotlib` указаны. Если нет, установите их с помощью этой же вкладки. (См. раздел [Вкладка "Пакеты" окна "Окружения Python"](python-environments-window-tab-reference.md).)
...
1. Выберите **Открыть интерактивное окно**, чтобы открыть интерактивное окно в режиме IPython. Если вы только что установили интерактивный режим, может потребоваться сброс установок окна, кроме того, может потребоваться нажать клавишу ВВОД, если отображается только приглашение ">>>", чтобы получить приглашение как "в [2]".
