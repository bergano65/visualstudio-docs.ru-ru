...
Как видите, в Poll сохраняется описание в поле `text` и дата публикации в `pub_date`. Эти поля являются односторонними, они существуют для Poll в базе данных. Значение поля `total_votes` вычисляется во время выполнения.
