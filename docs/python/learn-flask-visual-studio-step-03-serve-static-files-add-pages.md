---
title: Руководство. Сведения о Flask в Visual Studio — шаг 3
...
1. Замените содержимое файла `templates/index.html` приложения следующим кодом, который заменяет элемент `<strong>`, используемый на шаге 2, элементом `<span>`, ссылающимся на класс стиля `message`. Использование класса стиля таким образом дает большую гибкость в стилизации элемента.
...
Flask предоставляет функцию с именем `serve_static_file`, которую можно вызвать из кода для ссылки на любой файл в папке `static` проекта. В рамках следующей процедуры создается простая конечная точка API, которая возвращает файл статических данных.
...
1. В `views.py` добавьте функцию с маршрутом, API и данными, которая возвращает статический файл данных с помощью метода `send_static_file`:
...
Ответ. Вы можете добавить другие файлы CSS, JavaScript и HTML в свою папку `static` любым способом. Типичный способ организации статических файлов — это создание подпапок под названием `fonts`, `scripts` и `content` (для таблиц стилей и других файлов).
...
1. Откройте файл `templates/index.html` и добавьте следующую линию прямо в элемент `<body>`, чтобы связать страницу дополнительных сведений (опять же, вы заменяете эту ссылку навигационной панелью на шаге 3.4):
...
1. В папке `templates` приложения создайте HTML-файл (с помощью контекстного меню **Добавить** > **Новый элемент** или **Добавить** > **HTML-страница**) с именем `layout.html` и замените его содержимое указанным ниже кодом. Вы увидите, что этот шаблон содержит блок под названием content, который должен быть заменен для всех ссылающихся страниц:
...
1. Измените `templates/index.html` для ссылки на базовый шаблон и перезаписи блока content. Можно увидеть, что с помощью наследования этот шаблон упрощается:
...
1. Измените `templates/about.html`, чтобы ссылаться на базовый шаблон и перезаписать блок content:
