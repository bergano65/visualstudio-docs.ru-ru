---
title: "Среды Python в Инструментах Python для Visual Studio | Документация Майкрософт"
ms.custom: 
ms.date: 3/7/2017
...
Код Python всегда выполняется в рамках определенной *среды* Python, состоящей из интерпретатора, библиотеки (обычно стандартной библиотеки Python) и набора установленных пакетов. Вместе они определяют, какие языковые конструкции и синтаксис допустимы, какие возможности операционной системы доступны и какие пакеты можно использовать.

Рабочая нагрузка Python в Visual Studio упрощает управление несколькими средами Python и легко переключается между ними для различных проектов. Среда также включает в себя базу данных IntelliSense для библиотек среды. Благодаря этому, если, к примеру, ввести инструкцию `import` в редакторе Visual Studio, автоматически отобразится список доступных библиотек, а также содержащихся в них модулей.
