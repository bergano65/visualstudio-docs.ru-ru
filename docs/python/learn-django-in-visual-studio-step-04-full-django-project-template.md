---
title: Руководство. Сведения о Django в Visual Studio — шаг 4
...
Теперь, когда вы ознакомились с основами Django, создав в Visual Studio приложение на основе шаблона для пустого проекта приложения Django, вам будет легче создать более полное приложение с помощью шаблона "Веб-проект Django".
...
- Файлы в корневом каталоге проекта:
  ...
  - `db.sqlite3` — стандартная база данных SQLite.
...
В папке `templates/app` есть еще и файл четвертой страницы `login.html` вместе с файлом `loginpartial.html`, указанном в `layout.html` с использованием `{% include %}`. Эти файлы шаблона рассматриваются на шаге 5 с инструкциями по аутентификации.
