...
| **Пустой веб-проект Flask** | Создается минимальное приложение для вывода одной страницы "Hello World!" для `/`. Это приложение похоже на результат выполнения инструкций в [кратком руководстве по созданию первого веб-приложения Python с помощью Visual Studio](../ide/quickstart-python.md?context=visualstudio/python/default). См. также [шаг 1 руководства по работе с Flask](learn-flask-visual-studio-step-01-project-solution.md).
