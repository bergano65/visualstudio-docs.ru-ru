...
Каждый проект Python имеет один назначенный файл запуска, выделенный полужирным шрифтом в **обозревателе решений**. Это файл, который запускается, когда вы запускаете отладку (**F5** или **Отладка** > **Начать отладку**) или выполняете проект в **интерактивном** окне (**SHIFT**+**ALTF**+**5** или **Отладка** > **Выполнить проект в интерактивном окне Python**). Чтобы изменить его, щелкните правой кнопкой мыши файл и выберите **Set as Startup File** (Назначить файлом запуска).
