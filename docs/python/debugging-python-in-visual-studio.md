...
1. Кроме того, можно обновить пакет ptvsd до последней версии. Введите `ptvsd --upgrade -pre` в поле поиска и выберите **Выполнить команду: pip install ptvsd --upgrade -pre**. (Также можно использовать эту из PowerShell.)

    ![Указание команды обновления в окне "Окружения Python"](media/debugging-experimental-upgrade-ptvsd.png)
