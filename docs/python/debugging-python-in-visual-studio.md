...
| --- | --- | --- |
| `$continue`значение `$cont`значение `$c` | Запускает выполнение программы, начиная с текущего оператора. |
...
| `$stepin`значение `$step`значение `$s` | При возможности выполняет шаг с заходом в следующий вызов функции. |
| `$stepout`значение `$return`значение `$r` | Выходит из текущей функции. |
| `$stepover`значение `$until`значение `$unt` | Обходит вызов следующей функции. |
...
| `$where`значение `$w`значение `$bt` | Отображает список кадров текущего потока. |
