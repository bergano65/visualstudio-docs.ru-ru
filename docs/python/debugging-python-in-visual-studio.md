...
При остановке отладчика можно проверить и изменить значения переменных. Окно **контрольных значений** позволяет отслеживать отдельные переменные, а также пользовательские выражения. (Общие сведения см. в статье [Проверка переменных](../debugger/getting-started-with-the-debugger.md#inspect-variables-with-the-autos-and-locals-windows).)
...
При попытке использовать экспериментальный отладчик с несовместимой средой Visual Studio отображает ошибку **Отладчик несовместим с данной средой**:
...
Если вы установили в текущей среде более раннюю версию ptvsd (например, для удаленной отладки нужна более ранняя версия 4.0.x версии 3.x), Visual Studio отображает ошибку **Не удалось загрузить пакет отладчика** или предупреждение **Пакет отладчика устарел**:
