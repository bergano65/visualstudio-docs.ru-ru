...
## <a name="use-the-experimental-debugger"></a>Использование экспериментального отладчика

Начиная с Visual Studio 2017 (предварительная версия 4.0), можно согласиться на использование экспериментального отладчика, основанного на ptvsd версии 4.1+. Для этого выберите команду меню **Средства** > **Параметры**, перейдите к разделу **Python** > **Экспериментальные** в диалоговом окне "Параметры" и выберите **Использовать экспериментальный отладчик**.

Экспериментальный отладчик совместим только с ограниченным набором сред Python, как показано в следующей таблице:

| Версия Python | Совместимость с экспериментальным отладчиком |
| --- | --- |
| 2.6 | Нет |
| 2.7 | Да |
| 3.1–3.4 | Нет |
| 3.5 и более поздние версии | Да |
| IronPython. | Нет |

При попытке использовать экспериментальный отладчик с несовместимой средой Visual Studio отображает ошибку "Debugger is incompatible with this environment" (Отладчик несовместим с данной средой):

![Ошибка "Debugger is incompatible with this environment" (Отладчик несовместим с данной средой) при использовании экспериментального отладчика](media/debugging-experimental-incompatible-error.png)

Выберите команду **Disable the experimental debugger** (Отключить экспериментальный отладчик), которая отменяет выбор параметра **Использовать экспериментальный отладчик**.

> [!Note]
> Сейчас это предупреждение не отображается для Python 3.3 и 3.4.

Если вы установили в текущей среде более раннюю версию ptvsd (например, для удаленной отладки нужна более ранняя версия 4.0.x версии 3.x), Visual Studio отображает ошибку "Debugger package could not be loaded" (Не удалось загрузить пакет отладчика) или предупреждение "Debugger package is outdated" (Пакет отладчика устарел):

![Ошибка "Debugger package could not be loaded" (Не удалось загрузить пакет отладчика) при использовании экспериментального отладчика](media/debugging-experimental-version-error.png)

![Предупреждение "Debugger package is outdated" (Пакет отладчика устарел) при использовании экспериментального отладчика](media/debugging-experimental-version-warning.png)

Для управления установкой ptvsd используйте вкладку **Пакеты** окна **Окружения Python** или следующие команды из командной строки:
