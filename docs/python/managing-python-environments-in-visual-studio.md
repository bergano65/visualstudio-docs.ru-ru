...
1. Настройте следующие поля:
    ...
    | Имя | Имя среды Conda. |
    | Добавить пакеты из | Выберите **Файл окружения**, если у вас есть файл *environment.yml* с описанием зависимостей, или выберите **Имена одного или нескольких пакетов Anaconda** и отметьте хотя бы один пакет Python или версию Python в поле ниже. Этот список пакетов передает Conda задание на создание окружения Python. Чтобы установить последнюю версию Python, используйте `python`. Чтобы установить определенную версию, используйте `python=,major>.<minor>`, например: `python=3.7`. Кнопка пакета позволяет выбрать версии Python и стандартные пакеты из нескольких меню. |
    | Установить в качестве текущего окружения | После создания окружения активирует его в выбранном проекте. |
    | Установить в качестве окружения по умолчанию для новых проектов | Автоматически устанавливает среду Conda и активирует ее во всех новых проектах, которые создаются в Visual Studio. Этот параметр эквивалентен параметру **Назначить это окружение по умолчанию для новых проектов** в окне **Окружения Python**. |
    | Просмотреть в окне окружений Python | Указывает, нужно ли отобразить окно **Окружения Python** после создания окружения. |
...
1. Выберите **Создать** и отслеживайте ход выполнения в окне **Вывод**. Выходные данные после завершения создания возвращают несколько инструкций CLI:
...
1. Перейдите к разделу **HKEY_LOCAL_MACHINE\SOFTWARE\Python**. Если используется IronPython, вместо этого ищите **IronPython**.
