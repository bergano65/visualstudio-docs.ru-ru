...
ms.locfileid: "34454692"
---
# <a name="tutorial-step-2-create-a-django-app-with-views-and-page-templates"></a>Руководство (шаг 2). Создание приложения Django с представлениями и шаблонами страниц

**Предыдущий шаг. [Руководство (шаг 1). Начало работы с веб-платформой Django в Visual Studio](learn-django-in-visual-studio-step-01-project-and-solution.md)**
...
- Шаблоны. В **обозревателе решений** щелкните проект правой кнопкой мыши и выберите пункты **Добавить** > **Новый элемент**. В диалоговом окне **Добавить новый элемент** выберите шаблон "Приложение Django 1.9", укажите имя приложения в поле **Имя** и нажмите кнопку **OК**.

- Интегрированная команда. В **обозревателе решений** щелкните правой кнопкой мыши проект и выберите **Добавить** > **Приложение Django**. Эта команда запрашивает имя и создает приложение Django 1.9.
...
При использовании имени HelloDjangoApp `app.py` имеет следующее содержимое:
...
1. В папке `BasicProject` (созданной на шаге 1) измените значение `urls.py`, чтобы оно по крайней мере соответствовало следующему коду (при необходимости можно оставить полезные комментарии):
...
1. Нажмите кнопку изменения в нижней части Visual Studio (выделено кругом ниже), чтобы перейти к **Team Explorer**.
...
> [!Tip]
> Чтобы быстро остановить и перезапустить проект, выполните команду меню **Отладка** > **Перезапустить** (Ctrl+Shift+F5) или нажмите кнопку перезапуска на панели инструментов отладки.
>
...
1. В папке `BasicProject`, содержащей проекты Django, откройте файл `settings.py` и добавьте имя приложения HelloDjangoApp в список `INSTALLED_APPS`. Добавление приложения в список указывает проекту Django, что в приложении уже есть папка с идентичным именем.
...
1. Кроме того, в `settings.py` убедитесь, что объект `TEMPLATES` содержит следующую строку (включена по умолчанию), которая указывает Django, что нужно найти шаблоны в папке `templates` установленного приложения.

    ```json
    'APP_DIRS': True,
    ```

1. В папке `HelloDjangoApp` откройте файл шаблона страницы `templates/index.html`, чтобы увидеть, что в нем находится только одна переменная, `{{ content }}`.
...
1. Запустите проект и просмотрите выходные данные. Должно отобразиться идентичное сообщение, что и на шаге 2–2. Оно означает, что шаблон работает.
    ...
    Обратите внимание, что HTML, используемый в свойстве `content`, отображается только в качестве обычного текста, так как функция `render` автоматически экранирует этот HTML. Хотя вы можете обойти экранирование, идеально было бы не использовать встроенный HTML с самого начала. Форматирование и оформление лучше всего хранить в шаблоне страницы, а не в коде. Это простой способ создания дополнительных переменных там, где это необходимо.

    Например, измените `templates/index.html` в соответствии со следующими исправлениями, в которых добавлен заголовок страницы и сохранено все форматирование в шаблоне страницы.
...
1. <a name="template-namespacing"></a>В качестве завершающего этапа переместите шаблоны в подпапку, имя которой совпадает с именем приложения, которое создает пространство имен и позволяет избежать возможных конфликтов с другими приложениями, добавленными в проект. Другими словами, создайте подпапку в `templates` с именем `HelloDjangoApp`, переместите `index.html` в эту подпапку и измените функцию представления `index` для ссылки на новый путь шаблона, `HelloDjangoApp/index.html`. Затем запустите проект, убедитесь, что страница отображается должным образом и остановите сервер.
...
Ответ. Использовать расширение `.html` для файлов шаблона страницы совсем необязательно, так как вы всегда определяете точный относительный путь к файлу во втором аргументе для функции `render`. Однако Visual Studio (и другие редакторы) обычно предоставляет функции, например завершение кода или разметка синтаксиса в файлах `.html`, что значительно важнее того, что шаблоны страницы не имеют строгого HTML.
