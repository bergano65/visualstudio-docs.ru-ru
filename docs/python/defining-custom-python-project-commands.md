...
| ErrorRegex<br>WarningRegEx | Optional | Используется, только если ExecuteIn имеет значение `output`. Оба значения указывают регулярное выражение, с помощью которого Visual Studio анализирует выходные данные команды, чтобы отобразить ошибки и предупреждения в окне **Список ошибок**. Если значение не указано, команда не затрагивает окно **Список ошибок**. Дополнительные сведения о том, что ожидает Visual Studio, см. в разделе [Именованные группы записи](#named-capture-groups-for-regular-expressions). |
