...
1. В папке `HelloFlask` создайте файл с именем `views.py` со следующим содержимым. Имя `views.py` важно, так как вы использовали `import HelloFlask.views` в `__init__.py`; если имена не будут совпадать, во время выполнения возникнет ошибка.
    ...
    Помимо переименования функции и маршрутизации к `home`, этот код содержит код отрисовки страницы из app.py и импортирует объект `app`, объявленный в `__init__.py`.

1. Создайте вложенную папку в `HelloFlask` с именем `templates`, которая пока остается пустой.

1. В корневой папке проекта переименуйте `app.py` в `runserver.py` и сделайте так, чтобы содержимое совпадало со следующим кодом:
    ...
    if __name__ == '__main__':
        ...
        app.run(HOST, PORT)
    ```
1. Структура проекта должна выглядеть следующим образом:

    ![Структура проекта после рефакторинга кода](media/flask/step02-project-structure.png)

1. Выберите **Отладка** > **Начать отладку** (F5) или нажмите кнопку **Веб-сервер** на панели инструментов (браузер может отличаться), чтобы запустить приложение и открыть браузер. Попробуйте оба маршрута URL-адресов: / и/home.

1. Также можно установить точки останова в разных частях кода и перезапустить приложение для выполнения последовательности запуска. Например, задайте точку останова в первых строках `runserver.py` и `HelloFlask\__init__.py`, а затем в строке `return "Hello Flask!"` в `views.py`. Затем перезапустите приложение (с помощью пункта **Отладка** > **Перезапуск**, Ctrl + F5 или кнопки на панели инструментов, показанной ниже) и запустите пошаговое выполнение кода (F10) или выполнение из каждой точки останова, нажав клавишу F5.

    ![Кнопка перезапуска на панели инструментов отладки в Visual Studio](media/debugging-restart-toolbar-button.png)

1. По завершении остановите приложение.
...
1. Для начинающих разработчиков рекомендуется изменить `views.py` так, чтобы он содержал следующий код, который использует встроенный HTML-код для страницы с некоторым динамическим содержимым:
...
1. Измените функцию `home` так, чтобы она использовала `render_template` для загрузки шаблона, и укажите значение для свойства content, для чего требуется указать именованный аргумент, соответствующий имени заполнителя. Flask автоматически выполняет поиск шаблонов в папке `templates`, поэтому OATH-код шаблона указывается относительно этой папки:
...
Ответ. Использовать расширение `.html` для файлов шаблона страницы совсем необязательно, так как вы всегда определяете точный относительный путь к файлу в первом аргументе для функции `render_template`. Однако Visual Studio (и другие редакторы) обычно предоставляет функции, например завершение кода или разметка синтаксиса в файлах `.html`, что значительно важнее того, что шаблоны страницы не имеют строгого HTML.
