---
title: Руководство. Сведения о Flask в Visual Studio — шаг 4
...
---
# <a name="tutorial-step-4-use-the-full-flask-web-project-template"></a>Руководство (шаг 4). Использование полного шаблона веб-проекта Flask
...
1. Когда Visual Studio завершит настройку виртуального окружения, назначьте проект FlaskWeb в качестве проекта по умолчанию для решения Visual Studio. Для этого в **обозревателе решений** щелкните правой кнопкой мыши имя проекта и выберите **Назначить запускаемым проектом**. Запускаемый проект, выделенный полужирным шрифтом, выполняется при запуске отладчика.
...
- Файлы в корневом каталоге проекта:
  - `runserver.py`, сценарий для запуска приложения на сервере разработки;
  - `requirements.txt` — файл, который содержит зависимости от Flask 0.x.
- Папка `FlaskWeb` содержит все файлы приложения:
  - `__init.py__` помечает код приложения как модуль Python, создает объект Flask и импортирует представления приложения.
  - `views.py` содержит код для отображения страниц.
  - Папка `static` содержит подпапки с именами `content` (CSS-файлы), `fonts` (файлы шрифтов) и `scripts` (файлы JavaScript).
...
Чтобы можно было включить Jade, шаблон проекта должен включать пакет pyjade в `requirements.txt`. 
...
- Исходный код, используемый в руководстве, на сайте GitHub: [Microsoft/python-sample-vs-learn-flask](https://github.com/Microsoft/python-sample-vs-learn-flask)