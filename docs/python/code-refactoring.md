...
1. В окне **Переименовать** введите новое имя для выбранного идентификатора и нажмите кнопку **ОК**.
  ...
  ![Переименовать. Запрос на ввод нового имени идентификатора](~/python/media/code-refactor-rename-1.png)
...
1. В следующем диалоговом окне выберите файлы и экземпляры, к которым следует применить переименование. Здесь можно выбрать любой отдельный экземпляр для предварительного просмотра конкретных изменений:
  ...
  ![Переименовать. Диалоговое окно для выбора мест применения изменений](~/python/media/code-refactor-rename-2.png)
...
1. В открывшемся диалоговом окне введите имя для нового метода, укажите место для его создания и выберите переменные для закрытия. Переменные, которые не будут выбраны для закрытия, преобразуются в аргументы метода:
  ...
  ![Диалоговое окно извлечения метода](~/python/media/code-refactor-extract-method-1.png)

1. Нажмите кнопку **ОК**, и код будет изменен соответствующим образом:

  ![Результат извлечения метода](~/python/media/code-refactor-extract-method-2.png)
...
![Смарт-тег для добавления импорта](~/python/media/code-refactor-add-import-1.png)
...
![Результат добавления импорта](~/python/media/code-refactor-add-import-2.png)
...
![Меню удаления инструкций import](~/python/media/code-refactor-remove-imports-1.png)

Visual Studio внесет соответствующие изменения в код:

![Результат удаления инструкций import](~/python/media/code-refactor-remove-imports-2.png)
