...
1. Когда в Visual Studio будет создано это окружение, поищите в **обозревателе решений** файл `app.py`, сопровождающий `requirements.txt`. Откройте `app.py`, чтобы убедиться, что шаблон сформировал код, аналогичный приведенному в [кратком руководстве по созданию веб-приложения с помощью Flask](../ide/quickstart-python.md). К коду должно быть добавлено несколько разделов. Весь код, приведенный ниже, создан с помощью шаблона, поэтому вам не нужно ничего вставлять в `app.py`.
