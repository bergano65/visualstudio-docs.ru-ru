...
1. В диалоговом окне **Новый проект** выберите **Установленные** > **Visual Basic** >  категорию **Рабочий стол Windows**, а затем выберите шаблон **Приложения WPF (.NET Framework)**. Назовите проект **HelloWPFApp**.
...
1. Измените значение свойства **Имя файла** на `Greetings.xaml`.
     ...
     В **обозревателе решений** видно, что файл теперь имеет имя *Greetings.xaml*, а имя вложенного файла кода поменялось на *Greetings.xaml.vb*. Этот файл с текстом программы вложен в узел файла *.xaml*, что означает их тесную связь.
