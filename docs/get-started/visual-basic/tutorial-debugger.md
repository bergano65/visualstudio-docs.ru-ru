...
1. Запустите Visual Studio.
    ...
    ::: moniker range=">=vs-2019"
    Нажмите клавишу **ESC**, чтобы закрыть окно запуска. Нажмите клавиши **CTRL+Q**, чтобы открыть поле поиска, введите **visual basic**, щелкните **Шаблоны**, а затем выберите **Создать новый проект консольного приложения (.NET Core)** или **Создать новый проект консольного приложения (.NET Framework)** . В появившемся диалоговом окне введите такое имя, как **get-started-debugging**, а затем выберите **Создать**.
    ::: moniker-end
    ::: moniker range="vs-2017"
...
1. В файле *Module1.vb* замените весь код по умолчанию.
...
1. Во время приостановки в вызове метода `shape.Draw` в функции `Main` нажмите клавишу **F11** (или выберите **Отладка > Шаг с заходом**), чтобы перейти в код для класса `Rectangle`.
     ...
     ![Использование клавиши F11 для перехода в код](../visual-basic/media/get-started-f11-vb.png "F11 Шаг с заходом")
...
1. В редакторе кода прокрутите вниз и наведите указатель мыши на метод `Console.WriteLine` в классе `Triangle`, чтобы в левой части появилась зеленая кнопка **Выполнение до щелкнутого** ![Выполнение до щелкнутого](../../debugger/media/dbg-tour-run-to-click.png "RunToClick"). В подсказке для кнопки выводится "Выполнить до этого места".
...
2. Нажмите кнопку **Выполнение до щелкнутого** ![Выполнение до щелкнутого](../../debugger/media/dbg-tour-run-to-click.png "RunToClick").
    ...
    Отладчик перемещается к реализации метода `Console.WriteLine` для класса `Triangle`. (Если отладчик сразу приостанавливается в заданной ранее точке останова, нажмите кнопку **Выполнение до щелкнутого** снова, чтобы отладчик перешел к `Console.WriteLine`.)
...
1. Нажмите сочетание клавиш **SHIFT** + **F11** (или **Отладка > Шаг с выходом**).
     ...
     Вы должны вернуться в цикл `For Each` в методе `Main`. В противном случае нажмите клавиши **SHIFT** + **F11** во второй раз.
