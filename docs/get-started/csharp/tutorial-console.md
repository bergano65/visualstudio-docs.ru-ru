...
Во-первых, мы рассмотрим некоторые базовые расчеты для целых чисел в C#. Затем мы добавим код для создания простого калькулятора. Далее мы будем изменять этот код, постепенно добавляя функциональные возможности. После этого нам предстоит отладить приложение, чтобы найти и исправить ошибки. И, наконец, мы оптимизируем код для повышения эффективности.

Давайте начнем с расчетов целых чисел в C#.
...
1. Выберите **Calculator**, чтобы запустить программу, или нажмите клавишу **F5**.
   ...
   Откроется окно консоли с суммой 42 + 119.

1. Теперь попробуйте изменить строку кода `int c = a + b;` с помощью различных операторов, таких как `-` для вычитания, `*` для умножения или */* для деления.

    Обратите внимание на то, что при изменении оператора и запуске программы результат меняется.
