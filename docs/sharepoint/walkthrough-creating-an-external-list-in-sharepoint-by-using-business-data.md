...
2. В **визуальном C#**  элементе или **Visual Basic**разверните узел **SharePoint** , а затем выберите элемент **2010** .
...
8. В строке меню выберите **View** > **Обозреватель сервера**.
...
2. На вкладке **BusinessDataConnectivity** в **области элементов**добавьте **сущность** в конструктор BDC.
     ...
     Новая сущность появится в конструкторе. Visual Studio добавляет файл с именем *ентитисервице. vb* (в Visual Basic) или *EntityService.CS* (in C#) в проект.
...
10. Повторите шаг 6, чтобы создать дескриптор типа для каждого из следующих полей.
    ...
    |емаилпромотион|System.Int32|
    |наместиле|System.Boolean|
    |PasswordHash|System.String|
    |пассвордсалт|System.String|

11. В конструкторе BDC в сущности **Contact** откройте метод **ReadItem** .

     Файл с кодом службы Contact откроется в редакторе кода.

12. В классе `ContactService` замените метод `ReadItem` следующим кодом. Этот код выполняет следующие задачи:
...
5. В классе `ContactService` замените метод `ReadList` следующим кодом. Этот код выполняет следующие задачи:
...
[Создание модели подключения к бизнес-](../sharepoint/creating-a-business-data-connectivity-model.md) данным
