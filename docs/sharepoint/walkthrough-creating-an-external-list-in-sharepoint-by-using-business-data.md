...
В этом пошаговом руководстве показано, как создать модель для службы BDC, которая возвращает сведения о контактах в образце базы данных. Затем создается внешний список в SharePoint с помощью этой модели.
...
- Добавление определенного метода Finder.
...
## <a name="create-a-project-that-contains-a-bdc-model"></a>Создание проекта, содержащую модель BDC

1. В строке меню в Visual Studio, выберите **файл** > **New** > **проекта**.

     Откроется диалоговое окно **Новый проект** .

2. В группе **Visual C#** или **Visual Basic**, разверните **SharePoint** узел, а затем выберите **2010** элемента.

3. В **шаблоны** области выберите **проект SharePoint 2010**, присвойте проекту имя **AdventureWorksTest**, а затем выберите **ОК** кнопку .
...
4. Выберите **развернуть как решение фермы** переключатель, чтобы задать уровень доверия.
...
2. Добавьте подключение к базе данных SQL Server AdventureWorks.
     ...
     Дополнительные сведения см. в разделе [Добавить/изменить подключение (Microsoft SQL Server)](https://msdn.microsoft.com/fa400910-26c3-4df7-b9d1-115e688b4ea3).

3. В области **Обозреватель решений**выберите узел проекта.

4. В строке меню выберите **Проект** > **Добавить новый элемент**.

5. В **установленные шаблоны** панели выберите **данных** узла.

6. В **шаблоны** панели выберите **LINQ to SQL Classes**.
...
9. В **обозревателя серверов**, разверните узел, представляющий образца базы данных AdventureWorks, а затем разверните **таблиц** узла.

10. Добавить **Contact (Person)** таблицы в конструктор O/R.

     Создается класс сущностей и появляется в области конструктора. Класс сущностей имеет свойства, которые сопоставляются со столбцами в таблице Contact (Person).

## <a name="remove-the-default-entity-from-the-bdc-model"></a>Удаление сущности по умолчанию из модели BDC

**Business Data Connectivity Model** проект добавляет сущность по умолчанию с именем Entity1 модели. Удалите эту сущность. Позже вы добавите новую сущность. Начиная с пустой модели сокращает количество шагов, необходимых для выполнения данного пошагового руководства.

1. В **обозревателе решений**, разверните **BdcModel1** узел, а затем откройте *BdcModel1.bdcm* файла.
...
2. На **BusinessDataConnectivity** вкладке **элементов**, добавьте **сущности** в конструктор BDC.
     ...
     В конструкторе отобразится новая сущность. Visual Studio добавляет файл, который называется *EntityService.vb* (в Visual Basic) или *EntityService.cs* (в C#) в проект.

3. В строке меню выберите **представление** > **свойства** > **окно**.

4. В **свойства** окне **имя** значение свойства **контакт**.
...
## <a name="add-a-specific-finder-method"></a>Добавление определенного метода Finder

Чтобы включить службу каталога бизнес-данных для отображения определенного контакта, необходимо добавить конкретный метод поиска. Служба BDC вызывает конкретный метод поиска, когда пользователь выбирает элемент в списке и затем выбирает **элемент** кнопку на ленте.

Добавление определенного метода Finder сущность "Контакт" с помощью **Подробности метода BDC** окна. Чтобы возвратить определенную сущность, добавьте код в метод.
...
4. В **Подробности метода BDC** окно, откройте список, отображаемый для **контакт** дескриптор типа, а затем выберите **изменить**.

     **Обозреватель BDC** открывает и обеспечивает иерархическое представление модели.

5. В **свойства** окно, откройте список рядом с полем **TypeName** свойства, выберите **текущий проект** вкладке, а затем выберите **контакт**свойство.
...
7. В **свойства** окне **имя** значение свойства **ContactID**.

8. Откройте список рядом с полем **TypeName** свойства, а затем выберите **Int32**.

9. Откройте список рядом с полем **идентификатор** свойства, а затем выберите **ContactID**.
...
12. В `ContactService` класса, замените `ReadItem` метод следующим кодом. Этот код выполняет следующие задачи:
    ...
    - Извлекает записи из таблицы Contact базы данных AdventureWorks.

    - Возвращает сущность "Контакт" для службы BDC.
...
## <a name="add-a-finder-method"></a>Добавление метода finder

Чтобы включить отображение списка контактов в службе BDC, необходимо добавить метод поиска. Добавление метода Finder в сущность "Контакт", с помощью **Подробности метода BDC** окна. Для возврата коллекции сущностей в службу BDC, добавьте код в метод.

1. В конструкторе BDC выберите **контакт** сущности.

2. В **Подробности метода BDC** окне Свернуть **ReadItem** узла.

3. В **добавьте метод** списке **ReadList** метод, выберите **создать метод поиска**.
...
При запуске проекта открывается сайт SharePoint, и Visual Studio добавляет модель к службе Business Data Connectivity. Создание внешнего списка в SharePoint, который ссылается на сущность "Контакт". В списке отображаются данные контактов в базе данных AdventureWorks.
...
3. На **создать** выберите **внешний список** шаблона, а затем выберите **создать** кнопки.

4. Имя пользовательского списка **контакты**.

5. Нажмите кнопку обзора рядом с полем **внешнем типе контента** поля.

6. В **внешнего содержимого диалогового окна Выбор типа** диалоговое окно, выберите **AdventureWorksContacts.BdcModel1.Contact** элемента, а затем выберите **создать** кнопки.
...
8. На ленте выберите **элементы** вкладке, а затем выберите **элемент** команды.
