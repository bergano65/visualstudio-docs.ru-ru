...
- Создание расширения [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], расширяющего узел **подключений SharePoint** **Обозреватель сервера** , следующим образом.
...
3. В диалоговом окне **Новый проект** разверните **визуальный C#**  или **Visual Basic** узел, а затем выберите пункт **расширяемость**.
...
2. В диалоговом окне **Новый проект** разверните  **C# визуальный** или **Visual Basic** узел, а затем выберите **Windows**.
...
7. В окне **пространства имен по умолчанию** (C#) или в поле **корневого пространства имен** (Visual Basic) введите **ServerExplorer. шарепоинтконнектионс. вебпартноде**.
...
9. В строке меню выберите **сборка** > **сборка решения**, а затем убедитесь, что решение компилируется без ошибок.
...
1. В экспериментальном экземпляре Visual Studio в строке меню выберите **View** > **Обозреватель сервера**.
...
4. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной ранее в методе `NodeChildrenRequested`, а затем нажмите клавишу **F5** , чтобы продолжить отладку проекта.
