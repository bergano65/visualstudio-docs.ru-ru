...
2. В диалоговом окне **Добавление нового проекта** разверните  **C# визуальный** элемент или **Visual Basic** узлы, а затем выберите узел **Windows** .
...
## <a name="define-the-project-item-extension"></a>Определение расширения элемента проекта
 Создайте класс, определяющий расширение для элемента проекта **модели подключения к бизнес-данным** . Чтобы определить расширение, класс реализует интерфейс <xref:Microsoft.VisualStudio.SharePoint.ISharePointProjectItemTypeExtension>. Реализуйте этот интерфейс всякий раз, когда требуется расширить существующий тип элемента проекта.
...
2. В проекте Бдкпрожектитемекстенсион откройте файл кода Прожектитемекстенсион, а затем добавьте точку останова в строку кода в методе `Initialize`.

3. Откройте файл кода Женератикстерналдаталистс, а затем добавьте точку останова в первую строку кода в методе `GenerateExternalDataLists_Execute`.
...
2. В диалоговом окне **Новый проект** разверните узел **шаблоны** , разверните узел  **C# визуальный** элемент, разверните узел **SharePoint** и выберите **2010**.

3. В списке в верхней части диалогового окна установите флажок **.NET Framework 3,5** . Проекты для [!INCLUDE[moss_14_long](../sharepoint/includes/moss-14-long-md.md)] нуждаются в этой версии .NET Framework.
...
11. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной в методе `Initialize` файла кода Прожектитемекстенсион.
...
17. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной в методе `GenerateExternalDataLists_Execute`. Нажмите клавишу **F5** или в строке меню выберите **Отладка** > **продолжить** , чтобы продолжить отладку проекта.
