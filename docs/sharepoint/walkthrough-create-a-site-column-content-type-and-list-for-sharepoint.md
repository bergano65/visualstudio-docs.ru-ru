...
1. В меню [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] **файл** выберите пункт **создать** > **проект**.

2. В диалоговом окне **Новый проект** в поле **визуальный C#**  или **Visual Basic**разверните узел **SharePoint** и выберите **2010**.

3. В области **шаблоны** выберите **проект SharePoint 2010**, измените имя проекта на " **курс**", а затем нажмите кнопку " **ОК** ".
...
4. На странице **Укажите сайт и уровень безопасности для отладки** введите URL-адрес локального сайта SharePoint, в который нужно добавить новый элемент настраиваемого поля, или используйте расположение по умолчанию (`http://<`*SystemName*`>/)`.
...
1. Добавьте новый столбец сайта. Для этого в **Обозреватель решений**откройте контекстное **меню для курса**и выберите команду **добавить** > **новый элемент**.

2. В диалоговом окне **Добавление нового элемента** выберите **столбец сайта**, измените имя на **Patient Name**, а затем нажмите кнопку **Добавить** .

3. В файле *Elements. XML* столбца site оставьте параметр **Type** **Text**и измените параметр **Group** на "курсовый переход к **столбцам сайта**". По завершении файл *Elements. XML* столбца site должен выглядеть, как в следующем примере.
...
4. С помощью той же процедуры добавьте в проект два дополнительных столбца сайта: **PATIENT ID** (Type = "integer") и **Doctor Name** (Type = "Text"). Задайте для их значения группы значение " **курс по столбцам сайта**".
...
3. В **визуальном C#**  элементе или **Visual Basic**разверните узел **SharePoint** , а затем выберите узел **2010** .
...
11. В строке меню выберите **файл** > **сохранить все**, а затем закройте конструктор типов содержимого.
...
5. Оставьте **настройку списка** **по умолчанию (пусто)** , а затем нажмите кнопку **Готово** .
...
11. На вкладке **представления** выберите пустую строку для создания представления. Введите **сведения о пациентах**.
...
- [Столбце](/previous-versions/office/developer/sharepoint-2010/ms196085(v=office.14))
