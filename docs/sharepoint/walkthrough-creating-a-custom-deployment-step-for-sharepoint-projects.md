...
- Проект библиотеки классов, который определяет пользовательские команды SharePoint. Этот проект должен быть предназначен для .NET Framework 3,5.
...
2. В диалоговом окне **Новый проект** разверните  **C# визуальный** элемент или **Visual Basic** узлы, а затем выберите узел **Windows** .
...
2. В диалоговом окне **Новый проект** разверните узел **визуальный C#**  элемент или **Visual Basic**, а затем выберите узел **Windows** .
...
## <a name="define-the-custom-deployment-step"></a>Определение настраиваемого шага развертывания
 Создайте класс, определяющий шаг развертывания обновления. Чтобы определить шаг развертывания, класс реализует интерфейс <xref:Microsoft.VisualStudio.SharePoint.Deployment.IDeploymentStep>. Реализуйте этот интерфейс всякий раз, когда необходимо определить пользовательский шаг развертывания.
...
## <a name="create-a-deployment-configuration-that-includes-the-custom-deployment-step"></a>Создание конфигурации развертывания, включающей этап настраиваемого развертывания
 ...
 Чтобы создать конфигурацию развертывания, класс реализует интерфейс <xref:Microsoft.VisualStudio.SharePoint.ISharePointProjectExtension>. Реализуйте этот интерфейс всякий раз, когда нужно создать расширение проекта SharePoint.
...
2. В проекте Деплойментстепекстенсион откройте файл кода Упградестеп, а затем добавьте точку останова в первую строку кода в методах `CanExecute` и `Execute`.
...
2. В диалоговом окне **Новый проект** разверните узел **визуального C#**  элемента или узел **Visual Basic** , разверните узел **SharePoint** , а затем выберите узел **2010** .

3. В верхней части диалогового окна убедитесь, что **.NET Framework 3,5** отображается в списке версий .NET Framework.
...
15. В файле *Elements. XML* замените XML-код по умолчанию в этом файле на следующий XML-код. Этот XML-код изменяет имя списка на **Employees** и добавляет сведения для сотрудника с именем Джим ханце.
...
2. Удалите элемент `Data` и его дочерние элементы, чтобы удалить из списка запись для Jim Ханце.
...
6. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной ранее в методе `CanExecute`.

7. Снова нажмите клавишу **F5** или в строке меню выберите **Отладка** > **продолжить**.

8. Убедитесь, что код останавливается в точке останова, заданной ранее в методе `Execute`.
