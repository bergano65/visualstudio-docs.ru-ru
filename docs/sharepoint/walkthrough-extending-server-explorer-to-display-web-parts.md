...
- Использование серверной объектной модели для SharePoint. Дополнительные сведения см. [в разделе Использование серверной модели объектов SharePoint Foundation](/previous-versions/office/developer/sharepoint-2010/ee538251(v=office.14)).
...
2. В диалоговом окне **Новый проект** разверните узел **визуального C#**  элемента или узел **Visual Basic** , а затем выберите узел **Windows** .
...
7. В окне **пространства имен по умолчанию** (C#) или в поле **корневого пространства имен** ([!INCLUDE[vbprvb](../sharepoint/includes/vbprvb-md.md)]) введите **ServerExplorer. шарепоинтконнектионс. вебпартноде**.
...
4. Щелкните стрелку рядом с кнопкой **Добавить** , а затем в появившемся меню выберите **Добавить как ссылку** .
     ...
     [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] добавляет файлы кода в проект Вебпарткоммандс в виде ссылок. В результате файлы кода находятся в проекте Вебпартнодикстенсион, но код в файлах также компилируется в проект Вебпарткоммандс.
...
9. В окне **пространства имен по умолчанию** (C#) или в поле **корневого пространства имен** ([!INCLUDE[vbprvb](../sharepoint/includes/vbprvb-md.md)]) введите **ServerExplorer. шарепоинтконнектионс. вебпартноде**.
...
## <a name="define-a-node-type-that-represents-a-web-part"></a>Определение типа узла, представляющего веб-часть
 ...
 Чтобы определить новый тип узла, класс реализует интерфейс <xref:Microsoft.VisualStudio.SharePoint.Explorer.IExplorerNodeTypeProvider>. Реализуйте этот интерфейс всякий раз, когда нужно определить новый тип узла в **Обозреватель сервера**.
...
## <a name="create-the-custom-sharepoint-commands"></a>Создание настраиваемых команд SharePoint
 Создайте пользовательские команды, вызывающие объектную модель сервера для SharePoint, чтобы получить данные о веб-части на сайте SharePoint. Каждая команда — это метод, к которому применен <xref:Microsoft.VisualStudio.SharePoint.Commands.SharePointCommandAttribute>.
...
13. В строке меню выберите **сборка** > **сборка решения**, а затем убедитесь, что решение компилируется без ошибок.
...
2. В проекте Вебпартнодикстенсион откройте файл кода Ситенодикстенсион, а затем добавьте точку останова в первую строку кода в методах `NodeChildrenRequested` и `CreateWebPartNodes`.
...
1. В экспериментальном экземпляре [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)]в строке меню выберите **View** > **Обозреватель сервера**.
...
4. Убедитесь, что код в другом экземпляре [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] остановлен на точке останова, заданной ранее в методе `NodeChildrenRequested`, а затем нажмите **клавишу F5** , чтобы продолжить отладку проекта.
...
6. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной ранее в методе `CreateWebPartNodes`, а затем нажмите клавишу **F5** , чтобы продолжить отладку проекта.
...
9. В экземпляре [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)], на котором выполняется отладка, убедитесь, что сведения о веб-части отображаются в окне **Свойства** .
