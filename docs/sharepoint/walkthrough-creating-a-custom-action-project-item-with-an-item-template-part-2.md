...
3. В диалоговом окне **Новый проект** разверните  **C# визуальный** элемент или **Visual Basic** узлы, а затем выберите узел **Windows** .
...
## <a name="define-the-default-location-and-id-strings-for-custom-actions"></a>Определение строк расположения и идентификатора по умолчанию для настраиваемых действий
 У каждого пользовательского действия есть расположение и идентификатор, указанные в атрибутах `GroupID` и `Location` элемента `CustomAction` в файле *Elements. XML* . На этом шаге вы определите некоторые из допустимых строк для этих атрибутов в проекте Итемтемплатевизард. По завершении этого пошагового руководства эти строки записываются в файл *Elements. XML* в элементе проекта настраиваемого действия, когда пользователи указывают расположение и идентификатор в мастере.
...
## <a name="implement-the-wizard"></a>Реализация мастера
 Определите функциональные возможности мастера, реализовав интерфейс <xref:Microsoft.VisualStudio.TemplateWizard.IWizard>.
...
1. В окне командной строки Visual Studio выполните следующую команду, заменив *пастовизардассембли* на полный путь к созданной сборке итемтемплатевизард. dll для проекта итемтемплатевизард на компьютере разработчика.

    ```xml
    sn.exe -T PathToWizardAssembly
    ```

     Токен открытого ключа для сборки *итемтемплатевизард. dll* записывается в окно командной строки Visual Studio.
...
2. Ближе к концу файла добавьте следующий элемент `WizardExtension` между тегами `</TemplateContent>` и `</VSTemplate>`. Замените значение *йоуртокен* атрибута `PublicKeyToken` на маркер открытого ключа, полученный в предыдущей процедуре.
...
1. В проекте ItemTemplate замените содержимое файла *Elements. XML* следующим XML-кодом.
    ...
    ```
     ...
     Новый XML-файл изменяет значения `Id`, `GroupId`, `Location`, `Description`и `Url` атрибутов на заменяемые параметры.
...
6. В строке меню выберите **сборка** > **сборка решения**, а затем убедитесь, что решение компилируется без ошибок.
...
2. В проекте Итемтемплатевизард откройте файл кода Кустомактионвизард, а затем добавьте точку останова в первую строку кода в методе `RunStarted`.
...
2. Разверните **визуальный C#**  или **Visual Basic** узел (в зависимости от языка, поддерживаемого шаблоном элемента), разверните узел **SharePoint** , а затем выберите узел **2010** .
...
8. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной ранее в методе `RunStarted`.

9. Продолжайте отладку проекта, нажав клавишу **F5** , или в строке меню выберите **Отладка** > **продолжить**.
