...
## <a name="project-templates"></a>Шаблоны проектов
 Ниже приведен список шаблонов проектов SharePoint. Чтобы просмотреть шаблоны проектов SharePoint в Visual Studio, в диалоговом окне **Новый проект** разверните узел **SharePoint** в любом  **C# визуальном** элементе или **Visual Basic**, а затем выберите **2010**.
 ...
 *Пустые элементы* чаще всего используются для определения элементов проектов SharePoint, в которых отсутствует проект или шаблон элемента проекта в Visual Studio. При добавлении пустого элемента в проект узел с именем Емптелемент [x] (где [x] — это уникальное число,\) создается. Емптелемент [x] содержит один файл с именем *Elements. XML.* Используйте инструкции [!INCLUDE[TLA2#tla_xml](../sharepoint/includes/tla2sharptla-xml-md.md)], чтобы определить требуемые элементы в *Elements. XML*.
...
- [Продукты и технологии SharePoint](/previous-versions/office/developer/sharepoint-2010/dd776256(v=office.12))
