...
## <a name="what-happens-when-you-import-a-solution"></a>Что происходит при импорте решения
 При импорте решения с помощью шаблона "Импорт пакета решения SharePoint" [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] копирует все содержимое *WSP* -файла и пытается выверять и хранить как можно больше ассоциаций и ссылок между импортируемыми элементами и их файлами.
...
### <a name="handle-special-cases"></a>Обрабатывайте особые случаи
 ...
 Например, если импортировать определение списка ExpenseForms, определение списка с таким именем появится в папке **определения списка** в **Обозреватель решений** вместе с его *Elements.xml* и *Schema.xml* файлов. Однако его связанные формы ASPX и HTML могут быть помещены в папку с именем **ExpenseForms** в папке **Прочие импортированные файлы** . Чтобы завершить импорт, переместите эти файлы в определение списка ExpenseForms в **обозревателе решений** и измените значение свойства **DeploymentType** каждого файла с **NoDeployment** на **ElementFile**.

 При импорте приемников событий файл *Elements.xml* копируется в нужное расположение, но необходимо вручную включить сборку в пакет решения, чтобы она была развернута вместе с решением. [!INCLUDE[crabout](../sharepoint/includes/crabout-md.md)]инструкции см. в разделе [как добавлять и удалять дополнительные сборки](../sharepoint/how-to-add-and-remove-additional-assemblies.md).
...
## <a name="import-fields-and-property-bags"></a>Импорт полей и контейнеров свойств
 При импорте решения с несколькими полями все отдельные определения полей объединяются в один файл *Elements.xml* в узле в **Обозреватель решений** , именуемых **полями**. Аналогичным образом все записи контейнера свойств объединяются в файл *Elements.xml* в узле с именем **пропертибагс**.
