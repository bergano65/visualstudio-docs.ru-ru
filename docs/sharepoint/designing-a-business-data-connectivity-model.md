...
|Создание|Добавляет новые данные в источник данных сущности. Вызывается, когда пользователь наберет кнопку " **создать элемент** " на ленте списка, основанного на модели. Дополнительные сведения см. [в разделе инструкции. Добавление метода Creator](../sharepoint/how-to-add-a-creator-method.md).|
...
## <a name="define-method-parameters"></a>Определение параметров метода
 ...
 Visual Studio позволяет копировать дескрипторы типов между параметрами в модели. Например, можно определить дескриптор типа с именем `CustomerTD` для возвращаемого параметра метода `GetCustomer`. Можно скопировать дескриптор типа `CustomerTD` в **обозревателе BDC**, а затем вставить этот дескриптор типа в входной параметр метода `CreateCustomer`. Это не позволит вам определять один и тот же дескриптор типа более одного раза.
...
|Задача|Параметр|
|----------|------------|
|Для развертывания моделей в службе BDC.|Правка|
|Для создания списков и веб-части с помощью внешних типов содержимого (сущностей) в модели.|Выбираемые в клиентах|
|Для создания, чтения, обновления и удаления данных сущности.|Выполнение|
