...
- Создание шаблона проекта [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] для элемента проекта.

- Создание пакета расширения [!INCLUDE[vsprvs](../sharepoint/includes/vsprvs-md.md)] (VSIX) для развертывания шаблона проекта и сборки расширения.
...
4. Разверните **Visual Basic** или **визуальные C#**  узлы, а затем выберите узел **расширяемость** .
...
3. Разверните **визуальный C#**  элемент или узел **Visual Basic** , а затем выберите узел **расширяемость** .

4. В списке шаблонов проектов выберите шаблон  **C# проекта** или шаблон **шаблона проекта Visual Basic** .
...
3. Разверните **визуальный C#**  элемент или **Visual Basic** узлы, выберите узел **Windows** , а затем выберите шаблон **Библиотека классов** .
...
## <a name="define-the-new-sharepoint-project-item-type"></a>Определение нового типа элемента проекта SharePoint
 Создайте класс, реализующий интерфейс <xref:Microsoft.VisualStudio.SharePoint.ISharePointProjectItemTypeProvider>, чтобы определить поведение нового типа элемента проекта. Реализуйте этот интерфейс всякий раз, когда нужно определить новый тип элемента проекта.
...
3. В файле проекта выберите следующий элемент `VSTemplate`.
...
4. Замените этот элемент следующим XML-кодом.
    ...
    ```
     ...
     Элемент `OutputSubPath` указывает дополнительные папки в пути, по которому шаблон проекта создается при построении проекта. Указанные здесь папки гарантируют, что шаблон проекта будет доступен только тогда, когда клиенты открывают диалоговое окно **Новый проект** , разверните узел **SharePoint** , а затем выберите узел **2010** .
...
1. В проекте Ситеколумнпрожекттемплате откройте файл *AssemblyInfo.CS* или *AssemblyInfo. vb* , а затем добавьте следующий оператор в его верхнюю часть:
    ...
    ```
     ...
     Если свойство **"изолированное решение"** проекта SharePoint имеет значение **true**, Visual Studio добавляет <xref:System.Security.AllowPartiallyTrustedCallersAttribute> в файл кода AssemblyInfo. Однако файл кода AssemblyInfo в шаблоне проекта не импортирует пространство имен <xref:System.Security> по умолчанию. Чтобы избежать ошибок компиляции, необходимо добавить инструкцию **using** или **Imports** .
...
1. В проекте Ситеколумнпрожекттемплате замените содержимое файла *шарепоинтпрожектитем. данных* следующим XML-кодом.
   ...
   - Изменяет атрибут `Type` элемента `ProjectItem` на ту же строку, которая передается в <xref:Microsoft.VisualStudio.SharePoint.SharePointProjectItemTypeAttribute> определения элемента проекта (класс `SiteColumnProjectItemTypeProvider`, созданный ранее в этом пошаговом руководстве).

   - Удаляет атрибуты `SupportedTrustLevels` и `SupportedDeploymentScopes` из элемента `ProjectItem`. Эти значения атрибутов не нужны, так как уровни доверия и области развертывания указываются в классе `SiteColumnProjectItemTypeProvider` в проекте Прожектитемтипедефинитион.
...
1. В проекте Ситеколумнпрожекттемплате замените содержимое файла *Feature1. Feature* следующим XML-кодом.
   ...
   - Изменяет значения `Id` и `featureId` атрибутов элемента `feature` на `$guid4$`.

   - Изменяет значения атрибута `itemId` элемента `projectItemReference` на `$guid2$`.
...
1. В проекте Ситеколумнпрожекттемплате замените содержимое файла *Package. Package* следующим XML-кодом.
   ...
   - Изменяет значения `Id` и `solutionId` атрибутов элемента `package` на `$guid3$`.

   - Изменяет значения атрибута `itemId` элемента `featureReference` на `$guid4$`.
...
1. В проекте Ситеколумнпрожекттемплате замените содержимое файла Ситеколумнпрожекттемплате. vstemplate одним из следующих разделов XML.
   ...
   - Если вы создаете шаблон визуального C# проекта, используйте следующий код XML.
   ...
   - Устанавливает элемент `Name` в **столбец сайта**значения. (Это имя отображается в диалоговом окне **Новый проект** ).

   - Добавляет `ProjectItem` элементов для каждого филесат, входящего в каждый экземпляр проекта.

   - Использует `http://schemas.microsoft.com/developer/vstemplate/2005`пространства имен. Другие файлы проекта в этом решении используют пространство имен `http://schemas.microsoft.com/developer/msbuild/2003`. Поэтому будут созданы предупреждающие сообщения схемы XML, но их можно игнорировать в этом пошаговом руководстве.
...
1. В проекте Ситеколумнпрожекттемплате замените содержимое файла *ProjectTemplate. csproj* или *ProjectTemplate. vbproj* одним из следующих разделов XML.
    ...
    - Использует элемент `TargetFrameworkVersion` для указания .NET Framework 3,5, а не 4,5.

    - Добавляет `SignAssembly` и `AssemblyOriginatorKeyFile` элементы для подписания выходных данных проекта.
...
10. В списке **тип** выберите **Microsoft. VisualStudio. MefComponent**.
    ...
    > [!NOTE]
    > Это значение соответствует элементу `MefComponent` в файле Extension. vsixmanifest. Этот элемент задает имя сборки расширения в пакете VSIX. Дополнительные сведения см. в разделе [элемент MEFComponent (Схема VSX)](/previous-versions/visualstudio/visual-studio-2010/dd393736\(v\=vs.100\)).
...
2. В файле кода Ситеколумнпрожектитемтипепровидер добавьте точку останова в первую строку кода в методе `InitializeType`, а затем нажмите клавишу **F5** , чтобы начать отладку.
...
2. Разверните **визуальный C#**  или **Visual Basic** узел (в зависимости от языка, поддерживаемого шаблоном проекта), разверните узел **SharePoint** , а затем выберите узел **2010** .
...
5. Убедитесь, что код в другом экземпляре Visual Studio остановлен в точке останова, заданной ранее в методе `InitializeType`, а затем нажмите клавишу **F5** , чтобы продолжить отладку проекта.
