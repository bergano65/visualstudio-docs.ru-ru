...
description: "Описываются действия, необходимые для установки сертификатов для автономной установки Visual Studio"
ms.date: 08/30/2017
ms.reviewer: tims
ms.suite: 
...
1. Скопируйте [средство "Диспетчер сертификатов"](https://msdn.microsoft.com/library/e78byta0.aspx) (certmgr.exe) в общую папку установки (например, \\server\share\vs2017). Программа Certmgr.exe не является компонентом Windows, но доступна как часть пакета [Windows SDK](https://developer.microsoft.com/windows/downloads/windows-10-sdk).
