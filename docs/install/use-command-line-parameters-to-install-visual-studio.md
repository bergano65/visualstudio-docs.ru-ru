...
| `--installChannelUri <uri>` | **Необязательно**. Универсальный код ресурса (URI) манифеста канала, используемого для установки. Универсальный код ресурса (URI), указанный с помощью параметра `--channelUri` (который является обязательным при наличии параметра `--installChannelUri`), используется для обнаружения обновлений. Если обновления не нужны, параметр `--channelUri` следует указать без аргумента. Может использоваться для команды установки; для других команд игнорируется. |
