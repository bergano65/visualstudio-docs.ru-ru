...
| `--remove <one or more workload or component IDs>` | **Необязательно**. Один или несколько идентификаторов рабочих нагрузок или компонентов, которые нужно удалить. Дополнительные сведения см. в статье о [рабочей нагрузке и идентификаторах компонента](workload-and-component-ids.md).|
