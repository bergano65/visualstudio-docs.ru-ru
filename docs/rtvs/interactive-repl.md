...
![Ввод многострочного оператора в интерактивном окне](~/rtvs/media/repl-multiline-entry.png)
...
![Интерактивное окно с панелью инструментов](~/rtvs/media/repl-window.png)
...
| --- | --- | --- | --- |
| ![Кнопка сброса](~/rtvs/media/repl-toolbar-01-reset.png) | Сбросить | Ctrl+Shift+F10 | Сбрасывает сеанс работы с интерактивным окном, очищает все переменные и журнал. |
| ![Кнопка "Сбросить"](~/rtvs/media/repl-toolbar-02-clear.png) | Clear | CTRL+L | Очищает выходные данные, которые отображаются в интерактивном окне; переменные сеанса и журнал остаются без изменений. |
| ![Кнопки работы с журналом](~/rtvs/media/repl-toolbar-03-history.png) | Команда "Назад"<br/>Команда "Далее" | СТРЕЛКА ВВЕРХ, СТРЕЛКА ВНИЗ<br/>ALT + СТРЕЛКА ВВЕРХ, ALT + СТРЕЛКА ВНИЗ | Прокрутка журнала; для блоков с несколькими строками кода имеется определенная функциональность. См. раздел [Журнал](#history). |
| ![Кнопка "Загрузить рабочую область"](~/rtvs/media/repl-toolbar-04-load-workspace.png) | Загрузить рабочую область | Н/Д | Загрузка сохраненной ранее рабочей области (см. раздел [Рабочие области и сеансы](#workspaces-and-sessions). |
| ![Кнопка "Сохранить рабочую область как"](~/rtvs/media/repl-toolbar-05-save-workspace-as.png)| Сохранить рабочую область как | Н/Д | Сохранение текущего состояния сеанса в качестве рабочей области (см. раздел [Рабочие области и сеансы](#workspaces-and-sessions)). |
| ![Кнопка "Использовать скрипт R в качестве источника"](~/rtvs/media/repl-toolbar-06-source-r-script.png) | Использовать скрипт R в качестве источника | Ctrl+Shift+S | Вызов `source` с текущим активным скриптом R в редакторе Visual Studio, который выполняет код.  Эта кнопка доступна только в том случае, если R-файл открыт в редакторе Visual Studio. | 
| ![Кнопка "Использовать скрипт R в качестве источника с эхо-выводом"](~/rtvs/media/repl-toolbar-07-source-r-script-with-echo.png) | Использовать скрипт R в качестве источника с эхо-выводом | Ctrl+Shift+ВВОД | Аналогична кнопке "Использовать скрипт R в качестве источника", только содержимое скрипта отображается в интерактивном окне. | 
| ![Кнопка "Прервать R"](~/rtvs/media/repl-toolbar-08-interrupt-r.png)| Прервать R | ESC | Остановка выполняемого кода в интерактивном окне, например цикла `while` на снимке экрана выше. |
| ![Кнопка "Присоединить отладчик"](~/rtvs/media/repl-toolbar-09b-attach-debugger.png)| Присоединить отладчик | Н/Д | Также можно воспользоваться командой **Отладка > Подключить к интерактивному окну R**. | 
| ![Кнопка "Задать расположение исходных файлов в качестве рабочего каталога"](~/rtvs/media/repl-toolbar-10-set-working-directory-source.png)| Задать расположение исходных файлов в качестве рабочего каталога | Ctrl+Shift+E | Задает расположение недавно загруженного в интерактивное окно исходного файла в качестве рабочего каталога (с помощью `source`). См. раздел [Рабочий каталог](#working-directory). |
| ![Кнопка "Задать расположение проекта в качестве рабочего каталога"](~/rtvs/media/repl-toolbar-11-set-working-directory-to-project.png) | Задать расположение проекта в качестве рабочего каталога | Ctrl+Shift+P | Задает корневой каталог текущего загруженного в Visual Studio проекта в качестве рабочего каталога. См. раздел [Рабочий каталог](#working-directory). |
