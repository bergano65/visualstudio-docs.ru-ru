...
Интерактивное окно также интегрировано с обычными окнами редактора R. Можно выбрать код и нажать клавиши CTRL + ВВОД или щелкнуть правой кнопкой мыши и выбрать команду **Выполнение в интерактивном режиме**, после чего код будет выполнен построчно в интерактивном окне как при вводе напрямую. Когда курсор находится в одной из строк в окне редактора, при нажатии клавиш CTRL + ВВОД эта строка отправляется в интерактивное окно, а курсор затем перемещается на следующую строку. Таким образом можно просто нажать клавиши CTRL + ВВОД для пошагового выполнения кода.

Чтобы ознакомиться с этой возможностью в действии, выполните пошаговое руководство [Начало работы с R](getting-started-with-r.md), а также действия в следующих разделах.
...
Все предыдущие входные и выходные данные в REPL доступны только для чтения, и изменить их не удастся. Однако можно в любое время выбрать и скопировать текст из окна, а затем вставить его. Вставленный код будет выполняться так, как если бы он был введен построчно.
...
![Ввод многострочного оператора в интерактивном окне](~/docs/rtvs/media/repl-multiline-entry.png)
...
![Интерактивное окно с панелью инструментов](~/docs/rtvs/media/repl-window.png)
...
| --- | --- | --- | --- |
| ![Кнопка сброса](~/docs/rtvs/media/repl-toolbar-01-reset.png) | Сбросить | Ctrl+Shift+F10 | Сбрасывает сеанс работы с интерактивным окном, очищает все переменные и журнал. |
| ![Кнопка "Сбросить"](~/docs/rtvs/media/repl-toolbar-02-clear.png) | Clear | CTRL+L | Очищает выходные данные, которые отображаются в интерактивном окне; переменные сеанса и журнал остаются без изменений. |
| ![Кнопки работы с журналом](~/docs/rtvs/media/repl-toolbar-03-history.png) | Команда "Назад"<br/>Команда "Далее" | СТРЕЛКА ВВЕРХ, СТРЕЛКА ВНИЗ<br/>ALT + СТРЕЛКА ВВЕРХ, ALT + СТРЕЛКА ВНИЗ | Прокрутка журнала; для блоков с несколькими строками кода имеется определенная функциональность. См. раздел [Журнал](#history). |
| ![Кнопка "Загрузить рабочую область"](~/docs/rtvs/media/repl-toolbar-04-load-workspace.png) | Загрузить рабочую область | Н/Д | Загрузка сохраненной ранее рабочей области (см. раздел [Рабочие области и сеансы](#workspaces-and-sessions). |
| ![Кнопка "Сохранить рабочую область как"](~/docs/rtvs/media/repl-toolbar-05-save-workspace-as.png)| Сохранить рабочую область как | Н/Д | Сохранение текущего состояния сеанса в качестве рабочей области (см. раздел [Рабочие области и сеансы](#workspaces-and-sessions)). |
| ![Кнопка "Использовать скрипт R в качестве источника"](~/docs/rtvs/media/repl-toolbar-06-source-r-script.png) | Использовать скрипт R в качестве источника | Ctrl+Shift+S | Вызов `source` с текущим активным скриптом R в редакторе Visual Studio, который выполняет код.  Эта кнопка доступна только в том случае, если R-файл открыт в редакторе Visual Studio. | 
| ![Кнопка "Использовать скрипт R в качестве источника с эхо-выводом"](~/docs/rtvs/media/repl-toolbar-07-source-r-script-with-echo.png) | Использовать скрипт R в качестве источника с эхо-выводом | Ctrl+Shift+ВВОД | Аналогична кнопке "Использовать скрипт R в качестве источника", только содержимое скрипта отображается в интерактивном окне. | 
| ![Кнопка "Прервать R"](~/docs/rtvs/media/repl-toolbar-08-interrupt-r.png)| Прервать R | ESC | Остановка выполняемого кода в интерактивном окне, например цикла `while` на снимке экрана выше. |
| ![Кнопка "Присоединить отладчик"](~/docs/rtvs/media/repl-toolbar-09b-attach-debugger.png)| Присоединить отладчик | Н/Д | Также можно воспользоваться командой **Отладка > Подключить к интерактивному окну R**. | 
| ![Кнопка "Задать расположение исходных файлов в качестве рабочего каталога"](~/docs/rtvs/media/repl-toolbar-10-set-working-directory-source.png)| Задать расположение исходных файлов в качестве рабочего каталога | Ctrl+Shift+E | Задает расположение недавно загруженного в интерактивное окно исходного файла в качестве рабочего каталога (с помощью `source`). См. раздел [Рабочий каталог](#working-directory). |
| ![Кнопка "Задать расположение проекта в качестве рабочего каталога"](~/docs/rtvs/media/repl-toolbar-11-set-working-directory-to-project.png) | Задать расположение проекта в качестве рабочего каталога | Ctrl+Shift+P | Задает корневой каталог текущего загруженного в Visual Studio проекта в качестве рабочего каталога. См. раздел [Рабочий каталог](#working-directory). |
...
Выполнение кода в интерактивном окне создает контекст в текущем сеансе, состоящий из глобальных переменных, определений функций, загрузок библиотек и т. д. В совокупности этот контекст именуется *рабочей областью*. Вы можете в любое время сохранять и загружать рабочие области. 
...
Чтобы сохранить рабочую область под определенным именем файла (значение по умолчанию — `.RData`), нажмите кнопку "Сохранить рабочую область" в среде REPL.
...
Лучше всего, конечно, самостоятельно изучить эти возможности интерактивного окна на практике. В следующем примере кода предоставлено несколько подходящих одно- и многостроковых операторов. Однако для создания соответствующего журнала необходимо скопировать и вставить каждый оператор по отдельности. (То же самое можно сделать путем вставки кода в отдельный файл кода, а затем отправить строки в интерактивное окно, нажав CTRL + ВВОД.)
