...
Параметры доступны в меню **Инструменты R > Параметры**, а также в меню **Сервис > Параметры** в разделе **Инструменты R**.
  ...
  ![Диалоговое окно параметров инструментов R](~/docs/rtvs/media/options-dialog.png)

В следующих разделах описаны различные параметры, доступные на этой странице.

> [!Note]
> При доступе к параметрам через меню **Сервис > Параметры** может потребоваться выбрать **Показать все параметры** внизу окна, чтобы отобразить страницу **Инструменты R**.

<a name="data-scientist-layout"</a>

Также имеется отдельный пункт меню **Инструменты R > Параметры обработки и анализа данных** для настройки Visual Studio IDE с использованием макета, оптимизированного для обработки и анализа данных. В частности, с помощью этого параметра можно открыть окна [Интерактивное окно](interactive-repl.md), [Обозреватель переменных](variable-explorer.md) и [Рабочие области](workspaces.md).

![Макет окна для обработки и анализа данных в Visual Studio](~/docs/rtvs/media/installation-data-scientist-layout-result.png)
...
Эти параметры определяют порядок обработки значений в [обозревателе переменных](variable-explorer.md) и в окнах инструмента отладки, таких как "Контрольные значения" и "Локальные"(см. раздел [Отладка](debugging.md).
...
| Параметр | Значение по умолчанию | Описание | 
| --- | --- | --- |
| Проверка новостей и опросов | `Once a week` | Указывает периодичность, с которой инструментам R рекомендуется проверять на сервере наличие новостей и опросов. | 
...
| Параметр | Значение по умолчанию | Описание | 
| --- | --- | --- |
| Всегда сохранять журнал | `True` | Определяет, записывает ли RTVS журнал команд в файл `.RHistory` в рабочем каталоге при закрытии проекта. Обратите внимание, что это происходит даже в том случае, если вы не сохранили проект перед закрытием. |
...
| Отражение CRAN | `(Use .Rprofile)` | Задает зеркало CRAN по умолчанию для установки пакетов. Значение по умолчанию `Use .Rprofile` учитывает параметры отражения CRAN в файле `.RProfile`. Его можно переопределить, выбрав одно из зеркал CRAN, доступных в раскрывающемся списке. |
| Рабочий каталог | Пользовательская папка | Задает текущий рабочий каталог, который обычно задается при каждом открытии проекта. |
