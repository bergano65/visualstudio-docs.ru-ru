...
Пункт меню **Инструменты R > Параметры обработки и анализа данных** также используется для настройки Visual Studio IDE с использованием макета, оптимизированного для обработки и анализа данных. В частности, с помощью этого параметра можно открыть окна [Интерактивное окно](interactive-repl.md), [Обозреватель переменных](variable-explorer.md) и [Рабочие области](workspaces.md).
...
Эти параметры определяют порядок обработки значений в [обозревателе переменных](variable-explorer.md) и в окнах отладчика, таких как "Контрольные значения" и "Локальные" (см. раздел [Отладка](debugging.md).
