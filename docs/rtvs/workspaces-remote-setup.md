...
- [Установка служб R в Ubuntu](#install-r-services-on-ubuntu)
...
## <a name="install-r-services-on-ubuntu"></a>Установка служб R в Ubuntu
...
1. Скачайте, извлеките и запустите скрипт установки [пакета управляющей программы RTVS](https://aka.ms/r-remote-services-linux-binary-current). Он должен установить необходимые пакеты, их зависимости и управляющую программу RTVS:

    - Скачивание: `wget -O rtvs-daemon.tar.gz https://aka.ms/rtvs-daemon-current`
    - Извлечение: `tar -xvzf rtvs-daemon.tar.gz`
    - Запустите установщик: `sudo ./rtvs-install`. Для установки пакета DotNet нужно добавить новый доверенный ключ подписывания. Для установки без предупреждений или в автоматическом режиме используйте команду `sudo ./rtvs-install -s`.
    

1. Включите и запустите управляющую программу:

    - Включение: `sudo systemctl enable rtvsd`
    - Запуск управляющей программы:`sudo systemctl start rtvsd`

1. Убедитесь, что управляющая программа работает, и выполните команду `ps -A -f | grep rtvsd`. Должен появиться процесс, запущенный в качестве пользователя `rtvssvc`. Теперь вы должны быть в состоянии подключиться из Инструментов R для Visual Studio, привязав URL-адрес к этому компьютеру с Linux.

Чтобы настроить `rtvs-daemon`, см. `man rtvsd`.
