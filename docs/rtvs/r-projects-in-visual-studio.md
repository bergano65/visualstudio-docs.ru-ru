...
| EnableCodeIndexing | Yes | Определяет необходимость запуска фоновой задачи индексирования для ускорения поиска кода. |
| UseSpacesForTab | Yes | Определяет, следует ли вставлять пробелы ("Да") или символ табуляции ("Нет") при нажатии клавиши **TAB** в редакторе. |
