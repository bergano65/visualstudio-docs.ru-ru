...
|Изменение директивы x:Uid не поддерживается|Н/Д|Не поддерживается|Отсутствуют|
|Несколько процессов | Не поддерживается | Не поддерживается | Горячую перезагрузку можно использовать только для одного процесса одновременно. |
