...
ms.locfileid: "34693829"
---
# <a name="walkthrough-use-xml-editor-features"></a>Пошаговое руководство: Использование функций редактора XML

В этом пошаговом руководстве подробно описан процесс создания нового XML-документа. В нем также используются некоторые возможности XML Editor, благодаря которым это приложение становится очень полезным при создании XML-документов.

> [!NOTE]
> Прежде чем приступать к выполнению задач, сохранения *hireDate.xsd* файла (содержится ниже в этом разделе) для локального компьютера.
...
5.  Выберите *hireDate.xsd* файла и нажмите кнопку **откройте**.
...
6.  Редактор добавляет закрывающий тег. Закрывающий тег после добавления будет подчеркнут волнистой линией, обозначающей ошибку проверки правильности. **Подсказка** отображает сообщение: **элемента «сотрудник» имеет неполные содержимое. Ожидалось «ID»**.
...
8.  Тип **abc**.
     ...
     **Abc** текст подчеркивается волнистой линией. **Подсказка** отображает сообщение: **элемент «ID» имеет недопустимое значение в соответствии с типом данных**.

9. Щелкните правой кнопкой мыши на элемент ID и выберите **перейти к определению**.

     Откроется редактор *hireDate.xsd* файл в новом окне документа и помещает курсор на определении элемента схемы идентификатор.

10. Вернитесь к XML-файле и замените **abc** текста с **123**.

     Подчеркивание волнистой линией и **подсказка** установлен в значение идентификатора элемента. **Подсказка** для сотрудника конечного тега теперь отображает сообщение: **элемента «сотрудник» имеет неполные содержимое. Ожидалось «hire-date»**.

11. Поместите курсор после закрывающего тега ID, введите `<`выберите **даты найма** из списка членов, а затем введите в `>`.
...
- Выберите **форматировать документ** кнопки на панели инструментов редактора XML.

    XML-документ будет переформатирован.

## <a name="to-save-the-xml-document"></a>Сохранение XML-документа

1.  Из **файл** последовательно выберите пункты **Сохранить как**.

     **Сохранить файл как** диалоговое окно. Имя файла по умолчанию — *«XMLFile1»*.

2.  Введите имя и расположение файла для XML-документа и нажмите кнопку **Сохранить**.
