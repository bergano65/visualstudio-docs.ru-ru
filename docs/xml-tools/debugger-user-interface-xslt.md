...
## <a name="command-windowimmediate-window"></a>Командное окно и интерпретация окно
 ...
 **Стек вызовов** окно используется для просмотра имен функций в стеке вызовов, типы параметров и значения параметров. Сведения стека вызовов отображаются, только если отлаживаемая программа находится в состоянии останова.
 ...
 Стек вызовов представляет различные контексты, через которые проходит выполнение XSLT. Например, если имеется вызов на основе шаблона «a» шаблон «б», шаблон «» и шаблон «б» появляются в **стек вызовов** окна с текущим контекстом в самом верху списка. Пользователь может видеть запрос, который выполняется в текущий момент.
