---
title: Пользовательский интерфейс отладчика (XSLT)
ms.date: 11/04/2016
 ...
 **Список задач** включает все ошибки, возникающие в блоках скрипта в файле XSLT.

> [!NOTE]
> Отладчик XSLT не имеет не выдает предупреждений, поэтому они никогда не появляются в **списка задач**.
...
## <a name="command-windowimmediate-window"></a>Окно команд окно и интерпретация
 ...
 **Стек вызовов** окно используется для просмотра имен функций в стеке вызовов, типы параметров и значения параметров. Сведения стека вызовов отображаются, только если отлаживаемая программа находится в состоянии останова.
 ...
 Стек вызовов представляет различные контексты, через которые проходит выполнение XSLT. Например, если имеется вызов на основе шаблона «a» шаблон «б», шаблон «» и шаблон «б» появляются в **стек вызовов** окно с текущим контекстом на самой верхней части списка. Пользователь может видеть запрос, который выполняется в текущий момент.
...
- [Отслеживание значений переменных в окнах "Видимые" и "Локальные" в Visual Studio](../debugger/autos-and-locals-windows.md)