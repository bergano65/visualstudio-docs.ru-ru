...
Аргумент ref или out должен являться переменной, которой можно присвоить значение  
 ...
 Только переменную можно передать как параметр [ref](/dotnet/csharp/language-reference/keywords/ref) в вызове метода. Значение `ref` аналогично передаче указателя.  
