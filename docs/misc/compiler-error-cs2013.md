...
Недопустимый номер базы образа "значение"  
 ...
 Недопустимое значение \(не число\) было передано в параметр компилятора [\/baseaddress](/dotnet/csharp/language-reference/compiler-options/baseaddress-compiler-option).  
