...
Требуется ";" или "\=" \(невозможно задать аргументы конструктора в объявлении\).  
 ...
 Ссылка на класс сформирована таким образом, как если бы создавался объект класса. Например, была попытка передать переменную в конструктор. Для создания объекта класса используйте оператор [new](/dotnet/csharp/language-reference/keywords/new).  
