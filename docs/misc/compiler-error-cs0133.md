...
Выражение, назначаемое переменной "переменная", должно быть константным  
 ...
 Переменная [const](/dotnet/csharp/language-reference/keywords/const) не может принимать в качестве своего значения выражение, которое не является константой. Для получения дополнительной информации см. [Константы](/dotnet/csharp/programming-guide/classes-and-structs/constants).  
