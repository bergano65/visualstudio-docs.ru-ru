...
-   **Инклюзивный размер \(байт\)** содержит размер объектов, на которые указаны ссылки.  
 ...
 Вы можете выбрать значок экземпляров \(![Значок экземпляра в столбце "Тип объекта"](~/docs/profiling/media/dbg_mma_instancesicon.png "DBG\_MMA\_InstancesIcon")\) в столбце **Тип объекта**, чтобы просмотреть список экземпляров этого типа.  
