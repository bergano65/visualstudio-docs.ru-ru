...
Требуется директива \#endif.  
 ...
 Для указанной директивы `#if` не найдена соответствующая [директива препроцессора](/dotnet/csharp/language-reference/preprocessor-directives/index)`#endif`. Возможно также, что внутри блока `#if` компилятор обнаружил директиву `#endregion` без соответствующей директивы `#region`.  
