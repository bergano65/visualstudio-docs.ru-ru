---
title: "Аргумент &lt;имя_аргумента&gt; должен быть больше или равен нулю | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vbrArgument_GEZero1"
ms.assetid: 6f54fdc6-e263-4b2f-a66f-f206f3a59818
caps.latest.revision: 8
caps.handback.revision: 8
author: "stevehoag"
ms.author: "shoag"
manager: "wpickett"
---
# Аргумент &lt;имя_аргумента&gt; должен быть больше или равен нулю
Значение аргумента меньше нуля. Значения аргумента должны быть больше или равны нулю.  
  
### Исправление ошибки  
  
1.  Убедитесь в том, что значение аргумента больше или равно нулю.  
  
2.  Если аргумент является результатом вычисления, проверьте значения, обрабатываемые при вычислении.  
  
## См. также  
 [Передача аргументов по значению и по ссылке](/dotnet/visual-basic/programming-guide/language-features/procedures/passing-arguments-by-value-and-by-reference)   
 [Parameter Passing Mechanism for Visual Basic 6.0 Users](http://msdn.microsoft.com/ru-ru/0fa2b0dc-aa1c-4797-bbd6-aa13c611cab2)