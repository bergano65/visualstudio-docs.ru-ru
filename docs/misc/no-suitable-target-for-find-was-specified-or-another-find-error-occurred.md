---
title: "No suitable target for find was specified or another find error occurred. | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vs.message.0x800A00E5"
  - "vs.message.VS_E_EXT_FINDFAILURE"
ms.assetid: 5a0a7f0f-f64c-437c-9f2b-e7ebc871f846
caps.latest.revision: 6
caps.handback.revision: 6
author: "mikeblome"
ms.author: "mblome"
manager: "douge"
---
# No suitable target for find was specified or another find error occurred.
Эта ошибка обычно возникает при попытке выполнения поиска, когда для области поиска установлено "Текущий документ", а ни один из файлов не открыт.  
  
### Чтобы исправить эту ошибку  
  
1.  Откройте файл и снова выполните поиск или измените область поиска.  
  
## См. также  
 [Поиск и замена текста](../ide/finding-and-replacing-text.md)   
 [Поиск в файлах](../ide/find-in-files.md)