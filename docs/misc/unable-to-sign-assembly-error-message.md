...
Компилятор [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] вызывает компоновщик сборок \(Al.exe, который также называется Alink\) для создания сборки с манифестом. Компоновщик сообщил об ошибке подписи сборки после ее создания. Это может быть вызвано недопустимым ключом.  
