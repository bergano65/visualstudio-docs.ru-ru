---
title: "Directory or file specified in Look In is not available or does not exist. | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-csharp"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vs.message.0x800A00DC"
  - "vs.message.VS_E_FRLookInMissingFile"
ms.assetid: 6b655780-eb65-4985-9b7e-e9dd3959be37
caps.latest.revision: 6
caps.handback.revision: 6
author: "mikeblome"
ms.author: "mblome"
manager: "douge"
---
# Directory or file specified in Look In is not available or does not exist.
Эта ошибка обычно возникает, если не удается найти путь или имя, указанные в списке обзора.  
  
### Чтобы исправить эту ошибку  
  
1.  Заново введите путь и имя файла.  
  
     —или—  
  
2.  Перейдите к правильному файлу или папке, используя список "Обзор" и поле **Доступные элементы**.  
  
## См. также  
 [Поиск и замена текста](../ide/finding-and-replacing-text.md)   
 [Choose Search Folders](http://msdn.microsoft.com/ru-ru/85af6458-dcde-4a84-9ea4-f5cc6550dc80)   
 [Поиск в файлах](../ide/find-in-files.md)