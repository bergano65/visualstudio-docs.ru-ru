---
title: "Требуется &quot;Next&quot; | Microsoft Docs"
ms.custom: ""
ms.date: "11/16/2016"
ms.prod: "visual-studio-dev14"
ms.reviewer: ""
ms.suite: ""
ms.technology: 
  - "devlang-visual-basic"
ms.tgt_pltfrm: ""
ms.topic: "article"
f1_keywords: 
  - "vbc30003"
  - "bc30003"
helpviewer_keywords: 
  - "BC30003"
ms.assetid: 75a68984-decd-43e2-808a-f14b36dd2c60
caps.latest.revision: 8
caps.handback.revision: 8
author: "stevehoag"
ms.author: "shoag"
manager: "wpickett"
---
# Требуется &quot;Next&quot;
Конструкция `On Error Resume` используется без завершающего ключевого слова `Next`.  
  
 **Идентификатор ошибки:** BC30003  
  
### Исправление ошибки  
  
-   Добавьте ключевое слово `Next` в конец оператора `On Error`.  
  
## См. также  
 [Оператор On Error](/dotnet/visual-basic/language-reference/statements/on-error-statement)   
 [Оператор Error](/dotnet/visual-basic/language-reference/statements/error-statement)   
 [Оператор Resume](/dotnet/visual-basic/language-reference/statements/resume-statement)   
 [Общие сведения об обработке неструктурированных исключений \(Visual Basic\)](http://msdn.microsoft.com/ru-ru/d2d84b66-ff3a-4878-a578-484c0c6d5c3d)