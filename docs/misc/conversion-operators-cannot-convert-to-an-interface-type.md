...
Оператор преобразования объявлен с типом интерфейса в качестве типа возвращаемых данных.  
 ...
 Во время компиляции [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)] определяет возможность выполнения предопределенного преобразования любого ссылочного типа в любой интерфейс. Такое преобразование может вызвать сбой во время выполнения, однако компилятор не может предсказать результаты выполнения, поэтому он позволяет компилировать такие преобразования.  
