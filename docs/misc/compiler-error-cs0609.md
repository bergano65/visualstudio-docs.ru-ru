...
Невозможно назначить атрибут IndexerName для индексатора, помеченного как override  
 ...
 Атрибут name \(**IndexerNameAttribute**\) не может применяться к индексированному свойству, которое является переопределением.  Дополнительные сведения см. в разделе [Индексаторы](/dotnet/csharp/programming-guide/indexers/index).  
