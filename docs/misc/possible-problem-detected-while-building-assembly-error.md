...
Средство ALink, вызываемое компилятором [!INCLUDE[vbprvb](../code-quality/includes/vbprvb_md.md)], сообщает об ошибке при построении сборки. Одна из возможных причин — подписанная сборка создает ссылку на сборку без подписи.  
  
 Это сообщение является предупреждением. Компилятор продолжает создавать сборку. Дополнительные сведения о скрытии предупреждений и обработке предупреждений как ошибок см. в разделе [Настройка предупреждений в Visual Basic](../ide/configuring-warnings-in-visual-basic.md).  
