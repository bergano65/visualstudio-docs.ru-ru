...
Оператор "оператор" для операнда типа "тип" является неоднозначным  
 ...
 Компилятору доступно несколько преобразований, и ему не удается определить, какое преобразование использовать перед применением оператора. Дополнительные сведения см. в разделах [Шаблонные пользовательские преобразования](../misc/templated-user-defined-conversions.md) и [Операторы преобразования](/dotnet/csharp/programming-guide/statements-expressions-operators/conversion-operators).  
