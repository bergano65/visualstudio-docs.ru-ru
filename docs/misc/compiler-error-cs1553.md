...
Декларация недопустима; используйте вместо нее "modifier operator \<dest\-type\> \(..."  
 ...
 Тип возврата для [оператора](/dotnet/csharp/language-reference/keywords/operator) должен находиться непосредственно перед списком параметров, и *модификатор* либо `implicit`, либо **explicit**.  
