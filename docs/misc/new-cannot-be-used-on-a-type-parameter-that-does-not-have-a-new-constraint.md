...
В операторе объявления используется предложение [Оператор New](/dotnet/visual-basic/language-reference/operators/new-operator) для задания параметра типа в качестве типа, который должен быть создан, и параметр типа объявляется без ограничения `New`.  
