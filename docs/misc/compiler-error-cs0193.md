...
К указателю должен быть применен оператор \* или \-\>.  
 ...
 Оператор **\*** или **\-\>** используется с типом, не являющимся указателем. Дополнительные сведения см. в разделе [Типы указателей](/dotnet/csharp/programming-guide/unsafe-code-pointers/pointer-types).  
