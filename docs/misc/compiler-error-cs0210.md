...
Требуется указать инициализатор в объявлении оператора fixed или using.  
 ...
 Переменная должна быть объявлена и инициализирована в [операторе fixed](/dotnet/csharp/language-reference/keywords/fixed-statement). Для получения дополнительной информации см. [Небезопасный код и указатели](/dotnet/csharp/programming-guide/unsafe-code-pointers/index).  
...
```  
 ...
 В приведенном ниже примере также возникает ошибка CS0210, так как в [операторе using](/dotnet/csharp/language-reference/keywords/using-statement) не содержится инициализатор.  
